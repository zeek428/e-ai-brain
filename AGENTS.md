...
This checkout contains the AI Brain documentation set under [docs/](docs/) plus the current MVP implementation under [apps/api](apps/api) and [apps/web](apps/web). The backend is a FastAPI app with a `MemoryStore` runtime that persists to PostgreSQL by default: users are managed through the `users` table, product configuration is mirrored into `products`, `product_versions`, `product_modules`, `product_git_repositories`, and `related_systems`, requirement ledger data is mirrored into `requirements`, AI task core fields are mirrored into `ai_tasks`, workflow review/checkpoint state is mirrored into `human_reviews`, `graph_runs`, and `graph_checkpoints`, GitLab MR review artifacts are mirrored into `gitlab_mr_snapshots` and `code_review_reports`, knowledge operations are mirrored into `knowledge_documents` and `knowledge_deposits`, audit events are mirrored into `audit_events`, Bug records are mirrored into `bugs`, GitLab daily metrics are mirrored into `gitlab_daily_code_metrics`, Jenkins release records are mirrored into `jenkins_release_records`, online log metrics are mirrored into `online_log_metrics`, mock Issue writebacks are mirrored into `mock_issues`, model gateway configuration and metadata logs are mirrored into `model_gateway_configs` and `model_gateway_logs`, and the remaining business runtime state is still stored through `app_state_snapshots` JSONB while fine-grained repositories are added module by module. `development_planning` and `automated_testing` can now be created from a confirmed `technical_solution`; confirmed automated-testing outputs create `ai_auto_test` Bug records from `bug_suggestions`. Always verify implementation files before assuming a module is complete.
