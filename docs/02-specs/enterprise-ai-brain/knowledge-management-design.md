...
4. 响应指标包含 `hit_count`、`no_result`、`no_result_rate`、`latency_ms`、`citation_count`、`quality_event_id` 和 `rag_citation_accuracy_proxy`。
5. 前端引用点击和“有用/无用”反馈必须携带 `quality_event_id` 写入质量事件，避免反馈和原始问答脱钩。
...
3. 已在检索和 RAG 响应中返回 `hit_count`、`no_result`、`no_result_rate`、`latency_ms`、`citation_count`、`retrieval_modes`、`quality_event_id` 和引用准确性代理指标，作为后续评测集和运营看板的数据基础。
4. 前端问答卡片已支持引用点击和“有用/无用”反馈，相关事件进入 `knowledge_quality_events`，用于引用点击率、反馈数量和 RAG 引用准确率 proxy 汇总。
5. 尚未落地低命中率评测集、重复文档检测和跨空间知识治理队列，应作为后续 Phase 4 增强继续推进。
