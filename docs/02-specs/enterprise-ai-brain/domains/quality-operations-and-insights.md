...
- 执行诊断列表和详情优先读取 `execution_trace_snapshots`，快照刷新必须在单个数据库事务中完成 upsert 与过期快照删除，短 TTL 内可复用快照降低重复查询开销；详情未命中关联 ID 时必须强制刷新后再返回 404，节点元数据必须脱敏；列表支持 `source_id` 按任一节点来源 ID 精准定位，前端深链命中唯一链路时自动打开详情；AI 助手草案、AI 助手运行状态最近失败、AI 助手运行诊断卡片、定时作业运行详情、代码巡检报告详情、插件管理 Runner 执行日志和审计列表通过统一 `ExecutionTraceLink` 生成深链，审计列表按 `source_type=audit_event` 和审计 ID 跳转执行诊断；代码巡检详情需提供巡检报告、来源运行和插件调用三个诊断入口，Runner 日志需提供任务诊断、Runner 诊断和来源运行诊断入口；Runner 节点支持按 `source_type=ai_executor_runner` 和 Runner ID 下钻，任务节点通过 `assigned_runner` 边关联 Runner，节点只展示心跳、协议、工作区、健康状态和 token 是否配置，不暴露 `token_hash`；模型网关日志需吸附通过 subject、payload 或 `ai_task_id` 指向它的审计事件，按审计 ID 下钻应回到同一条模型调用链路；列表和详情需返回从失败、取消、运行中或排队节点派生的 `diagnostic_nodes` 安全摘要，不返回节点 metadata；详情需优先用 `diagnostic_nodes` 汇总失败/运行中节点，提供来源 ID 深链、单节点“问 AI”和整条链路“问 AI 分析链路”入口；整条链路诊断包只包含 trace/root/status/summary/related_ids 和最多 5 个脱敏诊断节点，可复制或按根对象带入 AI 助手，详情内关联对象、诊断建议节点和节点表的来源 ID 链接必须同时携带 `source_id` 与 `source_type`，避免跨来源 ID 冲突时定位错误；跳转 AI 助手时必须通过 `reference_type/reference_id/prompt` 深链解析为“本次上下文”，且同标签页点击需写入短期待带入 prompt 兜底，避免 URL prompt 被截断后只跳页面不带问题；不得只带入纯文本问题。
