...
- 执行诊断来源深链进入助手时必须同时带入 `prompt` 和可解析引用上下文；执行诊断详情页“问 AI 分析链路”点击时还需写入短期同标签页待带入 prompt，助手页在 URL `prompt` 被浏览器、路由或复制过程截断时可从该兜底恢复输入框问题，并消费后清除，避免污染后续会话。`assistant_chat_run`、`assistant_message`、`model_gateway_log`、`plugin_invocation_log`、`ai_executor_task`、`ai_executor_runner`、`code_inspection_report`、`audit_event` 等来源类型仅在用户具备 `diagnostics.execution_traces.read` 或系统管理员权限时可解析和注入。
...
- @ 能力配置页必须复用统一管理列表底座，支持标题/关键词/角色/权限/URL 搜索、启停状态筛选、角色筛选、批量启停、横向滚动、表格设置、刷新和本地筛选视图保存；主表默认携带 `page/page_size/sort_by/sort_order` 调用 `/api/assistant/action-reference-configs` 服务端 read model，筛选项下推 `keyword/status/role/permission/enterprise_id/template_version` 并展示 `query/performance` 查询耗时。新增/编辑、灰度、删除和审计跳转继续消费 `/api/assistant/action-reference-configs` 系列管理 API，不得在前端维护硬编码动作清单。
- 角色快捷任务配置页必须复用统一管理列表底座，支持关键词、任务启停状态、分组启停状态、角色、权限、企业、草案模板和模板版本筛选，以及横向滚动、表格设置、刷新和本地筛选视图保存；主表默认携带 `page/page_size/sort_by/sort_order` 调用 `/api/assistant/role-quick-task-configs` 服务端 read model，筛选项下推 `keyword/status/group_status/role/permission/enterprise_id/target_draft_type/template_version` 并展示 `query/performance` 查询耗时。新增/编辑、启停、灰度和审计跳转继续消费 `/api/assistant/role-quick-task-configs` 系列管理 API，无分页全量读取仅保留兼容。
- 草案任务台必须支持待确认、失败、已采纳、已修改筛选，并展示采纳率、处理率、用户修改率、继续编辑入口、失败草案重新打开入口和来源链路入口；列表与详情弹窗的“继续编辑”统一按草案 ID 生成 `/assistant?draft_id=...`，助手页按 `draft_id` 加载草案卡，来源链路继续独立跳转执行诊断。失败草案只能先通过 retry 重新打开为待确认状态，保留失败历史并写审计，重新确认前不得写入业务配置。草案任务台列表必须透传远程分页 `performance` 元数据，由统一列表底座展示查询耗时和慢查询提示；PostgreSQL 运行态优先使用草案分页 read model 完成当前用户、动作、状态、时间、关键词、排序和分页，不得读取当前用户全量草案后再分页，实时预检 `validation_status` 筛选可保留服务层兼容路径。动作草案公开响应必须派生 `governance` 治理摘要，任务台列表展示影响对象、权限状态、审计事件数、失败次数和重试次数，详情弹窗集中展示风险等级、影响对象、必需/缺失权限、执行前后差异、失败重试和审计链路，让用户确认前能判断真实写入影响。草案任务台必须基于 summary 和当前页行派生治理优先队列，按权限阻断、校验阻断、失败重试、高风险和审计缺口展示处理优先级、代表草案和继续处理入口，避免用户只看到统计数字却不知道先处理哪一类草案。摘要指标条、治理优先队列、详情弹窗和草案状态/风险/校验展示 helper 必须保持组件化，分别由 `AssistantDraftSummaryStrip`、`AssistantDraftGovernanceQueue`、`AssistantDraftDetailModal` 和 `assistantDraftWorkbenchPresentation` 承接；助手页草案卡中的配置向导、步骤状态、前置草案提示和手动调整入口由 `AssistantDraftWizardBlock` 承接，应用前预检、字段差异、校验问题和修复动作入口由 `AssistantDraftPreviewBlock` 与 `assistantDraftPreviewHelpers` 承接，助手页草案详情弹窗由 `AssistantDraftDetailModal` 承接，页面主文件只保留远程查询、确认/取消和详情打开编排。
- AI 动作草案服务入口 `assistant_action_drafts.py` 只负责草案创建/查看/修改/确认/失败/重试、治理摘要、写入和审计编排；草案状态/动作枚举、默认 payload、基础动作校验和 Cron 表达式校验统一由 `assistant_action_draft_common` 承接，并由架构守护测试防止通用规则回流主服务文件。
