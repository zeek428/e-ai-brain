...
- 知识中心知识文档主列表属于管理型列表；`GET /api/knowledge/documents` 传入 `page/page_size` 时必须在 PostgreSQL read model 层完成知识空间权限、角色权限、关键字、知识空间、目录、类型、索引状态、权限角色筛选和白名单排序，并返回 `query/performance`；未分页全量返回仅用于兼容旧调用或下拉类轻量场景。
