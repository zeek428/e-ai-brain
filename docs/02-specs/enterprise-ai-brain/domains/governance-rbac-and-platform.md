...
- 管理列表型接口必须在 SQL/repository/read model 层完成分页、排序和筛选，并返回查询性能元数据；前端统一列表底座需展示查询耗时，`performance.slow=true` 时显示慢查询阈值提示并指引结合接口 trace、筛选条件和数据库慢查询日志排查。需求、任务、Bug、用户洞察、代码巡检、角色等核心列表必须透传该性能元数据，不得在页面侧静默丢弃。
