...
- 钉钉官方 MCP 以统一钉钉提供方接入，但 P0 能力按能力边界拆成标准插件：`dingtalk_doc`、`dingtalk_wiki`、`dingtalk_drive`、`dingtalk_aitable`、`dingtalk_bot` 和 `dingtalk_contact`；六个插件统一使用 `protocol=mcp_streamable_http`、`auth_type=url_key`、`endpoint_url=https://mcp-gw.dingtalk.com/mserver/{serverName}` 和连接级 query `provider=dingtalk/mcp_id/server_name/auth_subject_type`，授权 URL Key 作为连接密钥按个人、系统或应用授权实例单独配置，不作为全局固定 token。连接测试使用 JSON-RPC `tools/list`，动作调用使用 JSON-RPC `tools/call`，请求头必须接受 `application/json, text/event-stream`，请求回放、动作预览、调用日志和测试历史必须脱敏 query key。钉钉官方动作模板必须标记 `risk_tier`，至少覆盖文档搜索/读取/创建、知识库空间搜索、钉盘文件列表、AI 表格记录查询、机器人消息发送和通讯录用户搜索，便于后续高风险读写和通知类动作接入人工确认。
