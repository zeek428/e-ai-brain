...
- AI 执行器任务列表也是 Runner 运维管理列表；`GET /api/system/ai-executor-tasks` 传入 `page/page_size` 时必须优先走 PostgreSQL read model 完成 `ai_task_id/runner_id/scheduled_job_run_id/status` 筛选、白名单排序和 count/page 查询，并返回 `query/performance` 观测信息；未带分页的全量返回仅用于 Runner 日志弹窗、单任务兼容和测试 helper。
