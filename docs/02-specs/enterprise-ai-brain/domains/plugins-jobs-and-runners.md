...
- 内部数据源连接表单只展示业务源数据、字段模式、时间窗口、通用过滤和按源过滤字段；需求状态/优先级、Bug 状态/严重级别等常用过滤必须以可视化字段维护并写入 `request_config.query.source_filters`，高级过滤 JSON 仅作为更细粒度过滤补充，不应把常用过滤引导回手写 JSON。
