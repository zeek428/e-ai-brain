...
- 结果写入记录虽由定时作业运行和动作调用日志派生，但仍属于产品范围受控排障数据；`GET /api/system/result-write-records` 必须通过 `scheduled_job_id` 或 `scheduled_job_run_id` 解析定时作业产品并按当前用户产品 scope 过滤，scope 外写入记录不返回且不计入 total；带 `page/page_size` 时返回 `query/performance` 并支持白名单排序，未带分页时仅作为运行详情、助手诊断、执行诊断和测试 helper 兼容；无产品归属的独立动作调用写入记录仅对全局产品范围用户可见。
