...
- 关键接口清单见 [../api/catalog.md](../api/catalog.md) 的 Plugin、Scheduled Jobs、AI Executor 和 RD Executor Policy 条目；定时作业与代码巡检作业细节见 [../api/devops-quality-and-code-inspection.md](../api/devops-quality-and-code-inspection.md)，执行诊断聚合见 [../api/lifecycle-dashboard-and-diagnostics.md](../api/lifecycle-dashboard-and-diagnostics.md)。
...
- 定时作业配置和运行记录列表是管理型列表，分页、排序和筛选必须优先走 PostgreSQL read model；`GET /api/system/scheduled-jobs` 传入 `page/page_size` 时按名称、关键字、产品、来源、类型、启停、状态过滤并返回 `query/performance` 观测信息；`GET /api/system/scheduled-job-runs` 传入 `page/page_size` 时按运行 ID、作业 ID、状态和产品 scope 过滤，并支持开始/完成时间、状态、触发方式和导入数排序；运行记录响应应投影 `scheduled_job_name`，前端运行记录列表和详情基础信息优先展示作业名称；MemoryStore 全量读取只允许作为旧客户端、助手按 runId 拉详情和测试 helper 兼容。
- 定时作业运行观测同样属于产品范围受控读接口；`GET /api/system/scheduled-job-runs/observability` 允许 `system.scheduled_jobs.manage` 或 `system.scheduled_jobs.run` 访问，但健康汇总、状态/作业类型/触发/写入目标分布、失败原因、最近失败和慢运行都必须先按当前用户产品 scope 过滤运行记录，不能因为是聚合指标而绕过产品范围。存在 `execution_nodes.result_actions[]` 时，动作写入次数、成功率和写入目标分布必须按每个动作明细统计，旧运行缺少数组时才回退 `result_action`。
...
- 内部数据源连接表单只展示业务源数据、字段模式、时间窗口、通用过滤和按源过滤字段；产品过滤必须使用当前用户产品 scope 内的可搜索产品选择器，保存值仍写入 `request_config.query.product_id`，不得要求用户手工填写产品 ID。需求状态/优先级、Bug 状态/严重级别等常用过滤必须以可视化字段维护并写入 `request_config.query.source_filters`，并按已选 `source_types` 联动展示，取消对应源数据后隐藏过滤不得继续提交；时间字段的系统变量通过字段内图标菜单插入，避免在多列布局中挤占独立行；高级过滤 JSON 仅作为更细粒度过滤补充，不应把常用过滤引导回手写 JSON。
- 钉钉官方 MCP 以统一钉钉提供方接入，但 P0 能力按能力边界拆成标准插件：`dingtalk_doc`、`dingtalk_wiki`、`dingtalk_drive`、`dingtalk_aitable`、`dingtalk_bot` 和 `dingtalk_contact`；六个插件统一使用 `protocol=mcp_streamable_http`、`auth_type=url_key`，连接 Endpoint 必须使用钉钉 AI Hub MCP 详情复制出来的实例级 `StreamableHttp URL`（形如 `https://mcp-gw.dingtalk.com/server/<instance>`），系统可从完整 URL 中提取 `key` 并移入 `auth_config.secret_ref`，不再向钉钉网关追加旧的 `provider/mcp_id/server_name` query。授权 URL Key 作为连接密钥按个人、系统或应用授权实例单独配置，不作为全局固定 token。连接测试使用 JSON-RPC `tools/list`，动作调用使用 JSON-RPC `tools/call`，请求头必须接受 `application/json, text/event-stream`，请求回放、动作预览、调用日志和测试历史必须脱敏 query key。钉钉官方动作模板必须标记 `risk_tier`，至少覆盖文档搜索/读取/创建、知识库空间搜索、钉盘文件列表、AI 表格记录查询、机器人消息发送和通讯录用户搜索，便于后续高风险读写和通知类动作接入人工确认。
...
- 多数据连接失败策略必须在运行层落地：默认 `failure_policy=fail_fast` 时首个失败连接中断后续连接，但失败运行仍保留数据连接失败节点和 Trace；`failure_policy=continue_on_error` 时失败连接进入 `data_connection.items[]` 与 Trace DAG，后续连接继续执行，成功响应按合并策略进入后续 AI执行或动作。若数据连接失败导致 AI 链路未开始，`skill_processing` 必须记录 `status=not_run`、`model_gateway_called=false` 和 `processing_mode=not_started`。
