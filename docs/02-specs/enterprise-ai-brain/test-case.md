...
| TC-AIBRAIN-AUTH-API-024 | MVP 角色目录和用户管理角色选择 | P1 | MVP | admin 已登录 | 1. 调用 `/api/auth/roles` 2. 进入系统管理/用户管理新增用户 3. 打开角色字段 4. 尝试提交未定义角色 | 接口返回 6 个明确角色及权限说明；页面使用固定多选；后端拒绝目录外角色并返回 `VALIDATION_ERROR`；`role_definitions` 迁移脚本可重复执行 | 是 |
...
| TC-AIBRAIN-AUTH-API-024 | GET /api/auth/roles + POST/PATCH /api/users | 查询角色目录并按目录维护用户角色 | 返回 `admin/product_owner/rd_owner/reviewer/knowledge_owner/viewer` 六个角色；未知角色返回 `VALIDATION_ERROR`。 |
