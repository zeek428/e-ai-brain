...
| 2 | MVP-C: GET `/api/writeback/results/{task_id}` | 未写回时返回 `not_written` 和空 issues，不创建 mock issue。 |
| 3 | MVP-C: POST `/api/writeback/results/{task_id}` | 返回模拟 Issue 和 idempotency_key。 |
...
| 2 | GET `/api/system/model-gateway-configs` | 返回 `api_key_configured=true`，不返回明文 API Key、密钥前缀或后缀。 |
