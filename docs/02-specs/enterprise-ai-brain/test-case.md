...
| TC-AIBRAIN-REVIEW-FUNC-023D1 | 本地完整代码静态扫描 | P0 | v1.2 | 产品已配置可 clone 的 Git 仓库，仓库存在目标分支和包含硬编码凭据/内部地址的测试文件 | 1. 创建 `code_repository_inspection` 作业，设置 `config_json.scan_mode=native_full_scan`、`repository_id`、`branch` 和 `scan_rules` 2. 不配置 `plugin_action_id/plugin_connection_id` 保存作业 3. 手动运行作业 4. 查看运行详情 5. 打开代码巡检报告详情 | 作业保存成功且插件单值字段为空、数组为空；运行时服务端 clone 仓库并 checkout 指定分支，`plugin_invocation_log_id` 为空，`records_imported` 等于 finding 数量；运行摘要包含 `native_scan` 节点，返回扫描模式、分支、commit、文件数、行数和扫描器名称，`data_connection.processing_mode=native_full_scan`；报告返回 `scan_mode=native_full_scan`、`scanner_name=ai_brain_builtin_static`、`is_full_scan=true`、`files_scanned`、`lines_scanned`、`rules_loaded` 和 `coverage_warning`；finding 包含 `secrets.hardcoded_credential`、`metadata.internal_address_exposure`、文件路径、行号、修复建议和 git blame 提交人，raw 证据脱敏不保存明文密钥。回归见 `apps/api/tests/test_code_inspection_governance.py::test_native_repository_inspection_clones_branch_scans_files_and_blames_committers` 和 `apps/web/tests/ScheduledJobsPage.test.tsx::submits native full scan mode without requiring a plugin connection` | 是 |
...
| TC-AIBRAIN-REVIEW-FUNC-023E | 代码巡检 AI执行配置 | P0 | v1.2 | 管理员已配置代码巡检扫描插件、active AI 模型、AI角色 和 Skill；扫描插件返回未完全归一的仓库扫描 JSON | 1. 创建 `code_repository_inspection` 作业并在“AI执行”中选择 `ai_generated` 或 `ai_assisted` 2. 先不绑定模型、AI角色、Skill 并尝试保存 3. 补齐模型、AI角色、Skill、扫描动作和连接 4. 手动运行作业 5. 查看运行详情和代码巡检报告 | 前端保存前必须按 AI执行方式识别 AI 装配要求，代码巡检切换为 `ai_assisted/ai_generated` 时缺少 AI 模型、AI角色或 Skill 应直接显示校验提示且不提交请求；服务端创建/修改时也必须校验 active AI角色、Skill 和模型网关；运行时先通过数据连接获取扫描 JSON，再调用模型网关按 AI角色/Skill 输出 `repository_id/branch/commit_sha/risk_level/summary/findings` 结构，模型输出再进入 `write_code_inspection_report` 等结果动作；运行详情“运行链路”必须展示 `data_connection`、`skill_processing` 和 `result_action`，其中 `skill_processing.model_gateway_called=true`、包含 `model_log_id` 和模型处理输出，`result_action.write_target=code_inspection_reports`；最终代码巡检报告和 finding 必须来自模型归一化后的输出 | 是 |
