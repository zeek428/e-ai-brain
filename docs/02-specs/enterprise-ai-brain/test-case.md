...
| TC-AIBRAIN-ASSISTANT-FUNC-015C | v1.1 | P0 | 具备 `system.scheduled_jobs.manage` 但不是 admin 角色的用户在 AI 助手输入 `@提取每周用户反馈有价值信息 执行一次` | @ 候选和引用解析必须允许该用户看到并引用 `scheduled_job` / `scheduled_job_run`，聊天执行链路也允许手动运行；若未命中现成可执行周反馈洞察作业，也必须生成带 `assistant_run_once_request` 的待确认服务端草案，而不是退回“请选择候选”；该权限不得顺带开放插件连接、插件动作、AI角色或 Skill 候选，仍需分别具备 `system.plugins.manage` 或 `system.ai_capabilities.manage`。回归见 `apps/api/tests/test_assistant_chat.py::test_ai_assistant_reference_candidates_allow_scheduled_job_managers`、`apps/api/tests/test_assistant_chat_service.py::test_assistant_chat_allows_run_once_with_scheduled_job_permission` 和 `apps/api/tests/test_assistant_chat_service.py::test_assistant_chat_generates_run_once_draft_with_scheduled_job_permission`。 |
...
| TC-AIBRAIN-ASSISTANT-FUNC-015E | v1.1 | P0 | AI 助手 run-once 返回无运行记录的工具结果 | 当前端检测到输入包含 `@... 执行一次/立即执行/run once` 等定时作业手动运行意图，且当前用户缺少管理员、`system.admin` 或 `system.scheduled_jobs.manage` 权限时，必须在发送前展示“执行权限提示”，明确当前账号没有执行定时作业权限、本次不会直接执行，并提示所需权限；当 `assistant.scheduled_job_run` 没有 `run_id` 或 `items[]`，且 `summary.status` 为 `permission_denied`、`needs_scheduled_job_reference`、`needs_single_reference` 或 `failed` 时，前端必须展示“执行状态”卡片，显示“权限不足/未执行/失败”等状态、原始查询或作业名、所需权限或运行未创建原因，避免用户只看到普通回复而误以为命令被忽略。回归见 `apps/web/tests/AssistantPage.test.tsx::warns product users before sending scheduled job run-once commands without permission` 和 `apps/web/tests/AssistantPage.test.tsx::shows run-once non-execution status when the assistant cannot start a run`。 |
