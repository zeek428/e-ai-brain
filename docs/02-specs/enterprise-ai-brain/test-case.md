...
**状态**: 已提供可执行生产就绪门禁脚本 `scripts/production_readiness_check.py`，脚本覆盖 Docker Compose、pgvector/pgcrypto、Redis、模型网关配置脱敏和 GitLab 只读 preview/snapshot；真实目标环境仍必须执行并通过脚本，不能以本地 API/Web 单元测试替代。
