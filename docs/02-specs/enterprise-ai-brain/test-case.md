...
| 4 | 输入超过 diff 限制的 MR | 返回 `GITLAB_MR_DIFF_TOO_LARGE`，不得静默截断后继续；审计包含 `gitlab_mr.snapshot_failed`、`diff_size_bytes` 和 `diff_limit_bytes`。 |
...
4. diff 超限失败必须留下可审计指标，便于判断拆分 MR 或调整限制。
