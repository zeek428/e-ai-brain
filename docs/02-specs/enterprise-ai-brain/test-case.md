...
| TC-AIBRAIN-CONFIG-API-008B | 产品配置 PostgreSQL 结构表持久化 | P1 | MVP | Docker 本地栈以 `PERSISTENCE_MODE=postgres` 启动 | 1. 创建产品、版本、模块和 Git 资源 2. 查询 PostgreSQL 对应结构表 3. 重启 API 后重新查询接口 | `products`、`product_versions`、`product_modules`、`product_git_repositories` 有对应记录；API 重启后仍能从结构表恢复产品配置 | 是 |
...
| TC-AIBRAIN-AUTH-API-024 | MVP 角色目录和用户管理角色选择 | P1 | MVP | admin 已登录 | 1. 调用 `/api/auth/roles` 2. 进入系统管理/用户管理并打开角色目录 3. 新增用户时打开角色字段 4. 进入知识中心导入文档并打开权限角色字段 5. 尝试提交未定义角色 | 接口返回 6 个明确角色、职责、数据范围、决策范围、权限点和排序；用户管理和知识权限配置均从接口加载固定多选；后端拒绝目录外角色并返回 `VALIDATION_ERROR`；`role_definitions` 迁移脚本可重复执行 | 是 |
...
**前置条件**:
1. 用户已登录。
2. 系统至少存在产品、需求、AI 任务、知识文档和审计基础数据；Bug 管理在 v1.1 可加载真实列表或空列表，研发运营看板和用户洞察/迭代规划在 MVP 可没有业务数据。
...
| TC-AIBRAIN-AUTH-API-024 | GET /api/auth/roles + POST/PATCH /api/users | 查询角色目录并按目录维护用户角色 | 返回 `admin/product_owner/rd_owner/reviewer/knowledge_owner/viewer` 六个角色及职责、数据范围、决策范围；未知角色返回 `VALIDATION_ERROR`。 |
