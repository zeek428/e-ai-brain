...
| TC-AIBRAIN-REVIEW-FUNC-023D | 定期仓库质量/安全/规范巡检 | P0 | v1.2 | 管理员已配置产品 Git 仓库、仓库扫描插件、连接和扫描动作；插件可返回包含提交人、critical/blocker 和 minor finding 的结构化 JSON | 1. 新增定时作业时选择“代码仓库质量 / 安全 / 规范巡检”场景模板 2. 确认自动带出产品、GitHub/GitLab 巡检动作、同插件连接、Cron、来源系统和多个 `result_actions` 3. 手动运行作业 4. 查看运行详情 5. 打开运营治理 / 代码巡检页面查看报告详情 6. 点击来源运行链接 7. 使用提交人筛选报告 8. 再次运行同一扫描 9. 用授权产品范围用户查询报告 10. 使用嵌套响应和动作 `result_mapping` 再跑一次 11. 查询 Bug 列表和通知记录 | 模板创建的作业必须保存扫描插件动作、数据连接和默认 `result_actions`，包括写入巡检报告、严重问题创建 Bug 和邮件通知；扫描器 severity mapping 可把 blocker/minor 等外部等级归一为平台等级；运行时先通过插件连接获取扫描结果，再按作业 `plugin_output_mapping` 或动作 `result_mapping` 提取 `repository_id/branch/commit_sha/risk_level/summary/findings`，支持 `$` 根数组路径，然后写入 `code_inspection_reports/code_inspection_findings`；报告列表和详情返回 `scheduled_job_id`、`scheduled_job_run_id`、`plugin_connection_id`、`plugin_action_id` 和 `plugin_invocation_log_id`，详情弹窗固定展示来源链路，来源作业和来源运行是可点击链接；点击来源运行进入 `/tasks/scheduled-jobs?tab=runs&run_id=<运行 ID>` 后必须自动切换运行记录页签并打开对应运行详情；报告返回 `committer_summary` 和 `committer_count`，finding 返回提交人姓名、邮箱和用户名；达到阈值的问题创建 `source=code_inspection` Bug，Bug evidence 包含报告、finding、规则、文件、行号、提交人和 fingerprint；同一开放 finding 再次运行不重复建 Bug，运行摘要返回 `deduplicated_bug_ids`；通知动作写入邮件/钉钉机器人记录；运行摘要展示 `data_connection`、`code_inspection_report`、`bug_creation`、`notifications` 和 `result_actions` 节点；代码巡检列表展示风险级别、问题数、严重问题数、提交人并可查看 finding 明细；非全局用户只能读取授权产品范围内报告 | 是 |
