...
| 版本 | 日期 | 变更内容 | 作者 |
|------|------|----------|------|
| v1.1.395 | 2026-06-11 | 补充代码仓库定期巡检验收：定时作业扫描质量/安全/规范问题，结果写入代码审查表，严重问题自动建 Bug，通知动作记录反馈 | Codex |
...
| TC-AIBRAIN-REVIEW-FUNC-023D | 定期仓库质量/安全/规范巡检 | P0 | v1.2 | 管理员已配置产品 Git 仓库、仓库扫描插件、连接和扫描动作；插件可返回包含 critical 和 minor finding 的结构化 JSON | 1. 创建 `code_repository_inspection` 定时作业并配置多个 `result_actions` 2. 手动运行作业 3. 查看运行详情 4. 打开运营治理 / 代码审查页面查看报告详情 5. 查询 Bug 列表和通知记录 | 作业必须保存扫描插件动作和 `result_actions`；运行时先通过插件连接获取扫描结果，再写入 `code_inspection_reports/code_inspection_findings`；达到阈值的问题创建 `source=code_inspection` Bug，Bug evidence 包含报告、finding、规则、文件和行号；通知动作写入邮件/钉钉机器人记录；运行摘要展示 `data_connection`、`code_inspection_report`、`bug_creation`、`notifications` 节点；代码审查列表展示风险级别、问题数、严重问题数并可查看 finding 明细 | 是 |
