...
| v1.1.444 | 2026-06-13 | 补充插件动作模板唯一来源验收：前端和助手不得在模板缺失时硬编码生成官方动作 | Codex |
| v1.1.443 | 2026-06-13 | 补充代码巡检治理概览验收：规则统计、仓库/分支/提交人排行和严重问题 SLA 必须从真实报告聚合 | Codex |
| v1.1.442 | 2026-06-13 | 补充结果写入目标注册表验收：服务端返回目标默认映射和字段，插件动作表单按 registry 渲染 | Codex |
...
| v1.1.439 | 2026-06-13 | 补充插件动作模板唯一来源验收：模板接口返回版本，AI 助手动作草案必须携带模板 code/version 并复用模板 payload | Codex |
| v1.1.438 | 2026-06-13 | 补充插件动作动态模板验收：新增动作表单从服务端模板目录回填请求配置和结果映射 | Codex |
...
| v1.1.421 | 2026-06-12 | 补充 AI 助手插件动作草案验收：GitHub/GitLab/邮箱动作草案必须可展示并回填插件管理新增动作表单 | Codex |
| v1.1.420 | 2026-06-12 | 补充插件动作试运行审计验收：试运行成功/失败必须写 `plugin_action.trial_*` 轻量审计事件 | Codex |
...
| v1.1.402 | 2026-06-12 | 补充插件动作试运行写入预览验收：页面展示写入目标、预计写入、候选数量和样例数据 | Codex |
| v1.1.401 | 2026-06-12 | 补充代码巡检运行时映射验收：动作 `result_mapping` 必须参与巡检报告字段和 finding 提取 | Codex |
| v1.1.400 | 2026-06-12 | 补充官方插件动作模板验收：GitHub/GitLab 代码巡检模板自动生成请求配置和代码巡检报告映射 | Codex |
...
| v1.1.397 | 2026-06-11 | 补充插件动作代码巡检写入目标验收：新增动作可选择代码巡检报告并填写巡检报告 JSONPath 映射 | Codex |
...
| v1.1.387 | 2026-06-11 | 补充插件动作写入目标验收：MaxCompute 场景默认写入用户洞察表，定时作业未配置输出覆盖时复用动作 `result_mapping`，运行摘要展示 Skill 处理信息和写入目标 | Codex |
| v1.1.388 | 2026-06-11 | 补充插件动作写入目标联动验收：切换结果写入目标时，下方 JSONPath 映射字段必须按目标动态变化并同步高级 JSON | Codex |
...
| v1.1.374 | 2026-06-10 | 补充 MaxCompute 每周用户反馈洞察验收：插件动作引导配置可生成 MCP 查询 JSON，定时作业运行后将插件返回洞察写入用户反馈洞察表 | Codex |
...
| v1.1.373 | 2026-06-10 | 补充插件管理验收：插件、连接、动作和调用日志需脱敏、审计，定时作业可引用插件动作并保存插件快照和调用日志 | Codex |
...
| 编号 | 阶段 | 优先级 | 用例 | 预期 |
|------|------|--------|------|------|
| TC-AIBRAIN-ASSISTANT-FUNC-010 | v1.1 | P0 | 在 AI 助手输入框通过 `@` 搜索知识文档、插件动作、AI Agent/Skill 和定时作业 | 候选列表只展示当前用户有权读取或管理的对象，选择后显示引用 chip，并随聊天请求提交结构化 `references`。 |
...
| TC-AIBRAIN-PLANNING-FUNC-022B | MaxCompute 每周用户反馈洞察抽取 | P1 | v1.2 | 管理员已配置 MaxCompute MCP 插件、生产连接、AI 模型、Agent、每周反馈分析 Skill、可检索知识文档和用户反馈产品；结果动作可返回 `row_count`，模型可输出 `insights` | 1. 在插件动作页选择“MaxCompute 每周用户反馈”场景 2. 检查表名、时间字段、返回字段、最大行数和结果写入目标默认值 3. 新增定时作业时选择“每周用户反馈洞察抽取”场景模板 4. 确认表单自动带出产品、数据连接、AI 模型、Agent、Skills、知识引用、结果动作、Cron 和来源系统，且不展示时间参数、连接输入参数和结果写入覆盖 JSON 5. 保存后检查提交的 `plugin_input_mapping` 包含 `{{last_full_week.start}}` 和 `{{last_full_week.end}}` 6. 手动运行作业 7. 查询用户反馈洞察列表和模型日志 | 页面无需人工从零编写复杂 JSON；动作保存为 `mcp_tool` 且包含 `tool_name=maxcompute.execute_sql` 和 `result_mapping.write_target=user_feedback_insights`；定时作业模板必须保存 `plugin_connection_id/model_gateway_config_id/agent_id/skill_ids/plugin_action_id`，可选保存 `knowledge_document_ids`，并默认提交周起止动态变量映射；运行时先通过数据连接获取源数据，若配置知识引用则读取可检索知识 chunk 并以 `knowledge_references` 注入模型请求，再调用模型网关按 Agent/Skill 输出结构化 JSON，最后按动作结果映射写入用户洞察表；`records_imported` 为新增洞察数，运行摘要展示数据连接源表行数、Skill 节点 `model_gateway_called=true`、`model_log_id`、`input.knowledge_references`、模型处理输出、写入目标和结果动作反馈 | 是 |
| TC-AIBRAIN-PLANNING-FUNC-022B-1 | AI 类型定时作业强校验 | P1 | v1.2 | 管理员进入任务中心 / 定时作业新增页 | 1. 新增 `online_log_ai_analysis` 作业但不选择 Agent/Skills/AI 模型 2. 保存 3. 查看页面校验提示 4. 补齐 active Agent、Skill 和模型网关后再次保存 | AI 类型作业包括 `user_feedback_insight_extract`、`online_log_ai_analysis` 和 `iteration_plan_suggestion_generate`；前端保存前必须提示缺少 AI 模型、Agent 和 Skills，且不得提交创建请求；服务端必须按有效 `ai_generated` 校验，不允许缺少 Agent/Skill/模型网关配置的作业保存；缺少 Agent 返回 `AI_AGENT_REQUIRED`，缺少 Skill 返回 `AI_SKILL_REQUIRED`，缺少模型网关返回 `MODEL_GATEWAY_CONFIG_REQUIRED`；补齐配置后可保存并在运行快照中记录 AI 配置 | 是 |

补充验收约定：`TC-AIBRAIN-PLANNING-FUNC-022C` 的动作模板目录响应必须包含 `template_version`，AI 助手生成插件动作草案时必须携带 `template_code/template_version`，并复用同一服务端模板目录生成请求配置和结果映射。

| TC-AIBRAIN-PLANNING-FUNC-022C | 插件配置可视化与试运行 | P1 | v1.2 | 管理员已进入插件管理；存在 HTTP 或 MCP HTTP 插件、连接和执行 | 1. 新增/编辑插件，确认分类、协议、风险等级和状态可回填并保存 2. 打开插件市场，查看 MaxCompute/GitLab/GitHub/邮箱/AI 执行器官方标准插件，确认推荐场景、连接模板版本、连接默认 payload、连接 schema、执行模板、安装状态、连接数和执行数展示正确，并从 GitHub 市场项点击配置连接和创建执行 3. 查看官方标准插件，确认页面显示官方标签且无编辑/删除入口 4. 新增连接时选择 MaxCompute、GitLab、GitHub、邮箱和 AI 执行器，确认 endpoint、认证方式、Params/Headers 平台参数自动带出并可继续人工修改 5. 新增/编辑连接时通过认证字段填写 Header/Bearer 配置，并通过 Params/Headers 表格填写公共 query/header，展开高级请求 JSON 验证同步和反向应用 6. 在连接列表选择“生产”等环境筛选并检查请求参数，再调用非法环境筛选接口 7. 点击连接测试，关闭诊断弹窗后查看连接列表最近测试列 8. 调用动作模板目录接口，确认返回 MaxCompute、GitHub、GitLab、邮箱和 AI 执行器结构化模板 9. 新增/编辑 HTTP 执行时通过服务端模板或 Params/Headers 表格配置接口级参数、系统变量和结果写入目标，切换写入目标确认 JSONPath 字段联动，展开高级 JSON 验证同步，再从 JSON 应用回表格 10. 在执行配置场景中分别选择 GitHub 代码巡检、GitLab 代码巡检、邮箱通知发送、邮件收取和 AI 执行器下达指令 11. 查看请求预览 12. 对执行试运行并查看映射命中、写入预览和试运行审计 13. 新增定时作业时选择场景模板，确认页面不展示连接输入参数和输出覆盖 JSON，但提交的模板默认映射保留 `{{last_full_week.start}}` 等动态变量表达式 | 自定义插件、连接和执行新增后均能从列表进入编辑维护；插件市场只读展示 MaxCompute/GitLab/GitHub/邮箱/AI 执行器官方插件的发布方、简介、推荐场景、`connection_template_version`、`connection_defaults`、`connection_schema`、执行模板、安装状态、插件 ID、连接数和执行数，页面必须把 `connection_schema.sections[].fields[]` 渲染为可读的连接表单字段摘要，字段包含 label、path、type、required 和变量支持标记；点击“配置连接”必须打开新增连接弹窗并带出服务端 `connection_defaults` 中的官方插件、默认名称、endpoint、认证方式、Params 和 Headers，AI 助手 GitHub/GitLab/邮箱/AI 执行器连接草案也必须复用同一连接模板 payload，不得在前端或助手中私有维护默认 endpoint、认证或 Params/Headers；点击“创建执行”必须打开新增执行弹窗并优先套用服务端 `plugin-action-templates` 返回的对应模板；MaxCompute/GitLab/GitHub/邮箱/AI 执行器官方标准插件返回 `is_system=true`，页面不得提供编辑/删除入口，后端 PATCH/DELETE 返回 `409 PLUGIN_STANDARD_PLUGIN_LOCKED`；GitLab 连接默认生成 `PRIVATE-TOKEN` Header 认证和 `api_version/group_id/project_id` Params，GitHub 连接默认生成 Bearer Token、`Accept`/`X-GitHub-Api-Version` Headers 和 `owner/repo` Params，邮箱连接默认生成邮件网关/API endpoint、`Authorization` Header 认证、`Content-Type=application/json` Header 和 `send_protocol/receive_protocol/smtp_host/smtp_port/imap_host/imap_port/pop3_host/pop3_port/mailbox_folder/poll_since/default_from/default_to/subject_template` 收发参数，AI 执行器连接默认生成 `runner_polling` endpoint、`executor_type=codex`、`supported_executor_types=codex/claude/hermes/openclaw`、`runner_id`、`workspace_root`、`instruction_timeout_seconds` 和 `result_callback_url`；连接保存 `request_config.query/headers` 并参与连接测试；连接列表环境筛选必须发送 `GET /api/system/plugin-connections?environment=<env>`，仅展示对应 `environment` 的连接，后端只接受 `default/dev/test/staging/prod/sandbox`，非法环境返回 `400 VALIDATION_ERROR`；连接测试展示 endpoint、协议、认证、HTTP/MCP 调用等诊断步骤，并在请求调试台展示最终 URL、解析后的 query、headers 明文值、Header 来源、原始请求配置、动态变量解析区块、可复制 cURL、完整请求 JSON 和远端响应摘要；动态变量解析明细必须展示变量位置、原始表达式、变量 token、偏移天数、解析值、最终请求值、状态和解析时区；无动态变量时仍显示动态变量解析区块并提示未检测到动态变量；连接测试完成后服务端必须保存 `last_test_summary`，连接列表必须展示最近测试状态、耗时和错误码/失败步骤，HTTP 远端错误还应展示响应状态码摘要；认证配置里的同名认证 Header 必须覆盖 Headers 表格旧值或占位，若最终请求仍包含 `***` 必须明文展示并在诊断中标记；动作模板目录必须返回 `code/name/plugin_code/action_type/default_code/default_name/request_config/result_mapping/form_defaults`，前端新增执行场景下拉只能由目录生成，并按模板动态回填真实插件、默认连接、请求路径、Params、Headers、结果写入目标和 JSONPath 映射；服务端模板缺失时，前端和 AI 助手不得硬编码生成 MaxCompute/GitHub/GitLab/邮箱/AI 执行器官方执行，只能提示模板缺失或要求刷新模板目录；执行页面无需手写复杂 JSON 即可生成 `request_config.query/headers` 和 `result_mapping.write_target`，结果写入目标切换后下方 JSONPath 字段必须按目标变化；请求预览和试运行按“连接默认值 + 执行覆盖项”合并同名 query/header；高级 JSON 与表格双向一致；PATCH 编辑时 `auth_config/request_config` 中的脱敏占位 `***` 不得覆盖服务端真实密钥；试运行返回 `request_preview/response_summary/mapping_hits/write_preview`，页面必须展示写入目标、预计写入、候选记录、预览值、报告字段预览、邮件投递字段或样例记录，并写入 `plugin_action.trial_succeeded/failed` 审计事件，payload 只包含插件、执行、连接、连接环境、输入字段名、写入目标、状态、耗时和错误码，不得保存完整请求响应、输入 payload 或密钥；定时作业保存的 `plugin_input_mapping` 保留动态变量表达式，输出映射为空时复用执行结果映射 | 是 |
补充验收约定：`TC-AIBRAIN-PLANNING-FUNC-022C` 中旧的“服务端模板不可用时使用本地兼容兜底”表述废止；官方执行模板必须以 `/api/system/plugin-action-templates` 为唯一配置源，模板缺失时前端和 AI 助手只能提示缺失，不得硬编码生成 MaxCompute/GitHub/GitLab/邮箱执行。
| TC-AIBRAIN-PLANNING-FUNC-022D | 定时作业维护入口 | P1 | v1.2 | 管理员已进入任务中心 / 定时作业；存在至少一个绑定数据连接、Skill 和结果动作的作业 | 1. 在作业配置列表点击编辑 2. 检查作业名称、产品、Agent、Skills、数据连接、结果动作、连接输入参数和调度字段回填 3. 修改名称或启停状态并保存 4. 在作业列表点击复制 5. 确认新增草稿回填原作业配置并保存 6. 再次打开列表点击删除 7. 确认后刷新列表并检查列表刷新 | 列表每行必须提供编辑、复制、运行、删除操作，并在存在 `config_json.template_source` 时显示来源作业或运行快照；编辑弹窗复用新增表单并回填现有配置，保存调用 PATCH 且 `plugin_input_mapping` 继续保留动态变量表达式；复制只打开新增作业草稿，不直接写库，复制弹窗必须显示来源作业，确认保存后新作业 `config_json.template_source.source_type=scheduled_job`、`source_id=<原作业 ID>`，并保留原作业 `plugin_input_mapping/plugin_output_mapping/result_actions`；删除调用 `DELETE /api/system/scheduled-jobs/{job_id}`，作业定义从列表移除并记录 `scheduled_job.deleted` 审计 | 是 |
...
| TC-AIBRAIN-PLANNING-FUNC-022F | 定时作业运行结果详情 | P1 | v1.2 | 管理员已进入任务中心 / 定时作业；存在一次用户反馈洞察抽取运行记录，运行记录包含 `result_summary.execution_nodes`、插件调用 ID、模型日志 ID 和 Skill 快照 | 1. 打开运行记录页签 2. 点击运行记录详情 3. 查看三段式执行链路卡片 4. 查看数据连接获取内容、经过 Skill 处理后的内容、结果写入反馈内容、结果写入记录、结果摘要、插件快照、Skill 快照、Prompt 快照和作业配置快照 5. 手动触发一次作业 6. 在运行记录行点击复跑并查看复跑对比 7. 在运行记录行或详情弹窗点击复制配置 8. 通过接口传入非法 `trigger_type` 和非法 `source_run_id` 9. 配置邮件通知插件执行并运行 `plugin_action_invoke` 作业 10. 查询 `GET /api/system/result-write-records?scheduled_job_run_id=<run_id>` 11. 查询 `scheduled_job_run.succeeded/failed` 与 `scheduled_job.created` 审计事件 | 运行记录每行必须提供详情、复跑和复制配置入口；详情弹窗展示运行状态、作业类型、执行模式、AI 模型、Agent、Skills、导入数、复跑来源、模板来源、插件调用 ID、错误码/错误信息，并优先以三段式执行链路卡片展示 `data_connection`、`skill_processing` 和 `result_action` 摘要；代码巡检创建整改任务时还必须展示 `task_creation` 节点和 `created_task_ids`；原始 JSON 区域继续展示 `result_summary.execution_nodes.data_connection`、`skill_processing`、`result_action`、`task_creation`、`result_summary`、`resolved_plugin_snapshot`、`resolved_skill_snapshots`、`resolved_prompt_snapshot` 和 `config_snapshot`；数据连接节点必须可看到连接 ID、连接环境、请求方法、请求 URL、HTTP 状态、耗时、请求摘要、输入映射、响应摘要和源数据数量；Skill 节点必须展示处理输入/输出、知识引用数量、候选结果数量、`model_gateway_called=true`、`processing_mode=model_gateway_json_transform` 和 `model_log_id`；结果写入节点必须展示写入目标、写入数量、生成 ID、报告 ID、Bug/任务/通知数量和反馈内容；结果写入记录必须在运行详情中按当前 `scheduled_job_run_id` 查询并展示写入目标、状态、写入数、插件调用日志、摘要字段、写入预览和执行反馈，不得放在插件管理配置页；邮件通知写入目标必须显示“邮件通知记录”、投递 ID、投递状态和收件人，运行摘要需保留 `write_preview`；手动触发和复跑时按钮进入执行中状态并禁用重复触发，触发完成后自动打开本次运行详情，便于判断取数成功、AI 处理结果、生成洞察数量、写入目标和失败原因；普通手动触发保存 `trigger_type=manual`，复跑使用历史运行的 `scheduled_job_id` 重新触发当前作业配置并保存 `trigger_type=manual_rerun` 与 `source_run_id`，不改写旧运行记录；复跑创建响应和运行列表必须返回轻量 `source_run_summary`，详情页展示“复跑对比”，包含来源运行、来源触发、来源状态、本次状态、来源导入数、本次导入数、来源错误码和来源耗时；复制配置只打开新增作业草稿，优先使用运行 `config_snapshot`，缺失字段回退来源作业，复制弹窗必须显示来源运行快照，确认保存后 `config_json.template_source.source_type=scheduled_job_run` 且 `source_id=<运行 ID>`，审计 payload 输出 `template_source`，后续作业列表和运行详情可见模板来源；服务端只接受 `manual/manual_rerun/scheduler`，非法触发类型返回 `400 VALIDATION_ERROR`，带 `source_run_id` 时必须校验来源运行存在、属于同一作业且触发方式为 `manual_rerun`；`dashboard_snapshot_refresh`、`lifecycle_context_refresh`、`plugin_action_invoke`、`pending_attribution_retry` 和 `code_repository_inspection` 运行创建 collector run 时不得因 `collector_type` 数据库约束失败返回 500；终态审计 payload 必须包含 `scheduled_job_id/job_type/product_id/status/trigger_type/records_imported/collector_run_id`，复跑时必须包含 `source_run_id`，存在插件调用时还必须包含 `plugin_invocation_log_id`，失败时包含 `error_code`；插件+AI+知识链路还必须记录 `execution_mode/agent_id/skill_ids/model_gateway_config_id/model_gateway_called/knowledge_document_ids/plugin_code/plugin_action_id/plugin_action_code/plugin_connection_id/plugin_connection_environment/result_write_target` 等轻量上下文，用于追踪生产/测试连接、AI 装配和结果写入目标，但不得保存完整请求响应、Prompt、模型输出或密钥 | 是 |
...
| TC-AIBRAIN-PLANNING-FUNC-022H | 定时作业配置编排预览 | P1 | v1.2 | 管理员已进入任务中心 / 定时作业；存在 MaxCompute 数据连接、AI 模型、Agent、Skill、知识文档和结果动作 | 1. 打开新增作业弹窗 2. 查看“任务编排流程” 3. 选择“每周用户反馈洞察抽取”场景模板 4. 点击数据连接节点的“测试数据连接” 5. 编辑已有作业并查看同一编排视图 | 新增/编辑弹窗必须展示“数据连接 → AI 处理 → 知识引用 → 结果动作”四段式编排视图；未配置时数据连接、AI 处理和结果动作节点显示待配置，知识引用显示可选；模板回填后节点显示已配置/已选择，并展示生产 MaxCompute 项目、定时作业模型、已选知识和获取本周用户反馈数据等核心资源名称；点击“测试数据连接”必须调用 `POST /api/system/plugin-connections/{connection_id}/test`，按钮进入测试中状态，完成后在数据连接节点展示连接测试状态、耗时和请求摘要；编排视图仅作为配置预览和测试入口，不向保存 payload 增加额外字段 | 是 |
...
| TC-AIBRAIN-PLANNING-FUNC-022J | 结果写入目标注册表与结果记录查看 | P1 | v1.2 | 管理员已进入任务中心；服务端已启用 `scheduled_job_result`、`user_feedback_insights`、`code_inspection_reports` 和 `email_notifications` 写入目标；存在一次邮件通知正式运行记录 | 1. 调用 `GET /api/system/result-write-targets` 2. 打开插件动作新增弹窗 3. 切换不同结果写入目标 4. 选择官方动作模板并检查回填 5. 试运行动作并查看写入预览 6. 调用 `GET /api/system/result-write-records?scheduled_job_run_id=<run_id>` 7. 打开任务中心 / 定时作业 / 运行记录详情 8. 查看结果写入记录 9. 进入插件管理确认不再出现“结果记录”页签 | 目标注册表接口必须返回目标 `code/label/form_label/default_result_mapping/mapping_fields`；插件动作表单下拉使用服务端 `form_label`，切换目标时使用 `default_result_mapping` 回填高级 JSON，并按 `mapping_fields` 渲染 JSONPath 字段；用户洞察表、代码巡检报告、邮件通知记录字段必须与注册表一致；官方动作模板的 `result_mapping` 必须复用注册表默认映射；试运行 `write_preview.write_target_label` 必须来自同一注册表，避免前端、模板和预览三处硬编码不一致；结果记录接口必须返回通用 `write_target/write_target_label/status/source_type/scheduled_job_run_id/plugin_action_id/plugin_invocation_log_id/records_imported/summary_fields/preview/feedback` 字段，并支持 `scheduled_job_run_id` 精确查询单次运行；定时作业运行详情必须展示邮件通知主题、投递状态、消息 ID、收件人摘要、写入预览和执行反馈；未知未来目标也必须能保留通用预览和详情 JSON，不要求新增目标专属页面；插件管理只负责配置和调用日志，不展示定时作业运行结果记录 | 是 |
| TC-AIBRAIN-PLANNING-FUNC-022K | 定时作业运行可观测性 | P1 | v1.2 | 管理员已进入任务中心 / 定时作业；存在成功、失败、含模型日志和含结果动作的运行记录 | 1. 调用 `GET /api/system/scheduled-job-runs/observability` 2. 打开运行记录页签 3. 查看运行健康概览 4. 查看最近失败和慢运行表格 5. 点击运行详情继续下钻三段式执行链路 | 接口必须返回总运行数、成功/失败/运行中/取消数量、成功率、失败率、平均耗时、平均导入数、AI 调用次数、模型 Token 总量、插件调用次数、动作写入次数、动作写入成功率、状态分布、作业类型分布、触发方式分布、写入目标分布、失败原因、最近失败和慢运行；Token 汇总只读取 `skill_processing.model_log_id` 对应模型日志的 tokens 元数据，不返回完整 Prompt 或模型输出；运行记录页签顶部必须展示总运行数、成功率、失败率、平均耗时、AI 调用次数、Token 总量、插件调用次数和动作写入成功率，并列出最近失败、失败原因和慢运行，用户可继续从列表打开运行详情查看数据连接、AI/Skill 处理和结果动作三段节点。回归见 `apps/api/tests/test_scheduled_ai_jobs.py::test_plugin_action_scheduled_job_run_records_request_trace_summary` 和 `apps/web/tests/ScheduledJobsPage.test.tsx` | 是 |
| TC-AIBRAIN-PLANNING-FUNC-022L | 定时作业模板市场 | P1 | v1.2 | 管理员已进入任务中心 / 定时作业；系统已有周反馈洞察、代码巡检、邮件摘要、GitLab MR 审查和 AI 执行器官方模板 | 1. 调用 `GET /api/system/scheduled-job-templates` 2. 打开新增作业弹窗 3. 查看作业模板下拉和任务创建向导 4. 选择每周用户反馈洞察模板 5. 选择代码仓库质量 / 安全 / 规范巡检模板 6. 让 AI 助手生成同类定时作业草案 | 模板接口必须只允许管理员访问，并返回模板 `code/name/category/description/publisher/recommended_scenarios/payload_defaults/resource_selectors/template_version/available_resource_counts/wizard_steps`；`wizard_steps` 至少包含数据连接、AI 处理、知识引用、结果写入和调度；周反馈洞察模板 payload 必须包含 `job_type=user_feedback_insight_extract`、`execution_mode=ai_generated`、`cron_expression=0 9 * * MON` 和 `plugin_input_mapping.week_start/week_end` 动态变量；代码巡检模板 payload 必须包含 `job_type=code_repository_inspection`、`cron_expression=0 2 * * MON` 和写巡检报告、严重问题建 Bug、严重问题建整改任务、发送通知四类 `result_actions`；前端作业模板下拉必须由该接口渲染，选择模板时按 `wizard_steps` 展示向导，按 `payload_defaults` 回填作业字段，按 `resource_selectors` 选择默认产品、插件执行、同插件连接和 AI 资源；AI 助手定时作业草案也必须复用同一模板目录默认 payload，避免页面和助手各自维护作业模板。回归见 `apps/api/tests/test_scheduled_ai_jobs.py::test_scheduled_job_templates_are_admin_managed_and_versioned` 和 `apps/web/tests/ScheduledJobsPage.test.tsx::creates scheduled jobs from scene templates` | 是 |
