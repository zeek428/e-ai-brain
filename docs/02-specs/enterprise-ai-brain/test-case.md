...
| TC-AIBRAIN-KNOWLEDGE-FUNC-012B | 知识空间目录和资产导入 | P0 | MVP-C | 知识维护者已登录，系统启用 PostgreSQL，配置私有 MinIO/S3-compatible 对象存储 | 1. 创建知识空间 2. 创建目录 3. 上传 Markdown 文件 4. 查询文档列表、检索和资产预览 5. 使用非空间成员重试列表、检索和预览 | 新空间、目录、资产、导入任务、chunk set、文档和 chunk 均写入结构表；原始文件写入对象存储但事实源仍在 PostgreSQL；文档列表和检索可按 `knowledge_space_id`、`folder_id` 过滤，结果包含 `source_asset_id`、`active_chunk_set_id`、`asset_id` 和 `chunk_set_id`；非成员不能读取空间文档或资产预览；页面支持空间筛选、目录选择、新建空间/目录和文件上传。后端回归见 `apps/api/tests/test_knowledge_space_assets.py`、`apps/api/tests/test_knowledge_management_foundation.py` 和仓储边界测试；前端回归见 `apps/web/tests/KnowledgePage.test.tsx` | 是 |
