...
| TC-AIBRAIN-REVIEW-FUNC-023D | 定期仓库质量/安全/规范巡检 | P0 | v1.2 | 管理员已配置产品 Git 仓库、仓库扫描插件、连接和扫描动作；插件可返回包含提交人、critical/blocker 和 minor finding 的结构化 JSON | 1. 创建 `code_repository_inspection` 定时作业并配置多个 `result_actions` 2. 手动运行作业 3. 查看运行详情 4. 打开运营治理 / 代码巡检页面查看报告详情 5. 使用提交人筛选报告 6. 再次运行同一扫描 7. 用授权产品范围用户查询报告 8. 查询 Bug 列表和通知记录 | 作业必须保存扫描插件动作和 `result_actions`；扫描器 severity mapping 可把 blocker/minor 等外部等级归一为平台等级；运行时先通过插件连接获取扫描结果，再写入 `code_inspection_reports/code_inspection_findings`；报告返回 `committer_summary` 和 `committer_count`，finding 返回提交人姓名、邮箱和用户名；达到阈值的问题创建 `source=code_inspection` Bug，Bug evidence 包含报告、finding、规则、文件、行号、提交人和 fingerprint；同一开放 finding 再次运行不重复建 Bug，运行摘要返回 `deduplicated_bug_ids`；通知动作写入邮件/钉钉机器人记录；运行摘要展示 `data_connection`、`code_inspection_report`、`bug_creation`、`notifications` 和 `result_actions` 节点；代码巡检列表展示风险级别、问题数、严重问题数、提交人并可查看 finding 明细；非全局用户只能读取授权产品范围内报告 | 是 |
