...
| TC-AIBRAIN-TASK-API-001B | AI 任务 PostgreSQL 结构表持久化 | P1 | MVP | Docker 本地栈以 `PERSISTENCE_MODE=postgres` 启动，存在已审批需求 | 1. 从需求生成产品详细设计任务 2. 查询 PostgreSQL `ai_tasks` 表 3. 启动或取消任务使状态变化 4. 重启 API 后重新查询任务列表 | `ai_tasks` 保存任务类型、标题、状态、需求快照、产品上下文、输入输出、当前步骤和创建人；API 重启后仍能从结构表恢复任务和 `task` 计数器，Review/Graph 临时关联由快照兼容层保留直到对应主体完成结构化迁移 | 是 |
