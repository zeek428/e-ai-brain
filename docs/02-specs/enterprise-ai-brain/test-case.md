...
| TC-AIBRAIN-ASSISTANT-FUNC-015C | v1.1 | P0 | 具备 `system.scheduled_jobs.manage` 但不是 admin 角色的用户在 AI 助手输入 `@提取每周用户反馈有价值信息 执行一次` | @ 候选和引用解析必须允许该用户看到并引用 `scheduled_job` / `scheduled_job_run`，聊天执行链路也允许手动运行；该权限不得顺带开放插件连接、插件动作、AI角色或 Skill 候选，仍需分别具备 `system.plugins.manage` 或 `system.ai_capabilities.manage`。回归见 `apps/api/tests/test_assistant_chat.py::test_ai_assistant_reference_candidates_allow_scheduled_job_managers` 和 `apps/api/tests/test_assistant_chat_service.py::test_assistant_chat_allows_run_once_with_scheduled_job_permission`。 |
