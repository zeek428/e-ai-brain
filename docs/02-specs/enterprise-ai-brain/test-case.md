...
| TC-AIBRAIN-ASSISTANT-FUNC-022B | v1.1 | P2 | 不同角色进入 AI 助手工作台 | 前端按当前用户角色展示角色快捷任务：`product_owner` 展示产品快捷任务（需求进展、反馈洞察、版本风险），`rd_owner` 展示研发快捷任务（任务阻塞、代码巡检、缺陷修复），`reviewer/test_owner/tester/release_owner` 展示测试快捷任务（测试缺陷、自动化测试、发布风险），`admin` 展示管理员快捷任务（插件连接、AI能力、定时作业、运行失败）；后端助手 API 访问白名单必须同步允许 `test_owner/tester/release_owner` 访问 `/api/assistant/conversations` 与 `/api/assistant/chat`，这些角色发送“我要新增任务”应返回任务类型向导且不调用模型网关；高权限运维候选和结构化引用解析仍只允许管理员或 `system.admin` 权限用户；显式 `@定时作业名称 执行一次` 可由 `system.scheduled_jobs.manage` 权限用户触发，缺少权限时返回 `permission_denied`；点击任一快捷任务只回填输入框，不自动发送、不创建草案、不触发外部动作；无匹配角色时不展示角色任务组。回归见 `apps/api/tests/test_assistant_chat.py::test_ai_assistant_allows_testing_delivery_roles_to_use_workbench_apis`、`apps/web/tests/AssistantPage.test.tsx::shows admin role quick tasks and fills the run-failure prompt`、`apps/web/tests/AssistantPage.test.tsx::shows engineering quick tasks for rd_owner users` 和 `apps/web/tests/AssistantPage.test.tsx::shows testing quick tasks for test_owner users`。 |
