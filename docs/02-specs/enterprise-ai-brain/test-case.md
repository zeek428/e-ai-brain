...
| TC-AIBRAIN-PLANNING-FUNC-022B-1 | AI 类型定时作业强校验 | P1 | v1.2 | 管理员进入任务中心 / 定时作业新增页 | 1. 新增 `online_log_ai_analysis` 作业但不选择 AI角色/Skills/AI 模型 2. 保存 3. 查看页面校验提示 4. 补齐 active AI角色、Skill 和模型网关后再次保存 | AI 类型作业包括 `user_feedback_insight_extract`、`online_log_ai_analysis` 和 `iteration_plan_suggestion_generate`；前端保存前必须提示缺少 AI 模型、AI角色 和 Skills，且不得提交创建请求；服务端必须按有效 `ai_generated` 校验，不允许缺少 AI角色/Skill/模型网关配置的作业保存；缺少 AI角色 返回 `AI_AGENT_REQUIRED`，缺少 Skill 返回 `AI_SKILL_REQUIRED`，缺少模型网关返回 `MODEL_GATEWAY_CONFIG_REQUIRED`；补齐配置后可保存并在运行快照中记录 AI 配置 | 是 |
