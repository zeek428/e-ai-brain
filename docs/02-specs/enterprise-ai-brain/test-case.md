...
| TC-AIBRAIN-PLANNING-FUNC-022C | 插件配置可视化与试运行 | P1 | v1.2 | 管理员已进入插件管理；存在 HTTP 或 MCP HTTP 插件、连接和动作 | 1. 新增/编辑插件，确认分类、协议、风险等级和状态可回填并保存 2. 新增/编辑连接时通过认证字段填写 Header/Bearer 配置，并通过 Params/Headers 表格填写公共 query/header，展开高级请求 JSON 验证同步和反向应用 3. 点击连接测试 4. 新增/编辑 HTTP 动作时通过 Params/Headers 表格配置接口级参数、系统变量和结果写入目标，切换写入目标确认 JSONPath 字段联动，展开高级 JSON 验证同步，再从 JSON 应用回表格 5. 查看请求预览 6. 对动作执行试运行并查看映射命中 7. 新增定时作业时用连接输入参数表格选择 `{{current_date-7}}` 等变量，并展开高级 JSON 验证同步 | 插件、连接和动作新增后均能从列表进入编辑维护；连接保存 `request_config.query/headers` 并参与连接测试；动作页面无需手写复杂 JSON 即可生成 `request_config.query/headers` 和 `result_mapping.write_target`，结果写入目标切换后下方 JSONPath 字段必须按目标变化，用户洞察表目标展示洞察列表、源表行数和原始行列表路径；请求预览和试运行按“连接默认值 + 动作覆盖项”合并同名 query/header；连接测试展示 endpoint、协议、认证、HTTP/MCP 调用等诊断步骤、最终 URL、解析后的 query、脱敏 headers、Header 来源和远端响应摘要；认证配置里的同名认证 Header 必须覆盖 Headers 表格旧值或占位，若最终请求仍包含 `***` 必须拦截并提示重新填写；高级 JSON 与表格双向一致；PATCH 编辑时 `auth_config/request_config` 中的脱敏占位 `***` 不得覆盖服务端真实密钥；试运行返回 `request_preview/response_summary/mapping_hits`；定时作业保存的 `plugin_input_mapping` 保留动态变量表达式，输出映射为空时复用动作结果映射 | 是 |
