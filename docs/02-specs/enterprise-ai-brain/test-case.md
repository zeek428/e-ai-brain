...
| TC-AIBRAIN-ASSISTANT-FUNC-013 | v1.1 | P0 | 用户要求创建 AI 能力、插件动作或定时作业 | 系统只生成 `assistant_action_draft` 草案和风险/差异摘要；未确认前不得写入 `ai_skills`、`ai_agents`、`plugin_actions` 或 `scheduled_jobs`。 |
...
| TC-AIBRAIN-PLANNING-FUNC-022B | MaxCompute 每周用户反馈洞察抽取 | P1 | v1.2 | 管理员已配置 MaxCompute MCP 插件、生产连接、AI 模型、Agent、每周反馈分析 Skill、可检索知识文档和用户反馈产品；结果动作可返回 `row_count`，模型可输出 `insights` | 1. 在插件动作页选择“MaxCompute 每周用户反馈”场景 2. 检查表名、时间字段、返回字段、最大行数和结果写入目标默认值 3. 创建 `user_feedback_insight_extract` 定时作业，确认表单顺序为数据连接、AI 模型、Agent、Skills、知识引用、结果动作，且不展示时间参数、连接输入参数和结果写入覆盖 JSON 4. 手动运行作业 5. 查询用户反馈洞察列表和模型日志 | 页面无需人工从零编写复杂 JSON；动作保存为 `mcp_tool` 且包含 `tool_name=maxcompute.execute_sql` 和 `result_mapping.write_target=user_feedback_insights`；定时作业必须保存 `plugin_connection_id/model_gateway_config_id/agent_id/skill_ids/plugin_action_id`，并可选保存 `knowledge_document_ids`；运行时先通过数据连接获取源数据，若配置知识引用则读取可检索知识 chunk 并以 `knowledge_references` 注入模型请求，再调用模型网关按 Agent/Skill 输出结构化 JSON，最后按动作结果映射写入用户洞察表；`records_imported` 为新增洞察数，运行摘要展示数据连接源表行数、Skill 节点 `model_gateway_called=true`、`model_log_id`、`input.knowledge_references`、模型处理输出、写入目标和结果动作反馈 | 是 |
| TC-AIBRAIN-PLANNING-FUNC-022B-1 | AI 类型定时作业强校验 | P1 | v1.2 | 管理员进入任务中心 / 定时作业新增页 | 1. 新增 `online_log_ai_analysis` 作业但不选择 Agent/Skills/AI 模型 2. 保存 3. 补齐 active Agent、Skill 和模型网关后再次保存 | AI 类型作业包括 `user_feedback_insight_extract`、`online_log_ai_analysis` 和 `iteration_plan_suggestion_generate`；服务端必须按有效 `ai_generated` 校验，不允许缺少 Agent/Skill/模型网关配置的作业保存；缺少 Agent 返回 `AI_AGENT_REQUIRED`，缺少 Skill 返回 `AI_SKILL_REQUIRED`，缺少模型网关返回 `MODEL_GATEWAY_CONFIG_REQUIRED`；补齐配置后可保存并在运行快照中记录 AI 配置 | 是 |
| TC-AIBRAIN-PLANNING-FUNC-022C | 插件配置可视化与试运行 | P1 | v1.2 | 管理员已进入插件管理；存在 HTTP 或 MCP HTTP 插件、连接和动作 | 1. 新增/编辑插件，确认分类、协议、风险等级和状态可回填并保存 2. 查看 GitLab/GitHub/邮箱官方标准插件，确认页面显示官方标签且无编辑/删除入口 3. 新增连接时选择 GitLab、GitHub 和邮箱，确认 endpoint、认证方式、Params/Headers 平台参数自动带出并可继续人工修改 4. 新增/编辑连接时通过认证字段填写 Header/Bearer 配置，并通过 Params/Headers 表格填写公共 query/header，展开高级请求 JSON 验证同步和反向应用 5. 点击连接测试 6. 新增/编辑 HTTP 动作时通过 Params/Headers 表格配置接口级参数、系统变量和结果写入目标，切换写入目标确认 JSONPath 字段联动，展开高级 JSON 验证同步，再从 JSON 应用回表格 7. 在动作配置场景中分别选择 GitHub 代码巡检、GitLab 代码巡检和邮箱通知发送 8. 查看请求预览 9. 对动作执行试运行并查看映射命中和写入预览 10. 新增定时作业时用连接输入参数表格选择 `{{current_date-7}}` 等变量，并展开高级 JSON 验证同步 | 自定义插件、连接和动作新增后均能从列表进入编辑维护；GitLab/GitHub/邮箱官方标准插件返回 `is_system=true`，页面不得提供编辑/删除入口，后端 PATCH/DELETE 返回 `409 PLUGIN_STANDARD_PLUGIN_LOCKED`；GitLab 连接默认生成 `PRIVATE-TOKEN` Header 认证和 `api_version/group_id/project_id` Params，GitHub 连接默认生成 Bearer Token、`Accept`/`X-GitHub-Api-Version` Headers 和 `owner/repo` Params，邮箱连接默认生成邮件网关/API endpoint、`Authorization` Header 认证、`Content-Type=application/json` Header 和 `mail_provider/default_from/default_to/subject_template` Params；连接保存 `request_config.query/headers` 并参与连接测试；动作页面无需手写复杂 JSON 即可生成 `request_config.query/headers` 和 `result_mapping.write_target`，结果写入目标切换后下方 JSONPath 字段必须按目标变化，用户洞察表目标展示洞察列表、源表行数和原始行列表路径，代码巡检报告目标展示仓库 ID、分支、提交 SHA、风险级别、摘要和 finding 列表路径；GitHub 代码巡检模板必须自动选择 GitHub 官方插件、填充 `/repos/{{owner}}/{{repo}}/code-scanning/alerts`、`state=open` 和代码巡检报告映射；GitLab 代码巡检模板必须自动选择 GitLab 官方插件、填充 `/api/{{api_version}}/projects/{{project_id}}/vulnerability_findings`、`state=detected` 和代码巡检报告映射；邮箱通知发送模板必须自动选择邮箱官方插件和可用邮箱连接，填充 `POST /messages/send`、`Content-Type=application/json`、`to/subject_template/body_template` 参数和 `scheduled_job_result` 映射；请求预览和试运行按“连接默认值 + 动作覆盖项”合并同名 query/header；连接测试展示 endpoint、协议、认证、HTTP/MCP 调用等诊断步骤，并在请求调试台展示最终 URL、解析后的 query、headers 明文值、Header 来源、完整请求 JSON 和远端响应摘要；认证配置里的同名认证 Header 必须覆盖 Headers 表格旧值或占位，若最终请求仍包含 `***` 必须明文展示并在诊断中标记；高级 JSON 与表格双向一致；PATCH 编辑时 `auth_config/request_config` 中的脱敏占位 `***` 不得覆盖服务端真实密钥；试运行返回 `request_preview/response_summary/mapping_hits/write_preview`，页面必须展示写入目标、预计写入、候选记录、预览值、报告字段预览或样例记录；定时作业保存的 `plugin_input_mapping` 保留动态变量表达式，输出映射为空时复用动作结果映射 | 是 |
| TC-AIBRAIN-PLANNING-FUNC-022D | 定时作业维护入口 | P1 | v1.2 | 管理员已进入任务中心 / 定时作业；存在至少一个绑定数据连接、Skill 和结果动作的作业 | 1. 在作业配置列表点击编辑 2. 检查作业名称、产品、Agent、Skills、数据连接、结果动作、连接输入参数和调度字段回填 3. 修改名称或启停状态并保存 4. 再次打开列表点击删除 5. 确认后刷新列表并检查列表刷新 | 列表每行必须提供编辑、运行、删除操作；编辑弹窗复用新增表单并回填现有配置，保存调用 PATCH 且 `plugin_input_mapping` 继续保留动态变量表达式；删除调用 `DELETE /api/system/scheduled-jobs/{job_id}`，作业定义从列表移除并记录 `scheduled_job.deleted` 审计 | 是 |
