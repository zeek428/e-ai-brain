...
| TC-AIBRAIN-PLANNING-FUNC-022C | 插件配置可视化与试运行 | P1 | v1.2 | 管理员已进入插件管理；存在 HTTP 或 MCP HTTP 插件、连接和动作 | 1. 新增/编辑插件，确认分类、协议、风险等级和状态可回填并保存 2. 打开插件市场，查看 GitLab/GitHub/邮箱官方标准插件，确认推荐场景、连接模板版本、连接默认 payload、动作模板、安装状态、连接数和动作数展示正确，并从 GitHub 市场项点击配置连接和创建动作 3. 查看 GitLab/GitHub/邮箱官方标准插件，确认页面显示官方标签且无编辑/删除入口 4. 新增连接时选择 GitLab、GitHub 和邮箱，确认 endpoint、认证方式、Params/Headers 平台参数自动带出并可继续人工修改 5. 新增/编辑连接时通过认证字段填写 Header/Bearer 配置，并通过 Params/Headers 表格填写公共 query/header，展开高级请求 JSON 验证同步和反向应用 6. 在连接列表选择“生产”等环境筛选并检查请求参数，再调用非法环境筛选接口 7. 点击连接测试，关闭诊断弹窗后查看连接列表最近测试列 8. 调用动作模板目录接口，确认返回 MaxCompute、GitHub、GitLab 和邮箱结构化模板 9. 新增/编辑 HTTP 动作时通过服务端模板或 Params/Headers 表格配置接口级参数、系统变量和结果写入目标，切换写入目标确认 JSONPath 字段联动，展开高级 JSON 验证同步，再从 JSON 应用回表格 10. 在动作配置场景中分别选择 GitHub 代码巡检、GitLab 代码巡检和邮箱通知发送 11. 查看请求预览 12. 对动作执行试运行并查看映射命中、写入预览和试运行审计 13. 新增定时作业时选择场景模板，确认页面不展示连接输入参数和输出覆盖 JSON，但提交的模板默认映射保留 `{{last_full_week.start}}` 等动态变量表达式 | 自定义插件、连接和动作新增后均能从列表进入编辑维护；插件市场只读展示 GitLab/GitHub/邮箱官方插件的发布方、简介、推荐场景、`connection_template_version`、`connection_defaults`、动作模板、安装状态、插件 ID、连接数和动作数，点击“配置连接”必须打开新增连接弹窗并带出服务端 `connection_defaults` 中的官方插件、默认名称、endpoint、认证方式、Params 和 Headers，AI 助手 GitHub/GitLab/邮箱连接草案也必须复用同一连接模板 payload，不得在前端或助手中私有维护默认 endpoint、认证或 Params/Headers；点击“创建动作”必须打开新增动作弹窗并优先套用服务端 `plugin-action-templates` 返回的对应模板；GitLab/GitHub/邮箱官方标准插件返回 `is_system=true`，页面不得提供编辑/删除入口，后端 PATCH/DELETE 返回 `409 PLUGIN_STANDARD_PLUGIN_LOCKED`；GitLab 连接默认生成 `PRIVATE-TOKEN` Header 认证和 `api_version/group_id/project_id` Params，GitHub 连接默认生成 Bearer Token、`Accept`/`X-GitHub-Api-Version` Headers 和 `owner/repo` Params，邮箱连接默认生成邮件网关/API endpoint、`Authorization` Header 认证、`Content-Type=application/json` Header 和 `mail_provider/default_from/default_to/subject_template` Params；连接保存 `request_config.query/headers` 并参与连接测试；连接列表环境筛选必须发送 `GET /api/system/plugin-connections?environment=<env>`，仅展示对应 `environment` 的连接，后端只接受 `default/dev/test/staging/prod/sandbox`，非法环境返回 `400 VALIDATION_ERROR`；连接测试展示 endpoint、协议、认证、HTTP/MCP 调用等诊断步骤，并在请求调试台展示最终 URL、解析后的 query、headers 明文值、Header 来源、原始请求配置、动态变量解析区块、可复制 cURL、完整请求 JSON 和远端响应摘要；动态变量解析明细必须展示变量位置、原始表达式、变量 token、偏移天数、解析值、最终请求值、状态和解析时区；无动态变量时仍显示动态变量解析区块并提示未检测到动态变量；连接测试完成后服务端必须保存 `last_test_summary`，连接列表必须展示最近测试状态、耗时和错误码/失败步骤，HTTP 远端错误还应展示响应状态码摘要；认证配置里的同名认证 Header 必须覆盖 Headers 表格旧值或占位，若最终请求仍包含 `***` 必须明文展示并在诊断中标记；动作模板目录必须返回 `code/name/plugin_code/action_type/default_code/default_name/request_config/result_mapping/form_defaults`，前端新增动作场景下拉只能由目录生成，并按模板动态回填真实插件、默认连接、请求路径、Params、Headers、结果写入目标和 JSONPath 映射；服务端模板缺失时，前端和 AI 助手不得硬编码生成 MaxCompute/GitHub/GitLab/邮箱官方动作，只能提示模板缺失或要求刷新模板目录；动作页面无需手写复杂 JSON 即可生成 `request_config.query/headers` 和 `result_mapping.write_target`，结果写入目标切换后下方 JSONPath 字段必须按目标变化，用户洞察表目标展示洞察列表、源表行数和原始行列表路径，代码巡检报告目标展示仓库 ID、分支、提交 SHA、风险级别、摘要和 finding 列表路径，邮件通知记录目标展示收件人、主题、投递状态和消息 ID 路径；GitHub 代码巡检模板必须自动选择 GitHub 官方插件、填充 `/repos/{{owner}}/{{repo}}/code-scanning/alerts`、`state=open` 和代码巡检报告映射；GitLab 代码巡检模板必须自动选择 GitLab 官方插件、填充 `/api/{{api_version}}/projects/{{project_id}}/vulnerability_findings`、`state=detected` 和代码巡检报告映射；邮箱通知发送模板必须自动选择邮箱官方插件和可用邮箱连接，填充 `POST /messages/send`、`Content-Type=application/json`、`to/subject_template/body_template` 参数和 `email_notifications` 映射；请求预览和试运行按“连接默认值 + 动作覆盖项”合并同名 query/header；高级 JSON 与表格双向一致；PATCH 编辑时 `auth_config/request_config` 中的脱敏占位 `***` 不得覆盖服务端真实密钥；试运行返回 `request_preview/response_summary/mapping_hits/write_preview`，页面必须展示写入目标、预计写入、候选记录、预览值、报告字段预览、邮件投递字段或样例记录，并写入 `plugin_action.trial_succeeded/failed` 审计事件，payload 只包含插件、动作、连接、连接环境、输入字段名、写入目标、状态、耗时和错误码，不得保存完整请求响应、输入 payload 或密钥；定时作业保存的 `plugin_input_mapping` 保留动态变量表达式，输出映射为空时复用动作结果映射 | 是 |
...
| TC-AIBRAIN-PLANNING-FUNC-022J | 结果写入目标注册表 | P1 | v1.2 | 管理员已进入任务中心 / 插件管理；服务端已启用 `scheduled_job_result`、`user_feedback_insights`、`code_inspection_reports` 和 `email_notifications` 写入目标 | 1. 调用 `GET /api/system/result-write-targets` 2. 打开插件动作新增弹窗 3. 切换不同结果写入目标 4. 选择官方动作模板并检查回填 5. 试运行动作并查看写入预览 | 接口必须返回目标 `code/label/form_label/default_result_mapping/mapping_fields`；插件动作表单下拉使用服务端 `form_label`，切换目标时使用 `default_result_mapping` 回填高级 JSON，并按 `mapping_fields` 渲染 JSONPath 字段；用户洞察表、代码巡检报告、邮件通知记录字段必须与注册表一致；官方动作模板的 `result_mapping` 必须复用注册表默认映射；试运行 `write_preview.write_target_label` 必须来自同一注册表，避免前端、模板和预览三处硬编码不一致 | 是 |
