...
补充验收约定：`TC-AIBRAIN-PLANNING-FUNC-022C` 中旧的“服务端模板不可用时使用本地兼容兜底”表述废止；官方动作模板必须以 `/api/system/plugin-action-templates` 为唯一配置源，模板缺失时前端和 AI 助手只能提示缺失，不得硬编码生成 GitHub/GitLab/邮箱/AI 执行器官方执行。
