...
| TC-AIBRAIN-DEPLOY-FUNC-007C | DB-first 迁移模式健康检查 | P1 | 生产就绪 | API 以 `PERSISTENCE_MODE=postgres` 启动 | 1. 请求 `/health` 2. 检查 `data_access_mode` 3. 切换测试环境 memory helper 运行单测 | PostgreSQL 运行时返回 `data_access_mode=db_first_migration`；测试 memory helper 返回 `memory_test_helper`；该字段用于识别生产 API 是否仍处于 DB-first 迁移期 | 是 |
