...
| 步骤 | 操作 | 预期结果 |
|------|------|----------|
| 1 | 进入首页 IT 团队看板 | MVP 可展示需求、研发进展、知识沉淀和审计摘要；尚未交付的线上系统健康、用户使用、用户反馈、迭代规划和发布能力展示空状态、占位说明或禁用态；Bug 指标可在 v1.1 数据可用后接入。 |
...
| 5 | 进入 Bug 管理 | v1.1 可展示真实 Bug 列表、权限校验和空列表兜底；登记、分派、验证、关闭和重复归并按 TC-AIBRAIN-BUG-FUNC-018 验收。 |
...
2. Bug 管理在 v1.1 使用真实 API 或空列表兜底；研发运营看板、用户洞察/迭代规划等后续阶段入口在 MVP 不误导为已完成能力，空状态、占位说明或禁用态清晰。
