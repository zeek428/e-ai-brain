...
| TC-AIBRAIN-ASSISTANT-FUNC-015D | v1.1 | P0 | 管理员在 AI 助手输入 `@定时作业名执行一次`，命令词和作业名之间没有空格，或中文输入法输入全角 `＠定时作业名 执行一次` | 前端 `@/＠` 候选查询必须剥离 `执行一次/运行一次/立即执行/run once/run now` 等命令尾巴，只用作业名查询候选；候选面板必须正常弹出匹配定时作业，用户按 Enter 或点击发送时必须携带唯一 `scheduled_job` 结构化引用提交聊天请求，避免后端收到缺失引用的裸文本而让用户误以为没有执行；即使后端直接收到全角 `＠` 文本且没有结构化引用，也必须按显式引用解析并触发同一条确定性执行链。回归见 `apps/web/tests/AssistantPage.test.tsx::shows and sends @ scheduled job run-once candidates when the command has no whitespace`、`apps/web/tests/AssistantPage.test.tsx::supports full-width @ scheduled job run-once commands` 和 `apps/api/tests/test_assistant_chat.py::test_ai_assistant_chat_runs_fullwidth_explicit_mention_job_once`。 |
