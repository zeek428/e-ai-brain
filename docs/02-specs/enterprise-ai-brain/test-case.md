...
| TC-AIBRAIN-PLANNING-FUNC-022M | AI 执行器 Runner 与 OpenClaw 回写 | P0 | v1.2 | 管理员已进入任务中心 / 插件管理；本地或远程受控 Runner 可访问目标工作区并支持 OpenClaw | 1. 创建 AI 执行器 Runner，选择 `executor_types=["codex","claude","hermes","openclaw"]` 并配置工作区白名单 2. 填写 Codex、Claude Code、Hermes、OpenClaw 本机命令、目标系统、CPU 架构和安装模式 3. 分别下载 Linux、macOS、Windows、Docker 和通用手动安装 Runner 包并检查 ZIP 中的 env/config/manifest/Skill/系统专属脚本 4. 查看列表中的健康状态、心跳年龄和启动命令 5. 通过 Runner token 发送心跳 6. 为官方 AI 执行器插件创建 `runner_polling` 连接和 OpenClaw 下达指令执行 7. 创建引用该执行的定时作业并手动运行 8. Runner 认领任务 9. Runner 提交成功或失败结果 10. 查看定时作业运行详情和插件调用日志 | 创建 Runner 响应只在创建时返回一次性 token，列表仅显示 `token_configured`；Runner 列表必须返回并展示 `health_status`、`heartbeat_age_seconds`、支持的 Codex/Claude Code/Hermes/OpenClaw、可复制 `setup_command` 和安装包下载入口，便于本地 Runner 根据远程 AI Brain 地址启动；安装包公共文件必须包含 `README.md`、`ai-brain-runner.env`、`manifest.json`、`runner_config.json` 和 `skills/ai-brain-runner/SKILL.md`，并写入 endpoint、runner_id、executor_types、workspace_roots、executor_commands、target_os、arch、install_mode，`AI_BRAIN_RUNNER_TOKEN` 只能是 `<runner_token>` 占位；Linux systemd 包必须包含 `install.sh` 和 `systemd/ai-brain-runner.service`，macOS launchd 包必须包含 `install.sh` 和 `launchd/com.ai-brain.runner.plist`，Windows service 包必须包含 `install.ps1` 和 `windows/ai-brain-runner-service.xml`，Docker 包必须包含 `Dockerfile` 和 `docker-compose.runner.yml`，通用手动包必须包含 `scripts/start-runner.sh` 和 `scripts/start-runner.ps1`；错误 token 心跳返回 401，正确心跳更新 `last_heartbeat_at`；定时作业下发 OpenClaw 指令后运行状态保持 `running`，`execution_nodes.runner_execution.status=queued/claimed/running`，结果写入显示 `waiting_runner`；Runner 完成回写后任务、插件调用日志、定时作业运行、collector run 和作业最近运行字段同步更新，运行详情展示 `AI 执行器执行内容` 节点，包含 executor type、Runner ID、任务 ID、工作区、日志条数和结果 JSON；OpenClaw 结果写入 `result_action.feedback.runner_result`，成功时运行终态为 `succeeded`，失败/超时/取消时运行终态为 `failed` 并保留错误码和错误信息。回归见 `apps/api/tests/test_plugin_management.py::test_ai_executor_runner_install_package_contains_remote_config_skill_and_os_assets`、`apps/api/tests/test_plugin_management.py::test_scheduled_ai_executor_runner_completion_updates_run_detail`、`apps/web/tests/PluginsPage.test.tsx::manages AI executor runners with remote executor options and install packages` 与 `apps/web/tests/ScheduledJobsPage.test.tsx::shows AI executor runner details in scheduled job run results` | 是 |
