...
| 编号 | 阶段 | 优先级 | 用例 | 预期 |
|------|------|--------|------|------|
| TC-AIBRAIN-ASSISTANT-FUNC-010 | v1.1 | P0 | 在 AI 助手输入框通过 `@` 搜索知识文档、知识片段、业务对象、动作、AI角色/Skill、定时作业和运行记录 | 候选列表只展示当前用户有权读取或管理的对象；普通用户可见业务对象、可读 `knowledge_document` 和可读 `knowledge_chunk`，管理员额外可见 `scheduled_job`、`scheduled_job_run`、`plugin_action`、`plugin_connection`、`ai_agent`、`ai_skill`；输入裸 `@` 时前端请求足量默认候选，后端按类型均衡返回，知识文档数量很多也不得挤掉需求、研发任务、定时作业、运行记录、插件动作、插件连接、AI角色和 Skill；`ai_task` 在候选分组、类型 Tag、已选引用 Chip 和本次上下文中必须展示为“研发任务”，`ai_skill` 必须展示为“Skill”；指定类型且查询词为空时不得先被其它类型全局截断，例如 `type=scheduled_job&query=` 仍应返回可引用定时作业；选择后显示带类型标签的引用 chip，并随聊天请求提交结构化 `references`。回归见 `apps/api/tests/test_assistant_chat.py::test_ai_assistant_default_reference_candidates_are_balanced_across_types`、`apps/api/tests/test_assistant_chat.py::test_ai_assistant_type_specific_default_candidates_are_not_globally_truncated` 和 `apps/web/tests/AssistantPage.test.tsx::opens default reference candidates when the user types a bare @`。 |
