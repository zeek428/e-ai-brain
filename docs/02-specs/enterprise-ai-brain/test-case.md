...
| TC-AIBRAIN-ASSISTANT-FUNC-020C | v1.1 | P1 | 管理员围绕一次失败的 `@scheduled_job_run` 继续追问“这次失败怎么修，帮我生成修复草案” | 聊天响应包含 `assistant.action_draft` 工具结果，`intent=scheduled_job_run_repair_draft`，服务端从失败运行的结果动作节点、插件调用日志和原插件动作配置生成 `create_plugin_action` 草案，payload 至少包含插件、连接、动作类型、修复动作编码、请求方法/路径和写入目标；工具项必须被自动持久化为 `assistant_action_drafts` 并返回 `server_draft_id/status=pending/preview`，前端按已有插件动作草案卡展示“确认创建/取消/查看草案/重新生成”和“应用到插件动作表单”；确认前不得修改原动作、创建真实动作或触发外部调用；模型日志不得保存完整插件请求/响应、Prompt、模型输出或密钥。回归见 `apps/api/tests/test_assistant_chat.py::test_ai_assistant_chat_generates_repair_action_draft_for_failed_run` 和 `apps/web/tests/AssistantPage.test.tsx::renders assistant plugin action drafts as configuration cards`。 |
