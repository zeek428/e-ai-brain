...
| TC-AIBRAIN-PLANNING-FUNC-022M | AI 执行器 Runner 与 OpenClaw 回写 | P0 | v1.2 | 管理员已进入任务中心 / 插件管理；本地或远程受控 Runner 可访问目标工作区并支持 OpenClaw | 1. 创建 AI 执行器 Runner，选择 `executor_types=["openclaw"]` 并配置工作区白名单 2. 查看列表中的健康状态、心跳年龄和启动命令 3. 通过 Runner token 发送心跳 4. 为官方 AI 执行器插件创建 `runner_polling` 连接和 OpenClaw 下达指令执行 5. 创建引用该执行的定时作业并手动运行 6. Runner 认领任务 7. Runner 提交成功或失败结果 8. 查看定时作业运行详情和插件调用日志 | 创建 Runner 响应只在创建时返回一次性 token，列表仅显示 `token_configured`；Runner 列表必须返回并展示 `health_status`、`heartbeat_age_seconds` 和可复制 `setup_command`，便于本地 Runner 根据远程 AI Brain 地址启动；错误 token 心跳返回 401，正确心跳更新 `last_heartbeat_at`；定时作业下发 OpenClaw 指令后运行状态保持 `running`，`execution_nodes.runner_execution.status=queued/claimed/running`，结果写入显示 `waiting_runner`；Runner 完成回写后任务、插件调用日志、定时作业运行、collector run 和作业最近运行字段同步更新，运行详情展示 `AI 执行器执行内容` 节点，包含 executor type、Runner ID、任务 ID、工作区、日志条数和结果 JSON；OpenClaw 结果写入 `result_action.feedback.runner_result`，成功时运行终态为 `succeeded`，失败/超时/取消时运行终态为 `failed` 并保留错误码和错误信息。回归见 `apps/api/tests/test_plugin_management.py::test_scheduled_ai_executor_runner_completion_updates_run_detail` 与 `apps/web/tests/ScheduledJobsPage.test.tsx::shows AI executor runner details in scheduled job run results` | 是 |
