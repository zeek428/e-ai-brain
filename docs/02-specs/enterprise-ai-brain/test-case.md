...
| 字段 | 说明 |
|------|------|
| 适用阶段 | MVP-A、MVP-B、MVP-C、v1.1、v1.2 或 MVP 占位。 |
...
| MVP-C 知识与治理闭环 | TC-AIBRAIN-KNOWLEDGE-FUNC-005、TC-AIBRAIN-OUTPUT-FUNC-004B、TC-AIBRAIN-OUTPUT-FUNC-004C、TC-AIBRAIN-FLOW-FUNC-010、TC-AIBRAIN-KNOWLEDGE-FUNC-012 | 知识导入、权限过滤检索、知识沉淀审核、模拟 Issue、占位入口和主体级治理可跑通。 |
...
| TC-AIBRAIN-OUTPUT-FUNC-004A | MVP-A Markdown 导出 | P0 | MVP-A | 产品详细设计或技术方案已确认 | 1. GET Markdown 导出接口 2. 检查内容 | 返回 `text/markdown` 方案内容，并可关联 trace_id | 是 |
| TC-AIBRAIN-OUTPUT-FUNC-004B | MVP-C 模拟 Issue 幂等生成 | P0 | MVP-C | 任务已确认并进入输出阶段 | 1. 查询回写结果 2. 重复触发输出 | 生成 mock issues，重复触发不产生重复结果 | 是 |
...
| TC-AIBRAIN-CONFIG-API-008 | 产品、版本、模块和 Git 资源配置 | P1 | MVP | admin 已登录 | 1. 创建/更新配置 2. 查询 active_only 列表 | 配置可维护，任务可引用产品版本上下文 | 是 |
| TC-AIBRAIN-CONFIG-API-009 | 平台模型网关配置 | P1 | MVP | admin 已登录 | 1. 创建默认模型配置 2. 查询列表 | 只返回密钥掩码，不泄露明文 API Key | 是 |
| TC-AIBRAIN-FLOW-FUNC-010 | MVP 业务主体独立入口和占位状态可用 | P0 | MVP | 用户已登录且具备相应角色，至少存在产品、需求、AI 任务和知识文档基础数据 | 1. 进入首页 IT 团队看板 2. 进入产品管理 3. 进入需求管理 4. 进入任务中心 5. 进入 Bug 管理 6. 进入研发运营看板 7. 进入用户洞察/迭代规划 8. 进入知识中心 9. 进入审计与运行 | MVP 必交主体可独立查看或维护；后续阶段入口展示空状态、占位说明或禁用态，不误导为已完成闭环能力 | 是 |
...
| TC-AIBRAIN-DASHBOARD-FUNC-017 | v1.2 首页 IT 团队看板完整下钻 | P1 | MVP 占位 / v1.2 | 存在需求、任务、Bug、发布、线上日志、用户使用、用户反馈和迭代规划建议数据 | 1. 打开首页 2. 按产品筛选 3. 下钻明细 | 展示需求、研发进展、Bug、线上系统健康、核心业务运行、用户使用趋势、用户反馈趋势、AI 迭代规划建议摘要和发布状态；MVP 阶段只要求入口和空状态不阻塞验收 | 是 |
| TC-AIBRAIN-BUG-FUNC-018 | v1.1/v1.2 Bug 管理闭环 | P1 | MVP 占位 / v1.1 | 存在 AI 自动测试和人工测试输入 | 1. AI 自动测试登记 Bug 2. 人工登记 Bug 3. 分派修复 4. 验证关闭 | Bug 按产品归属，来源正确，状态完整流转；MVP 阶段只要求占位入口或空状态 | 是 |
...
| TC-AIBRAIN-PLANNING-FUNC-022 | v1.2 用户使用洞察与 AI 迭代规划 | P1 | MVP 占位 / v1.2 | 存在产品规划、需求池、用户使用数据、用户反馈、Bug、线上日志、发布记录和研发投入样本 | 1. 聚合使用指标 2. 收集用户反馈 3. 生成迭代规划建议 4. 检查未自动创建正式需求/变更路线图 5. 产品负责人确认后转需求 | 返回可追溯的下阶段优先迭代需求建议；AI 未经确认不自动创建正式需求或调整路线图/排期；MVP 阶段只要求占位入口或空状态 | 是 |
...
| 2 | MVP-C: GET `/api/writeback/results/{task_id}` | 返回研发、测试、文档类型模拟 Issue。 |
| 3 | MVP-C: 重复触发模拟输出 | 不产生重复 mock issue。 |
| 4 | MVP-C: 查询知识沉淀候选 | 返回 pending deposits。 |
| 5 | MVP-C: 批准或拒绝知识沉淀候选 | 候选状态正确流转，未批准内容不进入正式知识库。 |
...
| 4 | POST `/api/products/{product_id}/git-repositories` 绑定内部 GitLab 只读资源 | 返回 repository_id 和凭据引用，不返回明文 token。 |
| 5 | GET `/api/products/{product_id}/versions?active_only=true` | 不返回 archived 版本。 |
| 6 | 使用 archived 版本创建需求或任务 | 返回 `PRODUCT_VERSION_ARCHIVED` 或 `VALIDATION_ERROR`。 |
...
| 步骤 | 操作 | 预期结果 |
|------|------|----------|
| 1 | POST `/api/system/model-gateway-configs` 创建默认模型配置 | 返回 config_id、provider、model、purpose 和 enabled 状态。 |
| 2 | GET `/api/system/model-gateway-configs` | 返回 `api_key_configured=true` 和 `api_key_masked`，不返回明文 API Key。 |
| 3 | PATCH `/api/system/model-gateway-configs/{config_id}` 禁用配置 | 配置变为 disabled，新任务不能选择该配置。 |
| 4 | 使用缺失或无效模型配置启动任务 | 返回 `MODEL_GATEWAY_CONFIG_INVALID` 或 `MODEL_GATEWAY_FAILED`，任务不静默完成。 |

**预期结果**:
1. 模型网关配置可维护。
2. API 响应和日志不泄露模型 API Key。
3. 配置变更和模型调用失败均产生可追踪事件。
...
### TC-AIBRAIN-FLOW-FUNC-010: MVP 业务主体独立入口和占位状态可用

| 项目 | 内容 |
|------|------|
| 用例编号 | TC-AIBRAIN-FLOW-FUNC-010 |
| 用例名称 | MVP 业务主体独立入口和占位状态可用 |
| 优先级 | P0 |
| 模块 | FLOW |
| 创建人 | Claude |
| 创建日期 | 2026-05-28 |
...
**前置条件**:
1. 用户已登录。
2. 系统至少存在产品、需求、AI 任务和知识文档基础数据；Bug 管理、研发运营看板和用户洞察/迭代规划在 MVP 可没有业务数据。

**测试步骤**:
| 步骤 | 操作 | 预期结果 |
|------|------|----------|
| 1 | 进入首页 IT 团队看板 | MVP 可展示需求、研发进展、知识沉淀和审计摘要；尚未交付的 Bug、线上系统健康、用户使用、用户反馈、迭代规划和发布能力展示空状态、占位说明或禁用态。 |
| 2 | 进入产品管理 | 可以查看和维护产品、版本、模块和 Git 资源。 |
| 3 | 进入需求管理 | 可以查看需求列表、需求详情和审批状态。 |
| 4 | 进入任务中心 | 可以查看 AI 任务列表、任务详情、产品详细设计/技术方案/代码 Review 人工确认、回写结果和 Markdown 导出。 |
| 5 | 进入 Bug 管理 | MVP 可展示入口、权限校验和空状态；完整 Bug 分派、验证、关闭和重复归并按 v1.1/v1.2 验收。 |
| 6 | 进入研发运营看板 | MVP 可展示入口、待接入说明或空状态；GitLab/Jenkins/线上日志完整采集和下钻按 v1.2 验收。 |
| 7 | 进入用户洞察/迭代规划 | MVP 可展示入口、待接入说明或空状态；使用数据、反馈和 AI 迭代规划闭环按 v1.2 验收。 |
| 8 | 进入知识中心 | 可以查看知识文档、检索、索引状态和沉淀审核。 |
| 9 | 进入审计与运行 | 可以查看审计事件、运行记录、健康检查和失败排查信息。 |

**预期结果**:
1. 产品、需求、AI 任务、知识中心和审计与运行在 MVP 可独立查看或维护。
2. Bug 管理、研发运营看板、用户洞察/迭代规划等后续阶段入口在 MVP 不误导为已完成能力，空状态、占位说明或禁用态清晰。
...
### TC-AIBRAIN-BUG-FUNC-018: Bug 管理闭环
...
| 步骤 | 操作 | 预期结果 |
|------|------|----------|
| 1 | POST `/api/bugs`，source=`ai_auto_test` | Bug 创建成功，来源为 AI 自动测试。 |
| 2 | POST `/api/bugs`，source=`manual_test` | Bug 创建成功，来源为人工测试。 |
| 3 | PATCH `/api/bugs/{id}` 分派处理人 | 状态从 `open` 流转到 `assigned`。 |
...
**前置条件**:
1. 产品已绑定内部 GitLab 项目和凭据引用，当前用户具备该产品和 MR 的 Review 权限。
...
**前置条件**:
1. 已存在 MVP-A 生成的 MR diff 快照。
2. code-review 执行器默认适配 Claude Code `code-review` skill。

**测试步骤**:
| 步骤 | 操作 | 预期结果 |
|------|------|----------|
| 1 | POST `/api/ai-tasks` 创建 `task_type=code_review` 任务 | 任务为 `draft`，input 包含 requirement_snapshot、product_context 和 gitlab_mr_snapshot 引用。 |
| 2 | POST `/api/ai-tasks/{id}/start` | 调用 code-review 执行器，任务进入 `waiting_review`，返回 pending review。 |
| 3 | GET `/api/ai-tasks/{id}/code-review-report` | 返回 summary、risk_level、findings、文件/行号、建议、confidence、executor metadata 和 human_review。 |
| 4 | POST `/api/reviews/{id}/approve` 或 `edit-approve` | Review 报告归档到 AI Brain 内部，任务继续或完成。 |
| 5 | 查询 GitLab MR | 未新增评论，未改变审批状态、request changes、合并状态或分支。 |
| 6 | GET `/api/audit/events?ai_task_id={id}` | 返回执行器调用、报告生成、人工确认和归档审计事件。 |
...
| TC-AIBRAIN-CONFIG-API-009 | GET/POST/PATCH /api/system/model-gateway-configs | 配置平台模型网关 | 返回 api_key_configured 和 api_key_masked，不返回明文。 |
