...
| MVP-B GitLab Review 闭环 | TC-AIBRAIN-REVIEW-FUNC-023B、TC-AIBRAIN-REVIEW-API-023C、TC-AIBRAIN-REVIEW-FUNC-003 中 code_review 部分、TC-AIBRAIN-AUDIT-API-006、TC-AIBRAIN-AUDIT-API-013 | 基于 MVP-A 的 MR 快照生成 code_review 报告、人工确认、内部归档和结构表恢复可跑通，且不回写 GitLab。 |
...
| TC-AIBRAIN-TASK-API-001B | AI 任务 PostgreSQL 结构表持久化 | P1 | MVP | Docker 本地栈以 `PERSISTENCE_MODE=postgres` 启动，存在已审批需求 | 1. 从需求生成产品详细设计任务 2. 查询 PostgreSQL `ai_tasks` 表 3. 启动或取消任务使状态变化 4. 重启 API 后重新查询任务列表 | `ai_tasks` 保存任务类型、标题、状态、需求快照、产品上下文、输入输出、当前步骤和创建人；API 重启后仍能从结构表恢复任务和 `task` 计数器，Review/Graph 运行态由对应结构表恢复并回填任务关联 | 是 |
...
| TC-AIBRAIN-DEPLOY-FUNC-007 | Docker Compose 本地栈健康检查 | P1 | 生产就绪 | Docker 可用 | 1. 启动 compose 2. 请求 /health 3. 检查 postgres/redis | web/api/db/redis 服务正常，生产就绪门禁可验证 | 待环境验证 |
...
| TC-AIBRAIN-REQ-API-011B | 需求台账 PostgreSQL 结构表持久化 | P1 | MVP | Docker 本地栈以 `PERSISTENCE_MODE=postgres` 启动且存在产品版本 | 1. 创建需求 2. 审批或生成任务使需求状态变化 3. 查询 PostgreSQL `requirements` 表 4. 重启 API 后重新查询需求列表 | 需求标题、内容、产品、版本、状态、审批备注和 `task_ids` 写入 `requirements`；API 重启后仍能从结构表恢复需求和 `requirement` 计数器 | 是 |
...
| TC-AIBRAIN-REVIEW-API-023C | GitLab MR 快照和 Code Review 报告 PostgreSQL 结构表持久化 | P1 | MVP-B | Docker 本地栈以 `PERSISTENCE_MODE=postgres` 启动，已完成技术方案并存在 GitLab MR 快照 | 1. 生成 MR diff 快照 2. 创建并启动 code_review 任务生成报告 3. 查询 PostgreSQL `gitlab_mr_snapshots` 和 `code_review_reports` 表 4. 重启 API 后查询任务详情、生命周期上下文和报告接口 | MR 快照和 Review 报告写入结构表；报告保留 task_id、snapshot_id、executor、risk_level、findings、review_id、归档状态和不回写 GitLab 标记；API 重启后能恢复 `snapshot`/`report` 计数器并回填任务 `code_review_report_id` | 是 |
...
**状态**: 已自动化覆盖。后端证据见 `apps/api/tests/test_mvp_a_flow.py::test_requirement_to_product_detail_design_human_review_flow`、`apps/api/tests/test_graph_runtime.py::test_starting_task_creates_graph_run_checkpoint_and_task_detail_projection`；模型网关失败路径见 `apps/api/tests/test_model_gateway.py`。
...
**状态**: 待生产就绪环境验证。本用例保留为 Docker Compose、pgvector/pgcrypto、Redis、模型网关配置和 GitLab 只读凭据的发布门禁，不以本地 API/Web 单元测试替代。
...
| 5 | 模拟索引失败后调用 `POST /api/knowledge/documents/{document_id}/retry-index` | 展示失败原因；只有 `index_failed` 文档允许重试，成功后进入 `indexed`，失败时保留新的 `index_error`。 |
...
| 步骤 | 操作 | 预期结果 |
|------|------|----------|
| 1 | 打开首页 IT 团队看板 | MVP 展示真实需求、研发进展、待确认 Review、知识沉淀和审计摘要；v1.2 再扩展 Bug、线上系统健康、核心业务运行、用户使用趋势、用户反馈趋势、AI 迭代规划建议摘要和发布状态。 |
| 2 | GET `/api/dashboard/it-team?product_id=product_001&time_range=7d` | MVP 返回同一产品维度下的需求、AI 任务、待确认 Review、知识文档、知识沉淀和审计聚合指标，其他产品的知识文档和审计事件不计入当前产品。 |
| 3 | v1.2 按产品切换筛选 | 看板所有卡片同步切换产品归属。 |
| 4 | v1.2 从 Bug、发布或线上健康卡片下钻 | 跳转到对应主体列表或明细，并保留产品和时间范围上下文。 |
...
**状态**: 已自动化覆盖 MVP 聚合、页面产品筛选、知识文档和审计事件按产品过滤；v1.2 扩展下钻待后续切片
...
**状态**: development_planning / automated_testing / release_readiness / post_release_analysis 自动化通过
...
| 3 | 再次拉取相同 diff | 返回已有 snapshot_id，不重复写入 `gitlab_mr_snapshots`，并记录 `gitlab_mr.snapshot_reused` 审计事件。 |
| 4 | 输入超过 diff 字节数、变更文件数或单文件 diff 行数限制的 MR | 返回 `GITLAB_MR_DIFF_TOO_LARGE`，不得静默截断后继续；审计包含 `gitlab_mr.snapshot_failed`、`diff_size_bytes`、`diff_limit_bytes`、`changed_file_count`、`changed_file_limit`、`file_diff_line_count` 或 `file_diff_line_limit`。 |
...
4. diff 字节数、变更文件数或单文件 diff 行数超限失败必须留下可审计指标，便于判断拆分 MR 或调整限制。

**状态**: 已自动化覆盖。GitLab MR 预览、只读快照、相同 diff 复用和超限错误见 `apps/api/tests/test_gitlab_snapshot.py`；需真实企业 GitLab 凭据的端到端环境仍按生产就绪门禁验证。
...
| 8 | 让 code-review 执行器返回非法结构或调用失败后再次 POST `/api/ai-tasks/{id}/start` | 返回 `CODE_REVIEW_EXECUTOR_FAILED`，任务为 `failed`，`current_step=code_review_executor_failed`，审计包含 `code_review.executor_failed` 和 `ai_task.failed`。 |

**预期结果**:
1. v1 MVP 可以基于内部 GitLab MR diff 快照生成结构化 Review 报告。
2. Review 报告必须经过人工确认后才能归档为正式结论。
3. 系统不得向 GitLab 回写评论、审批状态、request changes、合并状态或分支变更。
4. 执行器失败时返回 `CODE_REVIEW_EXECUTOR_FAILED`，停留在可排查的失败步骤并写入审计。

**状态**: 已自动化覆盖。Code Review 报告生成、确认归档、编辑确认和执行器失败语义见 `apps/api/tests/test_code_review_report.py`；真实执行器/模型 provider 端到端按生产就绪门禁验证。
...
| 6 | 注入无法归属产品的提交 | 进入待归属队列，不参与需求级风险结论。 |
...
**状态**: 已自动化覆盖 MVP 证据主体链路追踪。MVP 需求下游、人工确认、Code Review 报告、模拟 Issue、知识沉淀和审计起点见 `apps/api/tests/test_lifecycle_context.py` 与 `apps/web/tests/App.test.tsx::opens real audit detail and lifecycle trace actions from audit rows`；v1.2 GitLab/Jenkins/线上日志/使用/反馈/迭代规划证据扩展仍待后续切片。
...
**状态**: 已自动化覆盖用户使用指标登记/查询、用户反馈和迭代规划基础闭环；外部采集器和模型驱动规划仍属后续增强
...
| TC-AIBRAIN-KNOWLEDGE-ERR-006 | 知识索引失败 | 文档内容无法切片、embedding 调用失败，或非 `index_failed` 文档调用 retry-index | 文档进入 `index_failed` 并保留失败原因；retry-index 仅允许失败状态，状态不匹配返回 `KNOWLEDGE_INDEX_STATE_INVALID`。 |
...
| TC-AIBRAIN-KNOWLEDGE-API-025 | POST /api/knowledge/documents/{document_id}/retry-index | 重试失败知识文档索引 | `index_failed` 文档重建 chunk 后进入 `indexed` 或保留新的 `index_error`；非失败状态返回 `KNOWLEDGE_INDEX_STATE_INVALID`。 |
