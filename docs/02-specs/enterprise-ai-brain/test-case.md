...
| v1.1.426 | 2026-06-12 | 补充定时作业模板来源可视化验收：作业列表、复制弹窗和运行详情必须显示来源 | Codex |
...
| TC-AIBRAIN-CONFIG-FUNC-008E | 迭代版本代码分支配置 | P1 | MVP | 用户已登录，存在启用产品、迭代版本和同产品 GitHub/GitLab 代码库 | 1. 在迭代版本页点击“代码分支” 2. 新增版本分支配置 3. 查询分支列表 4. 修改分支状态 5. 删除分支配置 | 同一版本可维护多个代码库分支；同一版本同一仓库只允许一条配置；分支记录包含基准分支、开发分支、状态、来源和仓库投影；仓库必须属于同一产品；数据写入 `product_version_branch_configs` 并记录审计；前端回归见 `apps/web/tests/IterationVersionsPage.test.tsx`，服务映射见 `apps/web/tests/ProductServices.test.ts`，后端接口见 `apps/api/tests/test_product_system_config.py` | 是 |
...
| TC-AIBRAIN-CONFIG-API-009C | 系统菜单管理和页面权限点 | P1 | MVP | admin 已登录，存在 `system.menus.manage` 权限 | 1. 调用 `/api/system/menus` 查看菜单资源 2. 在系统管理/菜单管理新增菜单 3. 编辑菜单名称、路由、图标、排序和访问权限点 4. 启停菜单并重排序 5. 尝试删除系统菜单和有子菜单的菜单 6. 打开真实 `/system/menus` 页面 | 菜单资源写入 `menu_resources`，新增/编辑/启停/重排序接口要求 `system.menus.manage`；`GET /api/system/menus` 允许 `system.menus.read/manage` 或历史 `system.roles.manage` 读取；系统菜单删除返回 `SYSTEM_MENU_PROTECTED`，有子菜单返回 `MENU_HAS_CHILDREN`；`/api/auth/me.menu_tree` 按授权菜单返回，左侧导航可见性由数据库菜单控制；前端只能加载已注册静态路由组件，不因数据库 path 动态执行未知页面；回归见 `apps/api/tests/test_rbac_system_api.py`、`apps/api/tests/test_rbac_foundation.py`、`apps/api/tests/test_router_boundaries.py` 和 `apps/web/tests/SystemManagementPages.test.tsx` | 是 |
...
| TC-AIBRAIN-REVIEW-FUNC-023F | 代码巡检治理概览 | P1 | v1.2 | 已存在通过定时作业写入的代码巡检报告、finding、严重问题 Bug 和提交人信息 | 1. 打开运营治理 / 代码巡检 2. 查看页面顶部治理概览 3. 调用 `GET /api/governance/code-inspections/dashboard` 4. 使用提交人或风险级别筛选后再次查看概览 | 概览接口必须按当前用户产品 scope 和列表筛选返回 `summary/trend/rule_distribution/repository_ranking/branch_ranking/committer_ranking/sla`；规则维度统计必须按 finding 的 rule/category/severity 聚合，仓库/分支/提交人排行必须按严重问题数和总问题数排序；严重问题 SLA 必须基于 high/critical finding 是否已关联 `code_inspection` Bug 计算覆盖率和未覆盖数量；页面必须在统一列表样式下展示报告数、问题数、严重问题、Bug 覆盖率、规则统计、仓库排行、分支排行、提交人排行和 SLA 摘要 | 是 |
...
| TC-AIBRAIN-PLANNING-FUNC-022I | 插件连接请求回放台 | P1 | v1.2 | 管理员已进入任务中心 / 插件管理；存在 MaxCompute 连接，连接 Params 使用 `{{current_date-7}}`，Header 使用真实 Authorization 值；另准备一个会返回 HTTP 400 的失败连接 | 1. 在连接列表点击测试 2. 查看请求调试台和请求回放台 3. 检查最近测试记录、变量解析前/后差异和完整请求 JSON 4. 展开最近测试记录查看历史完整请求响应 5. 点击“复制为动作模板” 6. 使用失败连接再次测试 | 测试接口返回并持久化最近 N 次 `test_history`，连接列表再次加载时可看到最近记录；诊断弹窗必须展示“请求回放台”和“最近测试记录”，每条记录包含测试时间、状态、耗时、方法、请求 URL 和响应码；最近测试记录可展开查看历史完整请求 JSON、历史远端响应信息、历史修复建议和历史动作模板草案；变量差异区必须明确展示 `query.start_pt` 从 `{{current_date-7}}` 解析为实际日期，保留解析状态和时区；“复制为动作模板”必须使用 `action_template_draft` 打开新增动作弹窗，并带入插件、连接、名称、编码、请求方法、路径、Params/Headers 和默认 `scheduled_job_result` 写入映射；失败测试必须返回 `repair_suggestions[]`，HTTP 400 给出检查请求参数/认证 Header 的建议，最终请求仍包含 `***` 占位时给出占位 Header 修复建议 | 是 |
| TC-AIBRAIN-PLANNING-FUNC-022J | 结果写入目标注册表与结果记录查看 | P1 | v1.2 | 管理员已进入任务中心；服务端已启用 `scheduled_job_result`、`user_feedback_insights`、`code_inspection_reports` 和 `email_notifications` 写入目标；存在一次邮件通知正式运行记录 | 1. 调用 `GET /api/system/result-write-targets` 2. 打开动作新增弹窗 3. 切换不同结果写入目标 4. 选择官方动作模板并检查回填 5. 试运行动作并查看写入预览 6. 调用 `GET /api/system/result-write-records?scheduled_job_run_id=<run_id>` 7. 打开任务中心 / 定时作业 / 运行记录详情 8. 查看结果写入记录 9. 进入插件管理确认不再出现“结果记录”和“调用日志”页签 | 目标注册表接口必须返回目标 `code/label/form_label/default_result_mapping/mapping_fields`；动作表单下拉使用服务端 `form_label`，切换目标时使用 `default_result_mapping` 回填高级 JSON，并按 `mapping_fields` 渲染 JSONPath 字段；用户洞察表、代码巡检报告、邮件通知记录字段必须与注册表一致；官方动作模板的 `result_mapping` 必须复用注册表默认映射；试运行 `write_preview.write_target_label` 必须来自同一注册表，避免前端、模板和预览三处硬编码不一致；结果记录接口必须返回通用 `write_target/write_target_label/status/source_type/scheduled_job_run_id/plugin_action_id/plugin_invocation_log_id/records_imported/summary_fields/preview/feedback` 字段，并支持 `scheduled_job_run_id` 精确查询单次运行；定时作业运行详情必须展示邮件通知主题、投递状态、消息 ID、收件人摘要、写入预览和动作反馈；未知未来目标也必须能保留通用预览和详情 JSON，不要求新增目标专属页面；插件管理只负责插件、连接和动作配置，不展示定时作业运行结果记录和独立调用日志 | 是 |
| TC-AIBRAIN-PLANNING-FUNC-022K | 定时作业运行可观测性 | P1 | v1.2 | 管理员已进入任务中心 / 定时作业；存在成功、失败、含模型日志和含动作反馈的运行记录 | 1. 调用 `GET /api/system/scheduled-job-runs/observability` 2. 打开运行记录页签 3. 查看运行健康概览 4. 查看最近失败和慢运行表格 5. 点击运行详情继续下钻运行链路 | 接口必须返回总运行数、成功/失败/运行中/取消数量、成功率、失败率、平均耗时、平均导入数、AI 调用次数、模型 Token 总量、插件调用次数、动作写入次数、动作写入成功率、状态分布、作业类型分布、触发方式分布、写入目标分布、失败原因、最近失败和慢运行；Token 汇总只读取 `skill_processing.model_log_id` 对应模型日志的 tokens 元数据，不返回完整 Prompt 或模型输出；运行记录页签顶部必须展示总运行数、成功率、失败率、平均耗时、AI 调用次数、Token 总量、插件调用次数和动作写入成功率，并列出最近失败、失败原因和慢运行，用户可继续从列表打开运行详情查看数据连接、AI执行处理和动作反馈三段节点。回归见 `apps/api/tests/test_scheduled_ai_jobs.py::test_plugin_action_scheduled_job_run_records_request_trace_summary` 和 `apps/web/tests/ScheduledJobsPage.test.tsx` | 是 |
