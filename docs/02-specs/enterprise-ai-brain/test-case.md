...
**状态**: 已自动化覆盖。产品详细设计、技术方案和 Code Review 人工确认门禁分别见 `apps/api/tests/test_mvp_a_flow.py`、`apps/api/tests/test_technical_solution_export.py`、`apps/api/tests/test_code_review_report.py` 与 `apps/web/tests/App.test.tsx` 任务操作弹窗用例。Review approve/edit-approve 主路径 DB-first no-persist 回归见 `apps/api/tests/test_database_persistence.py::test_approve_review_writes_completion_records_without_request_persist` 和 `apps/api/tests/test_database_persistence.py::test_edit_approve_review_writes_completion_records_without_request_persist`。
