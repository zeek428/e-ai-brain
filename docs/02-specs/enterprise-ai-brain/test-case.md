...
| TC-AIBRAIN-REQ-API-011D | 需求列表产品/迭代版本聚合字段 | P1 | MVP | 已存在产品、迭代版本和需求 | 1. 调用 `GET /api/requirements` 2. 按产品或状态过滤 3. 打开需求管理页面 | 列表响应直接返回 `product_code`、`product_name`、`version_code`、`version_name`；前端不再额外拉产品和版本列表翻译展示字段 | 是 |
...
| 6 | 打开真实迭代版本页面 | “推进状态”入口可见，弹窗可生成影响预览，页面无空白、无框架错误覆盖层、无相关控制台错误。 |
