...
| TC-AIBRAIN-TASK-FUNC-020 | 研发全链路 AI 任务类型覆盖 | P1 | v1.1 / v1.2 | 已存在产品、已审批需求、已确认产品详细设计、已确认技术方案、代码 diff、测试结果、Jenkins 发布记录和线上日志样本 | 1. 创建当前已实现 task_type 任务 2. 启动并查询详情 3. 检查输出结构 4. 后续补齐 release_readiness/post_release_analysis | 每类任务均保留 task_type、产品上下文、人工确认点和对应输出结构；当前自动化覆盖 development_planning 与 automated_testing | 是 |
...
| 2 | GET `/api/dashboard/it-team?product_id=product_001&time_range=7d` | MVP 返回同一产品维度下的需求、AI 任务、待确认 Review、知识文档、知识沉淀和审计聚合指标。 |
...
**预期结果**:
1. 已实现任务类型复用统一任务状态机、人工确认、审计和详情查询能力；`release_readiness` 和 `post_release_analysis` 仍按 v1.2 后续切片验收。
2. v1 系列不自动改代码、不自动提交 PR、不自动部署上线。
3. 自动化测试和上线后分析产生的 Bug 必须进入 Bug 管理闭环。

**状态**: development_planning / automated_testing 自动化通过；release_readiness / post_release_analysis 待实现
