...
| TC-AIBRAIN-PLANNING-FUNC-022H | 定时作业配置编排预览 | P1 | v1.2 | 管理员已进入任务中心 / 定时作业；存在 MaxCompute 数据连接、AI 模型、AI角色、Skill、知识文档和结果动作 | 1. 打开新增作业弹窗 2. 查看“任务编排流程” 3. 选择“每周用户反馈洞察抽取”场景模板 4. 点击数据连接节点的“测试数据连接” 5. 编辑已有作业并查看同一编排视图 | 新增/编辑弹窗必须展示“数据连接 → AI 处理 → 知识引用 → 结果动作”四段式编排视图；未配置时数据连接、AI 处理和结果动作节点显示待配置，知识引用显示可选；模板回填后节点显示已配置/已选择，并展示生产 MaxCompute 项目、定时作业模型、AI角色、已选知识和获取本周用户反馈数据等核心资源名称；点击“测试数据连接”必须调用 `POST /api/system/plugin-connections/{connection_id}/test`，按钮进入测试中状态，完成后在数据连接节点展示连接测试状态、耗时和请求摘要；编排视图仅作为配置预览和测试入口，不向保存 payload 增加额外字段 | 是 |
