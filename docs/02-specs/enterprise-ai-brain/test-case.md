...
| TC-AIBRAIN-PLANNING-FUNC-022B | MaxCompute 每周用户反馈洞察抽取 | P1 | v1.2 | 管理员已配置 MaxCompute MCP 插件、生产连接、AI 模型、Agent、每周反馈分析 Skill 和用户反馈产品；结果动作可返回 `row_count`，模型可输出 `insights` | 1. 在插件动作页选择“MaxCompute 每周用户反馈”场景 2. 检查表名、时间字段、返回字段、最大行数和结果写入目标默认值 3. 创建 `user_feedback_insight_extract` 定时作业，确认表单顺序为数据连接、AI 模型、Agent、Skills、结果动作，且不展示时间参数、连接输入参数和结果写入覆盖 JSON 4. 手动运行作业 5. 查询用户反馈洞察列表和模型日志 | 页面无需人工从零编写复杂 JSON；动作保存为 `mcp_tool` 且包含 `tool_name=maxcompute.execute_sql` 和 `result_mapping.write_target=user_feedback_insights`；定时作业必须保存 `plugin_connection_id/model_gateway_config_id/agent_id/skill_ids/plugin_action_id`；运行时先通过数据连接获取源数据，再调用模型网关按 Agent/Skill 输出结构化 JSON，最后按动作结果映射写入用户洞察表；`records_imported` 为新增洞察数，运行摘要展示数据连接源表行数、Skill 节点 `model_gateway_called=true`、`model_log_id`、模型处理输出、写入目标和结果动作反馈 | 是 |
