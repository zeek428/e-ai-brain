...
| TC-AIBRAIN-REVIEW-FUNC-023A | MVP-A GitLab MR / GitHub PR 预览和 diff 快照 | P0 | MVP-A | 产品已绑定 GitLab 或 GitHub 代码库，存在已审批需求、已确认技术方案和可访问 MR/PR | 1. 预览 MR/PR 2. 拉取 diff 快照 3. 查询快照摘要 | 返回变更元信息、snapshot_id、diff_size_bytes、diff_limit 和 created_at；快照不受后续远端变更静默影响 | 是 |
...
| 5 | GET `/api/model-gateway/logs?purpose=assistant_chat` | 返回助手模型调用元数据，包含 provider、model、tokens、latency、status 和 config_id，不包含完整用户问题、系统上下文、助手回答或 API Key。 |
| 6 | 模型网关未配置或调用失败 | API 返回 `MODEL_GATEWAY_CONFIG_INVALID` 或 `ASSISTANT_CHAT_FAILED`，页面展示错误消息并保留用户输入上下文。 |

**预期结果**:
1. AI 助手可以回答 AI Brain 系统配置、产品、需求、任务、Git 仓库和项目开发进展相关问题。
2. 助手聊天只依赖 Chat 模型网关，不因上游不支持 Embedding 而阻断。
3. 模型日志和审计事件只记录脱敏元数据，不能保存完整 prompt、完整输出或密钥。
...
| 步骤 | 操作 | 预期结果 |
|------|------|----------|
| 1 | GET GitLab `/api/devops/gitlab/merge-requests/{repository_id}/{mr_iid}/preview` 或 GitHub `/api/devops/github/pull-requests/{repository_id}/{pr_number}/preview` | 返回 MR/PR 标题、作者、source/head branch、target/base branch、changed_file_count、diff_refs 和 web_url。 |
| 2 | POST GitLab `/api/devops/gitlab/merge-requests/{repository_id}/{mr_iid}/snapshot` 或 GitHub `/api/devops/github/pull-requests/{repository_id}/{pr_number}/snapshot` | 返回 snapshot_id、diff_size_bytes、diff_limit 和 created_at；快照不受后续 MR/PR 变更静默影响。 |
| 3 | 再次拉取相同 diff | 返回已有 snapshot_id，不重复写入 `gitlab_mr_snapshots`，并记录 `gitlab_mr.snapshot_reused` 或 `github_pr.snapshot_reused` 审计事件。 |
| 4 | 输入超过 diff 字节数、变更文件数或单文件 diff 行数限制的 MR/PR | 返回 `GITLAB_MR_DIFF_TOO_LARGE`，不得静默截断后继续；审计包含 `*.snapshot_failed`、`diff_size_bytes`、`diff_limit_bytes`、`changed_file_count`、`changed_file_limit`、`file_diff_line_count` 或 `file_diff_line_limit`。 |
| 5 | 移除 GitLab/GitHub base URL 或只读 token 后重试 preview | 返回 provider 对应的配置或凭据错误，不得生成本地假 MR/PR。 |
...
**状态**: 已自动化覆盖。GitLab MR 预览、只读快照、相同 diff 复用和超限错误见 `apps/api/tests/test_gitlab_snapshot.py`；GitHub PR 预览、只读快照和 code_review 任务创建见 `apps/api/tests/test_github_snapshot.py`；需真实企业 GitLab/GitHub 凭据的端到端环境仍按生产就绪门禁验证。
...
| TC-AIBRAIN-ASSISTANT-API-027 | POST /api/assistant/chat | AI 助手系统问答 | 基于服务端注入的脱敏系统上下文和 Chat 模型网关返回助手回答；模型日志只记录 `purpose=assistant_chat` 元数据，不保存完整问题或回答。 |
