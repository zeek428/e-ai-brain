...
| 7 | 在需求管理页点击“全链路” | 弹窗一次展示需求、迭代版本、AI 任务、Review、PR/MR 快照、代码评审、Bug、发布和知识沉淀时间线，不需要跳转多个页面。 |
...
6. 需求全链路详情接口返回 `summary` 和按 `occurred_at` 升序排列的 `timeline`，页面入口只调用 `/api/requirements/{requirement_id}/full-chain` 获取详情。
