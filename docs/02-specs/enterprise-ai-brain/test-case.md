...
| TC-AIBRAIN-ASSISTANT-FUNC-015B | v1.1 | P0 | 管理员通过自然语言 `@提取每周用户反馈有价值信息 执行一次` 命中多个周反馈、MaxCompute 或原始同步类作业 | 后端不得仅凭 `source_system=aliyun-maxcompute`、停用精确同名作业或前端误带入的通用 `scheduled_job` 引用决定执行对象；必须按官方模板 `assistant_template.code=weekly_feedback_insight`、`job_type=user_feedback_insight_extract`、名称中的“用户反馈/洞察/提取/每周”语义和可执行状态评分，分数唯一最高且 `enabled=true/status=active` 时执行官方周反馈洞察作业；若仍不唯一才返回歧义或草案。回归见 `apps/api/tests/test_assistant_chat.py::test_ai_assistant_chat_prefers_official_feedback_insight_over_generic_maxcompute_job` 和 `apps/api/tests/test_assistant_chat.py::test_ai_assistant_chat_prefers_enabled_official_feedback_insight_over_disabled_exact_match`。 |
...
| TC-AIBRAIN-ASSISTANT-API-021 | v1.1 | P2 | 查询当前用户 AI 助手效果指标 | `GET /api/assistant/metrics` 只统计当前登录用户的助手草案、动作运行、定时作业运行和消息引用，返回草案总数、待确认/已确认/已取消/已过期/失败数、草案采纳率、草案处理率、用户修改率、动作运行成功率、定时作业运行成功率、失败复跑修复率、用户消息显式引用使用率、引用总数、知识引用数、知识引用命中率和 `drafts_by_action`；PostgreSQL/repository 运行态必须通过 `list_scheduled_job_runs()` 读取定时作业运行 read model，再按当前用户草案创建出的定时作业和消息引用过滤，不能依赖运行态内存集合；失败复跑修复率按失败运行被成功 `manual_rerun` 通过 `source_run_id` 引用计算，非 `manual_rerun` 的成功运行即使携带 `source_run_id` 也不得计入已修复；知识引用命中率按同一会话用户显式引用知识对象后助手回复也引用该对象计算；其他用户草案和运行不计入；响应不得包含完整对话正文、知识正文、密钥、完整外部请求/响应或模型 Prompt；前端 AI 助手工作台侧栏提供“助手效果指标”入口，按需加载并展示草案生成数、草案确认率、用户修改率、`@` 引用使用率、作业运行成功率、失败修复率和知识引用命中率，同时展示草案状态分布（待确认/已应用/已取消/已过期/失败）、`drafts_by_action` 草案类型拆分（总数、待确认、已应用、已取消、处理率）、运行追踪计数（成功/失败/总数、已修复/失败运行）和引用追踪计数（已引用用户消息/用户消息、知识命中/请求/引用），帮助定位哪类草案卡住并解释关键比率。回归见 `apps/api/tests/test_assistant_chat.py::test_ai_assistant_metrics_summarize_drafts_runs_and_reference_usage`、`apps/api/tests/test_assistant_chat.py::test_ai_assistant_metrics_reads_scheduled_job_runs_from_repository_read_model` 和 `apps/web/tests/AssistantPage.test.tsx::loads assistant effectiveness metrics from the workbench sidebar`。 |
...
| TC-AIBRAIN-ASSISTANT-FUNC-022B | v1.1 | P2 | 不同角色进入 AI 助手工作台 | 前端按当前用户角色展示角色快捷任务：`product_owner` 展示产品快捷任务（需求进展、反馈洞察、版本风险），`rd_owner` 展示研发快捷任务（任务阻塞、代码巡检、缺陷修复），`reviewer/test_owner/tester/release_owner` 展示测试快捷任务（测试缺陷、自动化测试、发布风险），`knowledge_owner` 展示知识快捷任务（知识库巡检、知识沉淀、知识权限），`admin` 展示管理员快捷任务（插件连接、AI能力、定时作业、运行失败）；产品角色点击“反馈洞察”必须回填周反馈洞察定时作业草案生成提示，点击“版本风险”必须回填发布风险分析草案生成提示，测试/发布角色点击“发布风险”必须回填发布风险分析草案提示，管理员点击“AI能力”必须回填“我要新增 AI能力配置”以进入新增任务向导中的 AI能力配置路径，避免入口停留在普通问答而不进入可确认草案闭环；后端助手 API 访问白名单必须同步允许 `test_owner/tester/release_owner/knowledge_owner` 访问 `/api/assistant/conversations` 与 `/api/assistant/chat`，这些角色发送“我要新增任务”应返回任务类型向导且不调用模型网关；高权限运维候选和结构化引用解析仍按对象类型限制，仅允许管理员、`system.admin` 或对应专项权限用户，其中定时作业和运行记录接受 `system.scheduled_jobs.run` 或 `system.scheduled_jobs.manage`；显式 `@定时作业名称 执行一次` 可由 `system.scheduled_jobs.run` 或 `system.scheduled_jobs.manage` 权限用户触发，缺少权限时返回 `permission_denied`；点击任一快捷任务只回填输入框，不自动发送、不创建草案、不触发外部动作；无匹配角色时不展示角色任务组。回归见 `apps/api/tests/test_assistant_chat.py::test_ai_assistant_allows_testing_delivery_roles_to_use_workbench_apis`、`apps/web/tests/AssistantPage.test.tsx::shows admin role quick tasks and routes AI capability into a draft-first guide prompt`、`apps/web/tests/AssistantPage.test.tsx::shows product quick tasks and routes feedback and risk into draft-first prompts`、`apps/web/tests/AssistantPage.test.tsx::shows engineering quick tasks for rd_owner users`、`apps/web/tests/AssistantPage.test.tsx::shows testing quick tasks for test_owner users` 和 `apps/web/tests/AssistantPage.test.tsx::shows knowledge quick tasks for knowledge_owner users`。 |
