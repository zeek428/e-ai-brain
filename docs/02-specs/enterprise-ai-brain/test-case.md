...
| TC-AIBRAIN-ASSISTANT-FUNC-022B | v1.1 | P2 | 不同角色进入 AI 助手工作台 | 前端按当前用户角色展示角色快捷任务：`product_owner` 展示产品快捷任务（需求进展、反馈洞察、版本风险），`rd_owner` 展示研发快捷任务（任务阻塞、代码巡检、缺陷修复），`reviewer/test_owner/tester/release_owner` 展示测试快捷任务（测试缺陷、自动化测试、发布风险），`admin` 展示管理员快捷任务（插件连接、AI能力、定时作业、运行失败）；点击任一快捷任务只回填输入框，不自动发送、不创建草案、不触发外部动作；无匹配角色时不展示角色任务组。回归见 `apps/web/tests/AssistantPage.test.tsx::shows admin role quick tasks and fills the run-failure prompt`、`apps/web/tests/AssistantPage.test.tsx::shows engineering quick tasks for rd_owner users` 和 `apps/web/tests/AssistantPage.test.tsx::shows testing quick tasks for test_owner users`。 |
