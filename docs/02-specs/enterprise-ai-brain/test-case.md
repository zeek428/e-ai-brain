...
| TC-AIBRAIN-KNOWLEDGE-FUNC-012B | 知识空间目录和资产导入 | P0 | MVP-C | 知识维护者已登录，系统启用 PostgreSQL，配置私有 MinIO/S3-compatible 对象存储 | 1. 创建知识空间 2. 创建目录 3. 上传 Markdown 文件 4. 查询文档列表、检索、文档资产、导入任务和资产预览 5. 使用非空间成员重试列表、检索、资产、导入任务和预览 | 新空间、目录、资产、导入任务、chunk set、文档和 chunk 均写入结构表；原始文件写入对象存储但事实源仍在 PostgreSQL；文档列表和检索可按 `knowledge_space_id`、`folder_id` 过滤，结果包含 `source_asset_id`、`active_chunk_set_id`、`asset_id` 和 `chunk_set_id`；文档资产列表和导入任务列表按知识空间/文档读权限过滤并返回文件名、解析器、切片策略、进度和目录信息；非成员不能读取空间文档、资产、导入任务或资产预览；页面支持空间筛选、目录选择、新建空间/目录、文件上传、文档资产查看和导入任务查看。后端回归见 `apps/api/tests/test_knowledge_space_assets.py`、`apps/api/tests/test_knowledge_management_foundation.py` 和仓储边界测试；前端回归见 `apps/web/tests/KnowledgePage.test.tsx` | 是 |
...
| TC-AIBRAIN-PLANNING-FUNC-022B | MaxCompute 每周用户反馈洞察抽取 | P1 | v1.2 | 管理员已配置 MaxCompute MCP 插件、生产连接和用户反馈产品；插件动作可返回 `row_count` 与 `insights` | 1. 在插件动作页选择“MaxCompute 每周用户反馈”场景 2. 检查表名、时间字段、返回字段和最大行数默认值 3. 展开高级 JSON 并修改请求配置 4. 创建 `user_feedback_insight_extract` 定时作业并绑定插件动作 5. 手动运行作业 6. 查询用户反馈洞察列表 | 页面无需人工从零编写复杂 JSON，但可直接修改请求配置和结果映射 JSON；动作保存为 `mcp_tool` 且包含 `tool_name=maxcompute.execute_sql`；定时作业从 `plugin_output_mapping.insights_path` 读取洞察，通过用户反馈 service 写入，`records_imported` 为新增洞察数，插件响应的源表行数保留在 `result_summary.plugin.response_summary` | 是 |
| TC-AIBRAIN-PLANNING-FUNC-022C | 插件配置可视化与试运行 | P1 | v1.2 | 管理员已进入插件管理；存在 HTTP 或 MCP HTTP 插件、连接和动作 | 1. 新增/编辑连接时通过认证字段填写 Header/Bearer 配置，并展开高级 JSON 反向应用 2. 点击连接测试 3. 新增 HTTP 动作时通过 Params/Headers 表格配置参数和系统变量，展开高级 JSON 验证同步，再从 JSON 应用回表格 4. 查看请求预览 5. 对动作执行试运行并查看映射命中 6. 新增定时作业时用插件输入映射表格选择 `{{current_date-7}}` 等变量，并展开高级 JSON 验证同步 | 连接测试展示 endpoint、协议、认证、HTTP/MCP 调用等诊断步骤和脱敏请求摘要；动作页面无需手写复杂 JSON 即可生成 `request_config.query/headers`，高级 JSON 与表格双向一致；试运行返回 `request_preview/response_summary/mapping_hits`；定时作业保存的 `plugin_input_mapping` 保留动态变量表达式 | 是 |
