...
| TC-AIBRAIN-PLANNING-FUNC-022F | 定时作业运行结果详情 | P1 | v1.2 | 管理员已进入任务中心 / 定时作业；存在一次用户反馈洞察抽取运行记录，运行记录包含 `result_summary.execution_nodes`、插件调用 ID、模型日志 ID 和 Skill 快照 | 1. 打开运行记录页签 2. 点击运行记录详情 3. 查看数据连接获取内容、经过 Skill 处理后的内容、结果动作反馈内容、结果摘要、插件快照、Skill 快照、Prompt 快照和作业配置快照 4. 手动触发一次作业 | 运行记录每行必须提供详情入口；详情弹窗展示运行状态、作业类型、执行模式、AI 模型、Agent、Skills、导入数、插件调用 ID、错误码/错误信息、`result_summary.execution_nodes.data_connection`、`skill_processing`、`result_action`、`result_summary`、`resolved_plugin_snapshot`、`resolved_skill_snapshots`、`resolved_prompt_snapshot` 和 `config_snapshot`；数据连接节点必须可看到脱敏请求摘要、输入映射、响应摘要和源数据数量；Skill 节点必须展示处理输入/输出、`model_gateway_called=true`、`processing_mode=model_gateway_json_transform` 和 `model_log_id`；结果动作节点必须展示写入目标、写入数量、生成 ID 和反馈内容；手动触发时按钮进入执行中状态并禁用重复触发，触发完成后自动打开本次运行详情，便于判断取数成功、AI 处理结果、生成洞察数量、写入目标和失败原因 | 是 |
