...
| TC-AIBRAIN-PLANNING-FUNC-022B | MaxCompute 每周用户反馈洞察抽取 | P1 | v1.2 | 管理员已配置 MaxCompute MCP 插件、生产连接、AI 模型、AI角色、每周反馈分析 Skill、可检索知识文档和用户反馈产品；动作可返回 `row_count`，模型可输出 `insights` | 1. 在动作页选择“MaxCompute 每周用户反馈”场景 2. 检查表名、时间字段、返回字段、最大行数和结果写入目标默认值 3. 新增定时作业时选择“每周用户反馈洞察抽取”场景模板 4. 确认表单按“基础信息、数据连接配置、AI执行配置、动作配置、调度配置”分区，自动带出产品、数据连接、AI执行方式、AI 模型、AI角色、Skills、知识引用、动作和 Cron，且不展示来源系统、时间参数、连接输入参数和结果写入覆盖 JSON 5. 保存后检查提交的 `plugin_input_mapping` 包含 `{{last_full_week.start}}` 和 `{{last_full_week.end}}` 6. 手动运行作业 7. 查询用户反馈洞察列表和模型日志 | 页面无需人工从零编写复杂 JSON；动作保存为 `mcp_tool` 且包含 `tool_name=maxcompute.execute_sql` 和 `result_mapping.write_target=user_feedback_insights`；定时作业模板必须保存 `plugin_connection_id/model_gateway_config_id/agent_id/skill_ids/plugin_action_id`，可选保存 `knowledge_document_ids`，并默认提交周起止动态变量映射；运行时先通过数据连接获取源数据，若配置知识引用则读取可检索知识 chunk 并以 `knowledge_references` 注入模型请求，再调用模型网关按 AI角色/Skill 输出结构化 JSON，最后按动作结果映射写入用户洞察表；`records_imported` 为新增洞察数，运行摘要展示数据连接源表行数、AI执行节点 `model_gateway_called=true`、`model_log_id`、`input.knowledge_references`、模型处理输出、写入目标和动作反馈 | 是 |
...
| TC-AIBRAIN-PLANNING-FUNC-022C | 插件配置可视化与试运行 | P1 | v1.2 | 管理员已进入插件管理；存在 HTTP 或 MCP HTTP 插件、连接和动作 | 1. 新增/编辑插件，确认分类、协议、风险等级和状态可回填并保存 2. 打开插件市场，查看 MaxCompute/GitLab/GitHub/邮箱/AI 执行器官方标准插件，确认推荐场景、连接模板版本、连接默认 payload、连接 schema、动作模板、安装状态、连接数和执行数展示正确，并从 GitHub 市场项点击配置连接和创建动作 3. 查看官方标准插件，确认页面显示官方标签且无编辑/删除入口 4. 新增连接时选择 MaxCompute、GitLab、GitHub、邮箱和 AI 执行器，确认 endpoint、认证方式、Params/Headers 平台参数自动带出并可继续人工修改 5. 新增/编辑连接时通过认证字段填写 Header/Bearer 配置，并通过 Params/Headers 表格填写公共 query/header，展开高级请求 JSON 验证同步和反向应用 6. 在连接列表选择“生产”等环境筛选并检查请求参数，再调用非法环境筛选接口 7. 点击连接测试，关闭诊断弹窗后查看连接列表最近测试列 8. 调用动作模板目录接口，确认返回 MaxCompute、GitHub、GitLab、邮箱和 AI 执行器结构化模板 9. 新增/编辑 HTTP 动作时通过服务端模板或 Params/Headers 表格配置接口级参数、系统变量和结果写入目标，切换写入目标确认 JSONPath 字段联动，展开高级 JSON 验证同步，再从 JSON 应用回表格 10. 在动作配置场景中分别选择 GitHub 代码巡检、GitLab 代码巡检、邮箱通知发送、邮件收取和 AI 执行器下达指令 11. 查看请求预览 12. 对动作试运行并查看映射命中、写入预览和试运行审计 13. 新增定时作业时选择场景模板，确认页面不展示连接输入参数和输出覆盖 JSON，但提交的模板默认映射保留 `{{last_full_week.start}}` 等动态变量表达式 | 自定义插件、连接和动作新增后均能从列表进入编辑维护；插件市场只读展示 MaxCompute/GitLab/GitHub/邮箱/AI 执行器官方插件的发布方、简介、推荐场景、`connection_template_version`、`connection_defaults`、`connection_schema`、动作模板、安装状态、插件 ID、连接数和执行数，页面必须把 `connection_schema.sections[].fields[]` 渲染为可读的连接表单字段摘要，字段包含 label、path、type、required 和变量支持标记；点击“配置连接”必须打开新增连接弹窗并带出服务端 `connection_defaults` 中的官方插件、默认名称、endpoint、认证方式、Params 和 Headers，AI 助手 GitHub/GitLab/邮箱/AI 执行器连接草案也必须复用同一连接模板 payload，不得在前端或助手中私有维护默认 endpoint、认证或 Params/Headers；点击“创建动作”必须打开新增动作弹窗并优先套用服务端 `plugin-action-templates` 返回的对应模板；MaxCompute/GitLab/GitHub/邮箱/AI 执行器官方标准插件返回 `is_system=true`，页面不得提供编辑/删除入口，后端 PATCH/DELETE 返回 `409 PLUGIN_STANDARD_PLUGIN_LOCKED`；GitLab 连接默认生成 `PRIVATE-TOKEN` Header 认证和 `api_version/group_id/project_id` Params，GitHub 连接默认生成 Bearer Token、`Accept`/`X-GitHub-Api-Version` Headers 和 `owner/repo` Params，邮箱连接默认生成邮件网关/API endpoint、`Authorization` Header 认证、`Content-Type=application/json` Header 和 `send_protocol/receive_protocol/smtp_host/smtp_port/imap_host/imap_port/pop3_host/pop3_port/mailbox_folder/poll_since/default_from/default_to/subject_template` 收发参数，AI 执行器连接默认生成 `model-gateway://default` endpoint、`executor_type=model_gateway`、`runner_id=ai_executor_runner_system_default`、`supported_executor_types=model_gateway/codex/claude/hermes/openclaw`、`workspace_root`、`instruction_timeout_seconds` 和 `result_callback_url`；执行器列表必须展示只读“系统默认执行器”，健康状态为 `managed`，无编辑、删除、轮换 Token 或本地日志入口，系统默认动作调用应直接走平台默认模型并返回 `result_json`；连接保存 `request_config.query/headers` 并参与连接测试；连接列表环境筛选必须发送 `GET /api/system/plugin-connections?environment=<env>`，仅展示对应 `environment` 的连接，后端只接受 `default/dev/test/staging/prod/sandbox`，非法环境返回 `400 VALIDATION_ERROR`；连接测试展示 endpoint、协议、认证、HTTP/MCP 调用等诊断步骤，并在请求调试台展示最终 URL、解析后的 query、headers 明文值、Header 来源、原始请求配置、动态变量解析区块、可复制 cURL、完整请求 JSON 和远端响应摘要；动态变量解析明细必须展示变量位置、原始表达式、变量 token、偏移天数、解析值、最终请求值、状态和解析时区；无动态变量时仍显示动态变量解析区块并提示未检测到动态变量；连接测试完成后服务端必须保存 `last_test_summary`，连接列表必须展示最近测试状态、耗时和错误码/失败步骤，HTTP 远端错误还应展示响应状态码摘要；认证配置里的同名认证 Header 必须覆盖 Headers 表格旧值或占位，若最终请求仍包含 `***` 必须明文展示并在诊断中标记；动作模板目录必须返回 `code/name/plugin_code/action_type/default_code/default_name/request_config/result_mapping/form_defaults`，前端新增动作场景下拉只能由目录生成，并按模板动态回填真实插件、默认连接、请求路径、Params、Headers、结果写入目标和 JSONPath 映射；服务端模板缺失时，前端和 AI 助手不得硬编码生成 MaxCompute/GitHub/GitLab/邮箱/AI 执行器官方执行，只能提示模板缺失或要求刷新模板目录；动作页面无需手写复杂 JSON 即可生成 `request_config.query/headers` 和 `result_mapping.write_target`，结果写入目标切换后下方 JSONPath 字段必须按目标变化；请求预览和试运行按“连接默认值 + 动作覆盖项”合并同名 query/header；高级 JSON 与表格双向一致；PATCH 编辑时 `auth_config/request_config` 中的脱敏占位 `***` 不得覆盖服务端真实密钥；试运行返回 `request_preview/response_summary/mapping_hits/write_preview`，页面必须展示写入目标、预计写入、候选记录、预览值、报告字段预览、邮件投递字段或样例记录，并写入 `plugin_action.trial_succeeded/failed` 审计事件，payload 只包含插件、执行、连接、连接环境、输入字段名、写入目标、状态、耗时和错误码，不得保存完整请求响应、输入 payload 或密钥；定时作业保存的 `plugin_input_mapping` 保留动态变量表达式，输出映射为空时复用动作结果映射 | 是 |
补充验收约定：`TC-AIBRAIN-PLANNING-FUNC-022C` 中旧的“服务端模板不可用时使用本地兼容兜底”表述废止；官方动作模板必须以 `/api/system/plugin-action-templates` 为唯一配置源，模板缺失时前端和 AI 助手只能提示缺失，不得硬编码生成 MaxCompute/GitHub/GitLab/邮箱执行。
...
| TC-AIBRAIN-PLANNING-FUNC-022O | 官方插件模板版本和复制扩展 | P1 | v1.2 | 管理员进入任务中心 / 插件管理；系统已内置 GitLab、GitHub、邮箱、MaxCompute 和 AI 执行器官方插件 | 1. 打开插件市场 2. 查看每个官方插件模板版本状态 3. 打开插件列表 4. 点击 GitHub 官方插件的复制入口 5. 保存后编辑复制出的自定义插件 6. 尝试编辑/删除原官方插件 | 插件市场和插件列表必须展示 `template_version/latest_template_version/version_status/upgrade_available`，最新版本显示“v1 最新”或等价状态；`POST /api/system/plugins/{plugin_id}/copy` 生成 `is_system=false/source_plugin_id/source_plugin_code/template_version/version_status=custom` 的自定义插件；复制插件可编辑删除，原官方插件仍然不可修改删除并返回官方标准插件保护提示；复制时 code 冲突返回 409 | 是 |
