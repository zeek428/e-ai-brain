...
- P0 已落地插件连接、动作、AI 能力配置和定时作业创建草案确认闭环；显式 `@定时作业名称 执行一次` 已接入手动运行、运行追踪和未执行原因展示，删除/停用按后续切片扩展。
...
- P1/P2 已落地助手工作台工程化拆分：会话、引用、草案生命周期、运行轮询、运行状态和指标面板均拆入 hooks / components，助手页面样式迁移到页面级 `Assistant.css`，真实页面 smoke 覆盖桌面、窄屏和移动视口。
- P2 已落地确定性意图注册表雏形：任务向导、插件连接诊断、定时作业诊断、执行一次、指标解释和草案生成统一通过 registry 匹配 handler、priority 与 conflict_policy，文本 `@` 解析只作为结构化引用缺失时的兜底。
- P2 已落地助手效果指标增强：`/api/assistant/metrics` 支持 `window_days/date_from/date_to/product_id/role/action` 过滤，返回产品/角色维度、草案类型趋势、每日趋势、运行归因和导出接口，前端指标面板支持钻取和 CSV 导出。
...
| `ai_agent` | AI Agent | `system.ai_capabilities.manage` |
| `ai_skill` | AI Skill | `system.ai_capabilities.manage` |
