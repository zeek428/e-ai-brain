...
| assistant_action_drafts | AI 助手动作草案 | 记录聊天生成或显式创建的 `create_scheduled_job`、`create_plugin_connection`、`create_plugin_action`、`create_analysis_draft` 草案，包含用户、来源消息、客户端草案 ID、payload、风险、状态和确认/取消信息；确认前不写领域表或最终分析结果。 |
