...
自动派发是 DB-first 命令。准备阶段只构造 AI 任务、Runner 任务、attempt、执行上下文清单、可选 Agent Loop run/iteration、预算台账和审计事件记录；不得把 `PostgresRuntimeStore.audit_events` 或任何进程内集合当作中间传输。`dispatch_work_item_execution_bundle` 在同一 PostgreSQL 事务内完成工作项 compare-and-set、任务/Runner/attempt/上下文/自治循环/预算、协作事件与完整 `audit_events` bundle 的写入。Runner 插入、版本 compare-and-set 或任一 bundle 写入失败时全部回滚，重试与并发 Worker 都不可观察到孤立任务、清单、循环、预算或审计记录。
