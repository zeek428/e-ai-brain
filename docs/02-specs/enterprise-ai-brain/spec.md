...
| product_versions | 产品迭代版本 | 同一产品内 `code` 唯一，在需求交付菜单集中维护；主状态为 `planning / active / testing / ready_for_release / deploying / released`，`archived` 仅用于历史归档；自动需求归组只选择 `planning`。新增 `scope_version bigint NOT NULL DEFAULT 1` 作为需求范围事实版本，范围成员、纳入需求修订/验收标准及冻结仓库/分支基线变化时在版本行锁事务中递增。 |
...
| requirements | 需求实体 | 状态为 `draft / submitted / approved / planned / designing / ready_for_dev / developing / code_reviewing / testing / ready_for_release / deploying / released / accepted / rejected / deferred / cancelled / closed`；`source` 记录需求来源；`version_id` 可为空，最新正式评估未 accepted 或未归入规划版本时不能创建研发协作运行。 |
...
| rd_task_executor_policies | 统一研发执行策略 | 原表原位升级；保存 `policy_version bigint NOT NULL DEFAULT 1`、需求评估、规划版本归组、必需岗位、岗位执行器绑定、预算、质量门禁、Git、风险和交付终点配置。PATCH 使用乐观锁并在任何影响运行解释的变更后递增版本；同一业务大脑/产品最多一条 active 产品策略，同一业务大脑最多一条 active 默认策略。 |
| rd_task_executor_policy_role_bindings | 策略岗位绑定 | 保存策略内岗位、actor mode、显式真人用户/AI 数字员工候选、主/备用执行器、信任域、上下文、工具、预算、审核岗位和所需系统权限。 |
| rd_task_executor_policy_snapshots | 不可变研发执行策略快照 | 保存 `policy_id/policy_version/parent_snapshot_id/snapshot_kind/resolution_context_key/resolution_revision/schema_version/content_hash/payload_json/created_by/created_at`；除 base 的 parent 外身份和内容字段均 NOT NULL。base 使用 `resolution_context_key=policy:{policy_id}:version:{policy_version}`、parent null、revision 0；派生快照使用 `assessment:{assessment_id}`、parent 非空且 revision 1..2，父链触发器校验父子策略 ID/版本一致。数据库触发器禁止更新/删除，运行角色只可 INSERT/SELECT，策略和全部消费者外键均 `ON DELETE RESTRICT`。无收紧差异时 final/effective 直接复用 base。 |
...
| rd_collaboration_events | 协作事件 Inbox | 保存分配、问题、阻塞、交接、审核、返工、决策和重规划事件及幂等事件 ID，是 Graph 恢复的业务事件事实源。 |
| decision_requests | 人类决策请求 | 保存高风险、超权限、预算、冲突、门禁或部署边界问题的冻结 `options[]`、options_hash、证据、建议、决策人、`plan_version NOT NULL DEFAULT 0`、状态和乐观锁版本；option 固定包含 code/label/outcome/subject_transition/requires_comment/input_schema/effect_preview。状态为 `pending/waiting_more_info/approved/rejected/expired/cancelled`。 |
| rd_command_idempotency_records | 研发命令幂等记录 | 为评估创建、协作启动、claim、submit、review、decision、decision answer、工作项取消和经验审核保存 `command_type/aggregate_type/aggregate_id/idempotency_key/request_hash/result_type/result_id/http_status/response_hash/response_json/created_at`；脱敏响应快照与领域写入同事务提交，不设置 TTL/`expires_at`，不能重复使用已处理键。 |
| role_feedback_records | 岗位反馈证据 | 将审核、返工、门禁、交付结果和人工评价分别归因到 `brain_app_id/product_id/role_code/seat_id/human_user_id|ai_employee_id/executor_profile_id/work_item_id/attempt_id/strategy_snapshot_id`，并保存证据引用和 fingerprint；记录创建后不可覆盖。 |
...
| rd_collaboration_upgrade_state | 研发协同升级状态 | 保存维护围栏、Schema 版本、预检、切换、cleanup、v2 API/Worker/图版本、健康标记、写入冒烟和围栏解除证据；围栏只阻断研发写路径，不影响定时作业。 |
...
`109_requirement_driven_rd_collaboration.sql` 只做 v2.0 非破坏性扩展，创建评估、岗位、AI 数字员工、执行器档案、策略岗位绑定、版本协作、席位、会话、工作项、依赖、attempt、事件、决策、反馈和升级状态表，并增加可空关联字段；不得在该迁移中删除旧策略字段或取消任务。全部新代码完成后先启用维护围栏并运行预检/数据转换/切换脚本；新应用在不读取旧字段的状态下启动并通过健康验证后，显式 cleanup 命令以单事务执行 `110_requirement_driven_rd_cutover.sql` 删除旧字段。迁移 110 不注册到启动时 automatic additive runner。清理成功后恢复 v2 Worker、验证 Worker/Schema/图版本和 v2 写入冒烟检查，再解除围栏；解除后旧写路径仍永久拒绝。两次数据库迁移之间不启用第二套研发运行规则。
...
| 统一研发执行策略 | GET/POST/PATCH/DELETE | /api/delivery/rd-task-executor-policies, /api/delivery/rd-task-executor-policies/{policy_id} | 原入口原位升级；管理评估、版本归组、必需岗位、岗位执行器、预算、门禁、Git、风险和交付终点。写入必须整体校验并审计，不提供旧/新模式或策略外 fallback；已有快照的策略 DELETE 返回 `RD_POLICY_IN_USE`，只能停用。 |
| 需求评估 | POST/GET/POST | /api/requirements/{id}/assessments, /api/requirements/{id}/assessments/latest, /api/requirement-assessments/{id}/opinions, /api/requirement-assessments/{id}/answers, /api/requirement-assessments/{id}/decisions | 创建/查询正式评估，提交岗位意见、需求补充和 `accept/reject/request_more_info/request_rework/defer` 决策；请求使用 `request_id/requirement_revision/version`，评估引用初始/最终不可变策略快照；accepted 后进入 approved，再按自动或人工决策完成规划版本归组。 |
...
| 研发协作运行 | POST/GET | /api/product-versions/{id}/collaboration-runs, /api/requirements/{id}/collaboration-run, /api/delivery/rd-collaboration-runs/{id} | 产品版本是聚合根；启动请求包含 `request_id/scope_version`，并发同范围/快照幂等返回唯一活动运行，不同范围返回稳定冲突；运行引用不可变策略快照并保存人工暂停恢复字段。 |
| 协作工作项与决策 | GET/POST | /api/delivery/rd-collaboration-runs/{id}/work-items, /api/delivery/rd-work-items/{id}/claim, /api/delivery/rd-work-items/{id}/submit, /api/delivery/rd-work-items/{id}/review, /api/delivery/rd-work-items/{id}/cancel, /api/delivery/decision-requests/{id}/decide, /api/delivery/decision-requests/{id}/answers | claim 使用 `expected_version/lease_seconds/idempotency_key`；submit 使用 `attempt_id/lease_token/version/output/evidence/idempotency_key`；review 使用 `decision/comment/version/idempotency_key`；cancel 使用 `reason/version/idempotency_key`；decision 使用 `selected_option/comment/version/idempotency_key`；answers 使用 `answer/evidence/version/idempotency_key`。成功及幂等重放返回工作项、attempt、运行/决策版本、next_state、idempotent_replay 和 trace_id；状态写入由确定性编排服务控制。 |
| 岗位经验治理 | GET/POST | /api/delivery/rd-role-experiences, /api/delivery/rd-role-experiences/{id}, /api/delivery/rd-role-experiences/{id}/decide | 按岗位、产品、场景、状态、版本和证据主体查询版本化经验；使用 `decision/comment/version/idempotency_key` 执行批准、拒绝或退役。只有 approved 且范围/策略兼容的经验可作为带引用的后续上下文。 |
| 研发协同升级控制 | GET/POST | /api/system/rd-collaboration-upgrade/preflight, /api/system/rd-collaboration-upgrade/maintenance-fence, /api/system/rd-collaboration-upgrade/cutover | 管理员执行只读预检、研发维护围栏和一次性切换；围栏不影响定时作业。清理、v2 Worker/Schema/图版本验证和写入冒烟检查全部成功后才能解除围栏，解除后旧写路径仍永久拒绝。 |
...
**职责**: 原位升级需求交付下的研发执行器策略为统一研发执行策略。每个 active 产品必须解析到且只解析到一条有效策略快照；策略统一定义需求评估、版本归组、研发岗位、AI 岗位执行器、预算、质量门禁、Git 规则、风险和交付终点。

**核心规则**:
- 不新增第二套策略表、模式字段或兼容运行时。迁移 `109_requirement_driven_rd_collaboration.sql` 只做非破坏扩展；全部新代码完成后启用研发维护围栏并执行预检、策略转换和 draft 任务取消，切换为 Schema v2；新应用健康验证后由 `110_requirement_driven_rd_cutover.sql` 删除旧顶层字段。清理成功后恢复 v2 Worker、验证 Worker/Schema/图版本和写入冒烟检查，再解除围栏。围栏不影响定时作业；切换后旧请求 Schema 和旧写路径永久无效。
- 策略主记录以 `policy_version` 作为运行解释版本，创建为 1，受乐观锁保护并在运行契约变更后递增；同一产品最多一条 active 产品策略，未配置产品策略时才使用业务大脑 active 默认策略。
- `rd_task_executor_policy_snapshots` 是独立不可变事实表，不是 `rd_collaboration_runs` 内的可变 JSON 字段。base 快照按策略版本冻结；评估只有实际发生自动收紧时，才按 `assessment:{assessment_id}` 和 `resolution_revision=1..2` 创建引用父快照的 `assessment_resolved` 派生快照，无差异时 final/effective 复用 base。唯一身份列均非空，base/派生 CHECK 和父链触发器校验身份完整性；相同身份同哈希复用、不同哈希冲突。数据库触发器拒绝 UPDATE/DELETE，应用运行角色只授予 INSERT/SELECT；策略、父快照及 initial/final/effective 评估、岗位意见、协作运行、反馈、经验和经验来源全部使用 `ON DELETE RESTRICT` 外键。历史解释不得回读当前策略；存在快照的策略不得 DELETE，只能停用。
- 所有研发命令通过 `rd_command_idempotency_records` 统一持久化幂等键、规范化请求哈希、脱敏响应快照和结果引用；评估创建聚合键为 requirement，协作启动为 product_version，claim/review/cancel 为 work_item，submit 为 attempt，通用决策及补充答案为 decision_request，经验审核为 role_experience。幂等记录与领域状态同事务提交，相同键不同哈希统一返回 `RD_IDEMPOTENCY_CONFLICT`；claim 仅在有效租约内重放成功，过期后固定报租约失效且不创建 attempt。领域表的活动运行、初始快照评估和 attempt 等唯一约束继续作为第二道并发保护。
- 策略写契约唯一包含 `name/brain_app_id/product_id/status/matching_config/assessment_config/iteration_config/delivery_target/team_config/autonomy_config/quality_gate_config/git_config/deployment_config/role_bindings`；数据库将 `role_bindings` 保存到岗位绑定子表，不接受第二套同义字段。
...
- `autonomy_config`、`quality_gate_config`、`git_config`、`deployment_config` 和 `delivery_target` 统一控制轮次/预算、独立门禁、分支/提交/推送/合并、风险和交付边界。`delivery_target` 默认 `ready_for_release`；只有显式设为 `deployed` 时才可申请进入部署域。
...
**核心规则**:
- `role_feedback_records` 只保存原始归因证据；聚合器按 `role_code + product_scope + scenario + risk_scope + evidence_fingerprint` 创建或追加 `rd_role_experience_records` 新版本，不原地覆盖已审核内容。
- 经验状态为 `pending/approved/rejected/retired`。审核要求 `delivery.rd_role_experiences.decide`、产品范围和 `review_version` 乐观锁；唯一证据产生者不能自审。审核决定、退役和版本替代必须写审计。
- 查询接口按业务大脑、产品、岗位、工作项类型、场景、风险、仓库/工具信任域、最低置信度、状态、版本和证据主体分页筛选，并在数据库查询层执行调用者业务大脑/产品权限过滤。协作上下文检索只返回 `approved` 且未退役记录，并再次校验全部范围、信任域、置信度和当前策略允许版本。
...
- 工作项取消只通过 `POST /api/delivery/rd-work-items/{id}/cancel` 或其领域服务。低风险取消在一个事务内撤销租约并更新工作项、attempt、pending Review、关联 AI task、运行聚合、事件和审计，再写 Runner 取消 Outbox；高风险或影响必需交付时只创建决策请求并保持暂停。Runner 的迟到完成结果按已撤销租约拒绝，只能进入审计证据，不能复活工作项。
- 决策选项冻结 `outcome/subject_transition` 等映射，服务端只按映射执行主体状态迁移；`request_more_info` 进入 `waiting_more_info`，通过 answers 子资源补充证据、生成新 options/version 并回到 pending，补充期间不得恢复主体。
- 代码更改必须先通过平台独立质量门禁。策略允许的低风险提交/推送可自动执行；合并、敏感目录、迁移、安全问题、预算超限和权限越界按风险策略创建 `decision_requests`。默认不进入部署。
- 集成工作项通过 Outbox 完成远程 push/MR/PR 并保存 repository、分支、local/remote commit SHA、MR/PR、执行身份、Outbox 和对账状态的可信交付记录；发布就绪只验证这些证据，不在完成判定时临时 push。默认策略下产品版本进入 `ready_for_release`，协作运行进入 `completed(completion_reason=ready_for_release)`。
...
新增权限点统一为 `delivery.rd_roles.manage`、`delivery.rd_ai_employees.manage`、`delivery.rd_executor_profiles.manage`、`delivery.requirement_assessments.read/decide`、`delivery.rd_collaboration.read/plan/work`、`delivery.decision_requests.decide` 和 P1 `delivery.rd_role_experiences.read/decide`；策略继续使用 `delivery.rd_executor_policies.manage`，部署继续使用现有部署权限。所有写接口同时检查权限点、产品 scope、业务大脑 scope、运行席位和状态。默认只为 admin 授予全部新增权限，其他系统角色通过角色管理显式授权；岗位定义、员工档案和席位分配永不自动授权。
...
ready_for_release → completed（默认策略；completion_reason=ready_for_release，产品版本保持 ready_for_release）
...
awaiting_human → running/rework_required/cancelled（按 decision_request 冻结的 resume_state）
...
| testing | verify_release_readiness | ready_for_release | 协作编排/产品负责人/研发负责人 | 必需需求、测试、门禁全部通过，且每个必需仓库具有对账成功的远程分支/commit/MR-PR 可信交付记录；完成判定不得临时执行 push。 | rd_collaboration.ready_for_release / product_version.status_advanced |
...
→ product_version.scope_version 随范围事实变化递增；负责人从版本入口启动唯一 rd_collaboration_run，冻结最终策略、需求范围、仓库基线、岗位、员工和执行器
