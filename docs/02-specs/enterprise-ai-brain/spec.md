...
企业 AI 大脑平台采用基于 Ant Design Pro 的 React + TypeScript 前端、FastAPI 模块化单体、LangGraph、PostgreSQL + pgvector、Redis、GBrain 长期记忆层和 OpenAI-compatible 模型网关，覆盖产品、需求、详细设计、技术方案、代码变更、Review、测试、发布准备、部署、线上反馈、用户洞察、知识和审计。v1 系列建立了 AI 任务、Runner、Agent Loop、质量门禁、Git 写回、部署治理、定时作业和运营感知基础；v2.0 在不改变定时作业的前提下，将需求交付升级为正式需求评估、规划版本归组、统一研发执行策略和 AI/真人多岗位协作。研发策略绑定岗位和岗位执行器，但不复用定时作业的 `ai_agents` / `ai_skills` 装配；两条运行链路共享模型网关、Runner、权限、审计和基础设施能力，但保持独立的业务状态与策略快照。

v2.0 将产品需求确立为研发任务唯一入口，并把原研发执行器策略原位升级为单一统一研发执行策略。正式需求评估通过后，系统优先选择兼容的 `planning` 版本，无合适版本才创建新规划版本；随后创建版本级协作运行，将目标拆为有负责人、输入输出、验收标准和依赖关系的 DAG 工作项。LLM 只提出评估、拆解、风险和重规划建议，确定性服务负责状态、权限、调度租约、幂等、预算、风险下限和门禁。研发岗位可以动态增加，席位可由 AI 数字员工或真人账号承担；AI 数字员工、岗位和执行器是三个独立主体。默认策略在远程代码提交、开发测试和质量门禁完成后让产品版本停在 `ready_for_release`，协作运行进入 `completed(completion_reason=ready_for_release)`，不进入部署；部署仅在策略显式允许且人工门禁通过时复用现有部署域。现有定时作业的数据模型、调度和 AI角色/Skill 装配保持不变。
...
| AI 编排 | LangGraph + AgentLoopRun | LangGraph 负责状态图、检查点、人机中断和恢复；AgentLoopRun 负责跨 Runner 轮次、预算、独立门禁和停止原因。 |
...
| 回写集成 | 模拟 Issue 默认 + 受控 Outbox | 默认不产生远端副作用；显式授权的 GitHub/GitLab 评论、Review 和合并通过产品 scope、连接权限、门禁与幂等 Outbox 后执行。 |
| 运行部署 | Docker Compose | 满足本地演示和早期平台部署，避免过早引入 Kubernetes。 |
| 业务发布 | 人工 / SSH / Docker / Jenkins | 统一经过部署预检、产品环境执行资源授权、发布波次、健康验证、回滚和事务 Outbox；SSH/Docker 仅允许部署信任域 Runner 执行。 |
...
- 研发任务命中执行器策略后先冻结 `execution_context_manifests`，只保存需求、Bug、仓库、分支、知识版本与召回摘要、验收标准、权限和截断信息，不复制完整知识正文或凭据。
- `autonomy_mode=autonomous_loop` 按“计划 -> 隔离工作区编码 -> 独立验证 -> 失败分析 -> 下一轮”推进；每轮写入 `agent_loop_iterations`，并受最大轮次、总时长、Token/费用和安全边界约束。人工接管会停止继续派发并保留已有证据。
...
迭代版本驾驶舱由 `GET /api/product-versions/{version_id}/dashboard` 提供版本级交付健康聚合，入口位于需求交付/迭代版本列表行操作；版本列表默认按 `created_at desc` 展示最新版本，`/delivery/versions?version_id=<id>&view=dashboard` 必须直达版本总览，未携带 `view=dashboard` 的版本分支深链继续打开代码分支维护弹窗。接口先校验 `product.read` 和当前用户产品 scope，再聚合同版本需求、AI 任务、版本代码分支、Bug、代码巡检报告、分支质量治理、代码评审报告、知识沉淀、运维部署、发布记录、下一阶段状态影响和阻塞项；PostgreSQL 运行时必须通过版本范围专用 read model 读取该版本相关源数据，不得先调用全量 `get_task_workflow_source_rows()` 再在服务层过滤，知识 chunk 只读取计数和 embedding 覆盖所需轻量字段。Bug 明细要求 `bug.read`，代码巡检明细和分支质量治理要求 `code_inspection.read`，知识沉淀明细要求 `knowledge.read`，缺少子权限时对应明细降级为空并返回 `access_issues`，但不影响版本摘要、需求、任务和分支展示。代码评审报告通过版本内可见任务的 `code_review_report_id` 或报告 `task_id` 聚合，summary 必须返回 `code_review_reports` 和 `pending_code_review_reports`，前端展示报告摘要、待确认状态、风险等级、执行器、关联任务和代码评审报告入口。知识沉淀通过版本内可见任务的 `ai_task_id` 聚合，summary 必须返回 `knowledge_deposits`、`searchable_knowledge_deposits` 和 `vectorized_knowledge_deposits`，明细只展示沉淀 ID、标题、状态、来源任务、关联知识文档、知识文档标题、索引状态、chunk 数、embedding chunk 数、检索模式、索引错误摘要和更新时间，不返回知识正文，并提供沉淀全链路入口；`text_indexed` 对应关键词兜底，`indexed/vector_indexed` 且存在 embedding chunk 时对应混合检索，否则不可检索状态必须在明细中暴露。同版本代码巡检报告通过版本分支配置命中后，其 `created_bug_ids` 派生的 Bug 也必须纳入驾驶舱 Bug 明细、Bug 状态分布和发布阻塞项；版本总览还必须基于版本分支配置、同分支巡检报告和报告 finding 返回 `branch_quality_governance`，按分支展示报告数、问题数、严重问题数、活跃严重问题、严重问题 Bug 覆盖、整改需求覆盖、误报忽略、接受风险、过期接受风险、待审批忽略、质量门禁失败报告数、门禁失败项、最近报告和治理状态，summary 同步返回 `branch_quality_action_required`、`branch_quality_pending_scan`、`branch_quality_active_severe_findings`、`branch_quality_false_positives`、`branch_quality_accepted_risks`、`branch_quality_expired_accepted_risks` 和 `branch_quality_pending_suppressions`。接口必须返回 `governance_conclusion`，基于同一份 summary、阻塞项、分支治理和状态推进影响生成“版本暂不建议推进、版本需治理后推进、版本证据待补齐或版本具备推进基础”的总体判断、主要风险标签和下一步动作，供版本页、AI 助手和真实全链路回归复用；前端仅在旧响应缺少该字段时使用本地推导兜底。页面必须在摘要指标中展示待治理分支、门禁失败、待审批忽略和到期接受风险，并在下一步行动后展示版本治理结论，再在摘要指标外展示交付链路总览和发布准备清单，按需求范围、研发任务、代码分支、代码巡检、代码评审、Bug 收敛、知识沉淀、运维部署、发布证据和状态推进十类信息给出可推进、阻塞或待治理判断，其中交付链路总览必须按研发顺序突出红/黄风险环节，代码分支和代码巡检环节必须纳入待治理分支、待巡检分支、质量门禁失败、待审批忽略和到期风险，知识沉淀必须展示可检索数和向量就绪数，再展示需求、任务、Bug 状态分布，并把状态推进影响拆成“同步推进 / 阻塞 / 保持不变”明细，包含需求编号、标题、当前状态、目标状态和阻塞说明。阻塞项至少覆盖未完成需求、待确认 Code Review、发布阻塞 Bug、高风险代码巡检、缺少版本代码分支和缺少成功运维部署单；版本下一阶段为 `released` 时，若运维部署单中不存在 `success/succeeded/successful/passed/deployed/released` 等成功状态，也必须返回“缺少成功运维部署单”的发布阻塞项，`action_target_type=product_version`、`action_target_id=<version_id>`，前端跳转到按版本筛选的运维部署列表。每条阻塞项必须携带 `action_label`、`action_target_type`、`action_target_id` 和 `resolution_hint`，前端展示“解除条件”和处理入口，直接跳转需求、Bug、代码巡检、代码评审、版本分支或部署或发布记录管理页面；阻塞项区域必须先按严重级别和来源类型生成“阻塞处理队列”，展示优先级、风险来源、阻塞原因、解除条件和处理入口，再提供完整阻塞项明细表。前端必须用固定列宽与横向滚动承载明细，避免长标题、仓库 URL 或建议文本挤压操作区。版本总览弹窗展示逻辑由 `VersionDashboardModal` 独立承载，迭代版本主页面只保留列表数据、状态推进、需求归集和分支维护编排；弹窗内部摘要行动区、交付链路总览、发布准备清单、健康摘要和状态分布由 `VersionDashboardSummary` 承载，推进影响、阻塞处理队列、阻塞项、需求/任务和质量/交付明细表由 `VersionDashboardTables` 承载，日期/链接/状态影响/健康摘要/发布准备清单/阻塞处理队列计算由 `versionDashboardModel` 承载，后续版本健康展示优化应优先落在对应职责组件内。

接口还必须返回 `delivery_stage_overview`，由后端基于 summary、状态推进影响、阻塞项、分支质量治理、代码巡检、代码评审、Bug、知识沉淀、运维部署和发布记录生成研发顺序阶段投影。字段顺序固定为 `requirements/tasks/branches/inspections/code-reviews/bugs/knowledge-deposits/deployments/releases/status-impact`；每个阶段必须包含 `key/title/value/detail/level`，可处理阶段必须包含 `action_label/action_target_type/action_target_id`，并在可追踪时补充 `full_chain_subject_type/full_chain_subject_id`。版本页和 AI 助手必须优先消费该后端投影，只有旧响应缺失时才允许前端本地推导兜底。

接口还必须返回 `release_readiness_checklist`，作为发布准备清单的后端权威投影。该字段按 `requirements/tasks/branches/inspections/code-reviews/bugs/knowledge-deposits/deployments/releases/status-impact` 十项输出 `items[]`，每项包含 `key/title/value/detail/level/status/action_label/action_target_type/action_target_id`，`status` 至少覆盖 `ready/blocked/missing/risk/not_applicable`；清单级别必须返回 `blocked_items/missing_items/risk_items/ready_items/not_applicable_items/total_items/level/value/summary/title`。清单判断必须同时考虑状态推进阻塞、版本范围、任务关联、版本分支缺失或待治理、代码巡检质量门禁、待确认代码评审、严重或未关闭 Bug、知识沉淀检索可用性、运维部署成功/失败、发布记录成功/失败和已发布目标的成功部署及发布证据。版本总览“发布准备清单”区必须优先消费该字段，`delivery_stage_overview` 仅用于“交付链路总览”；旧响应缺少清单时才允许前端回退到交付阶段投影。真实全链路版本驾驶舱 helper 必须用 `validate_version_dashboard_release_readiness` 校验清单结构、顺序、计数、级别和阻塞压力。

接口还必须返回 `evidence_coverage`，作为版本推进前的证据覆盖评分。该字段按 `requirements/tasks/branches/inspections/code-reviews/bugs/knowledge-deposits/deployments/releases/status-impact` 十个证据域输出 `domains[]`，每个域包含 `key/title/value/detail/level/status/action_label/action_target_type/action_target_id`；`status` 取值至少覆盖 `covered/blocked/risk/missing/inaccessible/not_applicable`，其中缺少 `code_inspection.read`、`bug.read`、`knowledge.read` 或 `deployment.read` 时对应证据域必须标记为 `inaccessible`，并参与缺口计数但不泄露明细。`evidence_coverage` 必须同时返回 `covered_domains/gap_domains/blocking_domains/total_domains/score/level/summary`，供版本总览首屏展示、后续 AI 助手迭代问答和回归脚本复用同一份证据判断；前端版本总览必须在下一步行动后展示“证据覆盖”区，突出阻断域和待补齐域。
...
交付链路总览的十类阶段卡片必须直接暴露处理入口：需求范围进入版本需求筛选，研发任务进入首个任务或产品任务列表，代码分支进入版本分支维护，代码巡检进入按版本筛选的巡检页，代码评审进入首个待看评审，Bug 收敛进入版本 Bug 筛选，知识沉淀进入沉淀全链路，运维部署进入按版本筛选的部署单，发布证据进入按版本筛选的发布记录，状态推进直接触发版本推进弹窗。
...
运营治理下的运维部署使用独立菜单资源 `governance.deployments` 和路由 `/governance/deployments`，菜单要求 `deployment.read`。页面提供“部署单”和“部署方案”页签：部署单通过运营统一 read model 固定筛选“运维部署”并执行服务端分页，创建、执行和取消按钮分别要求 `deployment.create`、`deployment.execute`、`deployment.cancel`；部署方案维护要求 `deployment.scheme.manage`，支持人工、SSH、Docker 和 Jenkins 四种方式。人工方式使用 manual 通道；SSH/Docker 使用具备 `deployment` capability 的本地 Runner，平台只保存 Runner 和 target_code 引用，主机、私钥、known_hosts 与本地工作目录保留在 Runner。部署 Runner 的真实连通性由平台下发 `deployment_connectivity_probe` 任务验证，SSH 只执行 `true`，Docker 只执行 Engine/Compose 配置检查；Runner 的心跳或目标上报不能替代该证据，成功结果只在 10 分钟内有效。Jenkins 使用 integration 通道触发 active Jenkins 连接并由后台同步器收敛状态。部署单创建时固化方案快照，Runner/Jenkins 自动部署只能由执行通道回写终态，运行日志统一在部署页查看，外部取消先进入 `cancelling`。日志监控通过 `exclude_category=运维部署` 排除部署单，只保留 GitLab 指标、Jenkins 发布和线上日志。版本驾驶舱、需求全链路中的部署入口必须指向运维部署页面，Jenkins 发布入口继续指向日志监控。

v2.0 目标态以正式评估和协作编排为准：旧版批量生成任务入口停用；批量排期只用于有权限人员调整系统自动归组结果且目标必须为 `planning`。`batch-advance-status` 只允许在既无活动协作运行、也不存在非终态工作项时批量 `cancelled/closed`，任何设计、开发、评审、测试、待发布、部署、发布或验收目标返回 `RD_COLLABORATION_REQUIRED`。这些交付状态只能由协作工作项、审核、版本动作与门禁证据推进。公开 `POST /api/ai-tasks`、`/api/ai-tasks/{id}/start` 和 `/api/ai-tasks/batch-retry` 同样不接受真人或外部调用；协作编排器通过内部领域服务创建、派发和恢复工作项 AI 任务。批量分配需求业务负责人等不改变交付状态的管理动作继续保留并展示逐条结果。
...
v2.0 的任务失败恢复由工作项调度器复用内部任务派发状态机：只有关联工作项允许重试且当前失败可恢复、预算和策略仍有效时，调度器才能对原 task id 触发内部 retry；真人任务中心不再调用公开 `/start` 或批量重试推进研发。重试结果投影到工作项 attempt、任务、审计和协作事件；不可恢复、预算耗尽或风险升级时进入 `blocked/awaiting_human/rework_required/failed`。v1 历史任务的批量重试记录保留只读，切换时 draft 任务取消，不能成为 v2 旁路。
...
| 需求 | requirement, requirement_assessment | 业务问题、目标、约束、正式评估和版本组版入口 | 评估 accepted 后进入 approved 并优先组入规划版本；底层 AI 任务由版本协作工作项生成 |
...
| AI 数字员工 | rd_ai_employee | 可跨版本识别的 AI 员工身份、能力标签、人格版本和经验归因 | 与岗位和执行器分离；协作席位分别冻结员工与执行器，员工档案不携带密钥或授权 |
...
| 代码巡检 | code_inspection, scheduled_jobs | 周期性代码仓库质量、安全和规范巡检，按提交人沉淀报告、finding、通知反馈、严重问题 Bug、整改需求和治理概览 | 基于产品 Git 资源和插件扫描动作生成，关联产品、仓库、提交人、定时作业运行、Bug、整改需求和通知记录；治理概览从报告、finding、质量门禁、Bug 关联状态和整改需求派生状态聚合趋势、规则、仓库/分支/提交人排行和严重问题 SLA；历史整改任务字段只读 |
...
`requirement_assessments` 是需求进入研发迭代前的正式、可审计产物，不是页面临时预览。评估请求冻结需求版本和策略版本；LLM 输出价值、范围、完整度、风险、依赖、工作量、候选版本与理由，确定性服务校验 Schema、风险下限、权限和版本兼容性。评估 `accepted` 后需求原子进入 `approved`，组版成功后进入 `planned`；`waiting_human`、`needs_info`、`rework_required`、`deferred`、`rejected`、`failed` 或 `cancelled` 均阻止协作启动。`product_detail_design` 在评估通过和版本归组后生成正式详细设计，并继续通过 `human_reviews` 确认。`technical_solution`、`code_review`、Markdown 导出和知识沉淀应引用已确认的详细设计及其上游评估快照。Markdown 导出由独立 `export` router 调用 `task_workflow_context` 和 `markdown_export` service；代码评审报告读取由独立 `code_review_reports` router 调用 `task_workflow_context` 和 `code_review_report` service，二者均不得反向调用 `main.py` 中的 legacy handler/helper。
...
| `product_owner` | 产品负责人 | 维护产品上下文、处理需求评估与组版决策、确认产品详细设计和迭代建议 | 所负责产品、版本和模块下的需求、协作运行、AI 任务、Bug、知识引用和看板摘要 | 需求评估、产品详细设计确认、迭代规划采纳和产品侧优先级决策；不直接创建或启动 AI 任务 |
| `rd_owner` | 研发负责人 | 规划研发协作、确认技术方案、处理 Bug 和沉淀研发知识 | 授权产品下的协作运行、AI 任务、技术方案、GitLab 只读快照、Bug 和研发知识 | 技术方案确认、协作计划与异常决策、Bug 处理和研发知识沉淀；不直接创建或启动 AI 任务 |
...
| 产品管理 | admin, 产品负责人 | 产品列表、详情、版本、模块、Git 资源、相关系统和产品成员权限维护 |
| 需求管理 | product_owner, rd_owner | 需求列表、创建时间展示、新增需求、需求详情、正式评估意见/补充/决策、批量分配负责人、规划版本归组调整、安全批量取消/关闭；不提供交付状态批量推进、批量生成任务或直接启动 AI 任务 |
...
| user_external_identities | 外部登录身份绑定 | 记录 `provider=dingtalk`、`provider_subject`、`union_id/open_id/corp_id`、`user_id` 和状态；`provider + provider_subject` 唯一，同一用户同一 provider 只能有一条 active 绑定；已有 AI Brain 账号必须由当前登录用户显式发起自助绑定，不得仅凭邮箱或手机号静默关联；钉钉登录后仍以 AI Brain `users.id` 和 RBAC 权限作为业务授权边界。 |
...
| product_members | 产品成员和产品内职责 | 由产品管理页维护产品经理、研发负责人、开发工程师、测试负责人、测试人员、运维/发布负责人和观察者；保存后派生当前产品 scope、职责权限和菜单可见性，并写入成员变更审计。 |
| product_versions | 产品迭代版本 | 同一产品内 `code` 唯一，在需求交付菜单集中维护；主状态为 `planning / active / testing / ready_for_release / deploying / released`，`archived` 仅用于历史归档；自动需求归组只选择 `planning`。新增 `scope_version bigint NOT NULL DEFAULT 1` 作为需求范围事实版本；范围成员、纳入需求修订/验收标准/final-effective 策略快照及冻结仓库/分支基线只在版本尚未进入 `ready_for_release/deploying/released` 且不存在非终态协作运行时，于版本行锁事务中变更并递增；其他情况返回 `RD_SCOPE_FROZEN`，待发布后的变化必须进入新 planning 版本。 |
...
| requirements | 需求实体 | 状态为 `draft / submitted / approved / planned / designing / ready_for_dev / developing / code_reviewing / testing / ready_for_release / deploying / released / accepted / rejected / deferred / cancelled / closed`；`source` 记录需求来源；`version_id` 可为空，最新正式评估未 accepted 或未归入规划版本时不能创建研发协作运行。可选 `source_collaboration_run_id` 使用 `ON DELETE RESTRICT` 保存待发布后新增范围的来源运行，延续/替代既有需求时再保存可选 `supersedes_requirement_id`；CHECK 禁止只填 supersedes 而缺少来源运行。延迟约束触发器校验所有已填引用与新需求同产品且来源运行已达到待发布边界。后续需求仍从 submitted 独立评估，不能修改旧版本或继承 accepted 结论。 |
...
补充约定：结果写入目标注册表除 `scheduled_job_result`、`user_feedback_insights`、`code_inspection_reports` 和 `email_notifications` 外，必须包含 `dingtalk_document`。该目标的可视化字段包括钉钉文档链接或 ID、写入内容、写入方式、返回文档 ID JSONPath 和返回状态 JSONPath；钉钉连接仅表示 URL Key 授权入口，具体写入哪个文档由动作的 `result_mapping.write_target=dingtalk_document` 和 `request_config.arguments.nodeId/markdown/mode` 决定。用户可直接粘贴 `https://alidocs.dingtalk.com/i/nodes/...` 链接，系统保存和执行时自动提取 `/i/nodes/` 后的文档 ID 并写入 `nodeId`。“钉钉文档 - 更新内容”模板默认使用该写入目标，并在保存可视化动作时保留 `request_config.tool_name=update_document`、`request_config.mcp` 和写入参数，避免用户必须手工编辑高级 JSON；运行时继续兼容历史 `doc.update_document_content`、`document_id`、`content` 配置并转换为真实 MCP 参数。

补充约定：钉钉官方 MCP 标准插件的市场目录除通用字段外，还必须返回 `authorization_guide`、`capability_discovery`、`governance_policy`、`observability` 和 `business_scenario_templates`。`authorization_guide` 说明个人/系统/应用授权主体、URL Key 获取步骤和 Vault/env 凭据复用；`capability_discovery` 声明 `tools/list` 发现模式、已知工具和新增/下线/schema 变化策略；`governance_policy` 统一声明产品范围、允许角色和敏感读审计/写入前确认/通知防误发控制；`observability` 声明插件健康看板指标；`business_scenario_templates` 作为钉钉文档/知识库导入、巡检机器人通知和方案文档生成等组合模板入口。钉钉动作模板必须随 `risk_tier` 返回 `requires_human_review`、`governance` 和 `request_config.tool_schema`，敏感读、写入和通知类动作默认需要人工确认。钉钉连接列表提供动态发现入口，后端通过 `POST /api/system/plugin-connections/{connection_id}/discover-tools` 发起 JSON-RPC `tools/list` 并返回新增工具、缺失工具、schema 变化、处理建议和脱敏请求摘要；`GET /api/system/plugin-observability?provider=dingtalk` 聚合连接测试、插件调用日志和密钥过期信息，返回成功率、P95 延迟、失败原因、动作趋势、密钥预警和脱敏请求回放。

补充约定：钉钉官方动作模板名称统一使用“钉钉能力 - 操作”前缀，便于新增动作“配置场景”下拉集中检索；钉钉文档动作模板必须覆盖“钉钉文档 - 搜索文档”“钉钉文档 - 读取内容”“钉钉文档 - 创建文档”和“钉钉文档 - 更新内容”。“钉钉文档 - 更新内容”使用 `update_document` 工具，默认支持 `nodeId`、`markdown`、`format=markdown` 和 `mode=append|overwrite`，属于写入类高风险动作并默认要求人工确认。

补充约定：钉钉官方 MCP 连接以钉钉 AI Hub MCP 详情复制的实例级 `StreamableHttp URL` 为准，URL 形如 `https://mcp-gw.dingtalk.com/server/<instance>`，可直接携带 `?key=...`。服务端保存连接时必须从完整 URL 中提取 `key` 到 `auth_config.secret_ref`、去掉 Endpoint 中的 query，并清理旧模板残留的 `provider/mcp_id/server_name/auth_subject_type` 请求 query；旧的 `/mserver/{serverName}` 默认 Endpoint 仅作为历史兼容说明，不再作为新增连接默认值。
...
| assistant_action_drafts | AI 助手动作草案 | 记录需求、定时作业、插件、AI 能力和分析类确认草案；历史 `create_rd_task` 仅作为旧 key 兼容，v2.0 按 `create_requirement` 语义创建需求并返回评估入口，不写 AI 任务。草案继续保存用户、来源、payload、风险、状态、过期、查看和修改归因。 |
...
| ai_tasks | AI 工作项执行单元 | 评估 AI 意见任务由初始策略创建且禁止代码/Git/部署副作用；正式研发 AI 任务必须关联 `collaboration_run_id/work_item_id` 并使用冻结岗位执行器。任务中心可展示执行单元，但不能把单任务误报为版本完成。 |
| rd_task_executor_policies | 统一研发执行策略 | 原表原位升级；保存 `policy_version bigint NOT NULL DEFAULT 1`、需求评估、规划版本归组、必需岗位、岗位执行器绑定、预算、质量门禁、Git、风险、`experience_reuse_config` 和交付终点配置。PATCH 使用乐观锁并在任何影响运行解释的变更后递增版本；同一业务大脑/产品最多一条 active 产品策略，同一业务大脑最多一条 active 默认策略。评估/版本不保存显式策略覆盖。 |
| rd_task_executor_policy_role_bindings | 策略岗位绑定 | 保存策略内岗位、actor mode、显式真人用户/AI 数字员工候选、主/备用执行器、信任域、上下文、工具、预算、审核岗位和所需系统权限。 |
| rd_task_executor_policy_snapshots | 不可变研发执行策略快照 | 保存 `policy_id/policy_version/parent_snapshot_id/snapshot_kind/resolution_context_key/resolution_revision/schema_version/content_hash/payload_json/created_by/created_at`；除 base 的 parent 外身份和内容字段均 NOT NULL。base 使用 `policy:{policy_id}:version:{policy_version}`/parent null/revision 0；assessment_resolved 使用 `assessment:{assessment_id}`/parent 非空/revision 1..2；version_resolved 使用 `version:{version_id}:scope:{scope_version}`/同策略 base parent/revision 1。父链触发器校验父子策略 ID/版本一致；数据库禁止更新/删除，运行角色只可 INSERT/SELECT，策略和全部消费者外键均 `ON DELETE RESTRICT`。评估无收紧差异时 final/effective 直接复用 base；运行始终引用 version_resolved。 |
| rd_task_executor_policy_snapshot_sources | 版本策略快照来源 | 逐条关联 `version_resolved` 快照、来源 final/effective 快照、requirement 和 assessment，并保存 `created_at`；唯一约束 `snapshot_id + requirement_id`，不同需求可以共同引用同一个 base/final 快照，不对 `snapshot_id + source_snapshot_id` 建唯一约束。目标与来源必须属于同一 `policy_id/policy_version`，全部外键 `ON DELETE RESTRICT`；数据库拒绝 UPDATE/DELETE。延迟约束触发器还要保证来源集合与每个引用该快照的运行需求范围逐条完全一致。 |
| requirement_assessments | 正式需求评估 | 保存需求修订、`initial_strategy_snapshot_id/final_strategy_snapshot_id/strategy_snapshot_id`、结构化评估、完整度、风险、依赖、工作量、候选版本、归组理由、状态、LLM 建议和确定性校验结果；评估唯一键固定使用 initial 快照。 |
| requirement_assessment_opinions | 评估岗位意见 | 保存每个岗位、员工/执行器、输入版本、产生意见时的 `strategy_snapshot_id`、结构化结论、证据、置信度、风险、成本和意见轮次；策略收紧新增岗位时追加意见。 |
| rd_role_definitions | 研发岗位定义 | 独立于系统 RBAC，记录岗位编码、能力、职责、风险上限、可分配主体类型和启用状态；支持按实际团队增加岗位。 |
| rd_ai_employees | AI 数字员工 | 保存独立于岗位和执行器的稳定员工身份、业务大脑、编码、名称、能力标签、人格/工作风格版本和状态；不保存 Runner、模型密钥或执行授权。 |
| rd_executor_profiles | 研发执行器档案 | 记录 executor type、Runner/模型配置、工作区能力、并发、可承担岗位、健康状态和凭据引用，不保存员工身份或密钥明文。 |
| rd_collaboration_runs | 版本级研发协作运行 | 关联产品版本和单一 `version_resolved` 不可变策略快照，保存 `run_generation/supersedes_run_id`、scope/plan 版本、协作状态、交付终点、预算、图版本和时间。`supersedes_run_id` 只允许引用同版本 `failed/cancelled` 的最近一代运行并使用 `ON DELETE RESTRICT`；终态运行不得更新为非终态，同一版本的 generation 唯一且最多一个非终态运行。进入 `waiting_human` 时原子保存 `resume_state/suspended_decision_request_id/suspended_at`，恢复后清空。`suspended_decision_request_id` 外键 `ON DELETE RESTRICT`；CHECK 保证 waiting_human 时三字段同时非空且 resume_state 仅为 running/integrating/verifying，其他状态三字段同时为空。运行失败或取消后产品版本保持失败发生时的 active/testing 阶段，新一代 restart 另建运行和计划，不重开旧记录。 |
| rd_collaboration_run_requirements | 不可变运行需求范围 | 每条运行需求保存 `collaboration_run_id/requirement_id/requirement_revision/assessment_id/final_strategy_snapshot_id/acceptance_criteria_hash/repository_scope_hash/created_at`，不设置 `updated_at`；运行内 requirement 唯一，全部外键 `ON DELETE RESTRICT`，数据库触发器拒绝 UPDATE/DELETE。提交时延迟校验非空范围、assessment accepted、修订/快照对应，以及与运行 version_resolved 来源集合无缺失、无多余且逐条相等。 |
| rd_scope_change_requests | 受控范围变更请求 | 保存 `product_version_id/request_id/source_run_id/source_run_state/expected_scope_version/expected_run_generation/operations_json/operations_hash/reason/status/decision_request_id/applied_scope_version/requested_by/applied_at/created_at/updated_at`；status 只允许 `pending_decision/applied/rejected`。数据库触发器冻结版本、请求、来源、期望版本、operations、hash、reason 和申请人，只允许受控决策更新 status/applied 字段；CHECK 要求 pending 必有 decision，applied 必有 applied_scope_version/applied_at，其他状态不得残留 applied 字段。决策到期时请求保持 pending 并绑定升级后的决策，不引入无处理入口的 cancelled 状态。operations 仅允许增删需求、替换需求快照和更新仓库基线的类型化引用。来源运行与决策限制删除；running/integrating/verifying 来源使用运行暂停字段，draft/planning 来源由 pending 请求围栏继续调度，failed/cancelled 不改变终态，已有其他 waiting_human 决策时拒绝覆盖。批准时在同一事务中先按 run_generation 撤销租约/密文、终结全部非终态工作项/attempt/Review/AI task、写 Runner 取消与外部动作对账 Outbox，再终结非终态旧运行、应用全部 operations、仅递增一次 scope_version 并保存 applied 版本；迟到回写只进审计/对账。拒绝时仅恢复由本请求暂停的旧运行。 |
| rd_scope_change_request_operations | 不可变范围变更操作 | 保存 `scope_change_request_id/position/op/requirement_id/requirement_revision/assessment_id/final_strategy_snapshot_id/repository_id/branch_config_version/base_commit_sha/destination/created_at`；kind-specific CHECK 保证四类 operation 只填写各自允许字段，父请求内 position 唯一。requirement、assessment、snapshot、repository 和父请求均使用真实外键与 `ON DELETE RESTRICT`，数据库拒绝 UPDATE/DELETE；父表 JSON 仅作规范化命令快照，应用以类型化子行为准。 |
| rd_run_seats | 协作岗位席位 | 为运行内岗位绑定 AI 数字员工或显式真人用户；AI 席位另行冻结执行器档案，保存职责范围、容量、状态和替换历史。真人仍经系统 RBAC、产品范围、协作权限和席位鉴权。 |
| rd_role_sessions | 岗位连续会话 | 保存席位交接摘要、上下文游标和恢复元数据；不得作为工作项或决策业务状态事实源。 |
| rd_work_items | 协作工作项 | 保存目标、负责人席位、输入输出契约、验收标准、状态、`resume_state`、暂停 attempt、解除条件、风险、优先级、关联 AI 任务、审核人、租约和幂等键。自动派发退避状态为 `dispatch_failure_count/last_dispatch_error_code/next_dispatch_at`：只持久化稳定安全错误码，不保存异常原文、路径、Prompt、Token 或凭据；成功派发清零。PostgreSQL 候选保留同时携带工作项 `version` 作为 reservation version 和本次 `due_at`，最终派发事务在锁定行后必须同时重验版本与 `next_dispatch_at <= due_at`，不再到期的候选以 `RD_DISPATCH_RESERVATION_STALE` 无副作用跳过。 |
| rd_work_item_dependencies | 工作项依赖 | 保存 DAG 前后置关系、依赖类型和满足状态；写入时确定性校验无环，依赖满足后才能进入 ready。 |
| rd_work_item_attempts | 工作项尝试 | 保存每次领取、执行、结果、失败和返工证据；新 attempt 不覆盖历史结果。 |
| rd_collaboration_events | 协作事件 Inbox | 保存 `id/collaboration_run_id`、分配、问题、阻塞、交接、审核、返工、决策和重规划事件及幂等事件 ID，是 Graph 恢复的业务事件事实源；除主键外建立 `(collaboration_run_id,id)` 唯一键，供反馈使用同运行复合外键。 |
| decision_requests | 人类决策请求 | 保存高风险、超权限、预算、冲突、范围变更、门禁或部署边界问题的冻结 `options[]`、options_hash、证据、建议、决策人、`answer_actor_selector`、答案 Schema、`plan_version NOT NULL DEFAULT 0`、`expires_at/timeout_policy/escalation_target_selector/escalation_level/expired_at/expiry_event_id/supersedes_decision_request_id`、状态和乐观锁版本；option 固定包含 code/label/outcome/subject_transition/requires_comment/input_schema/effect_preview。范围变更固定以 `subject_type=rd_scope_change_request/decision_type=scope_change` 绑定其请求，并只冻结批准应用/重启或拒绝保留范围两个状态映射。状态为 `pending/waiting_more_info/approved/rejected/expired/cancelled`。默认 `timeout_policy=escalate_keep_paused`，到期不自动选择 option、不恢复主体，而是保持暂停并幂等创建后继升级请求。协作 Runner 安全阻断固定使用 `decision_type=runner_safety_approval`，不得由通用 Runner/插件审批接口代决或改写。 |
| rd_command_idempotency_records | 研发命令幂等记录 | 为评估创建、协作启动/restart、范围变更请求、claim、submit、review、decision、decision answer、工作项取消和经验审核保存 `command_type/aggregate_type/aggregate_id/idempotency_key/request_hash/result_type/result_id/http_status/response_hash/response_json/created_at`；脱敏响应快照与领域写入同事务提交，不设置 TTL/`expires_at`，不能重复使用已处理键。 |
| rd_command_replay_secrets | 限时命令重放密文 | 仅为 claim 有效期内重放保存 `command_record_id/secret_ciphertext/key_id/expires_at/scrubbed_at/created_at/updated_at`；每个命令记录最多一条，到期事务把密文置空并记录 scrub 时间，永久幂等事实仍由 command record 持有。 |

| role_feedback_records | 岗位反馈证据 | 将审核、返工、门禁、交付结果和人工评价分别归因到 `brain_app_id/product_id/collaboration_run_id/role_code/seat_id/human_user_id|ai_employee_id/executor_profile_id/work_item_id/attempt_id/strategy_snapshot_id`，并保存 `feedback_kind/source_event_id/feedback_fingerprint`、证据引用及实际生产者 `producer_subject_type/producer_subject_id/producer_role_code/producer_seat_id`。`(collaboration_run_id,source_event_id)` 必须以复合外键引用同一运行的持久化 `rd_collaboration_events(collaboration_run_id,id)` 且 `ON DELETE RESTRICT`；事件存在但属于另一运行时数据库拒绝反馈。规范化 fingerprint 至少覆盖 `run_generation/source_event_id/feedback_kind/role_code/seat_id/attributed subject/work_item_id/attempt_id/strategy_snapshot_id`，并以 `collaboration_run_id + feedback_fingerprint` 数据库唯一约束保证 Graph 恢复、事件重放和并发消费者只写一次。producer type/id 非空；类型化延迟约束触发器分别校验真人 `users`、AI 员工 `rd_ai_employees`，service 使用稳定系统主体代码，并由数据库 CHECK 限定为 `collaboration_orchestrator|quality_gate|delivery_reconciler|decision_expiry_worker`。`producer_role_code/producer_seat_id` 必须同时为空或同时非空；通过运行席位产生反馈时两者必填，`producer_seat_id` 限制删除并要求冻结岗位与席位一致。记录仅保留 `created_at`，创建后由数据库拒绝 UPDATE/DELETE。 |
| rd_role_experience_records | 版本化岗位经验 | 保存 `experience_key/version/brain_app_id/product_scope/role_code/work_item_type/scenario/risk_scope/repository_trust_domains/tool_trust_domains/content/evidence_refs/strategy_snapshot_id/confidence/status/review_version`；`strategy_snapshot_id` 外键指向候选生成时实际生效快照。状态为 `pending/approved/rejected/retired`；审核通过后才允许受控检索，不能直接修改生产策略。 |
| rd_role_experience_sources | 岗位经验来源 | 逐条关联经验版本、`role_feedback_record_id` 和该反馈的 `strategy_snapshot_id`，两者均使用真实外键和 `ON DELETE RESTRICT`；API 可聚合为 `source_strategy_snapshot_ids`。 |
| rd_collaboration_upgrade_state | 研发协同升级状态 | 保存 `fence_mode=disabled/draining/cutover_locked`、乐观锁版本、Schema 版本、围栏外/锁内预检、活动量、备份标记、切换、cleanup_started/cleanup_completed、v2 API/Worker/图版本、健康标记、写入冒烟、abort 原因/操作者/时间和围栏解除证据。draining 只阻断新研发写路径和 Runner 新领取，允许在途收敛和管理员受控取消；定时作业不受影响。cutover_locked 后禁止回到旧运行时。 |
...
| code_inspection_reports | 代码仓库巡检报告 | 记录产品、仓库、来源作业/运行、扫描快照、质量门禁、finding 汇总、`created_bug_ids`、`created_requirement_ids` 和通知；v2.0 不再新增整改 AI 任务，历史 `created_task_ids` 仅作只读审计兼容。 |
| code_inspection_findings | 代码仓库巡检问题 | 记录规则、严重级别、文件/行号、提交人、建议、Bug、`created_requirement_id` 和 suppression 审批；达到整改阈值时幂等创建或关联正式需求，report/finding 作为证据，之后进入评估。历史 `created_task_id` 仅只读。 |
| code_inspection_notifications | 代码巡检通知反馈 | 关联 code_inspection_report，记录 channel、target、status、message、request_config 和 response_summary；邮件渠道使用系统设置中的邮件发送配置作为统一发件箱投递到动作收件人，并在 response_summary 中记录 `delivery_status`、`message_id` 和收件人摘要；钉钉机器人等未接入真实发送器的渠道仍记录通知动作和反馈。 |
| graph_runs | LangGraph 运行记录 | 保存任务图或协作图 runtime、节点路径、thread/run 标识和 state snapshot 引用；v2.0 使用 PostgreSQL Checkpointer 持久化真实 interrupt/resume 状态。 |
...
| deployment_schemes | 部署方案 | 按产品和环境配置人工、SSH、Docker 或 Jenkins 部署方式、执行通道、Runner target 或 Jenkins Job、超时、默认标记、状态和乐观锁版本。 |
| deployment_requests | 运维部署单 | 按 product_id、version_id 和 requirement_ids 记录从测试完成到部署执行的准入、环境、风险、窗口、回滚方案、deployment_scheme_id、执行方式、执行通道、不可变方案快照和执行状态。 |
| deployment_request_requirements | 部署单需求范围 | 维护部署单与需求的多对多关系，支撑部署成功后批量推进需求到已发布。 |
| deployment_runs | 部署执行记录 | 记录部署单每次启动的方式/通道、Runner task 或插件调用、幂等键、外部 Job/Build、统一日志、同步租约、执行状态、失败原因和完成时间。 |
| jenkins_release_records | Jenkins 发布记录 | 按 product_id、version_id、可选 deployment_request_id、job_name、build_id 记录构建、部署、失败原因和触发人。 |
...
| status | enum | 是 | `draft`、`submitted`、`approved`、`planned`、`designing`、`ready_for_dev`、`developing`、`code_reviewing`、`testing`、`ready_for_release`、`deploying`、`released`、`accepted`、`rejected`、`deferred`、`cancelled`、`closed`。 |
...
| requirements | idx_requirements_supersedes_lineage | supersedes_requirement_id, source_collaboration_run_id | 组合索引 | 从待发布后的后续需求追溯可选旧需求与必需交付运行；CHECK 禁止 supersedes 脱离来源运行，同产品/待发布条件由延迟约束保护。 |
...
| rd_task_executor_policies | uk_rd_task_executor_policies_active_product | brain_app_id, product_id WHERE status='active' AND product_id IS NOT NULL | 部分唯一索引 | 同一业务大脑和产品最多一条 active 产品策略。 |
| rd_task_executor_policies | uk_rd_task_executor_policies_active_default | brain_app_id WHERE status='active' AND product_id IS NULL | 部分唯一索引 | 同一业务大脑最多一条 active 默认策略；产品策略优先于默认策略。 |
| rd_task_executor_policies | idx_rd_task_executor_policies_match | brain_app_id, product_id, status | 组合索引 | 按业务大脑和产品解析唯一 active 统一研发策略；matching_config 在单条策略内部承载细分规则，不再以多条 active 记录竞争。 |
| rd_task_executor_policy_snapshots | uk_rd_policy_snapshot_identity | policy_id, policy_version, snapshot_kind, resolution_context_key, resolution_revision | 唯一索引 | base 与各评估收紧轮次具有稳定独立身份；同一身份不同哈希必须冲突。 |
| rd_task_executor_policy_snapshots | idx_rd_policy_snapshot_content_hash | content_hash | 普通索引 | 校验和定位规范化快照内容；相同身份同哈希幂等返回既有记录。 |
| rd_task_executor_policy_snapshot_sources | uk_rd_policy_snapshot_requirement | snapshot_id, requirement_id | 唯一索引 | 每个 version_resolved 对同一需求只能有一条 final/effective 来源。 |
| requirement_assessments | uk_requirement_assessment_active_revision_initial_snapshot | requirement_id, requirement_revision, initial_strategy_snapshot_id WHERE status IN ('draft','evaluating','waiting_human','needs_info','rework_required','accepted') | 部分唯一索引 | 最终快照切换不能释放同一需求修订的评估业务键。 |
| rd_collaboration_runs | uk_rd_collaboration_runs_active_version | product_version_id WHERE status NOT IN ('completed','failed','cancelled') | 部分唯一索引 | 同一产品版本最多一个非终态协作运行。 |
| rd_collaboration_runs | uk_rd_collaboration_runs_generation | product_version_id, run_generation | 唯一索引 | failed/cancelled 后 restart 只能创建单调递增的新一代运行，旧运行不可重开。 |
| rd_collaboration_runs | uk_rd_collaboration_runs_supersedes | supersedes_run_id WHERE supersedes_run_id IS NOT NULL | 部分唯一索引 | 一个终态运行最多被一个直接后继代次引用，数据库层保证运行代际为单链。 |
| rd_collaboration_runs | idx_rd_collaboration_runs_suspended_decision | suspended_decision_request_id WHERE status='waiting_human' | 部分索引 | 决策处理和恢复时锁定当前暂停运行。 |
| rd_collaboration_run_requirements | uk_rd_collaboration_run_requirement | collaboration_run_id, requirement_id | 唯一索引 | 运行范围内每条需求只冻结一次；延迟触发器校验与策略来源逐条完全一致。 |
| rd_scope_change_requests | uk_rd_scope_change_request_idempotency | product_version_id, request_id | 唯一索引 | 相同版本和命令键只创建一个受控范围变更请求；operations 哈希不同则幂等冲突。 |
| rd_scope_change_requests | uk_rd_scope_change_request_pending_version | product_version_id WHERE status='pending_decision' | 部分唯一索引 | 同一产品版本最多一个待人工处理的范围变更，避免多个决策并发终结同一运行。 |
| rd_scope_change_request_operations | uk_rd_scope_change_request_operation_position | scope_change_request_id, position | 唯一索引 | 固定规范化 operations 顺序，防止重放时重复或覆盖类型化操作。 |
| decision_requests | uk_decision_requests_active_subject_plan | subject_type, subject_id, decision_type, plan_version WHERE status IN ('pending','waiting_more_info') | 部分唯一索引 | plan_version 非空；组版前使用 0，同一计划中的同一待决问题最多一个活动请求。 |
| decision_requests | idx_decision_requests_expiry_due | expires_at, id WHERE status IN ('pending','waiting_more_info') | 部分索引 | 协作维护 Worker 使用数据库时间批量领取到期待决请求。 |
| decision_requests | uk_decision_requests_expiry_event | expiry_event_id WHERE expiry_event_id IS NOT NULL | 部分唯一索引 | 并发和重复超时扫描只能生成一个过期事件。 |
| decision_requests | uk_decision_requests_supersedes | supersedes_decision_request_id WHERE supersedes_decision_request_id IS NOT NULL | 部分唯一索引 | 一个到期请求最多生成一个直接后继升级请求。 |
| rd_command_idempotency_records | uk_rd_command_idempotency | command_type, aggregate_type, aggregate_id, idempotency_key | 唯一索引 | 所有研发写命令按聚合和请求摘要幂等重放；相同键不同摘要冲突。 |
| rd_command_replay_secrets | uk_rd_command_replay_secret_record | command_record_id | 唯一索引 | 一个 claim 命令最多一条限时重放密文；过期后擦除密文而不删除永久幂等事实。 |
| rd_work_item_dependencies | uk_rd_work_item_dependency_edge | collaboration_run_id, plan_version, predecessor_work_item_id, successor_work_item_id, dependency_type | 唯一索引 | 防止 DAG 依赖边重复。 |
| rd_work_items | uk_rd_work_items_run_idempotency | collaboration_run_id, idempotency_key | 唯一索引 | 重规划或重复命令不能重复创建同一业务工作项。 |
| rd_work_items | idx_rd_work_items_dispatch_due | collaboration_run_id, next_dispatch_at, priority, id WHERE status IN ('ready','rework_required') | 部分索引 | 按到期时间读取可自动派发候选，跳过尚在退避窗口的工作项。 |
| rd_work_items | idx_rd_work_items_dispatch_due_page | collaboration_run_id, COALESCE(next_dispatch_at,'-infinity'), CASE WHEN priority=0 THEN 100 ELSE priority END, id WHERE status IN ('ready','rework_required') | 部分表达式索引 | 为有界候选页和每运行游标提供稳定的到期/优先级/ID 顺序。 |
| rd_dispatch_sweep_cursors | pk_rd_dispatch_sweep_cursors | id | 主键 | 全局自动派发轮询游标在事务中锁定，重启或多个 Worker 不会从同一运行永久开始。 |
| rd_dispatch_run_cursors | pk_rd_dispatch_run_cursors | collaboration_run_id | 主键 | 保存单个协作运行的续扫游标；同一事务写入最后检查候选，保证下轮可公平续扫。 |
| rd_work_item_attempts | uk_rd_work_item_attempt_number | work_item_id, attempt_no | 唯一索引 | 返工创建新 attempt，不覆盖历史尝试。 |
| rd_work_item_attempts | uk_rd_work_item_attempt_idempotency | work_item_id, idempotency_key | 唯一索引 | Runner 或人工重复提交不能创建重复尝试。 |
| rd_collaboration_events | uk_rd_collaboration_event_run_identity | collaboration_run_id, id | 唯一索引 | 为岗位反馈提供同运行复合外键目标，防止引用另一运行的事件。 |
| role_feedback_records | idx_role_feedback_producer | producer_subject_type, producer_subject_id, producer_role_code, producer_seat_id | 组合索引 | 经验审核通过全部来源反馈快速校验生产者主体及可选岗位/席位隔离。 |
| role_feedback_records | uk_role_feedback_idempotency | collaboration_run_id, feedback_fingerprint | 唯一索引 | 相同运行事件、反馈类型和归因上下文在 Graph 恢复、事件重放或并发消费时只生成一条不可变反馈。 |
| rd_role_experience_records | uk_rd_role_experience_version | experience_key, version | 唯一索引 | 同一经验键的版本不可覆盖。 |
| rd_role_experience_records | uk_rd_role_experience_one_approved | experience_key WHERE status='approved' | 部分唯一索引 | 同一经验键最多一个可检索批准版本，批准新版本前须退役旧版本。 |
| rd_role_experience_sources | uk_rd_role_experience_source_feedback | experience_id, role_feedback_record_id | 唯一索引 | 同一反馈证据不能在一个经验版本中重复引用。 |
...
| deployment_schemes | idx_deployment_schemes_one_active_default | product_id, environment | 条件唯一索引 | 每个产品环境仅允许一个 active 默认方案。 |
| deployment_schemes | idx_deployment_schemes_product_environment | product_id, environment, status, updated_at | 普通索引 | 查询产品环境可用部署方案。 |
| deployment_requests | idx_deployment_requests_product_status | product_id, status, updated_at | 普通索引 | 查询产品部署单和待执行/失败部署。 |
| deployment_requests | idx_deployment_requests_version_status | version_id, status, updated_at | 普通索引 | 查询迭代版本部署准入和成功部署证据。 |
| deployment_runs | idx_deployment_runs_request_time | deployment_request_id, created_at | 普通索引 | 查询部署单最近执行记录。 |
| deployment_runs | idx_deployment_runs_idempotency | idempotency_key | 条件唯一索引 | 防止同一次 Runner 或 Jenkins 启动重复创建外部执行。 |
| deployment_runs | idx_deployment_runs_sync_due | next_sync_at, sync_lease_until | 条件索引 | 后台领取需要同步的 Jenkins 运行。 |
...
Docker Compose 不得将 `apps/api/app/db/migrations` 挂载到 PostgreSQL 的 `/docker-entrypoint-initdb.d`，PostgreSQL 镜像也不得打包应用迁移 SQL。全新 volume 和存量 volume 都由 API 镜像内 `/app/app/db/migrations` 与 `api-entrypoint.sh` 作为普通 additive migration 的唯一控制面，避免 PostgreSQL initdb 与 API 启动形成两套执行顺序和例外规则。入口使用阻塞 PostgreSQL advisory lock 和 `app_schema_migrations(filename, checksum, applied_at)`：每个未登记普通 SQL 与其 SHA-256 账本记录必须在锁内作为独立事务提交，已登记文件必须校验和一致，否则失败启动并要求新增迁移；Repository 运行时不得重放普通迁移，只可处理明确例外的并发索引。该边界必须由 compose/PostgreSQL image 无应用迁移入口回归、API image 打包默认迁移目录回归、API entrypoint 账本/锁和排除列表回归共同固定。
...
`109_requirement_driven_rd_collaboration.sql` 只做 v2.0 非破坏性扩展，创建评估、岗位、AI 数字员工、执行器档案、策略岗位绑定、版本协作、席位、会话、工作项、依赖、attempt、事件、决策、反馈和升级状态表，并增加可空关联字段；不得在该迁移中删除旧策略字段或取消任务。全部新代码完成后先在围栏外运行只读预检；若 active 旧策略没有完整统一策略配置，预检返回待转换清单，必须由管理员在单一策略 API 中显式补齐岗位、账号和执行器映射，不能由切换服务猜测。随后进入 `draining` 收敛在途工作；零活动与备份条件满足后进入 `cutover_locked`，重跑锁内预检并执行单向切换。新应用在统一策略契约下启动并通过健康验证后，显式 cleanup 命令以单事务执行 `121_requirement_driven_rd_cutover.sql` 清理旧研发命令幂等记录。编号 110 已被增量统一策略迁移占用；迁移 121 不注册到启动时 automatic additive runner，普通 API 容器启动入口必须显式排除它。派发扩展的 `125_rd_dispatch_due_index.sql`、`126_rd_dispatch_page_index.sql`、`127_rd_active_run_dispatch_index.sql` 和 `128_rd_dependency_successor_index.sql` 同样不由逐文件启动事务直接执行；正常 API 运行时把这四个大索引委托给 repository schema-compatibility 路径，在 autocommit 连接上以同一非阻塞 PostgreSQL advisory lock 串行化，逐个检查索引 `indisvalid/indisready`，必要时使用 `DROP INDEX CONCURRENTLY` / `CREATE INDEX CONCURRENTLY` 修复或创建。未取得锁的并发启动实例立即跳过，不等待持锁实例；后续启动会再次校验。物理列收缩须在独立保留期评审后另行执行，不能与本次运行时切换混合。清理成功后恢复 v2 Worker、验证 Worker/Schema/图版本和 v2 写入冒烟检查，再解除围栏；进入 cutover_locked 后任一步失败都只能向前重试，解除后旧写路径仍永久拒绝。两次数据库迁移之间不启用第二套研发运行规则。

`080_system_settings.sql` 新增 `system_settings` 全局系统设置表，当前配置项为 `setting_key='system_admin_email'`，`setting_value.admin_email` / 兼容字段 `setting_value.email` 保存系统管理员邮箱，`setting_value.test_recipient_email` 保存常用测试邮件收件人，`setting_value.email_delivery` 保存系统级发信配置：`enabled`、发件邮箱、默认发件人、Reply-To、SMTP Host、端口、TLS/SSL 模式、SMTP 用户名、SMTP 密码或 `smtp_secret_ref`。邮箱客户端安全密码或应用授权码必须作为 SMTP 密码写入；系统设置页面不展示或提交 `smtp_secret_ref`，该字段仅作为 API 级高级能力表示 `env:SMTP_PASSWORD` 等外部密钥引用。首次配置或变更 `email_delivery` 敏感字段时，`PATCH /api/system/settings` 必须携带 `high_risk_confirmation.confirmed=true` 和确认原因，否则返回 `409 SENSITIVE_CONFIG_CONFIRMATION_REQUIRED`，错误详情只返回字段名，不返回密码、授权码或密钥值。`GET/PATCH /api/system/settings` 返回发信配置时必须脱敏，不得回显 `smtp_password`，仅返回 `smtp_password_configured` 与 `smtp_secret_ref_configured`；`POST /api/system/settings/email/test` 使用已保存配置发送带唯一测试编号、Date 和 Message-ID 的测试邮件，未显式传入收件人时依次回退到已保存测试收件人和系统管理员邮箱，响应返回投递状态、测试主题、消息 ID、收件人、发送时间和 SMTP 端点摘要；SMTP 拒收收件人时必须返回 `EMAIL_DELIVERY_TEST_FAILED`。迁移同时新增 `system.settings.manage` 权限点、系统管理 / 系统设置菜单 `/system/settings`，并为 admin 授权菜单和权限。系统设置写入必须通过 repository 直接写 PostgreSQL 并同步写入 `system.settings.updated` 审计事件；审计 payload 只记录已配置状态、变更字段、密码/密钥引用是否已配置和敏感配置确认状态，不记录 SMTP 密码、邮件正文、测试邮件内容或确认原因全文；MemoryStore 仅作为测试 helper 保留同名字段。
...
`043_official_devops_plugins.sql` 为 `integration_plugins` 增加 `is_system` 字段，并种子化 `gitlab`、`github`、`email`、`ai_executor` 官方标准插件；`074_internal_data_source_plugin.sql` 将 `internal_read_model` 纳入插件协议、将 `internal_query` 纳入动作类型，并种子化 `internal_data_source` 官方内部数据源插件；`075_internal_data_source_detail_permission.sql` 种子化 `system.internal_data_source.detail` 权限并默认授予管理员，用于控制内部数据源 detail 模式中的受保护字段；`081_dingtalk_mcp_plugins.sql` 将 `mcp_streamable_http` 纳入插件协议、将 `url_key` 纳入连接认证类型，并种子化钉钉文档、知识库、钉盘、AI 表格、机器人消息和通讯录官方 MCP 标准插件；`082_dingtalk_login_external_identities.sql` 新增 `user_external_identities`，用于保存钉钉登录身份与 AI Brain 用户的绑定关系、企业 corp 元数据和 active/unbound 状态；`087_user_profile_contact.sql` 为 `users` 增加 `mobile` 字段，支撑个人中心和用户管理展示联系方式。官方插件定义由系统维护，用户只能新增连接配置 endpoint、认证、HTTP 请求方法、Params/Headers/Body、URL Key 或内部只读过滤条件，不能修改或删除插件定义。标准动作场景使用系统模板冻结的调用方式、路径、工具名和动作级请求参数，页面只展示调用方式、连接和结果映射；仅无场景的自定义动作开放高级 HTTP/MCP 请求编辑。`ai_executor` 默认协议为 `runner_polling`，表示必须通过隔离 Runner 承接 Codex/Claude/Hermes/OpenClaw 等执行器，不能在 Web/API 进程中直接执行本机命令。
...
v1 基线迁移 `066_rd_task_executor_policies.sql` 新增 `rd_task_executor_policies`，并为 `ai_executor_tasks` 补齐可空 `ai_task_id` 反链。其原单任务匹配行为仅用于解释升级前数据；v2.0 由迁移 109 原位扩展为统一研发执行策略，并在迁移 110 后删除旧写字段，运行规则以“模块 B3.R”及 v2 API 契约为准。
...
`099_deployment_requests.sql` 新增 `deployment_requests`、`deployment_request_requirements` 和 `deployment_runs`，用于承载需求测试完成后的运维部署单、部署范围和执行记录；同时为 Jenkins 发布记录补充可选 `deployment_request_id`，并新增 `deployment.read/create/execute/cancel` 权限点。
...
| 产品成员 | GET/PUT | /api/products/{product_id}/members, /api/products/{product_id}/member-candidates | 在产品配置弹窗维护产品成员和产品内职责；查询要求 `product.member.read`，候选人与保存要求 `product.member.manage`，并按当前用户产品 scope 校验；产品范围内的成员管理员只能通过至少 2 个字符的 `keyword` 搜索候选人，全局产品管理员可查看完整候选列表；保存采用整体替换语义并支持 `expected_revision` 并发校验，成员职责会派生产品 scope、职责权限和菜单可见性。 |
...
| 迭代版本驾驶舱 | GET | /api/product-versions/{version_id}/dashboard | 查询单个迭代版本交付健康聚合，要求 `product.read` 并按版本归属产品校验 scope；PostgreSQL 运行时使用版本范围专用 read model，不先加载全量 task workflow source rows；聚合需求、AI 任务、版本分支、Bug、代码巡检、代码评审、知识沉淀、运维部署、发布记录、状态推进影响和阻塞项，Bug/代码巡检/知识沉淀明细按子权限降级隐藏。后端返回按阻塞严重级别和来源类型排序后的 blockers，并提供前三条 `next_actions`，包含优先级、来源标签、处理目标和全链路主体；前端版本总览应在“下一步行动”区域优先展示 `next_actions`，并保留阻塞处理队列明细；推进到已发布必须存在成功运维部署单，Jenkins 记录作为构建/发布证据而非唯一发布门禁。 |
...
| 系统设置 | GET/PATCH | /api/system/settings | 管理全局系统配置，维护系统管理员邮箱、常用测试收件人和系统级邮件发送配置；要求 `system.settings.manage`，邮箱类字段非空必须通过格式校验，SMTP 启用时必须配置 Host、端口、TLS 模式、用户名以及密码或密钥引用，写入 `system_settings` 并记录不含密钥的 `system.settings.updated` 审计。 |
| 系统设置邮件测试 | POST | /api/system/settings/email/test | 使用已保存的系统级邮件发送配置向指定收件人、常用测试收件人或系统管理员邮箱发送测试邮件；要求 `system.settings.manage`，响应返回投递状态、测试主题、消息 ID、发送时间和 SMTP 端点摘要，不返回密码、密钥值或邮件正文。 |
...
| AI 助手会话删除 | DELETE | /api/assistant/conversations/{conversation_id} | 删除当前登录用户指定助手会话；请求体可携带 `conversation_ids` 批量删除折叠重复组，同步清理消息、聊天运行和由消息生成的草案记录，并写入审计。 |
...
| 需求批量状态管理 | POST | /api/requirements/batch-advance-status | v2.0 仅允许在既无活动协作运行、也不存在非终态工作项时批量 `cancelled/closed`；任何研发交付状态目标固定返回 `RD_COLLABORATION_REQUIRED` 且不更新数据。合法操作逐条返回 updated/skipped 并记录取消/关闭审计。 |
| 迭代版本状态推进 | POST | /api/product-versions/{version_id}/advance-status | 预览并推进版本状态，按版本阶段同步可推进需求状态，阻塞项返回风险明细。 |
| 需求批量排期 | POST | /api/requirements/batch-schedule | 仅用于有权限人员调整自动归组结果，将同产品 `approved/planned` 需求归集到兼容 `planning` 版本；必须重新校验策略、容量、仓库、交付终点和硬依赖，合法需求更新为 `planned`，不合规需求返回 skipped 或决策请求。 |
| 旧需求批量生成任务 | POST | /api/requirements/batch-generate-tasks | v2.0 固定返回 `RD_COLLABORATION_REQUIRED`，不创建任务或推进状态；前端移除入口，历史成功记录只读。 |
| 旧需求审批 | POST | /api/requirements/{id}/approve, /api/requirements/{id}/reject | v2.0 不再直接改变需求状态；无对应评估决策时返回 `REQUIREMENT_ASSESSMENT_REQUIRED`，调用方转到评估 decisions 接口。 |
| 需求关闭 | POST | /api/requirements/{id}/close | 关闭无需继续推进的需求；端点由 `app.api.routers.requirements` 单一路由承载。 |
| 旧需求生成 AI 任务 | POST | /api/requirements/{id}/generate-task | v2.0 固定返回 `RD_COLLABORATION_REQUIRED`；底层 AI 任务只由版本协作工作项创建。 |
| 兼容创建 AI 任务 | POST | /api/ai-tasks | v2.0 对真人、前端、插件和其他外部客户端固定返回 `RD_COLLABORATION_REQUIRED`；协作编排器通过内部 `create_ai_task_for_work_item` 领域服务创建关联任务。 |
| 兼容启动 AI 工作项 | POST | /api/ai-tasks/{id}/start | v2.0 对真人和外部客户端固定返回 `RD_COLLABORATION_REQUIRED`；协作编排器通过内部 `dispatch_ai_task_for_work_item` 派发，并强制使用冻结的岗位、AI 数字员工与执行器快照。 |
| 兼容批量重试 AI 任务 | POST | /api/ai-tasks/batch-retry | v2.0 对真人和外部客户端固定返回 `RD_COLLABORATION_REQUIRED`；可恢复失败由工作项调度器按 attempt、预算、策略和风险内部处理。 |
...
| 研发运营指标列表 | GET | /api/devops/operational-metrics | 聚合 GitLab 指标、Jenkins 发布、运维部署和线上日志，支持服务端分页、排序和筛选；接口按 `devops.read` 与 `deployment.read` 自动收窄可见类别，只有部署权限时仅返回运维部署，只有研发运营权限时强制排除运维部署，避免绕过菜单访问无权数据。 |
| 运维部署方案 | GET/POST/PATCH/DELETE | /api/devops/deployment-schemes, /api/devops/deployment-schemes/{scheme_id} | 按产品 scope 查询人工、SSH、Docker 和 Jenkins 方案；写操作要求 `deployment.scheme.manage`，PATCH 使用 version 乐观锁，删除前检查部署单引用。 |
| 部署执行资源 | GET/POST | /api/devops/deployment-runner-targets, /api/devops/deployment-jenkins-connections, /api/devops/deployment-jenkins-connections/{connection_id}/connectivity-probe | 要求 `deployment.scheme.manage`，Runner 候选必须同时具备部署能力、部署信任域、在线心跳和上报目标；目标的 `ready=true` 还要求近期平台受控真实探测成功，并且探测证据的本地配置指纹与当前目标一致。Jenkins 预检使用 API 读取服务与指定 Job，保存短期脱敏证据；响应不返回本地密钥、机器路径或连接认证配置。 |
| 运维部署执行 | GET/POST | /api/devops/deployments, /api/devops/deployments/{id}/connectivity-probe, /api/devops/deployments/{id}/connectivity-probe/logs, /api/devops/deployments/{id}/start, /api/devops/deployments/{id}/complete, /api/devops/deployments/{id}/cancel, /api/devops/deployments/{id}/runs/{run_id}/sync, /api/devops/deployments/{id}/runs/{run_id}/logs | 部署单固化方案快照；人工部署允许人工 complete，SSH/Docker 由 Runner 回写，Jenkins 由后台或手工 sync 收敛。启动前必须校验按环境、风险和方案收紧的探测有效期；Runner 探测返回任务状态、剩余等待、失败分类、日志链接和安全重试条件，活动任务复用以避免重复派发。统一日志与取消中状态按产品 scope 返回。 |
...
| 统一研发执行策略 | GET/POST/PATCH/DELETE | /api/delivery/rd-task-executor-policies, /api/delivery/rd-task-executor-policies/{policy_id} | 原入口原位升级；管理评估、版本归组、必需岗位、岗位执行器、预算、门禁、Git、经验复用、风险和交付终点。写入必须整体校验并审计，不提供旧/新模式或策略外 fallback；已有快照的策略 DELETE 返回 `RD_POLICY_IN_USE`，只能停用。 |
| 需求评估 | POST/GET/POST | /api/requirements/{id}/assessments, /api/requirements/{id}/assessments/latest, /api/requirement-assessments/{id}/opinions, /api/requirement-assessments/{id}/answers, /api/requirement-assessments/{id}/decisions | 创建/查询正式评估，提交岗位意见、需求补充和 `accept/reject/request_more_info/request_rework/defer` 决策；请求使用 `request_id/requirement_revision/version` 且不接受 `strategy_id`，评估引用初始/最终不可变策略快照；需要改策略时先更新统一策略并重新评估；accepted 后进入 approved，再按自动或人工决策完成规划版本归组。 |
| 研发组织 | GET/POST/PATCH | /api/delivery/rd-roles, /api/delivery/rd-ai-employees, /api/delivery/rd-executor-profiles | 分别管理动态研发岗位、稳定 AI 数字员工身份和 AI 执行器档案；真人席位引用用户账号，AI 席位同时冻结员工与执行器。 |
| 研发协作运行 | POST/GET | /api/product-versions/{id}/collaboration-runs, /api/product-versions/{id}/collaboration-runs/restart, /api/requirements/{id}/collaboration-run, /api/delivery/rd-collaboration-runs/{id} | 产品版本是聚合根；启动请求包含 `request_id/scope_version`，服务端把同一 policy_id/version 的多需求 final/effective 快照确定性合并为 version_resolved，并让不可变运行需求范围与来源关系逐条完全一致；并发同范围/版本快照幂等返回唯一活动运行。提交后 Worker 对 `planning/plan_version=0` 运行以脱敏冻结范围、策略摘要和席位调用统一模型网关，模型仅提出结构化工作项/依赖；确定性计划服务校验范围、岗位、审核分离和无环 DAG 后才激活运行。网关失败或无效提案不生成本地兜底计划，运行保持 planning 等待重试。failed/cancelled 终态不可重开，restart 使用 `terminal_run_id/request_id/scope_version` 创建新 generation 并引用旧运行。 |
| 受控范围变更 | POST/GET | /api/product-versions/{id}/scope-change-requests, /api/delivery/rd-scope-change-requests/{id} | 使用 `request_id/expected_scope_version/expected_run_generation/source_run_id/reason/operations` 提交类型化范围变更。待发布前创建唯一人工决策，并暂停执行阶段或围栏 draft/planning 调度；批准事务原子围栏旧 generation、收敛非终态子任务与外部 Outbox、终结旧运行、应用全部 operations、仅递增一次 scope_version 并返回 terminal_run_id 供显式 restart。拒绝只恢复该请求暂停的原阶段。待发布后只返回新 planning 版本和后续需求指引，不修改原版本。 |
| 协作工作项与决策 | GET/POST | /api/delivery/rd-collaboration-runs/{id}/work-items, /api/delivery/rd-work-items/{id}/claim, /api/delivery/rd-work-items/{id}/submit, /api/delivery/rd-work-items/{id}/review, /api/delivery/rd-work-items/{id}/cancel, /api/delivery/decision-requests/{id}/decide, /api/delivery/decision-requests/{id}/answers | claim 使用 `expected_version/lease_seconds/idempotency_key`；submit 使用 `attempt_id/lease_token/version/output/evidence/idempotency_key`；review 使用 `decision/comment/version/idempotency_key`；cancel 使用 `reason/version/idempotency_key`；decision 使用 `selected_option/input/comment/version/idempotency_key` 并校验 option input_schema；answers 使用 `answer/evidence/version/idempotency_key`，调用者需 answer 权限与冻结 selector 资格。决策冻结到期和升级规则，过期保持主体暂停且不得自动批准。成功及幂等重放返回工作项、attempt、运行/决策版本、next_state、idempotent_replay 和 trace_id；状态写入由确定性编排服务控制。 |
| 岗位经验治理 | GET/POST | /api/delivery/rd-role-experiences, /api/delivery/rd-role-experiences/{id}, /api/delivery/rd-role-experiences/{id}/decide | 按岗位、产品、场景、状态、版本和证据主体查询版本化经验；使用 `decision/comment/version/idempotency_key` 执行批准、拒绝或退役，审核人与所有来源反馈生产者隔离。只有平台标志和冻结 `experience_reuse_config` 同时允许且通过全部容量/范围/策略约束的 approved 经验可作为带引用的后续上下文。 |
| 研发协同升级控制 | GET/POST | /api/system/rd-collaboration-upgrade/preflight, /api/system/rd-collaboration-upgrade/maintenance-fence, /api/system/rd-collaboration-upgrade/cutover | 管理员执行围栏外只读预检、`disabled/draining/cutover_locked` 维护围栏和一次性切换；draining 允许在途收敛与受控取消，cutover_locked 后不可回退旧运行时，围栏不影响定时作业。清理、v2 Worker/Schema/图版本验证和写入冒烟检查全部成功后才能解除围栏，解除后旧写路径仍永久拒绝。 |
| 插件管理 | GET/POST/PATCH/DELETE/POST(invoke/trial/copy) | /api/system/plugin-marketplace, /api/system/plugins, /api/system/plugins/{plugin_id}/copy, /api/system/plugin-connections, /api/system/plugin-connections/{connection_id}/test, /api/system/plugin-system-variables, /api/system/plugin-actions, /api/system/plugin-actions/{action_id}/invoke, /api/system/plugin-actions/{action_id}/trial, /api/system/result-write-targets, /api/system/result-write-records, /api/system/plugin-invocation-logs, /api/system/ai-executor-runners, /api/system/ai-executor-runners/{runner_id}/test, /api/system/ai-executor-runners/{runner_id}/install-package, /api/system/ai-executor-runners/{runner_id}/rotate-token, /api/system/ai-executor-runners/{runner_id}/attestation-key, /api/system/ai-executor-tasks/claim, /api/system/ai-executor-tasks/{task_id}/complete, /api/system/ai-executor-tasks/{task_id}/logs, /api/system/ai-executor-tasks/{task_id}/cancel, /api/system/ai-executor-tasks/timeout-scan | 管理 HTTP/MCP HTTP/MCP stdio/Runner 插件、三方系统连接、动作配置和 AI 执行器；插件定义、连接、动作、调用日志和 AI 执行器管理类列表必须由服务端校验 `system.plugins.manage` 或系统管理员权限，不能只依赖前端菜单隐藏；AI 执行器任务列表、日志查询、取消和超时扫描还必须按产品 scope 过滤，产品受限用户只能看到或操作其授权产品定时作业、运行快照或研发任务派生出的 Runner 任务，scope 外任务按 404 处理；AI 执行器配置还维护 `trust_domain/trust_boundary_id/attestation_status/attestation_key_fingerprint`，Runner 仅可通过自身 Token 注册一次 Ed25519 公钥，不能自行激活或换键；管理员核对独立信任边界和指纹后才可激活，缺键或缺边界不得成为 active。插件管理页面不展示独立调用日志页签，插件调用日志和结果写入记录由定时作业运行详情、AI 助手诊断和执行诊断统一消费，`/api/system/plugin-invocation-logs` 与 `/api/system/result-write-records` 保留为运行排障兼容 API，列表同样必须通过定时作业或运行实例解析产品并按当前用户产品 scope 过滤；官方插件市场只读展示 GitLab/GitHub/邮箱/AI 执行器标准插件的简介、推荐场景、`connection_defaults/connection_template_version/connection_schema`、动作模板、安装状态、模板版本状态和连接/动作数量，并可引导新增连接、直接创建官方动作模板或复制为自定义插件；`connection_schema` 是页面动态表单契约，按 section/field 声明字段 label、path、type、required、options 和是否支持系统变量，JSON 仅作为高级修改；非官方插件、连接和动作新增后必须可编辑、可删除维护；GitLab/GitHub/邮箱/AI 执行器作为官方标准插件返回 `is_system=true`，不能修改或删除，实例 endpoint、认证和平台参数在连接里维护；MaxCompute 作为普通 HTTP 插件/连接维护，历史官方 MaxCompute 需自动降级为 `is_system=false/protocol=http`，编辑连接时不展示“项目与表配置”官方 schema；官方插件可通过 copy 接口生成 `is_system=false/source_plugin_id/template_version/version_status=custom` 的自定义插件，用于企业私有扩展，源官方插件保持只读；邮箱连接模板覆盖发送和收取参数，AI 执行器模板默认使用系统默认执行器 `ai_executor_runner_system_default`、`executor_type=model_gateway` 和 `model-gateway://default`，由平台默认模型网关直接执行指令；如需 Codex/Claude/Hermes/OpenClaw，本地 Runner 连接可改选对应执行器类型、Runner、工作区和回写地址；执行器列表必须只读展示系统默认执行器，`health_status=managed`、无需 Token/心跳/启动命令，不允许编辑、删除或轮换 Token；本地 Runner 列表继续展示 `health_status`、`heartbeat_age_seconds`、可复制 `setup_command`、Token 版本、最近轮换时间、最近任务状态和按目标系统生成的安装包下载入口，便于本地 Runner 注册和运维；执行器列表每行提供“测试”入口，展示系统托管或 Runner 注册、Token、类型、endpoint、心跳诊断，工具栏提供“超时扫描”入口并展示 `summary/next_actions` 运维建议；具备插件管理权限的管理员可轮换本地 Runner Token、查看任务日志、取消运行中任务并触发超时熔断；连接列表 API 支持 `environment/plugin_id/status` 查询筛选，环境只接受 `default/dev/test/staging/prod/sandbox`，但插件管理页面默认不把环境作为用户配置项：新增/编辑连接不展示环境字段，连接列表不展示环境筛选或环境列，动作/试运行连接下拉只显示连接名称；环境字段作为后台元数据保留，用于审计、定时作业环境筛选、运行排障和未来隔离；删除前必须提示使用清单，后端对被引用资源返回 409；连接密钥脱敏，PATCH 编辑不得用脱敏占位覆盖真实密钥；连接测试返回诊断步骤、原始与最终请求摘要、可复制 cURL、`variable_resolutions` 动态变量解析明细、`test_history` 最近记录、`action_template_draft` 动作草案和 `repair_suggestions` 修复建议，动作新增/编辑页面只要求用户选择连接，隐藏插件字段并由连接或场景模板推导 `plugin_id`，后端同步校验连接和插件归属；动作支持请求预览和试运行；配置和调用必须审计。 |
...
→ assemble_role_seats
→ propose_and_validate_work_dag
→ dispatch_ready_work_items
→ collect_results_and_run_gates
→ review_or_create_rework
→ interrupt_for_human_decision (按需)
→ integrate_and_push
→ stop_at_delivery_target
...
任务图与协作图都使用 PostgreSQL LangGraph Checkpointer 保存 thread、checkpoint、pending writes 和 interrupt payload；API 收到人工补充、审核或决策后以原 `thread_id` 调用 resume。`rd_collaboration_events`/事件 Inbox 与领域表是业务恢复事实源：领域状态、事件消费、审计和 Outbox 在同一领域事务提交；Checkpointer 可以独立提交执行游标，不宣称跨连接原子。Graph 节点恢复时重新读取领域状态并只发出幂等命令，Checkpoint 或领域事务任一侧局部失败后重复 resume 都不得重复 Runner、Git 或部署副作用。`graph_runs/graph_checkpoints` 保留业务审计投影，不替代领域事实或 Checkpointer。

自动派发是 DB-first 命令。准备阶段只构造 AI 任务、Runner 任务、attempt、执行上下文清单、可选 Agent Loop run/iteration、预算台账和审计事件记录；不得把 `PostgresRuntimeStore.audit_events` 或任何进程内集合当作中间传输。`dispatch_work_item_execution_bundle` 在同一 PostgreSQL 事务内先以不加锁查询取得工作项的父运行 ID，再严格按全域一致的 `rd_collaboration_runs FOR UPDATE -> rd_work_items FOR UPDATE` 顺序加锁；两把锁均取得后，必须重验工作项仍属于该父运行且状态为 `ready/rework_required`、reservation version/到期边界未失效，并确认父运行仍处于允许派发的 `running/integrating/verifying`。`waiting_human`、终态或其他暂停状态一律 fail closed。校验通过后，事务才完成任务/Runner/attempt/上下文/自治循环/预算、协作事件、完整 `audit_events` bundle 与工作项 compare-and-set 写入。Runner 插入、运行/工作项重验、版本 compare-and-set 或任一 bundle 写入失败时全部回滚，重试与并发 Worker 都不可观察到孤立任务、清单、循环、预算或审计记录；因此候选保留后父运行被暂停或工作项被取消时，陈旧 Worker 会等待既有父运行锁、按相同锁序继续并在锁后重验失败，不产生 PostgreSQL `40P01` 死锁，也不能把该项派发到暂停运行。

Runner 高风险指令命中 `AI_EXECUTOR_APPROVAL_REQUIRED` 时，协作域不复用通用 Runner/插件审批路径，而是原子暂停该工作项并创建专用冻结 `runner_safety_approval` 决策和审批请求；不得创建 Runner 任务或 attempt。审批请求和决策身份由 `work_item_id + attempt_no` 确定，审批已过期时仅为同一 attempt 追加确定性 `renewal_no`，重复 sweep、重启或并发处理复用同一身份。批准只写覆盖冻结 `blocked_operations` 的不可变审批快照；拒绝取消工作项。下一次派发在同一事务锁定工作项、决策和审批请求，按当前指令重新生成安全快照并严格比对审批来源、身份、选项、证据、操作集合、策略版本和未过期时间；任一篡改、过期、通用审批代入或不完全覆盖均 fail closed，不写派发 bundle。

可重试派发故障只将安全码、连续失败次数和 `next_dispatch_at` 写回仍处于 `ready/rework_required` 的工作项。前三次连续失败分别退避 5、10、20 秒；到期前不再候选，成功后清零；第 4 次不再自动重试，而是暂停并创建既有冻结决策机制中的 `dispatch_fault_resolution`。容量耗尽保持 `ready` 并延后，不计入退避；永久配置/冻结快照/岗位故障和 Runner 安全审批均暂停受影响项而不阻塞其他可运行项。

PostgreSQL 自动扫描只保留 `ready/rework_required` 且已到期的候选，使用有界页和批量依赖前驱查询，禁止为单个候选逐条加载依赖。全局 sweep cursor 与每运行 cursor 在同一保留事务内更新，运行按轮转顺序继续、运行内按到期时间/规范化优先级/ID 继续；因此 Worker 重启和多 Worker 不会永久偏向一个运行。一次 sweep 的 `limit` 约束所有已检查候选的派发、容量延后、可重试、升级和人工决策结果。保留候选将 reservation version 与 `due_at` 传入最终派发命令；事务中的版本与到期重验把并发退避更新或陈旧保留绑定到无副作用跳过，而非创建新的执行记录。
...
**核心规则**:
- 原始文本、PDF 和图片资产继续写入 MinIO/S3-compatible 对象存储；`knowledge_document_versions` 为解析与索引版本事实，只有新版本成功完成解析/索引后才切换文档 `active_document_version_id`，失败版本不得归档或覆盖旧 active 版本。
- `knowledge_processing_profiles` 按产品配置 `builtin/http/mineru/paddleocr/gotenberg/multimodal_gateway` Provider 和 OCR、版面、表格、图片描述能力。配置只能保存 `env:`/Vault 类凭据引用，不接收 Token、密码、API Key 明文；HTTP Provider 校验 endpoint、请求/响应大小和结构化返回。
- `parser_engine=multimodal` 必须引用 active 处理 Profile。归一化产物包括 `parsed_markdown`、`ocr_json`、`page_layout_json`、`table_json` 和 `image_annotations`，资产/chunk 保存文档版本、模态、页码、位置框、内容哈希和脱敏 Provider 元数据。
- 文档版本保存过期时间和 `fresh/expiring/expired/flagged_outdated` 新鲜度；后台/人工扫描只更新新鲜度，不篡改历史内容。引用反馈关联具体文档版本与 chunk，支持 `useful/not_useful/partial/incorrect/outdated`。
- 搜索与 RAG source 必须返回实际命中的 `document_version_id/version_number/freshness_status/modality/page_number/bounding_boxes/structured_asset/provider_metadata`，并继续在返回 chunk 前执行知识空间、角色和产品权限过滤。
...
- 运维部署单按产品、版本和需求范围归属；创建时必须绑定同产品环境的 active 部署方案并固化快照。测试完成或待发布需求可发起部署。生产方案支持 `all_at_once/canary/batch/blue_green`，并配置 `strict/warn/disabled` 窗口、受控预检、健康检查和回滚策略。
- 启动部署必须重新校验严格时间窗口、产品/版本、制品版本/Commit/摘要、阻塞 Bug、发布评估、回滚配置以及 `execution_resource_grants` 中的产品/环境 Runner Target 或 Jenkins Connection 授权。预检失败保持待执行，不得先把需求错误推进为部署中。
- 部署请求、运行、运行步骤、审计和外部派发 Outbox 使用 repository 组合写事务；SSH/Docker、Jenkins、健康验证和回滚均由 execution worker 认领并幂等执行。每一发布波次健康检查与冒烟验证通过后才进入下一波；失败按策略创建独立 `operation=rollback` 运行，或进入 `waiting_takeover`。
- 人工部署由负责人登记结果，SSH/Docker 通过部署 Runner 隔离执行，Jenkins 通过连接触发并同步；部署成功后需求进入 `released`，部署失败或回滚后回到 `ready_for_release` 并生成 `deployment_failure` 来源 Bug，外部取消在确认前保持 `cancelling`。自动部署不得通过人工 complete 伪造成功。
- 发布候选的强制集成门禁必须复用既有非生产 Jenkins 验收 Job；CI 仅创建平台部署记录并触发、同步该 Job，不得自动创建、重配或销毁部署环境。容器化 SSH/Docker/Jenkins 环境只用于可选协议回归，不能作为发布候选通过条件的替代。
- 部署单和部署方案管理列表传入分页参数时必须使用 PostgreSQL read model 完成产品 scope、筛选、白名单排序、count/page 并返回查询耗时；部署详情聚合方案快照、质量门禁、波次运行、步骤、健康、回滚、派发 Outbox 和审计，不从前端拼接历史全量列表。
...
- 定时作业新增和编辑表单不展示连接环境筛选，数据来源下拉只显示连接名称；历史作业或运行摘要中的连接环境仅用于排障和旧客户端兼容，不参与新配置筛选或提交。数据来源阶段必须区分“直接取数连接”和“授权连接 + 读取动作”：前者选择可直接返回业务数据的连接，后者先选择钉钉文档 MCP、GitHub、GitLab、邮箱等授权连接，再选择读取动作并填写具体对象参数；页面将来源方式写入 `config_json.orchestration.data_source_mode`，读取对象参数写入 `plugin_input_mapping`，运行层继续按 `plugin_connection_ids` + `plugin_action_ids` 调用。
- 需求协作派发给 AI 岗位时，Runner 指令与 `input_payload` 必须同时带入同一冻结工作项契约：目标、需求摘要、`input_contract`、`output_contract` 和验收标准；执行器不得以缺失契约为由臆测实现范围。质量验证 Runner 必须携带触发它的 `rd_work_item_attempt_id`，失败结果只能投影到该次 attempt；仓库根目录在白名单内时，安装包可访问该仓库名下 `.ai-brain-worktrees/<repo>/...` 隔离目录，不能因此访问其他仓库或任意父目录。
...
| `ScheduledJobTemplateCatalog` | 提供官方定时作业模板目录，声明模板版本、默认 payload、推荐场景和资源选择规则；任务中心页面与 AI 助手草案共用该目录生成周反馈洞察、代码巡检、邮件摘要和“同步钉钉文档”等作业配置。 |
| `ScheduledJobCatalog` | 提供作业配置注册中心，声明作业类型、可创建/可运行状态、不可用原因、必填资源规则、执行/调度枚举、代码巡检扫描/规则/结果动作选项和通用结果动作选项；通用结果动作支持保存运行结果、发送通知，以及 AI 模式 `plugin_action_invoke` 下的创建需求和同步钉钉文档；连接环境字段仅作为旧客户端兼容输出保留，定时作业 service 复用该 catalog 做后端校验，前端仅消费 catalog 渲染选项和校验提示。 |
...
v2.0 对 `AssistantDraftBuilder` 的覆盖规则：历史 `create_rd_task` key 只保留兼容解析，确认后按 `create_requirement` 语义幂等创建或关联正式需求，返回需求和评估入口；不得写入 `ai_tasks`、不得接收 `task_type/auto_start`，也不得绕过正式评估和版本归组。上表其余 AI 能力、插件和定时作业草案行为保持不变。
...
**职责**: 原位升级需求交付下的研发执行器策略为统一研发执行策略。每个 active 产品必须解析到且只解析到一条有效策略快照；策略统一定义需求评估、版本归组、研发岗位、AI 岗位执行器、预算、质量门禁、Git 规则、经验复用、风险和交付终点。

**核心规则**:
- 不新增第二套策略表、模式字段或兼容运行时。迁移 `109_requirement_driven_rd_collaboration.sql` 只做非破坏扩展；全部新代码完成后先执行围栏外只读 preflight。旧策略若没有完整统一配置，预检会阻断并列出待转换项；管理员必须使用唯一策略 API 显式转换，避免平台猜测岗位、真人/AI 账号或执行器映射。转换完成后再进入 `draining` 阻止新增研发写入和 Runner 新领取、允许在途收敛与管理员受控取消。活动量归零、所有旧 draft 已取消且备份确认后原子进入 `cutover_locked` 并重跑锁内 preflight，再激活 Schema v2；新应用健康验证后由不注册到启动迁移器的 `121_requirement_driven_rd_cutover.sql` 清理旧研发命令记录（110 已用于增量统一策略）。物理列收缩另行经过保留期与回滚窗口评审，不能在本次运行时切换中隐式发生。只有仍在 draining、Schema v2 未激活且 cleanup 未开始时可以按乐观锁审计化中止；进入 cutover_locked 后只能向前修复。清理成功后恢复 v2 Worker、验证 Worker/Schema/图版本和写入冒烟检查，再解除围栏。围栏不影响定时作业；切换后旧请求 Schema 和旧写路径永久无效。
- 策略主记录以 `policy_version` 作为运行解释版本，创建为 1，受乐观锁保护并在运行契约变更后递增；同一产品最多一条 active 产品策略，未配置产品策略时才使用业务大脑 active 默认策略。
- `rd_task_executor_policy_snapshots` 是独立不可变事实表，不是 `rd_collaboration_runs` 内的可变 JSON 字段。base 快照按策略版本冻结；评估只有实际发生自动收紧时，才按 `assessment:{assessment_id}` 和 `resolution_revision=1..2` 创建引用父快照的 assessment_resolved 派生快照，无差异时 final/effective 复用 base。评估创建请求不接受 `strategy_id`；改变策略只能通过统一策略 API 形成新 policy_version 后重新评估。协作启动要求全部需求 final/effective 快照属于同一策略版本，再按 Schema 的 merge operator 生成 version_resolved：允许集交集、禁止项/岗位/门禁/人工点并集、上限取严、任一来源要求待发布则版本待发布，预算保存基础总上限及逐需求清单；`experience_reuse_config` 的 enabled 取 AND、最低置信度取最大、条数/Token/时效上限取最小、信任域取交集、策略兼容级别取最严、独立审核取 OR。未声明 operator 的字段必须相同，冲突则建立人工决策并阻止运行。version_resolved 以同策略 base 为 parent，来源表按 requirement 唯一地关联 final/effective 快照；启动事务同时写入不可变 `rd_collaboration_run_requirements`，冻结需求 revision、accepted assessment、final snapshot、验收标准与仓库范围哈希。`DEFERRABLE INITIALLY DEFERRED` constraint trigger 在提交时校验运行范围非空、source_count 等于范围行数、两组 requirement 集合无缺失无多余且逐条 assessment/snapshot 对应。唯一身份列均非空，kind-specific CHECK 和父链/来源触发器校验身份完整性；相同身份同哈希复用、不同哈希冲突。数据库触发器拒绝快照、来源和运行范围 UPDATE/DELETE，应用运行角色只授予 INSERT/SELECT；策略、父快照、来源及全部消费者使用 `ON DELETE RESTRICT` 外键。历史解释不得回读当前策略；存在快照的策略不得 DELETE，只能停用。
- 所有研发命令通过 `rd_command_idempotency_records` 统一持久化幂等键、规范化请求哈希、脱敏响应快照和结果引用；评估创建聚合键为 requirement，协作启动/restart 为 product_version，claim/review/cancel 为 work_item，submit 为 attempt，通用决策及补充答案为 decision_request，经验审核为 role_experience。幂等记录与领域状态同事务提交，相同键不同哈希统一返回 `RD_IDEMPOTENCY_CONFLICT`。claim 仅在有效租约内从 `rd_command_replay_secrets` 解密同一 token 重放；到期清理置空密文并记录 scrub 时间，之后固定报租约失效且不创建 attempt，永久 command record 不删除。领域表的活动运行、运行 generation、初始快照评估和 attempt 等唯一约束继续作为第二道并发保护。
- 策略写契约唯一包含 `name/brain_app_id/product_id/status/matching_config/assessment_config/iteration_config/delivery_target/team_config/autonomy_config/quality_gate_config/git_config/experience_reuse_config/deployment_config/role_bindings`；数据库将 `role_bindings` 保存到岗位绑定子表，不接受第二套同义字段。
- `assessment_config` 定义评估岗位、模型用途、完整度、风险阈值和人工门禁；`iteration_config` 定义兼容版本过滤、容量、评分阈值与新版本命名规则；归组只选择 `planning`。
- `team_config` 和 `role_bindings` 定义所需岗位、员工候选、执行器及 AI 席位容量。岗位来自 `rd_role_definitions`；AI 席位从显式 `rd_ai_employees` 候选选择稳定员工身份，并单独冻结 active `rd_executor_profiles` 和正整数 `capacity`；P0 真人席位只从显式 `user_ids` 选择并校验系统权限和产品范围，不自动推断团队容量。
- `autonomy_config`、`quality_gate_config`、`git_config`、`experience_reuse_config`、`deployment_config` 和 `delivery_target` 统一控制轮次/预算、独立门禁、分支/提交/推送/合并、经验注入、风险和交付边界。经验复用配置默认 disabled，只有平台 P1 标志和冻结策略同时启用才检索，并限制最低置信度、策略兼容级别、条数、上下文 Token、时效及仓库/工具信任域。`delivery_target` 默认 `ready_for_release`；P1 标志 `RD_COLLABORATION_DEPLOYMENT_ENABLED` 关闭时不能创建或激活 `deployed` 策略，P0 API/UI/测试不依赖部署实现；标志开启且显式设为 `deployed` 时才可申请进入现有部署域。
- 策略配置必须完整且可解析。创建需求评估、协作运行或工作项时若找不到 active 策略、岗位执行器不可用或关键配置缺失，应返回明确阻断错误并创建决策/配置修复入口，不得回退到策略外模型网关、code-review executor 或本地确定性输出。
- LLM 通过 `model_gateway` 生成结构化评估、工作分解、负责人候选、依赖、风险、返工和重规划建议；实现类工作项还必须声明 `repository_id/path/mode(read|write)` 资源意图，且至少一条为 `mode=write`，仅有非空 `read` 声明同样无效；非实现类工作项可保留合法的 `read` 声明。平台编排服务校验 JSON Schema、DAG 无环性、岗位能力、权限、冻结容量、资源声明、并发、租约、预算、幂等和风险下限，并拥有所有状态迁移的最终写权限。同仓库重叠路径的无序 write/write 声明由确定性服务补成稳定的 `finish_to_start` 依赖并持久化冲突证据；容量已满的 AI 席位只延后就绪项，绝不创建无 attempt 的临时任务。
- 初始策略只使用产品、来源/类型和人工优先级等稳定输入；评估后策略最多自动单调收紧两轮。自动收紧只能降低自动化、收紧权限/风险/预算、增加门禁/岗位或把部署改为待发布；新增评估岗位必须补齐兼容意见。不同维度不可比较、可能扩大权限/预算/自动化或超过两轮时创建人工决策。
- 工作项准备就绪后按岗位席位派发：AI 席位复用 `ai_tasks`、`agent_loop_runs`、`ai_executor_tasks` 和 Runner；真人席位生成可领取/执行的工作项。一次自动派发 sweep 的 `limit` 限制所有已检查 AI 候选的终局：派发、容量延后、可重试、永久故障升级和高风险人工决策创建均各消耗一个配额，并且必须在创建高风险决策前计数。公开 AI 任务创建/启动接口不能由真人或外部客户端调用，编排器通过内部领域服务处理。AI 和真人提交相同的结果契约与证据，审核人不能与被审核席位相同；失败审核或质量门禁失败进入 `rework_required` 并保留原 attempt。工作项列表在既有读权限和运行产品范围校验后，按工作项返回 `status=running` 的 `active_attempt_count`，并返回最多最近 20 条、按不可变序号升序的 `attempt_history={items,total,truncated}`。历史项只投影 attempt 序号/状态、AI Task/唯一编码 Runner 任务 ID 与状态、服务端从原始 workspace 路径计算的 SHA-256 指纹、安全失败码、返工证据数量、有界时间戳，以及从同内部 attempt 的持久化 fence 事件派生的 `late_result_fenced/fence_event_id`；Runner 关系只能由冻结内部 attempt 引用反查。读取时严格白名单净化，不暴露内部 attempt ID、租约/Token/Hash、执行器/员工、幂等键、原始 workspace、输入/结果/失败 JSON、返工正文、Runner 配置/日志/结果、Provider 回调、凭据或任意嵌套值；调用方不能读取并不存在于工作项表的 `attempt_id` 推断派发事实。`blocked/awaiting_human` 保存平台冻结的恢复目标，只能在条件重新校验或决策完成后恢复、返工或取消。
- 真实恢复回归必须从可验证的故障前状态开始。质量返工通过真实任务输入让首轮产物明确违反最终验收条件，由独立 verification Runner 真实判失败；Harness 不得写 gate 结果。超时恢复可在显式 opt-in 的专用测试产品上，以 `expected_policy_version` 临时把统一 active 策略收紧为低 `timeout_seconds` 和 `max_iterations=1`，只允许在需求评估冻结不可变快照的 `try/finally` 窗口内存在，随后立即用故障策略版本恢复完整原 payload/status；恢复首次失败必须先紧急重试清理并让场景失败关闭，未确认恢复不得继续写后续数据。报告记录原/故障/恢复三个策略版本。取消恢复必须等待 Runner 公共状态为 `cancelled` 且服务端已持久化同 attempt 的迟到结果 fence 后才允许 resume；Harness 不提交 Provider 回调或 fence。
- 工作项取消只通过 `POST /api/delivery/rd-work-items/{id}/cancel` 或其领域服务。低风险取消在一个事务内撤销租约并更新工作项、attempt、pending Review、关联 AI task、运行聚合、事件和审计，再写 Runner 取消 Outbox；高风险或影响必需交付时先把工作项置为 awaiting_human、撤销租约、把 attempt 标为 suspended_for_decision、写 Runner 取消 Outbox，再创建决策请求。批准后完成取消；拒绝取消或选择继续时旧 attempt/lease 不复活，工作项重新校验后进入 ready，由新 claim 创建 attempt/lease。迟到结果按已撤销租约拒绝，只能进入审计证据。
...
- 任务详情展示协作运行、工作项 DAG、岗位席位、真人用户或 AI 数字员工、执行器、预算、决策请求、门禁、返工链和反馈归因。协作运行详情在既有读权限和产品范围校验后，只读投影按运行过滤的可信 Git 交付与对账证据；对账状态使用 `pending | reconciled`，只返回交付/工作项/仓库/分支/local-remote SHA/对账/验证时间/证据哈希和有界测试证据，不返回 Outbox、工作区、审批、Runner、Provider 回调、凭据、Token 或原始 payload。原生 Runner 的顶层、`result` 或 `parsed_output` JSON 只能提供严格匹配冻结运行/工作项/仓库/工作区的本地 commit、隔离分支及白名单测试证据；其中任何远程 SHA、对账或回调自报字段均忽略且不落库。测试证据只允许有限枚举/长度/数值范围内的标量，写入和读取投影均重新净化，禁止通过白名单字段嵌套或超限编码任意原始对象。Runner 的 Token、心跳、安装包、日志和健康检测继续由插件管理 / AI 执行器维护。
...
**职责**: P1 把 P0 已落库的不可变岗位反馈证据治理为可检索、可审核、可版本化的岗位经验，并在后续协作中受控复用；P0 不依赖经验候选、审核或注入即可完成开发测试与可信远程交付。

**核心规则**:
- `RD_ROLE_EXPERIENCE_ENABLED=false` 时不注册/开放经验管理端点、不生成候选、不注入上下文，P0 `role_feedback_records` 与协作套件保持可用；平台标志开启后才执行候选与治理，后续注入还必须要求当前运行冻结的 `experience_reuse_config.enabled=true`。
- `role_feedback_records` 只保存原始归因证据，并把被归因主体与实际反馈生产者分开：生产者固定为 `producer_subject_type/producer_subject_id`，通过运行席位产生时同时保存 `producer_role_code/producer_seat_id`。每条反馈必须引用持久化 `source_event_id`，服务端按运行代次、事件、反馈类型、归因主体、工作项/attempt 和策略快照计算规范化 `feedback_fingerprint`，通过 `(collaboration_run_id, feedback_fingerprint)` 唯一键幂等写入；Graph 恢复或事件重放只读取原记录。聚合器按 `role_code + product_scope + scenario + risk_scope + evidence_fingerprint` 创建或追加 `rd_role_experience_records` 新版本，不原地覆盖已审核内容。
- 经验状态为 `pending/approved/rejected/retired`。审核要求 `delivery.rd_role_experiences.decide`、产品范围和 `review_version` 乐观锁；服务端经 `rd_role_experience_sources -> role_feedback_records` 聚合所有 producer subject，审核者不得匹配任一生产者。`require_independent_reviewer=true` 时还比较非空 producer role/seat。审核决定、退役和版本替代必须写审计。
- 查询接口按业务大脑、产品、岗位、工作项类型、场景、风险、仓库/工具信任域、最低置信度、状态、版本和证据主体分页筛选，并在数据库查询层执行调用者业务大脑/产品权限过滤。协作上下文检索只返回 `approved` 且未退役记录，并按冻结 `experience_reuse_config` 再次校验全部范围、信任域、最低置信度、`policy_compatibility` 和最大时效；空信任域按 deny-all，`same_policy_version` 要求 policy ID/version 相同，`same_policy_schema` 仍要求同业务大脑、同产品、同 schema_version 且经验约束不宽于当前策略。确定性排序后受 `max_items/max_context_tokens` 双重限制。
- 来源生产者隔离无条件生效；`require_independent_reviewer=true` 时还要求审核岗位/席位与全部来源岗位/席位分离，false 不能放行任何来源生产者自审。
- 注入 Prompt 的经验必须携带经验 ID、版本和证据引用；与策略冲突、跨业务大脑/产品、低置信度或权限不匹配的内容不得进入自动执行 Prompt。经验不能自动提高预算、放宽权限、跳过质量门禁、改变交付终点或直接修改 active 策略。

新增权限点统一为 `delivery.rd_roles.manage`、`delivery.rd_ai_employees.manage`、`delivery.rd_executor_profiles.manage`、`delivery.requirement_assessments.read/decide`、`delivery.rd_collaboration.read/plan/work`、`delivery.decision_requests.decide/answer` 和 P1 `delivery.rd_role_experiences.read/decide`；策略继续使用 `delivery.rd_executor_policies.manage`，部署继续使用现有部署权限。answers 还必须命中请求冻结的 `answer_actor_selector`。所有写接口同时检查权限点、产品 scope、业务大脑 scope、运行席位和状态。默认只为 admin 授予全部新增权限，其他系统角色通过角色管理显式授权；岗位定义、员工档案和席位分配永不自动授权。
...
**核心规则**:
- v2.0 保持代码巡检定时作业定义、调度、锁、重试、AI角色/Skill 装配和历史解释不变，但 `create_task_for_severe_findings` 结果动作升级为兼容别名，实际调用统一需求适配器并写入/回填 `created_requirement_ids/created_requirement_id`；不得直接创建可启动的整改 AI 任务。后续工作项由需求评估、组版和版本协作生成。
- 新增作业选择代码巡检类型、代码巡检场景模板或 AI 助手生成代码巡检作业草案时，默认执行模式为 `ai_assisted`：先执行本地完整扫描并冻结 native 扫描快照，再通过系统默认 AI 执行器/模型网关和已启用 AI角色、Skill 归一化风险摘要与 finding，最后进入代码巡检报告、Bug、整改需求和通知等结果动作；仅当用户显式选择确定性执行或表达“不调用 AI、纯扫描、只扫描、静态扫描”时，才创建 `deterministic` 纯扫描作业。
...
- 报告详情的 suppression 治理应区分“误报/忽略”和“接受风险”。`reason=accepted_risk` 用于业务决定短期接受的真实风险，必须记录责任人和到期时间；到期后不删除历史审批，但重新进入治理概览、提交人治理待办和报告详情 action items，推动复核、转 Bug 或创建/关联整改需求。
...
- 看板趋势必须由后端基于同一批 PostgreSQL source rows 按自然日聚合后返回 `trend`，覆盖交付、风险、工程和用户四类趋势；前端使用 Ant Design Charts 绘制趋势图，只能在无 Canvas/浏览器绘制能力的测试环境降级为文本摘要，不能在生产页面伪造趋势数据。
...
**状态结构**:
```text
Requirement: draft | submitted | approved | planned | designing | ready_for_dev | developing | code_reviewing | testing | ready_for_release | deploying | released | accepted | rejected | deferred | cancelled | closed
Product: active | inactive
Product version: planning | active | testing | ready_for_release | deploying | released | archived
...
Requirement assessment: draft | evaluating | waiting_human | needs_info | rework_required | accepted | deferred | rejected | failed | cancelled
R&D collaboration run: draft | planning | running | integrating | verifying | waiting_human | ready_for_release | deploying | completed | failed | cancelled
R&D work item: queued | waiting_dependencies | ready | running | blocked | awaiting_human | awaiting_review | rework_required | approved | failed | cancelled
R&D decision request: pending | waiting_more_info | approved | rejected | expired | cancelled
R&D role experience: pending | approved | rejected | retired
...
```text
Requirement:
draft → submitted → approved（assessment accepted）→ planned（组入 planning 版本）→ designing → ready_for_dev
ready_for_dev → developing → code_reviewing → testing → ready_for_release → deploying → released → accepted
...
running/waiting_* → failed/cancelled（关联 v2 工作项时只能由协作编排器原子推进）
...
draft → evaluating → accepted/deferred/rejected/failed
evaluating → waiting_human → accepted/needs_info/rework_required/deferred/rejected
needs_info/rework_required → evaluating（创建新评估版本或新意见轮次）

Assessment actions:
opinions → 被分配的真人评估者或受控 AI 评估执行单元提交结构化岗位意见
answers → 需求负责人补充事实并创建新的需求/评估版本
decisions.accept/reject/request_more_info/request_rework/defer → accepted/rejected/needs_info/rework_required/deferred

R&D collaboration run:
draft → planning → running → integrating → verifying → ready_for_release
running/integrating/verifying → waiting_human（同事务冻结 resume_state/suspended_decision_request_id/suspended_at）→ 原阶段（决策服务按运行 version 恢复并清空暂停字段）
draft/planning/running/integrating/verifying/waiting_human → failed/cancelled（终态不可重开；终态事务清空暂停恢复字段，产品版本保持失败发生时的 active/testing 阶段；ready_for_release/deploying 的部署失败或回滚按部署域返回 ready_for_release，不走本迁移）
failed/cancelled → 新建 run_generation+1 的 draft 运行（仅显式 restart；新运行 supersedes 旧运行，旧记录不变）
ready_for_release → completed（仅 delivery_target=ready_for_release；completion_reason=ready_for_release，产品版本保持 ready_for_release）
ready_for_release → deploying → completed（仅策略 delivery_target=deployed 且人工门禁通过；completion_reason=deployed）

R&D decision request:
pending/waiting_more_info → expired（数据库时间到期；主体继续暂停并创建/复用升级请求，不自动批准）
pending → approved/rejected/cancelled
pending → waiting_more_info → pending（补充证据后生成新 options/version；主体保持暂停）
...
approved → retired
rejected/retired → 终态（新证据只能创建新版本）
...
|----------|------|----------|------|----------------|----------|
| planning | start_collaboration | active | 产品负责人/研发负责人 | 必须存在 accepted/planned 需求和完整策略；对版本加锁并冻结 scope，重复启动返回同一活动运行，直接 PATCH 返回 `PRODUCT_VERSION_STATUS_ADVANCE_REQUIRED`。 | rd_collaboration.started / product_version.status_advanced |
...
| testing | record_ready_for_release_evidence | ready_for_release | 协作编排/产品负责人/研发负责人 | 必需需求、测试、门禁全部通过，且每个必需仓库具有对账成功的远程分支/commit/MR-PR 可信交付记录；证据记录不得临时执行 push。 | rd_collaboration.ready_for_release / product_version.status_advanced |
| ready_for_release | deployment_started | deploying | 发布负责人/运维负责人 | 仅 `delivery_target=deployed` 且部署审批、资源和门禁通过；默认待发布策略不创建部署单。 | deployment_request.started / product_version.status_advanced |
| deploying | deployment_succeeded | released | 执行通道 | 只有现有部署域登记真实成功和部署后验证证据才能发布；失败/回滚退回 `ready_for_release`。 | deployment_request.completed / product_version.status_advanced |
...
| submitted | assessment_accepted | approved | 评估服务/有权限决策人 | 必须存在同需求版本的 accepted 正式评估；旧 approve 端点不能直接触发。 | requirement.assessment_accepted / requirement.updated |
| submitted | assessment_rejected_or_deferred | rejected / deferred | 评估服务/有权限决策人 | 必须通过评估 `reject/defer` 决策并使用乐观锁。 | requirement.assessment_decided / requirement.updated |
| approved | schedule_iteration | planned | 组版服务/产品负责人 | 必须绑定同产品兼容 `planning` 迭代版本并重新校验策略、容量、仓库、交付终点和硬依赖；候选争议创建 decision request。 | requirement.updated |
...
| draft / submitted / approved / planned / 交付中状态 / rejected / deferred | batch_advance_status | cancelled / closed | 产品负责人/研发负责人 | 只允许取消或关闭；存在活动协作、非终态工作项、待决请求或不可取消副作用时跳过。任何交付状态目标整个请求返回 `RD_COLLABORATION_REQUIRED`。 | requirement.batch_cancelled / requirement.batch_closed / requirement.updated |
| approved / planned | batch_schedule_iteration | planned | 产品负责人/研发负责人 | 批量请求必须指定同产品兼容 `planning` 版本；重新校验策略、容量、仓库、交付终点和硬依赖，只更新合法需求。 | requirement.batch_scheduled / requirement.updated |
...
| testing / ready_for_release | deployment_request_created | pending ops | 产品负责人/研发负责人/发布负责人 | 创建运维部署单时选择测试中或待发布需求并绑定同产品、同环境 active 方案；若存在未关闭 blocker/critical Bug、release_readiness_task_id 未确认或方案执行资源不可用，返回阻塞错误。 | deployment_request.created |
| planned | work_item_product_detail_design_started | designing | 协作编排器 | 只有版本协作中的就绪工作项可通过内部领域服务创建任务；公开 AI 任务创建/启动端点拒绝。 | rd_work_item.started / ai_task.created |
| designing | product_detail_design_approved | ready_for_dev | 协作编排器/Reviewer | 产品详细设计工作项审核通过后按计划释放下游技术方案工作项。 | review.submitted / rd_work_item.approved |
| ready_for_dev | development_work_started | developing | 协作编排器 | 必须存在已批准技术方案和就绪开发工作项。 | rd_work_item.started / ai_task.created |
| developing | code_review_work_started | code_reviewing | 协作编排器 | 必须存在同需求、同产品版本下待审变更和 PR/MR diff 快照。 | rd_work_item.started / ai_task.created |
| code_reviewing | test_work_started | testing | 协作编排器/Reviewer | 独立代码审核通过后释放测试工作项。 | review.submitted / rd_work_item.approved |
| testing | release_readiness_verified | ready_for_release | 协作编排器/Reviewer | 测试、远程 Git 对账和版本门禁通过后进入待发布。 | rd_collaboration.completed / requirement.updated |
| ready_for_release | deployment_started | deploying | 发布负责人/运维负责人 | 部署单按不可变方案快照启动；人工进入 running，SSH/Docker 投递 Runner，Jenkins 触发 Job。 | deployment_request.started / deployment.run.started / requirement.updated |
| deploying | deployment_succeeded | released | 发布负责人/运维负责人或执行通道 | 人工确认或 Runner/Jenkins 可信回写成功后，部署单需求范围内需求进入已发布。 | deployment_request.completed / requirement.updated |
| deploying | deployment_failed_or_rolled_back | ready_for_release | 发布负责人/运维负责人或执行通道 | 人工确认或 Runner/Jenkins 回写失败/回滚后需求回到待发布，并创建 `deployment_failure` 来源 Bug 作为后续修复入口。 | deployment_request.completed / bug.created / requirement.updated |
| deploying | deployment_cancel_requested | deploying | 发布负责人/运维负责人 | Runner/Jenkins 运行先进入 cancelling 并请求外部取消，需求保持 deploying，直至外部通道确认终态。 | deployment.request.cancel_requested / deployment.run.cancel_requested |
| deploying | deployment_cancelled | ready_for_release | 执行通道 | 外部通道确认取消后部署单进入 cancelled，需求回到待发布。 | deployment.request.cancelled / deployment.run.cancelled / requirement.updated |
| ready_for_release | release_readiness_confirmed | ready_for_release | Reviewer | 发布评估确认只证明可发布，不直接视为已发布；后续必须通过运维部署单成功完成发布。 | review.submitted |
| released | post_release_analysis_confirmed | accepted | Reviewer | 上线后分析确认后进入验收完成。 | review.submitted |
| approved/planned/交付中状态/rejected | close | closed | rd_owner | 存在活动协作运行、非终态工作项、待决请求或不可取消副作用时不允许关闭。 | requirement.closed |

#### AI Task

| 当前状态 | 动作 | 目标状态 | 角色 | 幂等/冲突规则 | 审计事件 |
|----------|------|----------|------|----------------|----------|
| draft | dispatch_from_work_item | running | 协作编排器内部服务 | 必须绑定 ready 工作项并使用冻结岗位、员工、执行器和策略快照；公开 `/start` 固定拒绝真人或外部调用。 | ai_task.started / rd_work_item.started |
| running | coding_runner_succeeded | running / quality_gate_running | system | 编码成功只进入独立验证，不得直接 merge；保存上下文版本和编码结果摘要。 | ai_task.quality_gate_started |
| running / quality_gate_running | quality_gate_failed | running / agent_loop_retrying / waiting_review | system | 自治策略且可重试、预算充足时创建下一轮；否则进入人工确认或停止，失败证据必须保存。 | quality_gate.failed / agent_loop.retry_queued |
| running / quality_gate_running | quality_gate_passed | waiting_review / completed | system | manual_review 或高风险进入人工确认；auto_commit 仅在独立证据、风险阈值和保护规则全部满足时完成。 | quality_gate.passed / review.created / ai_task.executor_auto_committed |
| running | human_takeover | waiting_review | rd_owner/admin | 停止派发下一轮并取消仍在运行的循环 Runner 任务，保留已有证据和接管原因。 | ai_task.agent_loop_human_takeover |
| running | request_more_info | waiting_more_info | system/ai | 必须保存可操作问题。 | ai_task.more_info_required |
| waiting_more_info | submit_answers | draft | 工作项负责人 | 补充后回到 draft，由协作编排器按原工作项和 thread 内部恢复；真人不直接调用 start。 | ai_task.more_info_submitted / rd_collaboration.event_recorded |
...
| draft/running/waiting_more_info/waiting_review | cancel | cancelled | rd_owner/admin 或协作编排器 | 非协作历史任务可沿用公开取消；任务存在 `collaboration_run_id/work_item_id` 时，公开单个和批量取消整个请求返回 `RD_COLLABORATION_REQUIRED`。协作取消必须通过工作项取消或人工决策服务，在同一事务更新 AI task、pending Review、attempt、工作项和运行聚合。 | ai_task.cancelled / review.cancelled / rd_work_item.cancelled / rd_collaboration.event_recorded |
...
用户新增需求
→ requirement 持久化
→ 正式需求评估解析 base 策略快照，多岗位输出意见，平台完成风险、权限和完整度校验；有实际安全收紧时生成 assessment_resolved 派生快照
→ accepted 后 requirement 进入 approved，优先归入兼容 planning 版本；无合适版本时按规则创建新 planning 版本
→ 无活动运行时，product_version.scope_version 随已接受范围事实变化递增；负责人从版本入口启动唯一 rd_collaboration_run，平台把同 policy_id/version 的多需求 final/effective 快照确定性合并为 version_resolved 并保存逐需求来源，再冻结需求范围、仓库基线、岗位、员工和执行器
→ LLM 提出工作项 DAG、负责人、依赖、风险和重规划建议；确定性平台校验并持久化计划
→ ready 工作项由 AI 数字员工或真人席位领取；AI 工作项内部创建关联 ai_task，并通过 Agent Loop / Runner 执行，不接受外部直接创建或启动
→ 工作项按需检索 knowledge_chunks，long_memory 可选补充 GBrain 长期记忆、知识图谱和缺口分析
→ 提交结果后执行独立审核、质量门禁、返工和人工决策；领域事件 Inbox 与 Outbox 唤醒原 LangGraph thread 恢复
→ 集成工作项完成远程 push 或 MR/PR Outbox、SHA 对账和版本级集成测试，写入不可变交付证据与岗位反馈
→ 产品版本进入并保持 ready_for_release，协作运行进入 completed(completion_reason=ready_for_release)，P0 不创建部署请求
→ 仅 P1 且策略 delivery_target=deployed、资源预检与人工发布门禁通过时，才由 deployment_request 进入现有部署域；失败或回滚保持/返回 ready_for_release
...
→ code_repository_inspection 定时作业通过插件扫描仓库质量/安全/规范 finding，写入 code_inspection 报告，严重问题派生 Bug 并创建/关联正式整改需求，历史任务字段只读，并记录通知反馈
...
- 代码巡检治理快速回归逻辑必须由 `scripts/full_chain_regression_code_inspection.py` 承接，包括本地完整扫描、质量门禁、Bug/整改需求写回、治理压力、提交人治理、趋势对比和版本总览代码巡检/Bug 阻塞校验；`scripts/full_chain_regression.py` 只导入 `validate_code_inspection_governance_quick_regression` 并保持 suite 编排，不得重新贴回代码巡检治理快速套件细节。
...
| Git 回写副作用 | v1 MVP code_review 只归档内部报告；v1.2 写回必须校验连接写权限、产品仓库归属、当前质量门禁和幂等键，由独立 Outbox Worker 派发，外部结果不确定时转对账或人工处理。 |
