...
角色定义的运行时来源是后端角色目录常量，持久化字典为 PostgreSQL `role_definitions` 表；`users.roles` 和知识 `permission_roles` 只允许引用该目录中的角色 code。系统管理/角色管理页面必须从 `/api/auth/roles` 只读展示角色定义；用户管理页面和知识权限配置必须从同一接口加载固定多选控件，不能让管理员自由输入或创建未定义角色。
