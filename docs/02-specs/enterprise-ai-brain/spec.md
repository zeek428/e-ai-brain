...
产品配置、需求台账、AI 任务、人工确认、Graph 运行态、GitLab MR 快照、Code Review 报告、知识文档、知识 chunk、知识沉淀候选、审计事件、Bug 记录、GitLab 每日代码指标、Jenkins 发布记录、用户反馈、用户使用指标、迭代规划建议/确认、模拟 Issue 回写、模型网关配置和模型调用元数据已开始细粒度持久化：产品、版本、模块、Git 资源、相关系统、需求、AI 任务核心字段、Review、Graph Run、Checkpoint、GitLab MR 快照、Code Review 报告、知识文档、知识 chunk、知识沉淀候选、审计事件、Bug 记录、GitLab 每日代码指标、Jenkins 发布记录、用户反馈、用户使用指标、迭代规划建议、迭代规划确认、模拟 Issue 回写、模型网关配置和模型调用元数据会同步写入 `products`、`product_versions`、`product_modules`、`product_git_repositories`、`related_systems`、`requirements`、`ai_tasks`、`human_reviews`、`graph_runs`、`graph_checkpoints`、`gitlab_mr_snapshots`、`code_review_reports`、`knowledge_documents`、`knowledge_chunks`、`knowledge_deposits`、`audit_events`、`bugs`、`gitlab_daily_code_metrics`、`jenkins_release_records`、`user_feedback`、`user_usage_metrics`、`iteration_plan_suggestions`、`iteration_plan_decisions`、`mock_issues`、`model_gateway_configs`、`model_gateway_logs`，同时 `app_state_snapshots` 继续作为未完成细粒度迁移主体的兼容持久化兜底。
...
产品配置、需求、知识文档、Bug、用户管理、用户反馈和模型网关配置已具备当前管理页所需 CRUD 能力；用户使用指标已具备真实登记和查询能力。产品管理页面通过“配置”弹窗维护产品版本、模块和 Git 资源，Git 凭据仅在新增/编辑时提交，列表只展示 `credential_ref_configured` 对应的配置状态，不回显凭据引用或 token。知识中心已具备独立导入、列表、chunk embedding 生成、权限过滤后的向量排序检索、索引失败原因展示、`index_failed` 重试和沉淀审核入口；任务中心已具备真实产品详细设计启动、Review 确认、技术方案任务创建和 Markdown 导出入口；用户洞察/迭代规划页已具备真实用户反馈登记、用户使用指标登记与筛选、迭代建议生成、确认和可选转需求入口，用户反馈创建人可为任意已登录用户，状态处理、指标登记和迭代建议操作需 `product_owner`、`rd_owner` 或 `admin`。

审计与运行页面支持查看真实审计详情，并从审计主体优先发起生命周期链路追踪。生命周期视图和首页 IT 团队看板的 AI 任务、待确认 Review、知识沉淀和风险信号聚合会先按任务类型读权限过滤，不能通过聚合接口绕过任务详情权限。首页 IT 团队看板已基于真实产品、需求、AI 任务、待确认 Review、知识和审计数据返回 MVP 聚合摘要。内部 GitLab MR 预览和 diff 快照已接入 GitLab 只读 API：产品 Git 资源需提供可解析的 `remote_url` 或 `GITLAB_BASE_URL`，凭据引用推荐使用 `env:GITLAB_READONLY_TOKEN`，系统只读取 MR 元信息和 changes，不回写 GitLab。进程内 `MemoryStore` 仅用于测试和显式内存模式。GitLab 每日代码指标、Jenkins 发布记录和用户使用指标可通过真实登记/导入接口落库，不依赖前端示例数据；外部自动采集器尚未接入。线上日志接口在未接入采集器时返回真实空集合，不提供前端本地兜底行或后端伪造统计数据。
