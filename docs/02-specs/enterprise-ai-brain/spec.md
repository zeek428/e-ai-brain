...
**核心规则**:
- 知识文档创建、内容更新、权限更新和沉淀采纳会先重建文本 chunk；如 `model_gateway` 的 OpenAI-compatible `/embeddings` 可用，则生成 `knowledge_chunks.embedding` 并进入 `vector_indexed`。
...
- 检索必须先过滤用户无权读取的文档和 chunk，再对有 embedding 的 chunk 使用 cosine 相似度排序；没有可读向量 chunk 时不得额外调用 query embedding，直接关键词检索。返回结果包含 `retrieval_mode`，向量命中包含 `score`，关键词命中 `score=null`。
