...
当前源码实现状态：FastAPI 请求处理仍使用进程内 `MemoryStore` 支撑本地测试和演示；`001_init.sql` 已定义目标 PostgreSQL 持久化 schema。接入真实 PostgreSQL 仓储、真实 GitLab API 和真实 OpenAI-compatible provider 前，当前实现只能作为 MVP 流程骨架和契约验证环境，不能视为生产持久化或真实外部集成完成。
...
首个初始化脚本规划位于 `apps/api/app/db/migrations/001_init.sql`，负责 pgvector 扩展和核心表初始化。后续迁移按模块追加，并确保回滚脚本不会破坏生产数据。
