...
| 版本 | 日期 | 变更内容 | 作者 |
|------|------|----------|------|
| v1.1.287 | 2026-06-11 | 同步代码仓库巡检技术规格：`code_repository_inspection` 定时作业支持多结果动作，写入代码审查报告、自动创建严重问题 Bug 并记录邮件/钉钉通知反馈 | Codex |
...
| 代码仓库巡检 | 定时作业 + 插件扫描 + 多结果动作 | 面向仓库质量/安全/规范的周期性治理，不绑定单个 MR/PR；扫描结果写入代码审查表，严重 finding 可自动创建 `code_inspection` 来源 Bug，并记录邮件/钉钉通知反馈。 |
...
| 代码审查 | code_inspection, scheduled_jobs | 周期性代码仓库质量、安全和规范巡检，沉淀报告、finding、通知反馈和严重问题 Bug | 基于产品 Git 资源和插件扫描动作生成，关联产品、仓库、定时作业运行、Bug 和通知记录 |
...
| code_inspection_reports | 代码仓库巡检报告 | 记录 product_id、repository_id、repository 快照、scheduled_job_id、scheduled_job_run_id、collector_run_id、plugin_invocation_log_id、branch、commit_sha、summary、risk_level、finding_count、severe_finding_count、status、result_actions、created_bug_ids 和 notification_ids；用于运营治理 / 代码审查列表和详情。 |
| code_inspection_findings | 代码仓库巡检问题 | 关联 code_inspection_report，记录 rule_id、category、severity、title、description、file_path、line_number、recommendation、raw 和 created_bug_id；严重问题可派生 `source=code_inspection` Bug。 |
...
`041_code_inspection_governance.sql` 补齐代码仓库巡检治理结构：为 `scheduled_jobs` 增加 `result_actions` JSONB 字段，创建 `code_inspection_reports`、`code_inspection_findings` 和 `code_inspection_notifications`，并补充 `code_inspection.read` 权限、运营治理 / 代码审查菜单和角色授权。该迁移用于 `code_repository_inspection` 作业落地，不得把巡检报告只保存在插件调用日志或运行摘要中。
...
| 代码审查巡检报告 | GET | /api/governance/code-inspections, /api/governance/code-inspections/{report_id} | 查询周期性代码仓库巡检报告列表和详情，返回仓库、分支、风险级别、finding 统计、问题明细和通知反馈；端点由 `app.api.routers.code_inspections` 单一路由承载。 |
...
- `code_repository_inspection` 必须绑定产品、数据连接和插件扫描动作，插件响应推荐包含 `repository_id`、`branch`、`commit_sha`、`risk_level`、`summary` 和 `findings[]`。作业通过 `result_actions` 支持多个结果动作，默认顺序为 `write_code_inspection_report`、`create_bug_for_severe_findings` 和 `send_notification`；写报告必须落到 `code_inspection_reports/code_inspection_findings`，严重 finding 按阈值创建 `source=code_inspection` Bug，通知动作写入 `code_inspection_notifications` 并记录邮件、钉钉机器人等目标和反馈摘要。运行摘要必须包含 `execution_nodes.data_connection`、`code_inspection_report`、`bug_creation` 和 `notifications`，用于运行详情展示扫描取数、报告写入、Bug 派生和通知反馈四个节点。
...
- `risk_level` 可由插件响应提供；缺失时按 findings 中最高 severity 推导。`severe_finding_count` 当前按 `high/critical` 统计，用于列表风险扫描。
- `create_bug_for_severe_findings` 只针对达到阈值的 finding 创建 Bug，Bug `source=code_inspection`，evidence 必须包含 report_id、finding_id、rule_id、file_path、line_number、branch 和 commit_sha。
...
- 列表和详情接口位于 `/api/governance/code-inspections`，需要 `code_inspection.read` 权限；前端入口归属运营治理 / 代码审查。
