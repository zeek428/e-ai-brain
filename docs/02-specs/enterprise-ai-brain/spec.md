...
- 运维部署单按产品、版本和需求范围归属；创建时必须绑定同产品环境的 active 部署方案并固化快照。测试完成或待发布需求可发起部署，启动后需求进入 `deploying`；人工部署由负责人登记结果，SSH/Docker 通过部署 Runner 隔离执行，Jenkins 通过连接触发并同步；部署成功后需求进入 `released`，部署失败或回滚后回到 `ready_for_release` 并生成 `deployment_failure` 来源 Bug，外部取消在确认前保持 `cancelling`。
