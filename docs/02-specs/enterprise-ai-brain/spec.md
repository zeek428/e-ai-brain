...
Docker Compose 不得将 `apps/api/app/db/migrations` 挂载到 PostgreSQL 的 `/docker-entrypoint-initdb.d`，PostgreSQL 镜像也不得打包应用迁移 SQL。全新 volume 和存量 volume 都由 API 镜像内 `/app/app/db/migrations` 与 `api-entrypoint.sh` 作为普通 additive migration 的唯一控制面，避免 PostgreSQL initdb 与 API 启动形成两套执行顺序和例外规则。该边界必须由 compose/PostgreSQL image 无应用迁移入口回归、API image 打包默认迁移目录回归和 API entrypoint 排除列表回归共同固定。
