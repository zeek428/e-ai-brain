...
| 部署执行资源 | GET | /api/devops/deployment-runner-targets, /api/devops/deployment-jenkins-connections | 要求 `deployment.scheme.manage`，Runner 候选必须同时具备部署能力、部署信任域、在线心跳和就绪目标；返回可用于部署方案的脱敏候选，且不返回本地密钥、机器路径或连接认证配置。 |
