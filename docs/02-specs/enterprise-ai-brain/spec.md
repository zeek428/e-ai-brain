...
后续系统权限管理按 [RBAC 重设计](rbac-redesign.md) 演进：固定六角色保留为 MVP 兼容预置角色模板，并新增 `developer`、`test_owner`、`tester`、`release_owner` 等研发交付扩展预置角色；角色管理从只读目录升级为角色 CRUD，并支持配置功能菜单；用户登录后后端返回 `menu_tree`，前端左侧菜单按授权菜单渲染；业务接口逐步从角色 code 判断迁移为权限点和数据范围校验，角色、权限、菜单、用户授权和范围授权均以 PostgreSQL 结构表为事实源。
