...
v2.0 将产品需求确立为研发任务唯一入口，并把原研发执行器策略原位升级为单一统一研发执行策略。正式需求评估通过后，系统优先选择兼容的 `planning` 版本，无合适版本才创建新规划版本；随后创建版本级协作运行，将目标拆为有负责人、输入输出、验收标准和依赖关系的 DAG 工作项。LLM 只提出评估、拆解、风险和重规划建议，确定性服务负责状态、权限、调度租约、幂等、预算、风险下限和门禁。研发岗位可以动态增加，席位可由 AI 数字员工或真人账号承担。默认策略在远程代码提交、开发测试和质量门禁完成后停在 `ready_for_release`，不进入部署；部署仅在策略显式允许且人工门禁通过时复用现有部署域。现有定时作业的数据模型、调度和 AI角色/Skill 装配保持不变。
...
v2.0 目标态以正式评估和协作编排为准：旧版批量生成任务入口停用；批量排期只用于有权限人员调整系统自动归组结果且目标必须为 `planning`。设计、开发、评审、测试和待发布状态由协作工作项、审核与门禁证据推进，不允许列表批量操作绕过依赖、岗位或策略快照。批量分配需求业务负责人和关闭/延期等管理动作继续保留并展示逐条结果。
...
| rd_task_executor_policy_role_bindings | 策略岗位绑定 | 保存策略内岗位、actor mode、显式真人用户/AI 执行器候选、主/备用执行器、信任域、上下文、工具、预算、审核岗位和所需系统权限。 |
...
| rd_executor_profiles | 研发执行器档案 | 记录 AI 数字员工所用 executor type、Runner/模型配置、工作区能力、并发、可承担岗位、健康状态和凭据引用，不保存密钥明文。 |
| rd_collaboration_runs | 版本级研发协作运行 | 关联产品版本、冻结需求集合和策略快照，保存 scope/plan 版本、协作状态、交付终点、预算、图版本和时间；数据库约束同一版本最多一个非终态运行。 |
| rd_run_seats | 协作岗位席位 | 为运行内岗位绑定 AI 执行器档案或显式真人用户，保存职责范围、容量、状态和替换历史；真人仍经系统 RBAC、产品范围、协作权限和席位鉴权。 |
| rd_role_sessions | 岗位连续会话 | 保存席位交接摘要、上下文游标和恢复元数据；不得作为工作项或决策业务状态事实源。 |
| rd_work_items | 协作工作项 | 保存目标、负责人席位、输入输出契约、验收标准、状态、风险、优先级、关联 AI 任务、审核人、租约和幂等键。 |
...
| role_feedback_records | 岗位反馈与经验 | 将审核、返工、门禁、交付结果和人工评价归因到岗位、席位、执行器和策略版本，形成可审核、版本化经验候选。 |
| rd_collaboration_upgrade_state | 研发协同升级状态 | 保存维护围栏、Schema 版本、预检、切换和新应用健康标记；围栏只阻断研发写路径，不影响定时作业。 |
...
`109_requirement_driven_rd_collaboration.sql` 只做 v2.0 非破坏性扩展，创建评估、岗位、执行器档案、策略岗位绑定、版本协作、席位、会话、工作项、依赖、attempt、事件、决策、反馈和升级状态表，并增加可空关联字段；不得在该迁移中删除旧策略字段或取消任务。全部新代码完成后先启用维护围栏并运行预检/数据转换/切换脚本；新应用在不读取旧字段的状态下启动并通过健康验证后，显式 cleanup 命令以单事务执行 `110_requirement_driven_rd_cutover.sql` 删除旧字段。迁移 110 不注册到启动时 automatic additive runner。两次数据库迁移之间不启用第二套研发运行规则。
...
| 启动 AI 工作项 | POST | /api/ai-tasks/{id}/start | 由协作编排服务为已绑定 `rd_work_item` 的 AI 任务启动执行；必须使用协作运行冻结的统一策略和岗位执行器快照，创建 `ai_executor_tasks` 或模型任务并返回执行引用。缺失策略、岗位、执行器或授权时阻断，不允许策略外回退。 |
...
| 需求评估 | POST/GET/POST | /api/requirements/{id}/assessments, /api/requirements/{id}/assessments/latest, /api/requirement-assessments/{id}/decisions | 创建/查询正式评估并提交人工决策；accepted 后触发规划版本优先归组。 |
| 研发组织 | GET/POST/PATCH | /api/delivery/rd-roles, /api/delivery/rd-executor-profiles | 管理动态研发岗位和 AI 执行器档案；真人席位在协作运行内引用用户账号。 |
...
| 研发协同升级控制 | GET/POST | /api/system/rd-collaboration-upgrade/preflight, /api/system/rd-collaboration-upgrade/maintenance-fence, /api/system/rd-collaboration-upgrade/cutover | 管理员执行只读预检、研发维护围栏和一次性切换；围栏不影响定时作业，健康验证前不清理旧字段。 |
...
**核心规则**:
- 不新增第二套策略表、模式字段或兼容运行时。迁移 `109_requirement_driven_rd_collaboration.sql` 只做非破坏扩展；全部新代码完成后启用研发维护围栏并执行预检、策略转换和 draft 任务取消，切换为 Schema v2；新应用健康验证后由 `110_requirement_driven_rd_cutover.sql` 删除旧顶层字段。围栏阻断研发写入和 Runner 新领取但不影响定时作业。切换后旧请求 Schema 不再有效，也不能重新开放旧写路径。
- 策略写契约唯一包含 `matching_config/assessment_config/iteration_config/delivery_target/team_config/autonomy_config/quality_gate_config/git_config/deployment_config/role_bindings`；数据库将 `role_bindings` 保存到岗位绑定子表，不接受第二套同义字段。
- `assessment_config` 定义评估岗位、模型用途、完整度、风险阈值和人工门禁；`iteration_config` 定义兼容版本过滤、容量、评分阈值与新版本命名规则；归组只选择 `planning`。
- `team_config` 和 `role_bindings` 定义所需岗位及执行器。岗位来自 `rd_role_definitions`；AI 席位引用 active `rd_executor_profiles`，P0 真人席位只从显式 `user_ids` 选择并校验系统权限和产品范围，不自动推断团队容量。
...
- 工作项准备就绪后按岗位席位派发：AI 席位复用 `ai_tasks`、`agent_loop_runs`、`ai_executor_tasks` 和 Runner；真人席位生成可领取/执行的工作项。AI 和真人提交相同的结果契约与证据，审核人不能与被审核席位相同；失败审核创建关联返工项而不是覆盖原结果。
- 代码更改必须先通过平台独立质量门禁。策略允许的低风险提交/推送可自动执行；合并、敏感目录、迁移、安全问题、预算超限和权限越界按风险策略创建 `decision_requests`。默认不进入部署。
- 集成工作项通过 Outbox 完成远程 push/MR/PR 并保存 repository、分支、local/remote commit SHA、MR/PR、执行身份、Outbox 和对账状态的可信交付记录；发布就绪只验证这些证据，不在完成判定时临时 push。
- 每次运行冻结不可变 `strategy_snapshot`，历史运行不读取后续修改后的策略解释自身。管理列表继续使用 PostgreSQL read model 分页筛选；策略页面升级原入口，不新增“旧策略/新策略”切换。
- 任务详情展示协作运行、工作项 DAG、岗位席位、执行器、预算、决策请求、门禁、返工链和反馈归因。Runner 的 Token、心跳、安装包、日志和健康检测继续由插件管理 / AI 执行器维护。
- 定时作业继续引用既有 `ai_agents`、`ai_skills` 和作业执行器配置；统一研发执行策略、`rd_role_definitions` 与 `rd_executor_profiles` 只服务需求驱动研发，不改变定时作业的创建、触发、调度或运行快照语义。

新增权限点统一为 `delivery.rd_roles.manage`、`delivery.rd_executor_profiles.manage`、`delivery.requirement_assessments.read/decide`、`delivery.rd_collaboration.read/plan/work` 和 `delivery.decision_requests.decide`；策略继续使用 `delivery.rd_executor_policies.manage`，部署继续使用现有部署权限。所有写接口同时检查权限点、产品 scope、运行席位和状态。默认只为 admin 授予全部新增权限，其他系统角色通过角色管理显式授权；岗位定义和席位分配永不自动授权。
...
draft → planning → running → integrating → verifying → ready_for_release → completed
running/integrating/verifying → waiting_human → 原阶段
任意非终态 → failed/cancelled
ready_for_release → deploying → completed（仅策略 delivery_target=deployed 且人工门禁通过）
...
| approved / planned / ready_for_dev / designing / developing / code_reviewing / testing / ready_for_release / deploying / released | batch_advance_status | 按目标状态 | 产品负责人/研发负责人 | 只能按研发流程向前推进到允许目标；推进到交付链路状态时必须已归属迭代版本；`ready_for_release` 可进入 `deploying`，`deploying` 失败或回滚可退回 `ready_for_release`，终态、重复、缺失、未排期或不符合路径需求进入 skipped 明细，不回滚合法项。 | requirement.batch_status_advanced / requirement.updated |
