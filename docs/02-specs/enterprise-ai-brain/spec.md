...
| 产品成员 | GET/PUT | /api/products/{product_id}/members, /api/products/{product_id}/member-candidates | 在产品配置弹窗维护产品成员和产品内职责；查询要求 `product.member.read`，候选人与保存要求 `product.member.manage`，并按当前用户产品 scope 校验；保存采用整体替换语义，成员职责会派生产品 scope、职责权限和菜单可见性。 |
