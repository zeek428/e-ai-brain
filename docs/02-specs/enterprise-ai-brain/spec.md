...
当前源码实现状态：Docker 本地栈默认以 `PERSISTENCE_MODE=postgres` 启动，登录账号读取 PostgreSQL `users` 表，管理员可通过系统管理下的用户管理维护用户，业务运行状态写入 `app_state_snapshots` JSONB 快照表。产品配置、需求、知识文档、Bug 和用户管理已具备当前管理页所需 CRUD 能力。进程内 `MemoryStore` 仅用于测试和显式本地 fallback；后续阶段应继续把产品、需求、任务、Review、知识、审计等主体替换为细粒度 PostgreSQL 仓储。真实 GitLab API 和真实 OpenAI-compatible provider 尚未接入，当前实现仍只能作为 MVP 流程骨架和契约验证环境，不能视为真实外部集成完成。
...
| 产品维护 | POST/PATCH/DELETE | /api/products, /api/products/{product_id} | 管理产品，删除前校验版本、模块、Git 资源、需求和 Bug 依赖。 |
| 产品版本 | GET/POST/PATCH/DELETE | /api/products/{product_id}/versions, /api/product-versions/{version_id} | 管理产品版本，删除前校验需求和 Bug 依赖。 |
| 产品模块 | GET/POST/PATCH/DELETE | /api/products/{product_id}/modules, /api/product-modules/{module_id} | 管理产品模块，删除前校验需求和 Bug 依赖。 |
