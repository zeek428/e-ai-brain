...
| 系统设置邮件测试 | POST | /api/system/settings/email/test | 使用已保存的系统级邮件发送配置向指定收件人、常用测试收件人或系统管理员邮箱发送测试邮件；要求 `system.settings.manage`，响应仅返回投递状态和 SMTP 端点摘要，不返回密码、密钥值或邮件正文。 |
