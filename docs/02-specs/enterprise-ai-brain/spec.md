...
`109_requirement_driven_rd_collaboration.sql` 只做 v2.0 非破坏性扩展，创建评估、岗位、AI 数字员工、执行器档案、策略岗位绑定、版本协作、席位、会话、工作项、依赖、attempt、事件、决策、反馈和升级状态表，并增加可空关联字段；不得在该迁移中删除旧策略字段或取消任务。全部新代码完成后先在围栏外运行只读预检，再进入 `draining` 收敛在途工作；零活动与备份条件满足后进入 `cutover_locked`，重跑锁内预检并执行数据转换和单向切换。新应用在不读取旧字段的状态下启动并通过健康验证后，显式 cleanup 命令以单事务执行 `110_requirement_driven_rd_cutover.sql` 删除旧字段。迁移 110 不注册到启动时 automatic additive runner。清理成功后恢复 v2 Worker、验证 Worker/Schema/图版本和 v2 写入冒烟检查，再解除围栏；进入 cutover_locked 后任一步失败都只能向前重试，解除后旧写路径仍永久拒绝。两次数据库迁移之间不启用第二套研发运行规则。
...
**核心规则**:
- 不新增第二套策略表、模式字段或兼容运行时。迁移 `109_requirement_driven_rd_collaboration.sql` 只做非破坏扩展；全部新代码完成后先执行围栏外只读 preflight，再进入 `draining` 阻止新增研发写入和 Runner 新领取、允许在途收敛与管理员受控取消。活动量归零和备份确认后原子进入 `cutover_locked` 并重跑锁内 preflight，再执行策略转换、draft 任务取消和 Schema v2 激活；新应用健康验证后由 `110_requirement_driven_rd_cutover.sql` 删除旧顶层字段。只有仍在 draining、Schema v2 未激活且 cleanup 未开始时可以按乐观锁审计化中止；进入 cutover_locked 后只能向前修复。清理成功后恢复 v2 Worker、验证 Worker/Schema/图版本和写入冒烟检查，再解除围栏。围栏不影响定时作业；切换后旧请求 Schema 和旧写路径永久无效。
