...
当前源码实现状态：Docker 本地栈默认以 `PERSISTENCE_MODE=postgres` 启动，登录账号读取 PostgreSQL `users` 表，管理员可通过系统管理下的用户管理维护用户，并可在模型网关配置页维护 OpenAI-compatible 配置；模型网关列表只显示 `api_key_configured`，不显示明文密钥或密钥片段。业务运行状态写入 `app_state_snapshots` JSONB 快照表。产品配置、需求、知识文档、Bug、用户管理和模型网关配置已具备当前管理页所需 CRUD 能力；知识中心已具备独立导入、列表、真实权限过滤检索和沉淀审核入口；任务中心已具备真实产品详细设计启动、Review 确认、技术方案任务创建和 Markdown 导出入口。内部 GitLab MR 预览和 diff 快照已接入 GitLab 只读 API：产品 Git 资源需提供可解析的 `remote_url` 或 `GITLAB_BASE_URL`，凭据引用推荐使用 `env:GITLAB_READONLY_TOKEN`，系统只读取 MR 元信息和 changes，不回写 GitLab。进程内 `MemoryStore` 仅用于测试和显式本地 fallback；后续阶段应继续把产品、需求、任务、Review、知识、审计等主体替换为细粒度 PostgreSQL 仓储。DevOps/用户洞察采集器和真实 OpenAI-compatible provider 尚未接入；对应列表接口返回空集合，不提供前端本地兜底行或后端伪造统计数据。
