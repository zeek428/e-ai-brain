...
| product_versions | 产品迭代版本 | 同一产品内 `code` 唯一，在需求交付菜单集中维护；主状态为 `planning / active / testing / ready_for_release / deploying / released`，`archived` 仅用于历史归档；自动需求归组只选择 `planning`。 |
...
| rd_task_executor_policies | 统一研发执行策略 | 原表原位升级；保存需求评估、规划版本归组、必需岗位、岗位执行器绑定、预算、质量门禁、Git、风险和交付终点配置。active 策略必须能完整解析，运行时冻结不可变策略快照。 |
| rd_task_executor_policy_role_bindings | 策略岗位绑定 | 保存策略内岗位、actor mode、显式真人用户/AI 数字员工候选、主/备用执行器、信任域、上下文、工具、预算、审核岗位和所需系统权限。 |
| requirement_assessments | 正式需求评估 | 保存需求/策略版本、结构化评估、完整度、风险、依赖、工作量、候选版本、归组理由、状态、LLM 建议和确定性校验结果。 |
| requirement_assessment_opinions | 评估岗位意见 | 保存每个岗位、员工/执行器、输入版本、结构化结论、证据、置信度、风险、成本和意见轮次；策略收紧新增岗位时追加意见。 |
...
| rd_collaboration_runs | 版本级研发协作运行 | 关联产品版本、冻结需求集合和策略快照，保存 scope/plan 版本、协作状态、交付终点、预算、图版本和时间；数据库约束同一版本最多一个非终态运行。 |
...
| decision_requests | 人类决策请求 | 保存高风险、超权限、预算、冲突、门禁或部署边界问题的选项、证据、建议、决策人、状态和乐观锁版本。 |
| role_feedback_records | 岗位反馈与经验 | 将审核、返工、门禁、交付结果和人工评价分别归因到岗位、真人用户或 AI 数字员工、执行器和策略版本，形成可审核、版本化经验候选。 |
...
| rd_task_executor_policies | idx_rd_task_executor_policies_match | brain_app_id, product_id, status | 组合索引 | 按业务大脑和产品解析 active 统一研发策略；需求类型、来源和优先级等细分匹配由统一 matching_config 的规范化字段/索引承载，不再依赖旧顶层 task_type。 |
...
| 统一研发执行策略 | GET/POST/PATCH/DELETE | /api/delivery/rd-task-executor-policies, /api/delivery/rd-task-executor-policies/{policy_id} | 原入口原位升级；管理评估、版本归组、必需岗位、岗位执行器、预算、门禁、Git、风险和交付终点。写入必须整体校验并审计，不提供旧/新模式或策略外 fallback。 |
| 需求评估 | POST/GET/POST | /api/requirements/{id}/assessments, /api/requirements/{id}/assessments/latest, /api/requirement-assessments/{id}/opinions, /api/requirement-assessments/{id}/answers, /api/requirement-assessments/{id}/decisions | 创建/查询正式评估，提交岗位意见、需求补充和 `accept/reject/request_more_info/request_rework/defer` 决策；accepted 后进入 approved，再按自动或人工决策完成规划版本归组。 |
...
| 研发协作运行 | POST/GET | /api/product-versions/{id}/collaboration-runs, /api/requirements/{id}/collaboration-run, /api/delivery/rd-collaboration-runs/{id} | 产品版本是聚合根；并发启动同一版本幂等返回唯一活动运行，需求入口只读定位所属版本运行。 |
| 协作工作项与决策 | GET/POST/PATCH | /api/delivery/rd-collaboration-runs/{id}/work-items, /api/delivery/rd-work-items/{id}/submit, /api/delivery/decision-requests/{id}/decide | 查询/提交工作项结果，处理审核返工和人类决策；状态写入由确定性编排服务控制。 |
...
新增权限点统一为 `delivery.rd_roles.manage`、`delivery.rd_ai_employees.manage`、`delivery.rd_executor_profiles.manage`、`delivery.requirement_assessments.read/decide`、`delivery.rd_collaboration.read/plan/work` 和 `delivery.decision_requests.decide`；策略继续使用 `delivery.rd_executor_policies.manage`，部署继续使用现有部署权限。所有写接口同时检查权限点、产品 scope、运行席位和状态。默认只为 admin 授予全部新增权限，其他系统角色通过角色管理显式授权；岗位定义、员工档案和席位分配永不自动授权。
...
R&D decision request: pending | approved | rejected | expired | cancelled
...
→ 测试完成和发布评估确认后，由 deployment_request 发起运维部署，启动后需求进入部署中，成功后进入已发布，失败或回滚后回到待发布并生成 Bug
