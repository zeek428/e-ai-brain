...
任务中心不得依赖前端一键演示数据。MVP-A 的正式页面操作链路为：需求管理审批并生成 `product_detail_design` 任务，任务中心启动任务并通过人工确认台确认输出，再基于已确认产品详细设计创建 `technical_solution` 任务，确认后导出 Markdown。后续 GitLab MR 预览、diff 快照和 code_review 任务必须继续沿用正式 API 和真实持久化状态。
