...
| v1.1.41 | 2026-06-02 | 新增待归属数据队列结构表、API、审计、DevOps 处理入口和用户洞察只读可见性 | Codex |
...
| Git Code Review 输入 | GitLab MR 或 GitHub PR 元信息和 diff 快照、可插拔 code-review 执行器、结构化报告和内部归档 | GitLab/GitHub 只读 API + Claude Code `code-review` skill 适配器 |
...
审计与运行页面支持查看真实审计详情，并从审计主体优先发起生命周期链路追踪。生命周期视图和首页 IT 团队看板的 AI 任务、待确认 Review、知识沉淀和风险信号聚合会先按任务类型读权限过滤，不能通过聚合接口绕过任务详情权限。生命周期上下文已支持从真实 Bug、GitLab 每日代码指标、Jenkins 发布记录、线上运行日志指标、用户使用指标、用户反馈和迭代规划建议起点回溯同产品/版本/模块任务链路，并把这些真实证据写入 downstream 关系；缺少对应证据时返回动态 `missing_context`，不合成兜底关系。首页 IT 团队看板已基于真实产品、需求、AI 任务、待确认 Review、知识和审计数据返回 MVP 聚合摘要；传入 `product_id` 时，知识文档和审计事件也必须按产品归属过滤，不能把其他产品的知识或审计计数混入当前产品；后续需继续把看板十余类主体的聚合计算沉淀为专门 SQL/物化 read model，避免扩大运行时集合依赖。GitLab MR / GitHub PR 预览和 diff 快照已接入只读 API：GitLab 产品 Git 资源需提供可解析的 `remote_url` 或 `GITLAB_BASE_URL`，GitHub 产品 Git 资源需提供 `project_path=owner/repo` 或可解析 owner/repo 的 `remote_url`，GitHub Enterprise 可通过 `GITHUB_BASE_URL` 指定 API base；凭据引用推荐使用环境变量或服务端密钥引用，本地联调可直填只读 token，系统只读取变更元信息和文件摘要，不回写 GitLab/GitHub。GitLab 每日代码指标、Jenkins 发布记录、线上运行日志指标、用户使用指标、采集运行记录和待归属数据队列可通过真实登记/导入接口落库，不依赖前端示例数据；外部自动采集器尚未接入。无记录时返回真实空集合，不提供前端本地兜底行或后端伪造统计数据；采集运行记录只描述采集尝试，不自动创建 GitLab/Jenkins/线上日志/用户洞察指标；待归属队列只记录未归属事实和人工处理结果，不自动生成指标、反馈或迭代建议。
...
待归属数据队列已作为真实业务主体接入 `pending_attribution_items`。产品负责人、研发负责人或管理员可以登记无法映射产品、模块、需求或采集主体的真实导入事实，系统保存来源类型、来源系统、采集运行引用、原始主体 ID、摘要、原始 payload、建议归属、置信度、处理状态和人工处理结果，并记录 `pending_attribution.created`、`pending_attribution.resolved` 或 `pending_attribution.ignored` 审计事件。DevOps 页面提供全来源队列和弹窗处理入口，用户洞察页面只读展示用户使用/反馈来源的待归属项；队列处理不自动创建指标、反馈、需求或迭代建议。
...
| product_config | 产品、版本、模块、Git 资源配置、GitLab/GitHub 仓库绑定和凭据引用 | products, product_versions, product_modules, product_git_repositories |
...
| 需求管理 | product_owner, rd_owner | 需求列表、新增需求、需求详情、审批、驳回、批量排期/归集需求、生成任务、关闭 |
...
| 研发运营看板 | IT 管理者、研发负责人、平台运营 | GitLab 提交与代码质量、Jenkins 发布记录、线上日志指标、采集运行记录、用户使用分析、用户反馈分析、待归属数据队列 |
| 用户洞察 | 产品负责人、IT 管理者、研发负责人 | 用户使用趋势、用户反馈列表、AI 迭代规划建议、证据链、优先级、采纳/驳回和转需求入口 |
...
| requirements | 需求实体 | 状态为 `draft / submitted / approved / planned / designing / ready_for_dev / developing / code_reviewing / testing / ready_for_release / released / accepted / rejected / deferred / cancelled / closed`；`version_id` 可为空，未排期需求不能生成 AI 任务。历史 `pending_approval` 和 `task_created` 在运行时分别兼容为 `submitted` 和 `designing`。 |
...
| user_feedback | 用户反馈记录 | 记录来源渠道、反馈类型、满意度或情绪倾向、标签、关联产品模块、创建人和处理状态。 |
...
| 模型网关配置 | GET/POST/PATCH/DELETE/POST(test) | /api/system/model-gateway-configs, /api/system/model-gateway-configs/test, /api/system/model-gateway-configs/{config_id} | 管理平台默认模型网关，并使用临时参数检测连接。 |
...
| AI 助手聊天 | POST | /api/assistant/chat | 基于当前 AI Brain 系统上下文和模型网关 Chat 能力回答产品、任务、开发进展、Git 仓库和模型网关状态问题，并按用户保存会话历史。 |
| 需求列表 | GET | /api/requirements | 查询需求台账。 |
| 需求详情 | GET | /api/requirements/{id} | 查询单条需求详情和任务引用。 |
...
| 需求批量排期 | POST | /api/requirements/batch-schedule | 将同产品 `approved/planned` 需求批量归集到 `planning/active` 迭代版本；合法需求更新为 `planned`，不合规需求返回 skipped 明细。静态路径必须先于 `/api/requirements/{id}` 动态路由注册。 |
| 需求审批 | POST | /api/requirements/{id}/approve, /api/requirements/{id}/reject | 审批通过或驳回需求。 |
| 需求关闭 | POST | /api/requirements/{id}/close | 关闭无需继续推进的需求。 |
| 生成 AI 任务 | POST | /api/requirements/{id}/generate-task | 需求排期到有效迭代版本后基于需求实体生成 AI 任务。 |
| 创建 AI 任务 | POST | /api/ai-tasks | 低层任务创建接口，前端默认通过需求实体生成。 |
| 启动 AI 任务 | POST | /api/ai-tasks/{id}/start | 启动 LangGraph；`model_gateway_failed` 或 `code_review_executor_failed` 的失败任务可同 task_id 重试。 |
...
| Graph Run 列表 | GET | /api/graph-runs | 查询 AI 任务关联的 Graph 运行记录。 |
...
| GitLab MR 预览 | GET | /api/devops/gitlab/merge-requests/{repository_id}/{mr_iid}/preview | 读取 GitLab MR 标题、作者、分支、变更文件数和 diff refs。 |
...
| GitHub PR 列表 | GET | /api/devops/github/pull-requests/{repository_id} | 使用产品 GitHub 凭据列出可访问 PR，支持 state 和 limit。 |
| GitHub PR 预览 | GET | /api/devops/github/pull-requests/{repository_id}/{pr_number}/preview | 读取 GitHub PR 标题、作者、分支、变更文件数和文件摘要。 |
| GitHub PR diff 快照 | POST | /api/devops/github/pull-requests/{repository_id}/{pr_number}/snapshot | 拉取 PR 元信息和文件摘要，生成 code_review 任务输入快照。 |
| Code Review 报告 | GET | /api/ai-tasks/{id}/code-review-report | 查询 GitLab MR / GitHub PR 代码 Review 报告、执行器信息和确认状态。 |
...
| 采集运行记录 | GET/POST/PATCH | /api/collectors/runs, /api/collectors/runs/{run_id} | 查询、登记和结束 DevOps/洞察采集运行台账，不自动生成指标。 |
| 待归属数据队列 | GET/POST/POST | /api/attribution/pending-items, /api/attribution/pending-items/{item_id}/resolve | 查询、登记、归属或忽略无法映射产品/模块/需求的数据，不自动生成业务指标。 |
...
- 当前实现会在 PostgreSQL 运行时直接读取 repository source rows，按产品、权限和时间窗口组装看板指标，并通过单条 repository 写入把 `/api/dashboard/it-team` 的产品与时间窗口聚合结果同步到 `dashboard_metric_snapshots`，用于后续首页读取、运营复核和趋势分析。
...
| active | advance_status | testing | 产品负责人/研发负责人 | `code_reviewing` 需求自动推进到 `testing`；`planned/designing/ready_for_dev/developing` 等未完成开发评审需求进入阻塞明细，非强制推进返回 `PRODUCT_VERSION_STATUS_BLOCKED`，强制推进保留阻塞需求原状态并记录风险。 | product_version.status_advanced / requirement.updated |
...
| code_reviewing | version_advanced_to_testing | testing | 产品负责人/研发负责人 | 由迭代版本从 `active` 推进到 `testing` 触发；未完成开发评审的需求进入阻塞/风险明细。 | product_version.status_advanced / requirement.updated |
