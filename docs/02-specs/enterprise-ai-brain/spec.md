...
- `team_config` 和 `role_bindings` 定义所需岗位、员工候选及执行器。岗位来自 `rd_role_definitions`；AI 席位从显式 `rd_ai_employees` 候选选择稳定员工身份，并单独冻结 active `rd_executor_profiles`；P0 真人席位只从显式 `user_ids` 选择并校验系统权限和产品范围，不自动推断团队容量。
...
- LLM 通过 `model_gateway` 生成结构化评估、工作分解、负责人候选、依赖、风险、返工和重规划建议；平台编排服务校验 JSON Schema、DAG 无环性、岗位能力、权限、并发、租约、预算、幂等和风险下限，并拥有所有状态迁移的最终写权限。
