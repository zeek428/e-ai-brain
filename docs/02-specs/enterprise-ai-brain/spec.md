...
| deployment_requests | 运维部署单 | 按 product_id、version_id 和 requirement_ids 记录从测试完成到部署执行的准入、环境、风险、窗口、回滚方案和执行状态。 |
| deployment_request_requirements | 部署单需求范围 | 维护部署单与需求的多对多关系，支撑部署成功后批量推进需求到已发布。 |
| deployment_runs | 部署执行记录 | 记录部署单每次启动、外部 Job/Build、日志链接、执行状态、失败原因和完成时间。 |
...
- 运维部署单按产品、版本和需求范围归属；测试完成或待发布需求可发起部署，启动后需求进入 `deploying`，部署成功后进入 `released`，部署失败或回滚后回到 `ready_for_release` 并生成 `deployment_failure` 来源 Bug。
...
| testing / ready_for_release | deployment_request_created | pending ops | 产品负责人/研发负责人/发布负责人 | 创建运维部署单时选择测试中或待发布需求；若存在未关闭 blocker/critical Bug 或 release_readiness_task_id 未确认，返回阻塞错误。 | deployment_request.created |
...
| ready_for_release | deployment_started | deploying | 发布负责人/运维负责人 | 部署单启动后需求进入部署中；同一部署单可记录外部 Job/Build 和日志链接。 | deployment_request.started / requirement.updated |
| deploying | deployment_succeeded | released | 发布负责人/运维负责人 | 运维部署成功后，部署单需求范围内需求进入已发布。 | deployment_request.completed / requirement.updated |
| deploying | deployment_failed_or_rolled_back | ready_for_release | 发布负责人/运维负责人 | 部署失败或回滚后需求回到待发布，并创建 `deployment_failure` 来源 Bug 作为后续修复入口。 | deployment_request.completed / bug.created / requirement.updated |
