...
| MVP-C 知识与治理闭环 | 知识导入、索引状态、权限过滤检索、知识沉淀候选审核、模拟 Issue 幂等生成、主体级审计和 MVP 占位入口 | knowledge, long_memory, integration, audit, dashboard | 任务产出可沉淀、检索有权限过滤、后续阶段入口不误导为已完成 |
...
当前源码实现状态：Docker 本地栈默认以 `PERSISTENCE_MODE=postgres` 启动，登录账号读取 PostgreSQL `users` 表，管理员可通过用户管理接口维护用户，业务运行状态写入 `app_state_snapshots` JSONB 快照表。进程内 `MemoryStore` 仅用于测试和显式本地 fallback；后续阶段应继续把产品、需求、任务、Review、知识、审计等主体替换为细粒度 PostgreSQL 仓储。真实 GitLab API 和真实 OpenAI-compatible provider 尚未接入，当前实现仍只能作为 MVP 流程骨架和契约验证环境，不能视为真实外部集成完成。
...
| 系统角色 | 业务角色映射 | 技术权限边界 |
|----------|--------------|--------------|
| `admin` | 平台管理员 | 管理用户、业务大脑、产品主数据、Git 资源、模型网关配置和审计运行；不替代业务负责人做最终产品决策。 |
| `product_owner` | 产品负责人 | 创建和审批需求，确认产品详细设计，查看授权产品下任务、知识和占位看板。 |
| `rd_owner` | 研发负责人 | 创建和启动 AI 任务，确认技术方案，创建 GitLab MR 快照和 code_review 任务。 |
| `reviewer` | 研发负责人或指定 Reviewer | 预览 MR、创建 MR diff 快照、确认或修改后采纳 code_review 报告。 |
| `knowledge_owner` | 文档/知识维护者 | 导入知识文档、查看索引状态、审核知识沉淀候选。 |
| `viewer` | IT 管理者、测试负责人、测试人员或只读参与者 | 查看授权范围内的列表、详情、审计摘要和后续阶段空状态。 |

完整 `tester`、发布负责人和 IT 管理者写权限按 v1.1/v1.2 扩展；MVP 阶段不应为了占位页面提前实现复杂角色体系。

React 工作台应提供九个主入口，而不是只围绕任务详情组织全部能力：
...
| 任务中心 | rd_owner, 产品/研发/测试/发布负责人 | 任务列表、任务详情、任务类型筛选、人工确认台、GitLab MR 选择、MR diff 快照、Review 报告、Review 报告确认、自动化测试、发布上线评估、上线后分析、回写结果、Markdown 导出 |
...
MVP 演示环境可在任务中心提供一键演示动作，按登录、产品/版本创建、需求创建、审批、生成 AI 任务、启动工作流、读取任务详情和生命周期上下文的顺序调用正式 API，并在界面展示任务状态、人工确认点和下游关系摘要；该动作用于本地验收和演示，不替代正式页面的逐步业务操作。
...
| audit_events | 审计事件 | 记录任务、主体类型、主体 ID、事件类型、操作者、事件载荷和创建时间。 |
...
| actor_type | string | 是 | `user`、`system`、`ai`。 |
| actor_id | string | 否 | 操作者 ID。 |
| event | json | 是 | 事件摘要，不含完整 prompt、模型输出或密钥。 |
| created_at | datetime | 是 | ISO 8601。 |
...
| actor_type | string | 操作者类型，例如 `user`、`system`、`ai`。 |
| actor_id | string | 操作者 ID 或系统标识。 |
| event | object | 事件载荷，对应数据库 `event_json`。 |
| created_at | timestamptz/string | 创建时间。 |

API envelope 的 `trace_id` 用于请求追踪，不作为当前审计事件字段持久化。主体级审计字段用于跨产品、需求、任务和知识中心追踪关键写操作。
...
首个初始化脚本位于 `apps/api/app/db/migrations/001_init.sql`，负责 pgvector 扩展和核心表初始化。后续迁移按模块追加，例如 `002_persistence_users.sql` 补齐用户表种子数据和 `app_state_snapshots` 持久化快照表。已有环境必须通过可重复执行的 SQL 迁移脚本升级，不得通过清空 PostgreSQL 数据卷绕过迁移问题；PostgreSQL 镜像升级必须保持和数据目录相同的主版本，例如现有 PG18 数据卷使用 PG18 + pgvector 镜像，不能直接切到 PG16；回滚脚本不得破坏生产数据。
...
| 产品维护 | POST/PATCH | /api/products, /api/products/{product_id} | 管理产品。 |
| 产品版本 | GET/POST/PATCH | /api/products/{product_id}/versions, /api/product-versions/{version_id} | 管理产品版本。 |
| 产品模块 | GET/POST/PATCH | /api/products/{product_id}/modules, /api/product-modules/{module_id} | 管理产品模块。 |
| 产品 Git 资源 | GET/POST/PATCH | /api/products/{product_id}/git-repositories, /api/product-git-repositories/{repo_id} | 管理产品仓库资源。 |
| 相关系统 | GET/POST/PATCH | /api/system/related-systems, /api/system/related-systems/{system_id} | 管理相关系统配置。 |
| 模型网关配置 | GET/POST/PATCH | /api/system/model-gateway-configs, /api/system/model-gateway-configs/{config_id} | 管理平台默认模型网关。 |
| 模型调用日志 | GET | /api/model-gateway/logs | 查询模型调用元数据，不返回完整 prompt、输出或密钥。 |
| 需求列表 | GET | /api/requirements | 查询需求台账。 |
| 新增需求 | POST | /api/requirements | 创建待审批需求。 |
...
| 知识文档 | GET/POST | /api/knowledge/documents | 查询和导入知识文档。 |
...
| Markdown 导出 | GET | /api/export/tasks/{task_id}/markdown | 导出任务方案。 |
...
| Bug 管理 | GET/POST/PATCH | /api/bugs, /api/bugs/{bug_id} | 查询、登记和更新 Bug。 |
...
| report_pending | executor_failed | failed | system | 记录 executor 错误和 retryable。 | code_review.failed |
...
### 单元测试
- 覆盖状态机、权限判断、响应 envelope、幂等 key 生成、知识切片、模型 gateway schema 校验、GitLab MR 输入校验、diff 大小限制、Review 报告 schema 校验、凭据掩码和迭代规划建议排序规则。
- 目标覆盖率: 80%。

### 集成测试
- 覆盖 FastAPI 路由、PostgreSQL 迁移、pgvector 检索、Redis 依赖、人工确认恢复流程、GitLab MR 预览/拉取、code_review 任务创建、code-review 执行器调用、报告归档、用户洞察聚合和迭代规划确认流程。
