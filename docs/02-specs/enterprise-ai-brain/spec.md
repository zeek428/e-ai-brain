...
`063_assistant_chat_runs.sql` 创建 `assistant_chat_runs` 表，并为 `assistant_messages` 追加 `status`、`client_request_id`、`run_id`、`cancelled_at`、`completed_at`、`failed_at` 和 `error_code` 字段，用于支持服务端取消生成、消息生命周期追踪和历史可恢复状态。
