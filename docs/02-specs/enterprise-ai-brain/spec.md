...
补充约定：结果写入目标注册表除 `scheduled_job_result`、`user_feedback_insights`、`code_inspection_reports` 和 `email_notifications` 外，必须包含 `dingtalk_document`。该目标的可视化字段包括钉钉文档链接或 ID、写入内容、写入方式、返回文档 ID JSONPath 和返回状态 JSONPath；钉钉连接仅表示 URL Key 授权入口，具体写入哪个文档由动作的 `result_mapping.write_target=dingtalk_document` 和 `request_config.arguments.document_id/content/mode` 决定。用户可直接粘贴 `https://alidocs.dingtalk.com/i/nodes/...` 链接，系统保存和执行时自动提取 `/i/nodes/` 后的文档 ID。“钉钉文档 - 更新内容”模板默认使用该写入目标，并在保存可视化动作时保留 `request_config.tool_name=doc.update_document_content`、`request_config.mcp` 和写入参数，避免用户必须手工编辑高级 JSON。
...
补充约定：钉钉官方动作模板名称统一使用“钉钉能力 - 操作”前缀，便于新增动作“配置场景”下拉集中检索；钉钉文档动作模板必须覆盖“钉钉文档 - 搜索文档”“钉钉文档 - 读取内容”“钉钉文档 - 创建文档”和“钉钉文档 - 更新内容”。“钉钉文档 - 更新内容”使用 `doc.update_document_content` 工具，默认支持 `document_id`、`content` 和 `mode=append|overwrite`，属于写入类高风险动作并默认要求人工确认。
