...
| code_inspection | 周期性代码仓库质量/安全/规范巡检报告、finding 归档、严重问题派生 Bug 和通知反馈记录 | scheduled_jobs, plugin_management, product_config, bug, audit |
...
| 代码巡检 | code_inspection, scheduled_jobs | 周期性代码仓库质量、安全和规范巡检，按提交人沉淀报告、finding、通知反馈和严重问题 Bug | 基于产品 Git 资源和插件扫描动作生成，关联产品、仓库、提交人、定时作业运行、Bug 和通知记录 |
...
| integration_plugins | 集成插件定义 | 记录插件 `code`、`name`、`protocol=http/mcp_http/mcp_stdio`、`category`、risk_level、`is_system` 和状态；`category` 必须使用约定枚举 `general/data_warehouse/devops/issue_tracking/observability/knowledge_base/collaboration/ai_service/business_system`，前端以选择项维护，后端拒绝自由文本；非官方插件列表必须提供编辑和删除入口，允许维护名称、分类、协议、风险等级、状态和说明；`gitlab`、`github`、`email` 作为 `is_system=true` 官方标准插件由系统种子维护，页面展示官方标准标签且不提供编辑/删除，后端 PATCH/DELETE 返回 409；删除前必须检查下级连接、动作、定时作业和调用日志，被使用时返回 409 并提示使用清单；第一阶段 HTTP 与 MCP HTTP 可执行，MCP stdio 仅允许登记配置，真实执行需等待命令隔离方案。 |
| plugin_connections | 插件连接配置 | 记录插件归属、环境、endpoint_url、auth_type、auth_config、request_config、超时、重试和状态；`environment` 必须使用约定枚举 `default/dev/test/staging/prod/sandbox`，前端以选择项维护，后端拒绝自由文本；新增和编辑连接时认证配置默认按 `none/bearer/api_key_header/basic` 展示 Token、Header 或 Basic 字段并生成 `auth_config`，JSON 仅作为高级修改入口且可反向同步到可视化字段；选择 GitLab 官方插件时默认 endpoint 为 `https://gitlab.com`、认证为 `api_key_header`、Header 名为 `PRIVATE-TOKEN`，并提供 `api_version/group_id/project_id` Params；选择 GitHub 官方插件时默认 endpoint 为 `https://api.github.com`、认证为 Bearer Token，并提供 `Accept`、`X-GitHub-Api-Version` Headers 和 `owner/repo` Params；选择邮箱官方插件时默认 endpoint 为 `https://mail-gateway.example.com/api`、认证为 `api_key_header`、Header 名为 `Authorization`，并提供 `Content-Type=application/json` Header 和 `mail_provider/default_from/default_to/subject_template` Params；连接级公共 Params/Headers 默认以表格维护并生成 `request_config.query` 和 `request_config.headers`，高级请求 JSON 仅用于精修且可反向同步；连接列表提供编辑、测试和删除入口，测试 endpoint 可达性、认证和连接级 Params/Headers 配置并写入审计，测试结果必须包含 endpoint、协议、认证、HTTP 请求或 MCP `tools/list` 诊断步骤，并在请求调试台展示最终 URL、解析后的 query、headers 明文值、Header 来源、完整请求 JSON 和远端响应摘要；删除前必须检查动作、定时作业和调用日志引用，被使用时返回 409 并提示使用清单；认证配置生成的 Authorization/API Key 等同名认证 Header 优先于 Params/Headers 表格值，若最终请求仍包含 `***` 占位必须明文展示并在诊断中标记，便于判断是否误填；运行时先解析连接级动态变量，再与动作 `request_config` 合并，同名 query/header 由动作覆盖；管理员调试场景响应按配置明文展示 token、secret、password、api_key、Authorization 等字段，PATCH 编辑收到历史占位 `***` 时必须保留服务端原始密钥值，数据库不得把插件连接当成业务数据源。 |
| plugin_actions | 插件动作配置 | 记录插件、默认连接、action_type=http_request/mcp_tool、输入/输出 schema、request_config、result_mapping、requires_human_review 和状态；前端新增和编辑 HTTP 动作时默认以 Params/Headers 表格维护 `request_config.query` 和 `request_config.headers`，参数值支持 `{{current_date}}`、`{{current_date-7}}` 等系统变量表达式，高级 JSON 只用于精修完整 request_config，且必须支持可视化表格与 JSON 双向同步；动作配置页展示请求预览、结果写入目标和高级 `result_mapping`，并按 `write_target` 动态展示对应 JSONPath 字段：`user_feedback_insights` 展示洞察列表、源表行数和原始行列表路径，`code_inspection_reports` 展示仓库 ID、分支、提交 SHA、风险级别、摘要和 finding 列表路径，`scheduled_job_result` 展示可选导入数量路径；新增动作必须提供 MaxCompute 每周用户反馈、GitHub 代码巡检、GitLab 代码巡检和邮箱通知发送场景模板，GitHub/GitLab 模板自动选择官方插件、填充代码扫描或 vulnerability findings 请求路径、默认 Params 和 `code_inspection_reports` 映射，邮箱模板自动选择官方邮箱插件、填充 `/messages/send`、`Content-Type=application/json`、收件人/主题/正文模板参数和 `scheduled_job_result` 映射，管理员仍可在可视化字段和高级 JSON 中调整；代码巡检写入报告时必须按动作或作业输出 `result_mapping` 提取 `repository_id/branch/commit_sha/risk_level/summary/findings`，JSONPath 支持 `$` 根节点以承接根数组响应；动作列表支持编辑、删除、试运行并返回请求预览、响应摘要、`result_mapping` 命中情况和 `write_preview` 写入预览，写入预览需展示目标、预计写入数、候选数量、预览值、报告字段或样例记录；删除前必须检查定时作业和调用日志引用，被使用时返回 409 并提示使用清单；`result_mapping.write_target` 首批支持 `scheduled_job_result`、`user_feedback_insights` 和 `code_inspection_reports`；PATCH 编辑收到脱敏占位 `***` 时必须保留服务端原始敏感值；在定时作业业务语义中，连接负责取数、Skill 负责分析处理、动作负责把处理结果送到目标位置。 |
...
| code_inspection_reports | 代码仓库巡检报告 | 记录 product_id、repository_id、repository 快照、scheduled_job_id、scheduled_job_run_id、collector_run_id、plugin_invocation_log_id、branch、commit_sha、summary、risk_level、finding_count、severe_finding_count、committer_count、committer_summary、status、result_actions、created_bug_ids 和 notification_ids；用于运营治理 / 代码巡检列表和详情。 |
...
| 插件管理 | GET/POST/PATCH/DELETE/POST(invoke/trial) | /api/system/plugins, /api/system/plugin-connections, /api/system/plugin-connections/{connection_id}/test, /api/system/plugin-system-variables, /api/system/plugin-actions, /api/system/plugin-actions/{action_id}/invoke, /api/system/plugin-actions/{action_id}/trial, /api/system/plugin-invocation-logs | 管理 HTTP/MCP HTTP 插件、三方系统连接、动作配置和调用日志；非官方插件、连接和动作新增后必须可编辑、可删除维护；GitLab/GitHub/邮箱作为官方标准插件返回 `is_system=true`，不能修改或删除，实例 endpoint、认证和平台参数在连接里维护；删除前必须提示使用清单，后端对被引用资源返回 409；连接密钥脱敏，PATCH 编辑不得用脱敏占位覆盖真实密钥；连接测试返回诊断步骤，动作支持请求预览和试运行，配置和调用必须审计。 |
...
| 定时作业运行 | GET/POST(cancel) | /api/system/scheduled-job-runs, /api/system/scheduled-job-runs/{run_id}/cancel | 查询定时作业运行实例、AI 配置快照、collector run 关联、结果摘要和失败原因；前端运行记录必须提供详情和复跑入口，详情展示作业类型、执行模式、AI 模型、Agent、Skills、结果摘要、插件调用、Skill/Prompt 快照、作业配置快照和错误信息，复跑使用记录中的 `scheduled_job_id` 调用作业运行接口并打开新运行详情；手动触发或复跑期间必须显示执行中状态并禁用重复触发；运行中作业可由管理员取消。 |
