...
| 系统角色 | 中文名称 | 业务角色映射 | 技术权限边界 |
|----------|----------|--------------|--------------|
| `admin` | 系统管理员 | 平台管理员 | 管理用户、角色、业务大脑、产品主数据、Git 资源、模型网关配置和审计运行；不替代业务负责人做最终产品决策。 |
| `product_owner` | 产品负责人 | 产品负责人 | 创建和审批需求，确认产品详细设计，查看授权产品下任务、知识和真实空状态看板。 |
| `rd_owner` | 研发负责人 | 研发负责人 | 创建和启动 AI 任务，确认技术方案，创建 GitLab MR 快照和 code_review 任务。 |
| `reviewer` | 评审负责人 | 研发负责人或指定 Reviewer | 预览 MR、创建 MR diff 快照、确认或修改后采纳 code_review 报告。 |
| `knowledge_owner` | 知识负责人 | 文档/知识维护者 | 导入知识文档、维护权限角色、查看索引状态、审核知识沉淀候选。 |
| `viewer` | 查看者 | IT 管理者、测试负责人、测试人员或只读参与者 | 查看授权范围内的列表、详情、审计摘要和后续阶段空状态，不具备写权限。 |

完整 `tester`、发布负责人和 IT 管理者写权限按 v1.1/v1.2 扩展；MVP 阶段不应为了空状态页面提前实现复杂角色体系。
角色定义的运行时来源是后端角色目录常量，持久化字典为 PostgreSQL `role_definitions` 表；`users.roles` 只允许引用该目录中的角色 code。系统管理/用户管理页面必须使用固定多选控件，不能让管理员自由输入未定义角色。
...
| 表名 | 说明 | 关键约束 |
|------|------|----------|
| role_definitions | 系统角色字典 | `code` 主键；MVP 可分配角色为 `admin`、`product_owner`、`rd_owner`、`reviewer`、`knowledge_owner`、`viewer`。 |
...
首个初始化脚本位于 `apps/api/app/db/migrations/001_init.sql`，负责 pgvector 扩展和核心表初始化。后续迁移按模块追加，例如 `002_persistence_users.sql` 补齐用户表种子数据和 `app_state_snapshots` 持久化快照表，`003_role_definitions.sql` 补齐角色字典。当前产品配置运行时会将产品、版本、模块和 Git 资源同步到对应结构表，并在启动时用结构表覆盖快照中的同类集合；需求、任务、Review、知识和审计仍按后续切片逐步替换为细粒度仓储。已有环境必须通过可重复执行的 SQL 迁移脚本升级，不得通过清空 PostgreSQL 数据卷绕过迁移问题；PostgreSQL 镜像升级必须保持和数据目录相同的主版本，例如现有 PG18 数据卷使用 PG18 + pgvector 镜像，不能直接切到 PG16；回滚脚本不得破坏生产数据。
