...
| 业务发布 | 人工 / SSH / Docker / Jenkins | 统一经过部署预检、产品环境执行资源授权、发布波次、健康验证、回滚和事务 Outbox。 |
...
| 部署执行资源 | GET | /api/devops/deployment-runner-targets, /api/devops/deployment-jenkins-connections | 要求 `deployment.scheme.manage`，返回可用于部署方案的就绪 Runner 目标或脱敏 Jenkins 连接候选；只读用户不能枚举执行资源，响应不返回本地密钥、机器路径或连接认证配置。 |
