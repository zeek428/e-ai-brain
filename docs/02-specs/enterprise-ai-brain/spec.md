...
| AI 助手聊天 | POST | /api/assistant/chat | 基于当前 AI Brain 系统上下文和模型网关 Chat 能力回答产品、迭代进度、任务、阻塞需求、待确认 Review、代码评审结论、Bug 分布、知识沉淀、Git 仓库和模型网关状态问题，并按用户保存会话历史；对“新增任务”等泛化创建意图可直接返回 `assistant.task_creation_guide` 确定性工具结果，澄清研发任务、定时作业、插件动作、代码巡检和反馈洞察等任务类型，前端按草案优先向导展示。 |
