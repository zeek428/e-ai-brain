...
| 定时作业运行 | GET/POST(cancel) | /api/system/scheduled-job-runs, /api/system/scheduled-job-runs/observability, /api/system/scheduled-job-runs/{run_id}/cancel | 查询定时作业运行实例、AI 配置快照、collector run 关联、结果摘要和失败原因；运行可观测性接口聚合成功率、失败率、平均耗时、AI/Token/插件调用、动作写入成功率、失败原因、最近失败和慢运行，供运行记录页签顶部概览展示；前端运行记录必须提供详情和复跑入口，详情展示作业类型、执行模式、AI 模型、Agent、Skills、结果摘要、插件调用、Skill/Prompt 快照、作业配置快照和错误信息，复跑使用记录中的 `scheduled_job_id` 调用作业运行接口并传入 `trigger_type=manual_rerun` 与 `source_run_id=<当前运行 ID>` 后打开新运行详情；若响应包含 `source_run_summary`，详情页必须展示“复跑对比”，对比来源运行与本次运行的状态、导入数和错误码；手动触发或复跑期间必须显示执行中状态并禁用重复触发；运行中作业可由管理员取消。 |
...
| `AiExecutorRunnerService` | 管理隔离 Runner 注册、心跳、Token 校验、任务队列、OpenClaw/Codex/Claude/Hermes 执行类型校验、任务认领和完成回写；完成回写不得执行外部命令，只更新任务状态、插件日志、定时作业运行、collector run 和作业最近运行字段。 |
