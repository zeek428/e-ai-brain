...
| assistant_action_drafts | AI 助手动作草案 | 记录聊天生成或显式创建的 `create_rd_task`、`create_scheduled_job`、`create_plugin_connection`、`create_plugin_action`、`create_analysis_draft` 草案，包含用户、来源消息、客户端草案 ID、payload、风险、状态、`expires_at` 和确认/取消信息；确认前不写领域表或最终分析结果，过期 pending 草案自动转为 `expired`；用户打开详情或深链加载时通过查看接口写入 `metadata_json.viewed_at/detail_viewed_at/deeplink_viewed_at/last_viewed_at/view_count/viewed_by/last_view_surface`，供效果漏斗区分真实查看详情和深链打开；用户把草案应用到表单后如修改字段，客户端通过草案修改标记接口写入 `metadata_json.user_modified/modified_fields/modified_at/modified_by`，供助手效果指标统计用户修改率。 |
