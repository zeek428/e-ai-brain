...
- LLM 通过 `model_gateway` 生成结构化评估、工作分解、负责人候选、依赖、风险、返工和重规划建议；实现类工作项还必须声明 `repository_id/path/mode(read|write)` 资源意图。平台编排服务校验 JSON Schema、DAG 无环性、岗位能力、权限、冻结容量、资源声明、并发、租约、预算、幂等和风险下限，并拥有所有状态迁移的最终写权限。同仓库重叠路径的无序 write/write 声明由确定性服务补成稳定的 `finish_to_start` 依赖并持久化冲突证据；容量已满的 AI 席位只延后就绪项，绝不创建无 attempt 的临时任务。
...
- 工作项准备就绪后按岗位席位派发：AI 席位复用 `ai_tasks`、`agent_loop_runs`、`ai_executor_tasks` 和 Runner；真人席位生成可领取/执行的工作项。公开 AI 任务创建/启动接口不能由真人或外部客户端调用，编排器通过内部领域服务处理。AI 和真人提交相同的结果契约与证据，审核人不能与被审核席位相同；失败审核或质量门禁失败进入 `rework_required` 并保留原 attempt。`blocked/awaiting_human` 保存平台冻结的恢复目标，只能在条件重新校验或决策完成后恢复、返工或取消。
