...
- 策略匹配优先级为：同任务类型下产品专属策略优先于全局策略，随后按 `priority` 升序匹配；未命中策略的研发任务继续沿用现有模型网关 / code_review executor 路径。研发执行器策略页面必须展示“命中提示/命中预览”：列表中标识全局策略作为通用兜底、被同范围策略覆盖或不参与命中；新增/编辑弹窗需基于当前表单的任务类型、产品、优先级和状态即时预览最终会命中的策略，并提示同级 `priority` 冲突需要调整。
...
- 策略新增 `autonomy_mode=single_pass|autonomous_loop`、`max_iterations`、`max_duration_seconds`、可选 `token_budget/cost_budget`、`quality_gate_policy_id` 和 `auto_merge_risk_threshold`。自治模式每轮冻结上下文版本、计划、编码任务、门禁、验证证据、失败分析和预算；门禁失败且可重试、预算未耗尽时才创建下一轮，预算、安全边界或人工接管会停止继续派发。
- `code_change_review_mode` 默认 `manual_review`：编码 Runner 成功后必须先运行平台独立质量门禁，随后进入待确认，人工点击“确认通过/修改后通过”才向 Runner 下发 `merge` 决策。配置为 `auto_commit` 也不能在编码 Runner 成功时直接合入；只有门禁通过、至少一份独立证据、风险不高于阈值且未命中数据库迁移、受保护目录或安全阻断时，系统才自动创建已通过 Review、记录 `review.submitted/ai_task.executor_auto_committed` 审计并下发 `merge`。否则降级为人工确认。失败、取消、超时、拒绝或门禁不可恢复失败走 `discard`。
...
- Runner 认领、追加日志、租约重派、完成、取消、超时或死信回写时，服务端同步 `ai_tasks`：运行中和重派保持 `running/waiting_ai_executor`；编码成功进入 `quality_gate_running`，verifier 回写后由门禁决定下一轮、人工确认或受控自动合入；失败、取消、超时或死信写入错误码和错误信息，任务进入 `failed/cancelled` 或人工接管状态。
- 执行器输入包含研发任务摘要、需求/Bug 快照、产品上下文、仓库/分支、验收标准、版本化 `execution_context_manifest`、受限知识引用和输出契约；不下发产品 Git 凭据、插件连接密钥、模型网关密钥或 Agent/Skill 配置。知识引用只包含文档/chunk/版本、召回原因和限长摘要。Runner 可在受控工作区中使用本机 Codex/Claude Code/OpenClaw 完成工程修改，编码自报成功不能替代平台 verifier。
- 任务详情返回 `execution_context_manifest`、`agent_loop` 和 `quality_gate`。前端按页签展示上下文版本/哈希/截断、轮次与预算、每项独立检查和证据；运行中的自治循环可调用人工接管接口，服务端终止继续派发、取消仍在运行的 Runner 任务并写入审计。
...
| testing | advance_status | released | 产品负责人/研发负责人 | 不再自动把需求从 `testing/ready_for_release` 推进到 `released`；版本发布必须先由运维部署单登记成功部署，仍处于设计、开发、评审、测试或待发布等未完成状态的需求必须先延期、取消、关闭、部署成功或验收处理，否则阻止发布，`force=true` 不绕过发布阻塞。 | product_version.status_advanced |
