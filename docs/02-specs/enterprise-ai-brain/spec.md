...
草案状态为 `pending`、`confirmed`、`cancelled` 或 `failed`，确认执行记录写入 `assistant_action_runs`。`POST /api/assistant/action-drafts/{draft_id}/confirm` 只接受仍处于 `pending` 的草案，并重新走对应领域 service：定时作业草案调用 scheduled_jobs service，保存时把 `config_json.assistant_draft` 写入作业配置；插件连接草案调用插件连接 service；动作草案调用动作 service。确认成功写入 `assistant_action_draft.confirmed` 审计并返回领域资源 ID 和运行记录；取消写入 `assistant_action_draft.cancelled`，不产生业务写入。后续删除、停用、插件试运行、定时作业运行、AI 能力配置创建/修改等高影响动作继续沿用同一确认模型，确认后仍复用现有 service 校验、repository 写入、审计事件和运行快照。
...
| assistant_action_drafts | AI 助手动作草案 | 记录聊天生成或显式创建的 `create_scheduled_job`、`create_plugin_connection`、`create_plugin_action` 草案，包含用户、来源消息、客户端草案 ID、payload、风险、状态和确认/取消信息；确认前不写领域表。 |
