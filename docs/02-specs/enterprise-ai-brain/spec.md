...
| 部署执行资源 | GET | /api/devops/deployment-runner-targets, /api/devops/deployment-jenkins-connections | 要求 `deployment.scheme.manage`，Runner 候选必须同时具备部署能力、部署信任域、在线心跳和上报目标；目标的 `ready=true` 还要求近期平台受控真实探测成功，响应返回脱敏探测状态和时间，不返回本地密钥、机器路径或连接认证配置。 |
| 运维部署执行 | GET/POST | /api/devops/deployments, /api/devops/deployments/{id}/start, /api/devops/deployments/{id}/complete, /api/devops/deployments/{id}/cancel, /api/devops/deployments/{id}/runs/{run_id}/sync, /api/devops/deployments/{id}/runs/{run_id}/logs | 部署单固化方案快照；人工部署允许人工 complete，SSH/Docker 由 Runner 回写，Jenkins 由后台或手工 sync 收敛；统一日志与取消中状态按产品 scope 返回。 |
