...
`admin` 是内置超级管理员角色，不需要通过角色权限矩阵额外配置权限点或功能菜单。只要用户拥有 active 的 `admin` 角色，后端权限快照就按所有 active 权限点和所有 active 菜单资源动态展开，并自动拥有 `global:*` 管理范围；角色管理页可以展示该展开结果，但不应把 admin 的显式权限配置作为实际授权来源。
