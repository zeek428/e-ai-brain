...
| Scheduler | POST | `/api/system/scheduled-jobs` | 创建采集、AI 分析、动作调用、迭代建议或看板刷新作业；请求可传 `plugin_connection_ids` / `plugin_action_ids` 表达多个数据连接和动作，服务端去重保序并写入 `config_json.orchestration`，旧 `plugin_connection_id` / `plugin_action_id` 自动取数组第一项兼容当前执行入口。 |
| Scheduler | PATCH | `/api/system/scheduled-jobs/{job_id}` | 更新作业计划、启停、动作、AI 装配、重试和超时策略；支持更新 `plugin_connection_ids` / `plugin_action_ids`，响应与列表均返回完整数组和第一项兼容字段。 |
