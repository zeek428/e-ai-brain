...
| Plugins | Contract | 内部数据源动作 | 内部业务数据读取动作使用 `action_type=internal_query`，请求预览返回 `method=INTERNAL_READ`、`protocol=internal_read_model`、源数据类型和过滤条件；调用响应中的 `response_summary.json` 必须包含 `datasets`、`source_counts`、`source_types`、`labels`、`filters`、`schemas`、`row_count` 和 `total_rows`。该动作只负责读取内部数据，定时作业后续仍需通过 AI执行和结果动作完成分析与写入。 |
