...
| Plugins | GET | `/api/system/plugin-marketplace` | 查询官方插件市场只读目录；返回 GitLab/GitHub/邮箱标准插件的 `publisher/summary/recommended_scenarios/connection_defaults/connection_template_version/action_templates/installed/plugin_id/connection_count/action_count`，其中 `connection_defaults` 是可直接带入新增连接表单和 AI 助手连接草案的默认 payload，包含 `auth_type/auth_config/endpoint_url/environment/request_config/status/timeout_seconds/max_retries/name`；前端可据此引导新增连接或打开 GitHub/GitLab/邮箱官方动作模板，并在响应前确保官方插件种子存在。 |
| Plugins | GET | `/api/system/plugin-action-templates` | 查询插件动作场景模板目录；返回 MaxCompute 每周用户反馈、GitHub 代码巡检、GitLab 代码巡检和邮箱通知发送等模板，每个模板包含 `code/name/plugin_code/plugin_id/action_type/default_code/default_name/request_config/result_mapping/form_defaults/template_version`。前端新增动作表单必须使用该目录动态生成场景选项，并按模板回填请求配置、Params/Headers、结果写入目标和 JSONPath 映射；AI 助手生成插件动作草案时必须携带 `template_code/template_version`；若服务端模板缺失，前端和 AI 助手只能提示模板缺失或要求刷新模板目录，不得硬编码生成官方动作兜底。 |
