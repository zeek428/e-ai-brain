...
| Scheduler | Contract | 插件执行调用运行摘要 | `job_type=plugin_action_invoke` 的成功运行响应必须返回 `result_summary.job_type=plugin_action_invoke` 和 `result_summary.message=插件执行调用完成`；若存在插件调用摘要，仍返回 `result_summary.plugin` 与 `result_summary.execution_nodes`，用于运行详情展示数据连接、AI执行/跳过状态和动作反馈。响应不得返回 `No handler implemented` 等开发占位文案。 |
