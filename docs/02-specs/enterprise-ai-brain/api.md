...
| Plugins | POST | `/api/system/ai-executor-tasks/claim` | Runner 认领任务接口。调用方必须携带 Runner token，并提交 `runner_id` 和可选 `executor_type`；服务端只返回该 Runner 支持且处于 `queued` 的最早任务，任务进入 `claimed`，并在 `request_config.reliability` 写入 `lease_started_at/lease_expires_at/lease_timeout_seconds/max_reclaim_count` 等租约元数据；响应包含 `instruction/workspace_root/input_payload/request_config/timeout_seconds/scheduled_job_run_id/ai_task_id` 等执行上下文。 |
