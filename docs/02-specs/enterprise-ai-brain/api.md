...
| Scheduler | Error | `SKILL_OUTPUT_SCHEMA_INVALID` | 定时作业 AI 场景模型返回 JSON 后，若缺少 Skill 输出 Schema 必填字段、字段类型不匹配或已声明数组 item 结构不匹配，运行应失败在 `execution_nodes.skill_processing`，`result_action.status=not_run`，不得继续写入用户洞察、代码巡检报告、邮件通知或定时作业结果。 |
