...
### TC-AIBRAIN-RD-FUNC-035B: P1 并行资源冲突与冻结席位容量调度
...
**状态**: 已自动化覆盖，见 `apps/api/tests/test_rd_parallel_conflicts.py`、`apps/api/tests/test_rd_collaboration_plan_generation.py`、`apps/api/tests/test_rd_collaboration_auto_dispatch.py`、`apps/api/tests/test_rd_work_item_execution_postgres.py::test_postgres_dispatch_preserves_parallel_requirement_task_links_and_fields`、`::test_postgres_requirement_patch_after_dispatch_keeps_new_task_link`、`apps/api/tests/test_iteration_version_status_flow.py` 和 `apps/web/tests/VersionDashboardCollaborationPanel.test.tsx`。
...
4. 保留一个仍为 `ready` 的到期候选，让最终派发分别与工作项 cancel、父运行从 `running` 暂停到 `waiting_human` 并发；记录首次加锁对象、等待/完成顺序、SQLSTATE、最终运行/工作项状态和派发 bundle 表。
5. 创建多个活动协作运行及各自多个到期工作项和依赖；以低 `limit` 连续扫描、重启 Worker 并并发扫描。记录每次候选页、依赖前驱查询、全局/运行游标和最终派发结果。

**预期结果**: 任务、Runner、attempt、上下文、Agent Loop、预算、协作事件和显式构造的审计 bundle 只在同一 PostgreSQL 事务成功时同时存在；任一失败或并发落选均无孤立记录，审计以数据库事实为准。最终事务先不加锁读取父运行 ID，再严格按 `rd_collaboration_runs FOR UPDATE -> rd_work_items FOR UPDATE` 加锁，并在锁后重验归属、状态、version 与 due；与 cancel/suspend 并发时不得出现 SQLSTATE `40P01`。派发只原子追加对应需求的 `task_ids`，并发派发或随后保存需求快照不得丢失已关联任务；去重后保持已持久化任务顺序，新任务只追加到末尾。父运行已进入 `waiting_human` 或其他非活动状态时返回 `RD_WORK_ITEM_STATE_INVALID`，运行保持暂停、工作项仍为 `ready`；工作项已取消时取消状态保持不变，两种竞争均无任何派发 bundle 或新增派发审计。Runner 安全阻断只暂停受影响工作项，并创建一个以 `work_item_id + attempt_no (+ renewal_no)` 确定的 `runner_safety_approval` 决策和审批请求，不创建 Runner task 或 attempt；批准仅保存覆盖冻结操作的不可变审批快照，拒绝取消工作项，过期仅创建同 attempt 的确定性 renewal。通用审批路径不得替代协作决策；最终派发在同一事务锁定并重验指令、安全快照、审批来源/身份/证据/操作集、策略版本及过期时间，篡改、不完整覆盖或过期均 fail closed 且无 bundle 写入。前三次可重试故障仅保存安全码、失败计数和 5/10/20 秒 `next_dispatch_at`，到期前不再派发，成功清零；第 4 次暂停并创建冻结 `dispatch_fault_resolution` 决策，容量耗尽不计入退避。扫描只读取到期的 `ready/rework_required` 候选，候选与依赖均为有界批量页；全局轮转与每运行续扫游标持久化，使重启/多 Worker 后仍公平继续。`limit` 覆盖所有检查结果；reservation version 与 `due_at` 在最终事务中重验，陈旧保留返回无副作用跳过，不得绕过新的退避或产生执行记录。

**状态**: 已自动化覆盖，见 `apps/api/tests/test_rd_work_item_execution_postgres.py::test_postgres_dispatch_rolls_back_task_runner_attempt_event_and_audit_together`、`::test_postgres_dispatch_keeps_worker_scratch_empty_after_success`、`::test_postgres_autonomous_dispatch_persists_explicit_audit_bundle_without_reading_store_audits`、`::test_postgres_runner_safety_approval_is_atomic_replay_safe_and_dispatchable`、`::test_postgres_dispatch_rejects_approval_that_expires_while_waiting_for_its_row_lock`、`::test_postgres_expired_runner_safety_approval_renews_without_dispatch_artifacts`、`::test_postgres_stale_reserved_worker_cannot_bypass_new_retry_backoff`、`::test_postgres_final_dispatch_fences_bundle_reserved_before_parent_suspension`、`::test_postgres_final_dispatch_and_cancel_use_run_then_work_item_lock_order`、`::test_postgres_final_dispatch_waits_for_parent_suspension_without_artifacts`，以及 `apps/api/tests/test_rd_collaboration_auto_dispatch.py::test_auto_dispatch_persists_safe_retry_backoff_and_suppresses_early_retry`、`::test_auto_dispatch_does_not_record_retry_after_candidate_becomes_stale`、`apps/api/tests/test_rd_collaboration_repository.py::test_repository_reserves_fair_dispatch_pages_across_restart_and_workers` 和 `apps/api/tests/test_rd_work_item_scheduler.py::test_scheduler_batches_omitted_predecessors_for_repository_due_candidates`。
...
4. 真实 E2E 只接受 `task_kind=coding` 且 `runner_id` 等于预检配置的编码 Runner，并要求唯一成功的 `quality_gate` Runner 任务与 Gate ID 对应、使用独立 verification trust domain，聚合门禁含至少一条独立证据、一条已验证证明且 `verifier_trust_isolated=true`。在 implementation Review 批准前读取 DAG，确认 automated-testing 仍为 `blocked`、没有 AI task 且 `active_attempt_count=0`；批准后必须先观察 Worker 自动派发时 `active_attempt_count>0`，再等待质量门禁与 Review。分别构造质量返工、取消后恢复、超时人工恢复和高风险派发场景：质量场景从真实 Runner 的未失败首态开始，以任务输入令首轮产物明确缺少最终验收标记，再由真实独立 gate 判失败；取消场景必须同时等到 Runner `cancelled` 和服务端持久 fence，不能由 Harness 提交回调；超时场景通过统一策略 `expected_policy_version` 临时收紧、评估快照冻结、`finally` 完整恢复原 payload/status，记录原/故障/恢复版本，首次恢复失败时紧急重试清理并让场景失败关闭；高风险派发只接受冻结的 `approve_dispatch` 或 `cancel_work_item`，不得注入 `reject_dispatch`，浏览器通道必须从对应工作项来源的人工决策界面选择 `approve_dispatch`。校验 `attempt_history` 以不可变序号保留旧 attempt、关联同产品范围内唯一编码 Runner、只返回 workspace SHA-256 指纹、安全失败码、返工计数和持久化迟到围栏摘要，并在恢复后新增 attempt 而不覆盖旧项。另用跨产品账号读取工作项列表，并在 attempt、Runner 与 fence 事件中注入 ID、租约、Token Hash、执行器、员工、幂等键、原始 workspace、输入输出、异常、返工正文、Runner 配置/日志/结果、Provider 回调和嵌套敏感值；创建超过 20 条历史及非法状态、超长字符串、非法时间戳，覆盖 memory/repository 两条读取路径。
5. 在 Provider 回调前后分别读取协作运行详情：校验 `git_deliveries` 的待对账记录只含本地交付事实且远程字段为空；为两个产品配置相同 Provider/远端仓库地址，并让目标交付冻结到第二个产品仓库，确认签名回调按 `rd_delivery_id` 绑定冻结产品/仓库而不是命中第一条仓库配置；仓库、Provider 或分支不匹配时拒绝。完成签名 Provider Inbox 对账后校验状态为 `reconciled`、local/remote SHA 相等、对账 ID/验证时间/双证据哈希完整，并确认响应不含 Outbox、工作区/worktree、审批、Runner、回调、凭据、Token 或原始 payload。验签时冻结的错误上下文不得在重试时改写，修复配置后必须接收新 Provider Delivery；再用无该产品范围的账号读取同一运行。
6. 删除或篡改远程 commit/MR/PR 对账证据，重复完成判定。

**预期结果**: P0 集成工作项必须完成最小工作分支隔离、远程 push 或 MR/PR Outbox、版本级集成测试和不可变交付证据。原生 Runner 的三种本地结果形态均可归一化；冻结分支/仓库/工作区不匹配时拒绝，伪造远程、对账和回调字段不落库。测试证据只保留契约内有界标量，嵌套敏感值和非法/超限值在新写入及旧记录公共投影中均不可见。E2E 必须证明指定编码 Runner、独立可信质量门禁和审核前依赖阻塞，不能把缺失任务类型当作编码执行、读取不存在的工作项 `attempt_id`，或把提前派发当成正常依赖推进。工作项列表仅在权限和产品范围内返回 `active_attempt_count` 与最多最近 20 条的 `attempt_history`；历史按 attempt 序号升序，`total/truncated` 准确，Runner 关联和 workspace 指纹由服务端派生，迟到围栏只来自同内部 attempt 的持久化事件。跨产品读取拒绝；内部 attempt ID、租约/Token/Hash、执行器/员工、幂等键、原始路径和 payload、返工正文、Runner 配置/日志/结果及事件 payload 均不泄漏，历史脏数据被白名单净化。缺少或不匹配远程证据时不能进入待发布，完成判定不临时执行 push。运行详情只在既有读权限和产品范围内返回按运行过滤的 `git_deliveries`；未对账状态为 `pending`，完整对账状态为 `reconciled`，敏感内部证据不泄漏，跨产品读取返回 `FORBIDDEN`。发布就绪证据记录与目标终结分离；默认策略让产品版本进入并保持 `ready_for_release`，协作运行进入 `completed` 且 `completion_reason=ready_for_release`，不创建部署单，服务不得无条件关闭所有交付目标的运行。
...
3. 在 Schema v2 未激活且 cleanup 未开始时，以正确/过期版本中止 draining；然后再次 draining，待活动任务、Agent Loop、Runner lease 和协作命令归零并写备份标记后进入 `cutover_locked`，重跑锁内预检。分别模拟锁内预检阻断、cutover_locked 后尝试中止、转换成功、新应用健康失败、清理迁移 121 重试、Worker 图版本不一致、v2 写入冒烟失败和全部成功。
4. 对全新 volume 边界检查 Compose 和 PostgreSQL image 不向 `/docker-entrypoint-initdb.d` 挂载或打包应用迁移，API image 已打包 `/app/app/db/migrations` 与 entrypoint 默认路径。再构造普通 additive SQL、显式 cleanup 121 和四个派发大索引 125-128，运行正常 API 启动入口；最后以两个 repository compatibility 启动实例竞争同一 advisory lock，并覆盖有效索引、无效索引和未取得锁三种状态。

**预期结果**: 围栏外 preflight 只读；draining 阻断新增研发写入与 Runner 新领取并返回 `RD_UPGRADE_MAINTENANCE`，但允许在途终态回写和管理员受控取消，不阻断定时作业。合法早期中止审计后恢复旧写路径，过期版本返回 `RD_VERSION_CONFLICT` 且不改变围栏；零活动和备份是进入 cutover_locked 的硬前置，不满足时返回 `RD_UPGRADE_STATE_INVALID`。锁内预检失败不删除旧字段；进入 cutover_locked、Schema v2 激活或 cleanup 开始后，中止返回 `RD_UPGRADE_ABORT_NOT_ALLOWED`，只能向前重试。切换成功后只启用 v2 规则；健康、清理、Worker/Schema/图版本或写入冒烟任一步失败都保持 `cutover_locked`。PostgreSQL initdb 不执行任何应用迁移，API image/entrypoint 是全新和存量 volume 的唯一普通迁移控制面，入口以阻塞 advisory lock 与 `app_schema_migrations` 的文件名/SHA-256 账本在同一事务中串行执行未登记 SQL，校验和不一致必须失败启动。普通 API 启动只执行普通 additive SQL，明确跳过 121 和 125-128；121 只能由显式 cleanup 执行，125-128 由 autocommit repository compatibility 路径在非阻塞 advisory lock 下用 concurrent index DDL 创建或修复，有效索引不重建、未获锁实例不等待。全部成功后显式置为 disabled，v2 评估和协作写入恢复，旧 approve/generate/batch-delivery-advance/AI-task-create/start 仍永久拒绝。

**状态**: 已自动化覆盖启动迁移边界，见 `apps/api/tests/test_docker_migration_boundary.py::test_postgres_initdb_cannot_execute_application_migrations`、`::test_api_image_packages_migrations_at_entrypoint_default_path` 与 `apps/api/tests/test_api_entrypoint.py::test_api_entrypoint_runs_only_ordinary_additive_migrations`，以及 `apps/api/tests/test_persistence_repository_boundaries.py::test_postgres_schema_compatibility_only_applies_concurrent_dispatch_indexes`、`::test_schema_compatibility_rejects_missing_migration_files`、`::test_concurrent_index_compatibility_path_serializes_and_skips_valid_index`、`::test_concurrent_index_compatibility_path_does_not_wait_for_another_startup`；围栏与切换覆盖继续见 `apps/api/tests/test_rd_collaboration_migration.py` 和 `apps/api/tests/test_rd_maintenance_fence.py`。
