...
- `product_onboarding_scores`：按活跃产品计算接入完整度分数，评分来源包括产品主数据、版本、模块、代码仓库、可检索产品文档、关联系统、插件连接、产品权限范围和最近健康检查。每个产品返回 `missing_items`、`plugin_connection_count`、`plugin_failed_connection_count`、`plugin_total_connection_count`、`permission_scope_count`、`permission_scope_status`、`searchable_knowledge_document_count`、`recent_health_status=healthy|attention|degraded`、`recent_health_check` 和 `score_breakdown[]`；`score_breakdown[]` 按 `master_data/version/module/git_repository/knowledge/related_system/plugin_connection/permission_scope/recent_health` 返回 `score/max_score/status/evidence/suggestion`，用于解释插件连接、权限范围和最近健康检查如何影响总分。`recent_health_check` 汇总插件测试失败、知识索引失败、权限范围缺失和代码仓库缺失等真实健康信号。
            ...
            "score_breakdown": [
              {
                ...
                "max_score": 10,
                "status": "healthy",
                "evidence": "active 2 / total 2 / failed 0",
              ...
              },
              {
                ...
                "suggestion": "定期复核 viewer/read/write/admin scope 是否匹配产品边界"
              }
            ],
