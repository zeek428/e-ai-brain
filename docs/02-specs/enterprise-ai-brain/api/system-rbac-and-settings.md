...
`GET /api/system/health` 为系统健康与配置体检聚合接口，允许 `system.health.read`、`system.settings.manage` 或 `system.roles.read` 访问。响应不写入业务数据，聚合 `/health` 依赖状态、系统设置、钉钉登录配置、钉钉 MCP 插件连接、模型网关日志、知识索引健康、AI 执行器队列、定时作业运行、观测告警配置和产品初始化状态。每个 `checks[]` 项返回 `key`、`category`、`component`、`title`、`status=ok|configured|warning|degraded|error|not_configured|disabled|info`、`description`、`fix_suggestion`、可选 `last_error`、`metrics` 和 `action_href`；`summary` 返回总数、正常项、需关注项、阻断异常和按状态/分类计数；`recommendations[]` 按异常优先级返回可跳转的修复建议。

接口同时返回 `operations` 运维治理视图，供系统健康页展示和后续告警任务复用：

- `alert_center`：按检查项、产品接入评分和钉钉授权有效期聚合打开告警，返回 `summary.open_count/high_count/medium_count/low_count` 和脱敏后的 `alerts[]`。
- `ai_executor_ops`：聚合 Runner 数、队列状态、排队/运行/失败/死信/超时任务、待审批数、队列压力和最近失败摘要。
- `knowledge_quality_loop`：聚合知识空间、文档可检索率、索引失败、导入失败、待审核沉淀和质量门禁；检索日志、无结果率、RAG 引用准确性等尚未接入时必须返回 `instrumentation_pending`，不得伪造指标。
- `product_onboarding_scores`：按活跃产品计算接入完整度分数，评分来源包括产品主数据、版本、模块、代码仓库、产品文档和关联系统，并返回 `missing_items`。
- `permission_diagnostics`：基于 RBAC 权限矩阵汇总高风险权限、菜单权限缺口和未配置数据范围的角色；无 repository 的测试运行时返回降级诊断。
- `dingtalk_lifecycle`：聚合钉钉登录启用状态、企业白名单、用户绑定数、MCP 连接状态和 URL Key 到期提醒。
