...
- `alert_center`：按检查项、产品接入评分和钉钉授权有效期聚合告警，返回 `summary.open_count/high_count/medium_count/low_count/resolving_count/closed_count`、`alerts[]`、`subscriptions[]` 和最近趋势 `trend[]`。告警项包含 `status`、`owner`、`first_seen_at`、`last_seen_at`、`close_reason` 和 `postmortem`。
...
- `product_onboarding_scores`：按活跃产品计算接入完整度分数，评分来源包括产品主数据、版本、模块、代码仓库、产品文档、关联系统、插件连接和产品权限范围，并返回 `missing_items`、`plugin_connection_count`、`permission_scope_count` 与 `recent_health_status`。
