...
`GET /api/system/health` 为系统健康与配置体检聚合接口，允许 `system.health.read`、`system.settings.manage` 或 `system.roles.read` 访问。响应不写入业务数据，聚合 `/health` 依赖状态、系统设置、钉钉登录配置、钉钉 MCP 插件连接、模型网关日志、知识索引健康、AI 执行器队列、定时作业运行、观测告警配置和产品初始化状态。每个 `checks[]` 项返回 `key`、`category`、`component`、`title`、`status=ok|configured|warning|degraded|error|not_configured|disabled|info`、`description`、`fix_suggestion`、可选 `last_error`、`metrics` 和 `action_href`；`summary` 返回总数、正常项、需关注项、阻断异常和按状态/分类计数；`recommendations[]` 按异常优先级返回可跳转的修复建议。该接口不得返回 SMTP 密码、钉钉 Client Secret、模型 API Key、插件 URL Key 或任何密钥明文。
