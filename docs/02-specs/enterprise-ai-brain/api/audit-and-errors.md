...
| 协作运行内部 `create_ai_task_for_work_item` / `dispatch_ai_task_for_work_item` | 409 | RD_ROLE_ASSIGNMENT_REQUIRED / RD_WORK_ITEM_NOT_READY / RD_WORK_ITEM_STATE_INVALID | 否 | 记录工作项、父运行、冻结策略、缺失岗位、候选席位和安全失败原因；父运行暂停后的陈旧 reservation 不新增派发审计。 | 保持工作项阻断，补齐岗位席位或前置条件后由调度器恢复；父运行已暂停或终结时不得重试陈旧 reservation。 |
