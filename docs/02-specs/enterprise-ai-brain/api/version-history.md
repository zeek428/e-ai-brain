...
| 版本 | 日期 | 变更内容 | 作者 |
|------|------|----------|------|
| v1.1.565 | 2026-07-11 | 新增 Agent 自治循环/独立门禁/执行上下文详情、人工接管、执行资源授权、外部事件 Inbox、部署预检/波次/健康/回滚/详情/分页、Git 写回和多模态知识 Profile/版本/过期/引用反馈 API | Codex |
| v1.1.564 | 2026-07-10 | 定时作业平台模型网关 AI 处理新增数据连接响应采样压缩和失败诊断透出，避免周反馈/客服洞察大批量 rows 直接进入模型导致超时 | Codex |
| v1.1.563 | 2026-07-09 | 系统健康页权限诊断区新增“用户视角预览”入口，复用 `/api/system/permissions/menu-preview` 展示可见入口、阻断入口、缺失权限点和 scope 摘要 | Codex |
| v1.1.562 | 2026-07-09 | 产品接入完整度评分新增 `score_breakdown[]` 分项矩阵，按主数据、版本、模块、代码仓库、知识、关联系统、插件连接、权限范围和最近健康返回得分、状态、证据和建议 | Codex |
