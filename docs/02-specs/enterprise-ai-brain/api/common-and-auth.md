...
钉钉登录身份与 AI Brain 用户通过 `user_external_identities` 绑定，业务权限继续以 `users.id`、RBAC 权限点和数据范围为准。已有 AI Brain 账号不按邮箱或手机号静默自动关联钉钉账号；用户必须先用 AI Brain 账号密码登录，再从个人中心发起自助绑定，服务端通过钉钉 OAuth 返回的稳定身份 `provider_subject`（当前优先取 `union_id/open_id/subject`）建立一对一绑定。未绑定到系统用户的钉钉身份不得获得默认业务权限；若启用 `DINGTALK_AUTO_PROVISION`，默认角色为 `DINGTALK_AUTO_PROVISION_ROLE`，推荐保持 `viewer`，也可通过 `DINGTALK_PENDING_APPROVAL` 创建待审批账号。
...
`GET /api/auth/profile` 要求当前用户已登录，返回当前用户基础资料、RBAC 快照和钉钉绑定摘要。响应不会返回密码 hash、钉钉 access token、refresh token、client secret 或 OAuth code。
...
```json
{
  ...
  "email": "zhangsan@example.com",
  "mobile": "+86 13800000000",
  "current_password": "<required when changing email or password>",
...
```json
{
  "data": {
    ...
    "access_token": "<returned only when email changed>",
    "token_type": "bearer",
    "expires_in": 28800
  },
  "trace_id": "trace_profile_update_001"
}
```
