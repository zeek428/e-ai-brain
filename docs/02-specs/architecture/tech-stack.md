...
Docker Compose 中的 PostgreSQL/pgvector 镜像不包含应用迁移，也不从 `/docker-entrypoint-initdb.d` 执行 `apps/api/app/db/migrations`。普通迁移只由 API 镜像打包的迁移目录与 `api-entrypoint.sh` 执行，使全新和存量 volume 共享同一运维路径；121 显式 cleanup 和 125-128 运行时 concurrent compatibility 是仅有例外。
