...
- 人工决策和协同事件携带幂等事件 ID，通过持久化事件入口唤醒 Graph Run，并从最后成功 Checkpoint 恢复。
- 消费事件、推进业务状态、写入新 Checkpoint、审计和待派发 Outbox 必须满足事务一致性；重复事件只返回已有结果。
...
评估意见形成后，系统使用最终风险、所需能力、仓库范围、预计投入和交付约束再次解析并校验策略：

- 仍命中原策略时，将其作为迭代规划候选策略。
- 命中更严格策略时，自动升级到更严格策略，并重新校验缺失岗位、预算和人工门禁。
- 多个策略冲突，或策略变化涉及扩大权限、提高自动化程度、改变部署终点时，必须交给真人确认。
...
| `actor_selector` | 指定真人、AI 数字员工、团队或候选池，以及真人优先、AI 优先或按容量选择规则 |
...
`actor_mode=human` 时执行器为空，策略通过 `actor_selector` 从指定用户、团队或具备对应岗位资格的候选池选人；`actor_mode=hybrid` 时必须明确选择顺序。最终席位分配写入协同运行快照，不能因账号、岗位或策略配置变化而丢失历史解释。
...
- 不使用产品级 Feature Flag 维持两套研发流程；数据库、API、前端和工作流在同一发布窗口切换到新规则。
- 升级前暂停创建和启动新的研发任务，完成数据库备份并等待正在写状态的事务结束。
- 迁移预检必须生成活动任务、策略归并冲突、缺失岗位、失效 Runner、仓库和权限引用报告。处于 `running/waiting_more_info/waiting_review/writing_back` 或存在活动 Agent Loop、Runner 任务的研发任务数量必须为零；未归零时终止升级，不迁移运行中任务。
- 按业务大脑、产品和仓库范围整理现有策略，把每条旧任务类型执行配置转换为统一策略下的岗位执行器绑定；无法唯一归并或缺少必需岗位的策略标记为 `invalid`，补齐前不能启动研发。
- 为迁移后的策略写入安全默认值：`delivery_target=ready_for_release`、部署禁用、人工审核、预算上限和独立质量门禁。
- 迁移按“扩展结构、转换数据、完整校验、切换应用、删除旧字段”执行，但运行时只在切换点启用一套规则。完整校验失败时必须在删除旧字段前回滚数据迁移并保持系统暂停状态。
- 校验主策略、岗位绑定、Runner、仓库、工作区和权限引用完整后，删除旧顶层执行器字段并切换新版 API 与页面。
- 已完成任务和历史运行不重算，其冻结快照只用于审计。升级后不再读取旧策略配置。
- 现有 `draft` AI 任务统一标记为 `cancelled/superseded_by_v2_migration` 并写入审计，不允许升级后直接启动；其来源需求重新进入正式评估。历史 `completed/failed/cancelled` 任务保持只读，失败任务如需继续处理必须从来源需求创建新协同工作项。
