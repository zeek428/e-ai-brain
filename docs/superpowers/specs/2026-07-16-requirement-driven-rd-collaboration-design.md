...
保留现有研发执行策略的产品概念、菜单、权限、API 根路径和策略身份，直接扩展 `rd_task_executor_policies`。升级后的每条策略都使用同一种模型：主记录负责匹配范围、版本目标、需求评估、自动化边界、团队编制、质量门禁、Git 和部署规则；岗位绑定、策略版本和快照子表负责岗位到执行器、Runner、模板和预算，不再区分配置模式。

现有记录的 ID、名称和历史引用继续保留，原 `task_type/executor/runner/指令模板` 配置迁移为该策略的第一条岗位执行器绑定。后续可在同一策略中继续增加产品、技术、开发、测试、文档和运维岗位绑定。历史任务读取执行时已经冻结的旧策略快照；所有新任务和版本统一解析升级后的策略。
...
产品如果没有命中可用研发执行策略，或者策略缺少当前工作项所需的岗位执行器绑定，必须阻断组版确认或协同启动并提示配置，不得静默回退到策略外的模型网关或执行器。
...
现有表是唯一研发执行策略主表，不新增模式字段或并列主策略。原有记录原位补齐版本目标、需求评估、组版、团队、Git 和部署默认配置，并把当前任务类型、执行器、Runner、工作区、指令模板和输出契约复制为第一条岗位绑定。原记录 ID 和历史引用不变。

管理入口继续使用现有菜单、`delivery.rd_executor_policies.manage` 权限和 `/api/delivery/rd-task-executor-policies` API 根路径。API 在兼容旧字段读取的基础上增加需求评估、组版、生命周期、岗位绑定、Git 和部署配置；旧顶层执行器字段仅作为迁移期默认岗位绑定的兼容投影，不形成另一套命中逻辑。前端直接展示升级后的统一策略，不进行模式分组。
...
- 现有研发执行器策略记录原位补齐统一配置，并把原执行器字段迁移为默认岗位绑定；历史任务继续读取原策略快照，不能静默改变历史任务解释。
