...
- 组织：`rd_role_definitions`、`rd_ai_employees`、`rd_executor_profiles`、`rd_task_executor_policy_role_bindings`。
...
公开的需求批量状态推进、AI 任务创建、启动和批量重试接口不再是 v2 交付入口。`batch-advance-status` 对 `planned/designing/ready_for_dev/developing/code_reviewing/testing/ready_for_release/deploying/released/accepted` 等交付状态统一返回 `RD_COLLABORATION_REQUIRED`；它只保留在既无活动协作运行、也不存在非终态工作项时批量 `cancelled/closed` 的管理用途。外部调用 `POST /api/ai-tasks`、`/api/ai-tasks/{id}/start` 或 `/api/ai-tasks/batch-retry` 同样返回 `RD_COLLABORATION_REQUIRED`；协作编排器直接调用内部领域服务创建、派发和恢复关联工作项的 AI 任务，不通过真人角色或公开 HTTP 绕行。
...
27. 需求批量状态接口不能推进任何研发交付状态，公开 AI 任务创建/启动/批量重试接口不能由真人或外部客户端绕过协作运行。
