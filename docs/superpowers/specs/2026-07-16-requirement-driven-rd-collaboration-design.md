...
### 方案 A：把全部新配置直接塞入现有策略单表

直接向 `rd_task_executor_policies` 增加岗位、组版、生命周期和部署 JSON。改动表面较小，但会把主策略、多个岗位和单任务 Runner 参数混在一行，校验、版本化和历史解释会越来越困难。

### 方案 B：原位升级现有研发执行策略，并增加子配置（推荐）

保留现有研发执行策略的产品概念、菜单、权限、API 根路径和策略身份，把 `rd_task_executor_policies` 升级为同时支持 `task_compat` 与 `team_delivery` 两种模式。主记录负责匹配范围、版本目标、需求评估、自动化边界、团队编制、质量门禁、Git 和部署规则；新增岗位绑定、策略版本和快照子表，负责岗位到执行器、Runner、模板和预算的映射。

该方案对用户来说仍然只有一个“研发执行策略”，同时能够对整个版本冻结策略快照，并兼容当前 Runner、自治循环和历史单任务策略。

### 方案 C：另建一套版本级研发策略

保留现有研发执行器策略，再新增并列的版本级策略入口。物理边界清楚，但用户需要理解两套命中逻辑，容易出现同一任务同时命中或配置不一致，因此不采用。版本需要差异时，应在升级后的原策略上做受控覆盖和版本快照。
...
现有 `rd_task_executor_policies` 原位升级，建议增加 `policy_mode=task_compat|team_delivery` 和以下版本级配置：
...
`task_compat` 保持当前按任务类型、产品和优先级选择执行器的行为；`team_delivery` 用于 2.0 需求评估、组版和版本协同。策略匹配优先级为：版本显式策略 > 产品专属策略 > 业务大脑默认策略。优先级相同且同时命中时不得随机选择，应阻断并提示配置冲突。
...
现有表是唯一研发执行策略主表，不新增并列主策略。原有记录迁移为 `policy_mode=task_compat`，字段和命中行为保持兼容；2.0 团队策略使用 `policy_mode=team_delivery`，通过岗位绑定子表配置不同岗位的执行器。启用 2.0 的版本只匹配 `team_delivery`，旧单任务入口只匹配 `task_compat`，不能跨模式竞争。

管理入口继续使用现有菜单、`delivery.rd_executor_policies.manage` 权限和 `/api/delivery/rd-task-executor-policies` API 根路径。API 在兼容旧字段的基础上增加模式、需求评估、组版、生命周期、团队、Git 和部署配置；前端在同一页面按“单任务兼容策略 / 团队研发策略”分组展示，不创建第二个策略菜单。
...
原位升级当前单任务执行器配置，使同一策略管理入口同时支持兼容单任务策略和版本级团队策略。团队策略包含需求评估、组版、团队编制、岗位执行器、预算、质量门禁、Git 和部署，并提供命中预览、岗位资源可用性检查、冲突提示和版本影响预览。
...
- 现有研发执行器策略记录迁移为 `task_compat` 后保持可读和可用；新增 `team_delivery` 模式和子配置，但不新增并列策略产品，不能静默改变历史任务解释。
