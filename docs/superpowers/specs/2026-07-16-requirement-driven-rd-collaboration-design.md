...
- 现有 `graph_runs/graph_checkpoints` 抽象扩展为可关联 `ai_task` 或 `rd_collaboration_run`，旧任务关联方式保持兼容。
...
16. 版本级协同图和单工作项执行图均运行在原 LangGraph 工作流引擎上，能够通过持久化 Checkpoint 在事件或人工决策后恢复；定时作业引擎不参与该流程。
