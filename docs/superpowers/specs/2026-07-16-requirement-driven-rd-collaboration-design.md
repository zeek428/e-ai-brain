...
- `rd_run_seats`：本次运行的岗位席位、真人用户或 AI 数字员工、执行器和容量快照；AI 席位分别保存员工 ID、执行器 ID 和大于零的冻结 `capacity`。
...
只有依赖已通过、员工可用、权限有效、预算充足、工作区准备完成且无资源冲突的工作项才能进入 `ready`。P1 的实现工作项必须在输入契约中声明仓库 ID、相对路径和 `read/write` 资源意图；平台检测同仓库重叠路径的无序 write/write 声明，按稳定优先级新增 `finish_to_start` 依赖并保存冲突证据。AI 工作项由调度器以席位锁和运行中工作项计数原子领取；已达到冻结容量时保持 `ready` 并返回可重试的容量延后，不创建临时任务或 attempt。真人工作项进入个人待办，可接受、转交或拒绝。

自动派发的候选 reservation 只表示扫描时可尝试，不授予跨事务执行权。最终 bundle 事务先以不加锁查询取得父运行 ID，再统一按 `rd_collaboration_runs FOR UPDATE -> rd_work_items FOR UPDATE` 加锁；两把锁取得后重新校验工作项归属、父运行活动状态、工作项状态、version 与 due，再继续席位、安全审批和 bundle 写入。cancel、运行 suspend 与最终派发使用相同的父聚合优先锁序，因此并发竞争不能形成 SQLSTATE `40P01`；锁后发现 `waiting_human`、终态、取消或陈旧 reservation 时整体无副作用退出。

并行编码使用独立 worktree 和分支。多个工作项触碰同仓库同一路径或父子路径的 write 声明时，调度器确定性串行化；无法通过依赖安全表达的冲突才创建显式集成工作项或人工决策。集成人负责合并和冲突处理，普通开发 Agent 不直接覆盖其他工作项成果。
...
- P1 已覆盖以显式仓库/路径资源声明为依据的同仓库 write/write 冲突预测、确定性依赖串行化，以及按运行冻结 AI 席位容量的原子派发和容量延后；版本总览展示可用/已用容量和已串行化冲突数。
- 跨仓库语义冲突预测、基于历史吞吐的容量优化、团队日历和复杂失败恢复仍为后续增强。
...
- 全新与存量 PostgreSQL volume 使用同一迁移控制面：PostgreSQL initdb 不挂载或打包应用 SQL，API image 打包 `/app/app/db/migrations` 并由 entrypoint 执行普通 additive migrations。新应用在旧字段仍存在但不再读取的状态下启动；健康检查、策略解析、评估和只读页面验证通过后记录健康标记，再由显式 cutover cleanup 命令以单事务执行迁移 121 清理旧研发命令记录。破坏性清理不接入启动时自动 additive migration runner。普通 API 启动还必须跳过派发大索引迁移 125-128，由非测试 API repository compatibility 初始化在 autocommit 连接上使用共享的非阻塞 advisory lock、catalog valid/ready 校验和 concurrent index DDL 创建或修复；并发实例未获锁即继续启动，后续启动再次校验。清理或索引创建失败不回退业务规则；cleanup 失败保持维护状态并允许修复后重试，不能重新开放旧写路径。
