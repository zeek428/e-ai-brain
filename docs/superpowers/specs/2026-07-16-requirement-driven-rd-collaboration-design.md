...
- 现有 `graph_runs/graph_checkpoints` 抽象扩展为可关联 `ai_task` 或 `rd_collaboration_run`，已有任务运行记录继续可查询。
...
- 已完成任务和历史运行不重算；其冻结快照只用于审计。升级时已在运行的任务完成当前原子步骤后暂停，由迁移程序关联到工作项并在新策略快照下恢复，不能继续读取旧策略配置。
