...
与产品缺陷修复分离：缺陷修复继续留在当前开发分支；测试执行只能在独立的远程测试分支 `e2e/rd-collaboration/<run-id>` 及其 Runner 隔离 worktree 中写入。
...
3. 测试仓库已绑定统一研发执行策略，并允许专用 Git 写回身份只对 `e2e/rd-collaboration/*` 推送。
