...
- 告警中心会把异常检查项、产品接入低分和钉钉授权到期统一列出来，并标明建议处理人、处理状态、最近出现时间和跳转入口；管理员可直接点击“处理”认领、标记处理中、关闭或忽略告警，并填写关闭原因和复盘记录。处理弹窗会展示最近状态流转记录，便于复盘谁在什么时候把告警从打开推进到处理中或关闭。
- 告警中心会展示启用规则数、规则摘要和历史趋势；管理员可在页面点击“新增规则”配置 `source`、`component`、最低严重级别、负责人和规则条件，也可以直接启用或停用已有规则。
- 点击“新增订阅”可以配置告警通知渠道、通知目标、最低严重级别和订阅范围，用于把打开告警推送给负责人或运维组；已有订阅会在告警中心直接展示渠道、目标、级别和范围，可从页面启用或停用。系统会先按订阅生成幂等的待投递通知记录，管理员可点击“投递通知”立即处理 pending 通知；失败通知会显示尝试次数和失败原因，可点击“重试失败”再次投递。站内通知会直接标记送达，邮件通知使用系统设置中的 SMTP 配置，钉钉和 webhook 通知需要订阅目标为 `http(s)` 回调地址。
...
- 产品接入完整度评分按产品主数据、迭代版本、模块、代码仓库、可检索知识文档、关联系统、插件连接、产品权限范围和最近健康检查计算；产品卡片会展示分项得分、当前证据和健康摘要，插件测试失败、知识索引失败、权限范围缺失会直接反映到分项状态和缺失项里。
- 权限诊断增强区汇总菜单权限缺口、高风险角色、数据范围缺失、保存角色前风险预检、自动修复建议和 read/write/admin scope 对比。点击“用户视角预览”可输入用户 ID，直接查看该用户真实可见入口、被阻断入口、缺少权限点和 scope 摘要，方便从“菜单能看但接口拒绝”的问题继续排查。
...
- 帮助截图与数据归档策略区展示帮助截图覆盖情况，审计、执行链路、模型日志、作业运行、知识导入等保留天数，以及超过保留期的数据归档候选；对象存储同步清理会提示知识附件孤儿引用、对象信息不完整、可删除对象、仅清理引用和阻断复核数量。点击“预检并清理”会先生成 dry-run 计划，二次确认后才删除孤儿对象并清理对应知识资产引用，失败对象会保留记录供人工复核。
- 安全审计治理区展示敏感配置变更数量、高风险操作数量、审计导出入口、管理员周报摘要、密钥引用校验和直接密钥配置数量；系统会按异常密钥引用、直接密钥配置、高风险操作和审计留档生成治理动作，页面只展示状态、路径和处理建议，不展示密钥值。
