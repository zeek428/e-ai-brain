...
- 告警中心会把异常检查项、产品接入低分和钉钉授权到期统一列出来，并标明建议处理人、处理状态、最近出现时间和跳转入口；管理员可通过 API 认领、处理中、关闭或忽略告警，关闭时必须填写原因。
- 告警中心会展示启用规则数和规则摘要；管理员可通过告警规则 API 配置 `source`、`component`、最低严重级别、负责人和规则条件，系统健康物化告警时会记录命中的规则 ID。
- AI 任务执行运维台展示 Runner 可用数、排队任务、运行中任务、失败/死信/超时任务、待审批数、队列压力和失败原因分布，并说明重试、取消、超时扫描和 dead-letter 策略。
- 知识中心质量闭环展示文档可检索率、索引失败、待审核沉淀、无结果率、引用点击率、有用/无用反馈和 RAG 引用准确率 proxy；低质量或长期未更新文档会进入待关注列表。
- 产品接入完整度评分按产品主数据、迭代版本、模块、代码仓库、知识文档、关联系统、插件连接和产品权限范围计算，低分产品会展示缺失项。
- 权限诊断增强区汇总菜单权限缺口、高风险角色、数据范围缺失、保存角色前风险预检、自动修复建议和 read/write/admin scope 对比，方便从“菜单能看但接口拒绝”的问题继续排查。
- 钉钉授权生命周期区展示钉钉登录配置、用户绑定、MCP 连接失败、URL Key 到期提醒，以及个人授权、系统授权、应用授权的边界说明和连接授权主体。
- 帮助截图与数据归档策略区展示帮助截图覆盖情况，以及审计、执行链路、模型日志、作业运行、知识导入等保留天数。
...
- 帮助中心维护可运行 `node scripts/check_help_center_assets.mjs` 检查帮助路由、截图文件和截图过期状态；需要刷新截图时可在安装 Playwright 后运行 `node scripts/capture_help_screenshots.mjs`。

排查建议：

- 优先处理红色异常，再处理橙色或黄色待完善项。
- 产品接入分数低时，先补版本、模块、代码仓库和产品知识文档，再复检系统健康。
