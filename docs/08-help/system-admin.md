...
- 平台治理运维台把运维重点拆成告警中心、AI 任务执行、知识质量、产品接入、权限诊断、钉钉授权和帮助/归档策略七块。
- 告警中心会把异常检查项、产品接入低分和钉钉授权到期统一列出来，并标明建议处理人和跳转入口。
- AI 任务执行运维台展示 Runner 可用数、排队任务、运行中任务、失败/死信/超时任务、待审批数和队列压力。
- 知识中心质量闭环展示文档可检索率、索引失败、待审核沉淀和质量门禁；无结果率、RAG 引用准确性等指标未接入时会明确显示为待埋点，不使用假数据。
- 产品接入完整度评分按产品主数据、迭代版本、模块、代码仓库、知识文档和关联系统计算，低分产品会展示缺失项。
- 权限诊断增强区汇总菜单权限缺口、高风险角色和数据范围缺失，方便从“菜单能看但接口拒绝”的问题继续排查。
- 钉钉授权生命周期区展示钉钉登录配置、用户绑定、MCP 连接失败和 URL Key 到期提醒。
