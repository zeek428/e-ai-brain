...
任务详情中优先查看创建时间、更新时间、输出摘要、确认点、执行诊断，以及“自治循环”“质量门禁”“执行上下文”页签。时间字段仅展示后端任务记录中的实际值；历史记录缺少时间时显示 `-`，不会由页面补造时间：
...
- 需求带有验收标准时，应先维护验收计划并把标准映射到测试用例。若用例可由独立 Runner 自动验证，可配置“文件必须包含文本”：只能填写工作区相对路径和一个或多个必含文本，不能填写绝对路径或 `..`。计划激活后，用例和配置会冻结；质量门禁只接受本次冻结配置、当前门禁和当前编码 Commit 完全匹配的签名验证证据，不会使用后来替换的计划、历史门禁结果或未提交文件，也不会由脚本预先写入“通过”。验收证据、门禁结论、检查项和审计会一起保存，重复回调不会额外创建同一门禁用例的证据。每次验收运行会关联 Commit、制品和验证任务；同一 Commit 与输入出现通过/失败矛盾时，系统标记 Flaky 并转人工处理。
...
当任务状态为“待确认”且详情显示“确认编号”时，具备评审权限的用户可直接在详情的“人工评审确认”区域点击“处理待确认”。系统会只加载当前任务的评审项；核对输出后可选择“确认通过”“修改后通过”“拒绝并丢弃”或“要求补充”。该入口只进入既有评审状态机，不会绕过协作工作项，也不会触发部署。

人工确认模式下，确认通过后才会请求合入隔离工作区；自动提交模式也必须先通过平台独立门禁，迁移、高风险或受保护路径仍转人工确认。拒绝或普通执行失败会丢弃隔离工作区；工作项取消和执行超时会保留隔离工作区及补丁证据，供人工诊断或返工恢复使用，且不会回退已经存在于主工作区的用户改动。
...
在“任务中心 / 插件管理 / 执行器”配置本地 Codex 或验证 Runner 时，先设置它的信任域和唯一的“信任边界 ID”。编码 Runner 与验证 Runner 必须使用不同边界；不要把部署 Runner 用作研发验证 Runner。

![可信 Runner 配置表单](assets/screenshots/plugin-runner-trust-config.png)

1. 新建 Runner 时将“可信证明状态”保留为“待激活”，下载对应系统的安装包。
2. 在 Runner 机器按安装包 README 安装 `runner_requirements.txt`，配置一次性 Runner Token 后启动。首次启动会在本机生成 Ed25519 私钥，并用 Runner Token 注册公钥。
3. 返回执行器表，确认 Runner 在线、信任边界正确且已出现公钥指纹；私钥文件 `attestation_key.json` 只能保留在 Runner 本机，不能上传、提交或分享。
4. 管理员核对指纹后编辑 Runner，把“可信证明状态”改为“已激活”。没有已注册公钥或信任边界时平台会拒绝激活。

使用 Codex 的本地 Runner 需要保留安装包生成的 `exec --disable code_mode_host --ephemeral` 命令参数。它确保 Runner 只接收本次冻结工作项的输入，不继承桌面端正在进行的对话，也不会留下可被下一次任务续接的本地会话。

研发工作项的指令会显示冻结的目标、输入/输出契约与验收标准；若缺失这些内容，执行器会停止并要求修复配置，而不会猜测实现范围。验证 Runner 会在同一仓库名下的 `.ai-brain-worktrees` 隔离目录执行质量门禁；升级已有安装包后请保留这一受限目录授权，不能把工作区白名单放宽为任意父目录。Codex 在 linked worktree 中需要向安装包附加的工作树 Git 元数据目录写入 commit lock；安装包会用 Git 解析共享元数据目录，因此基础工作区本身是 linked worktree 时也可正确授权，但不会通过放宽到任意父目录来解决提交失败。

平台会为 Runner 的每次任务完成回写附加签名证明。自动合入仍只接受不同信任边界、已激活的验证 Runner 证明；缺少证明时协作会转人工确认，不会伪造验证结果。

远程推送完成后，平台仍需等待 GitHub/GitLab 的签名回调，才会把本地 Commit 标记为已对账并推进到“待发布”。同一个远程仓库同时配置在多个产品下时，系统按本次工作项冻结的交付记录绑定产品和仓库，不会使用列表中第一条同地址配置。若回调到达时配置错误，该条事件会保留为失败审计；修复配置后需要由 Provider 产生新的回调，历史事件不会被修改。默认研发策略到“待发布”即结束，不会自动部署。
...
![迭代版本总览与待发布协同状态](assets/screenshots/help-versions.png)
...
4. 提交结果后由独立审核人通过、拒绝或要求返工。返工会保留原 attempt 与证据，并生成下一次领取机会；当前有效 attempt 的普通编码失败会自动释放租约、保留错误和 Runner 证据，并转入“返工待重试”。Runner 超时则按本次冻结策略的 `max_iterations` 计数：未达到上限可自动重试；达到上限时工作项进入“待人工决策”，不再后台循环创建 attempt。评审人可选择“人工确认后重试”（恢复为就绪，Worker 才会创建新 attempt）或取消；如需增加超时时间，应调整统一策略并在新的协作代次中冻结，不能篡改当前运行的策略快照。如果已因故障处理被取消，先确认所有旧 Runner 已停止、修复治理或配置问题，再在“工作项 DAG”对应卡片选择“从取消恢复”；系统保留旧 attempt、任务和隔离工作区证据，只为后续派发创建新的 attempt，不会复活旧租约。
