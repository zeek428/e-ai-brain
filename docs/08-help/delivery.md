...
- 低风险工作可在策略约束内由 AI 自主推进；高风险、范围变更、策略冲突、预算超限和质量门禁失败必须生成给人类的决策。
...
1. 在“需求交付 / 研发协同”选择版本并启动协作。页面会显示冻结的策略快照。
2. 在“工作项 DAG”查看负责人、依赖、优先级、风险和状态；前置依赖满足后，不同岗位的工作可并行领取。
3. 提交结果后由独立审核人通过、拒绝或要求返工。返工会保留原 attempt 与证据，并生成下一次领取机会。
4. 高风险取消、范围变化、策略冲突和超权限问题会进入“人工决策”。只能从冻结选项中作答，不能通过任务中心的批量操作绕过协作状态机。
5. 协作完成后检查远程 Git、集成测试和可信交付证据，终点为 `ready_for_release`。不会部署。
