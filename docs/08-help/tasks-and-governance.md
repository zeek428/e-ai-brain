...
研发协同 2.0 不改变已有定时作业的定义、调度、Agent/Skill 快照或运行语义。定时作业仍用于洞察、巡检、通知和运营自动化；需求驱动的研发交付从“需求管理 → 研发评估 → 版本 → 研发协作”进入，不通过定时作业创建或推进协作工作项。
...
2. 点击手动触发或复跑后，平台会先创建并持久化运行记录，再在后台执行作业；页面会自动切到运行记录并置顶展示本次记录，可立即查看运行中、排队、成功或失败状态。
