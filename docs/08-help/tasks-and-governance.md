...
钉钉官方 MCP 连接使用 URL Key 授权。新增钉钉文档、知识库、钉盘、AI 表格、机器人或通讯录连接时，需要填写“查询参数名”和“URL Key / 密钥引用”；查询参数名通常为 `key`，URL Key 可以填写钉钉 MCP 授权 URL 中的 key 值，也可以填写 `vault/...` 或 `env:...` 这类密钥引用。
