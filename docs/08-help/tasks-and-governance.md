...
需要把 AI 处理结果新增到钉钉 AI 表格时，先在插件管理配置“钉钉 AI 表格”连接，在连接的“钉钉 AI 表格范围”里填写一次 Base ID。然后新增动作选择“钉钉 AI 表格 - 新增记录”，并把“结果写入目标”切换为“钉钉表格”；动作页面会从所选连接自动带出 Base ID，用户只需填写 Table ID 和新增记录内容。新增记录内容支持直接填写 JSON 数组，例如 `[{"cells":{"字段ID":"值"}}]`，也可以使用 `{{result_json}}`、`{{records}}` 等任务结果变量；系统会在运行时转换为钉钉 MCP `create_records` 工具的 `baseId/tableId/records` 参数。旧动作中保存的 Base ID 仍可兼容读取，但新建或维护动作应以连接中的 Base ID 为准。
