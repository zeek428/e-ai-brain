...
需要把用户声音自动沉淀为需求时，新增作业后选择“用户洞察需求机会挖掘”模板。模板会自动带入“内部业务数据连接”和“读取内部业务数据”，源数据默认选择“用户洞察数据”和“需求数据”，并配置 AI 分析与“创建需求”结果动作。内部业务数据连接由平台提供，无需到插件管理另行创建或填写密钥；请选择目标产品后再保存，作业运行时只会读取该产品范围内的用户洞察，并把模型筛选出的可验证改进机会写入该产品需求池。
