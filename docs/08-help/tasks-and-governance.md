...
“AI 执行器定时指令”用于按计划向系统默认 AI 模型，或已配置的本地 Codex、Claude、Hermes、OpenClaw Runner 下达任务指令并回写结果。该模板不预设代码仓库；需要周期性扫描代码质量、安全或规范问题时，请选择“代码仓库质量 / 安全 / 规范巡检”模板。

常用操作：

1. 在作业列表查看启停状态、上次执行时间、下次运行时间和产品归属。上次执行时间为空表示该作业尚未产生运行记录。
2. 点击某行“运行记录”可直接打开该作业的已筛选运行记录；如需恢复查看所有作业，点击筛选提示旁的“查看全部”。
3. 点击手动触发或复跑后，平台会先创建并持久化运行记录，再在后台执行作业；页面会自动切到运行记录并置顶展示本次记录，可立即查看运行中、排队、成功或失败状态。
4. 失败时查看错误摘要、trace_id 和关联审计记录。

Cron 和固定间隔作业由后端定时作业 Worker 扫描并触发。打开作业配置页只会读取当前状态，不会推进“下次运行”或跳过已经到期的作业；到期执行会生成触发方式为“定时调度”的运行记录。若某个作业在计划时间后仍没有运行记录，请先确认服务端已启用定时作业 Worker，再在该作业的“运行记录”中查看错误详情或使用“运行”手动补跑。

![作业配置运行记录入口](assets/screenshots/help-scheduled-job-run-entry.png)
...
“每周 AI 客服洞察抽取”或“每周用户反馈洞察抽取”的“数据读取动作”只用于读取客服与反馈数据，不会显示会修改外部内容的“钉钉文档 - 更新内容”。需要把洞察同步到钉钉时，在“结果动作”点击“新增钉钉文档更新”，选择已配置的“钉钉文档 - 更新内容”动作，填写文档链接或 ID，并选择追加或覆盖。系统先完成 AI 分析并生成 `dingtalk_markdown`，再执行文档写入，避免用未生成的内容调用外部写操作。
...
运行后，详情顶部的“结果动作执行情况”会直接展示每个动作的成功、失败或未执行状态。钉钉文档动作还会显示文档 ID、追加/覆盖方式和插件调用日志；失败时显示错误原因。对于历史运行，如果配置快照里存在钉钉动作但没有调用日志，会明确标记为“未执行”，不会把作业整体成功误认为钉钉文档已更新。执行中的作业可在“运行结果详情”标题栏点击“刷新”，重新读取当前运行状态、日志和结果动作写入记录。随后可按三段继续查看数据来源返回内容、AI 执行处理后的 `dingtalk_markdown` 和完整写入反馈；如果配置了创建需求动作，还会展示需求写入结果。

![定时作业运行详情刷新](assets/screenshots/help-scheduled-job-run-detail.png)

“创建需求”结果动作只能把候选需求写入定时作业所属产品，AI 输出中的其他产品编号不会被接受。同一次运行因 Runner 重试或重复回调再次执行结果动作时，系统会复用已经创建的需求，避免重复登记。

需要把用户声音自动沉淀为需求时，新增作业后选择“用户洞察需求机会挖掘”模板。模板会自动带入“内部业务数据连接”和“读取内部业务数据”，源数据默认选择“用户洞察数据”，开始时间和结束时间从下拉列表选择系统时间变量；AI 角色默认是 `product`，Skill 默认是“用户洞察内容转需求”，结果动作只保留“创建需求”。内部业务数据连接由平台提供，无需到插件管理另行创建或填写密钥；请选择目标产品后再保存，作业运行时只会读取该产品范围内的用户洞察，并把模型筛选出的可验证改进机会写入该产品需求池，需求优先级由 AI 判断。

![定时作业选择内部业务数据源](assets/screenshots/help-scheduled-jobs.png)
...
HTTP 连接负责维护可复用的接口地址、请求方法、Params、Headers 和高级请求 JSON。新增或编辑 HTTP 连接时，在“请求地址（Endpoint URL）”填写直接数据源地址，在“请求方法”选择 GET、POST、PUT、PATCH 或 DELETE；POST、PUT、PATCH、DELETE 的请求体可在高级请求 JSON 中的 `body` 配置。连接测试会使用这套方法、参数、请求头和请求体发起请求。

动作负责定义业务场景、结果写入目标和结果映射。选择 GitHub、GitLab、邮箱或钉钉等标准场景后，系统会显示“执行方式”，并自动使用场景模板的固定请求路径、工具名和必要参数，不再让用户重复编辑请求方法、路径、Params 或 Headers。只有不选择配置场景的自定义动作，才在高级配置中开放“调用方式”和 HTTP 请求字段；自定义动作用于确有独立外部接口的扩展场景。

定时作业中的“数据读取动作”只负责从 MaxCompute、钉钉或其他已授权连接读取原始数据；“结果动作”才负责 AI 处理后的额外输出。用户反馈洞察作业会自动写入用户洞察，通常无需增加结果动作；只有需要同步钉钉文档或发送通知时，才在结果动作中新增对应输出。代码巡检报告、自动创建 Bug 等结果动作只适用于代码仓库巡检，不会用于用户反馈洞察作业。

当新增动作的“配置场景”选择“写入内部业务数据”时，不需要也不能选择外部连接；具体写入哪张内部业务表由“结果写入目标”决定。该场景只提供“用户洞察表”“Bug 管理”和“代码巡检报告”三个内部目标，切换目标后系统会带入相应的结果映射字段；钉钉、邮件和仅保存运行结果等非内部目标请使用对应动作场景。创建定时作业时，还需要在“动作配置”中选择与目标匹配的结果动作，作业完成数据连接和 AI 分析后才会执行写入；用户反馈洞察抽取作业本身已经自动写入用户洞察，不需要重复添加该结果动作。

![写入内部业务数据动作](assets/screenshots/help-plugin-internal-write-action.png)

新增“内部数据源”连接时，先选择要读取的业务数据源、字段模式和时间范围；时间输入框右侧的变量图标可插入系统变量。需要限定产品时，在“产品”下拉中搜索并选择当前有权限的产品，系统保存对应产品 ID；无需也不能手工填写产品 ID。未选择产品时按当前数据源和产品权限范围读取数据。
...
需要把 AI 处理结果新增到钉钉 AI 表格时，先在插件管理配置“钉钉 AI 表格”连接，在连接的“钉钉 AI 表格范围”里填写一次表格链接或 Base ID。可直接粘贴形如 `https://alidocs.dingtalk.com/i/nodes/<Base ID>` 的钉钉 AI 表格地址，系统会立即提取并保存 `/i/nodes/` 后的 Base ID。然后新增动作选择“钉钉 AI 表格 - 新增记录”，并把“结果写入目标”切换为“钉钉表格”；动作页面会从所选连接自动带出 Base ID，用户只需填写 Table ID 和新增记录内容。若历史连接尚未配置 Base ID，动作页面允许先补填表格链接或 Base ID 并作为本动作兜底参数，建议随后回到连接配置中统一保存。新增记录内容支持直接填写 JSON 数组，例如 `[{"cells":{"字段ID":"值"}}]`，也可以使用 `{{result_json}}`、`{{records}}` 等任务结果变量；系统会在运行时转换为钉钉 MCP `create_records` 工具的 `baseId/tableId/records` 参数。旧动作中保存的完整表格链接仍可兼容读取，但新建或维护动作应以连接中的 Base ID 为准。
...
Jenkins 方案填写连接和 Job 后可直接执行预检。预检会使用配置连接请求 Jenkins API，并检查目标 Job 可读取；不展示或保存凭据明文。自动部署单的操作按钮为“探测并启动”：Runner 方案会展示探测任务的排队或运行状态、剩余等待和失败原因，可打开“查看 Runner 日志”；超时、失败、取消或目标配置变化后可以安全地重新探测，进行中的任务不会重复派发。Jenkins 方案会立即执行 API/Job 预检，成功后才自动继续启动。没有通过时，部署单保持待运维执行。

发布候选的集成验收由既有的非生产 Jenkins 验收 Job 执行。平台会创建部署记录、进行预检并触发、同步该 Job，但不会在 CI 中新建、重配或销毁服务器环境。容器化 SSH、Docker、Jenkins 环境只用于研发侧协议回归，不作为发布候选的替代门禁。
...
用户洞察用于沉淀用户反馈、行为指标和迭代建议。负向反馈可以进入治理视图，并关联到迭代建议、需求或 Bug。列表与详情中的“所属产品”优先显示当前可见范围内的产品名称；产品不存在或无权查看时才回退显示产品 ID。
