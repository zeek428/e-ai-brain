...
运行后，详情顶部的“结果动作执行情况”会直接展示每个动作的成功、失败或未执行状态。钉钉文档动作还会显示文档 ID、追加/覆盖方式和插件调用日志；失败时显示错误原因。对于历史运行，如果配置快照里存在钉钉动作但没有调用日志，会明确标记为“未执行”，不会把作业整体成功误认为钉钉文档已更新。随后可按三段继续查看数据来源返回内容、AI 执行处理后的 `dingtalk_markdown` 和完整写入反馈；如果配置了创建需求动作，还会展示需求写入结果。
