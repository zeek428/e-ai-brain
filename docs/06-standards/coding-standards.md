...
- 前端 API 调用必须先复用 `apps/web/src/services/apiClient.ts` 的请求基础设施，登录态、当前用户缓存、退出和 401 跳转必须复用 `apps/web/src/services/authClient.ts`；领域接口再放入对应业务 client。新增列表接口应复用统一远程分页参数和 `ApiRequestError`，不得在页面组件或领域 client 中重复拼装 base URL、认证错误解析和 401 跳转逻辑。
