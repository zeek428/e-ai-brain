...
### Added
- 验收计划用例现支持冻结的 `file_contains` 独立验证配置：仅允许受限工作区相对路径和必含文本；质量门禁在启动时冻结计划、用例指纹和编码 local commit，验证 Runner 从该提交的 Git blob 验证（脏工作树、缺失/超限 blob 均失败），并用 Ed25519 证明绑定完整结果摘要。服务端仅接收当前门禁、冻结指纹与 Commit 均匹配的 `runner://` 证据；替代计划、历史门禁和未提交文件不能解锁在途门禁。验收证据、门禁、检查和审计改为单一事务提交，并以门禁与用例唯一索引/稳定记录 ID 防止重试重复。未映射、无证据或验证失败仍会阻断协作，不会由 E2E 脚本预填通过结果。安装包也通过 Git 解析 linked worktree 的共享元数据目录，避免 Codex 因 `.git/index.lock` 无权限而失败。
...
- 研发协同新增按需 `generate-plan` API：对已冻结的协作运行调用统一模型网关生成工作项 DAG 提议，并仍经服务端确定性校验后持久化；模型不可直接选择执行器、批准风险或推进状态。该交互式入口不修改定时作业定义、调度或运行历史。
- AI 执行器 Runner 新增可信证明闭环：安装包首次启动在本机生成 Ed25519 私钥，以 Runner Token 注册不可替换的公钥，并对每次任务完成回写签名；插件管理页可配置信任边界、查看公钥指纹和待激活/已激活状态。Runner 不能自行激活、换键或修改信任域，管理员核对后才可启用可信证明；不涉及部署或定时作业行为。
- 研发协同 2.0 完成需求驱动交付闭环：需求先经岗位评估与人工决策，再优先归入兼容的规划中版本并启动协作；统一策略支持动态岗位、真人账号、AI 数字员工和受控执行器，协作 DAG 支持并行、审核、返工、范围决策、远程 Git 证据和 `ready_for_release` 收口。P0 不会创建或执行部署，P1 岗位经验沉淀保持独立开关与审核复用边界。同步新增维护围栏、预检、受控排空取消和显式非部署清理契约，定时作业行为不变；帮助中心、真实页面截图和用户手册同步更新。
- 插件动作新增“写入内部业务数据”配置场景：场景名称不再绑定具体业务表，默认选择用户洞察表，用户可在“结果写入目标”切换用户洞察表、Bug 管理或代码巡检报告；该场景不展示钉钉、邮件和仅保存运行结果等非内部目标，具体结果映射随目标自动带入。
- 定时作业新增“用户洞察需求机会挖掘”可执行模板：平台自动提供零配置的“内部业务数据连接”和“读取内部业务数据”动作，模板默认只读取当前产品范围内的用户洞察；开始时间、结束时间从系统变量下拉选择，AI 角色默认 `product`、Skill 默认“用户洞察内容转需求”，结果动作仅保留“创建需求”，需求优先级由 AI 判断。包含“创建需求”的新增、编辑和试运行请求未选择产品时，前后端会明确阻止保存。
- 插件管理新增“钉钉 AI 表格 - 新增记录”官方动作场景和“钉钉表格”结果写入目标：连接配置 Base ID，动作表单配置 Table ID 和新增记录 JSON，运行时自动调用钉钉 MCP `create_records` 并记录新增反馈。
- 钉钉 AI 表格新增记录动作改为从所选钉钉表格连接自动读取 Base ID；Base ID 在连接中配置一次，动作页面只维护 Table ID 和记录内容。
...
- 实现类研发协同工作项必须至少声明一项 `write` 资源意图；仅声明 `read` 的实现计划会在派发前被确定性拒绝，避免未授权的写入范围进入执行环节。
- 研发协同调度扫描的总配额现覆盖派发、延后和人工决策等全部已检查候选项；达到配额后不再继续创建高风险决策，保证单次扫描的工作量有严格上界。
- 协作工作项命中 Runner 高风险指令时，系统会创建专用且冻结的 `runner_safety_approval` 决策；通用 Runner/插件审批入口不能代决。批准快照按工作项、attempt 与续签轮次绑定，最终派发会在同一事务内重新校验指令、安全快照、审批证据和过期时间，篡改或过期均不会产生执行记录。
- 自动派发新增 PostgreSQL 持久化退避治理：可重试故障按 5/10/20 秒延后并只保存安全错误码，第 4 次转人工决策；候选扫描按到期时间、运行轮转和有界依赖页公平继续，预留版本与到期观察会在最终派发中重验，避免并发 Worker 绕过退避或提前清空状态。
- 本轮仅调整后端派发治理与源规格，未改变研发协同或迭代版本页面的路由、可见文案、表单和操作流程；帮助中心现有说明与真实页面一致，因此未更新帮助内容或截图。
- 开发环境浏览器端 API 请求改为同源 `/api`，复用已配置的 Umi 代理连接本地后端；不再让页面绕过代理直连 `127.0.0.1:8000`，避免本地页面因独立端口不可达而显示“Failed to fetch”。
- 版本协同启动后由执行 Worker 自动调用统一模型网关，基于脱敏冻结的需求、策略摘要和岗位席位生成工作项 DAG 建议；平台再确定性校验冻结需求范围、岗位容量、审核分离和依赖无环后激活运行并派发。模型失败或提案不合规不会生成本地兜底计划或改变运行状态，运行保留在 `planning` 等待恢复后重试；模型日志仅保存脱敏调用元数据。
- 研发协同执行 Worker 现按冻结工作项 DAG 自动派发 AI 席位的低/中风险工作；高风险工作项会在同一事务中生成 `high_risk_ai_dispatch` 决策、暂停工作项、记录协同事件和审计，只有指定真人确认后才允许派发，拒绝则取消该项。
- 研发协同 P1 新增确定性并行控制：实现工作项必须声明仓库/相对路径/读写资源意图，平台会把同仓库重叠路径的无序写入冲突持久化并补成稳定依赖；AI 岗位容量随席位冻结，派发使用席位锁与运行中计数原子限制，满载工作项保持就绪等待。迭代版本总览同步展示可用 AI 席位和已串行化资源冲突数。
- 迭代版本总览升级为研发交付总入口：版本驾驶舱新增受权限保护的研发协同摘要，聚合运行、岗位席位、工作项阻塞和待人工决策；可直接启动、继续或重启协同，并将协同工作台收敛为运行后的执行、审核和决策入口，不再允许手工填写版本 ID 或范围版本绕过版本范围与策略冻结。帮助文档同步更新；交付仍默认停在待发布，不新增部署操作。
- 修复研发协同页面的导航入口遗漏：后端菜单资源、PostgreSQL 迁移、管理员默认授权与前端路由一致，管理员登录后可从“需求交付 / 研发协同”进入页面，不再只能通过 URL 直达。
- 研发协同 2.0 进一步收口实现契约：新增受控范围变更命令和 `rd_scope_change_requests` 持久化模型，明确待发布前以人工决策原子终结旧运行、应用类型化 operations、仅递增一次 scope 并显式 restart；待发布后改为带旧需求/运行血缘的后续需求。岗位反馈新增来源事件、规范化 fingerprint 和数据库唯一键，保证 Graph 恢复/事件重放不重复；补齐策略缺失、岗位缺席、执行器不可用、策略人工决策/轮次超限、协作旁路和范围校验错误的 HTTP、details、重试和状态语义，并统一 restart 字段为 `terminal_run_id`。
- 研发协同 2.0 在实现前完成第二轮契约收口：新增不可变运行需求范围表并要求与版本策略来源集合逐条完全一致；允许多需求共用无差异 base/final 快照，不对 source snapshot 误设唯一约束；运行 generation 启动后范围完全冻结，进入待发布前的范围变更必须先终结旧运行并以新代重启，进入待发布/部署后只能进入新 planning 版本；失败或取消运行只能通过带代际和前序引用的新运行重启；人工决策补齐到期升级且保持暂停；交付终点明确区分 P0 待发布完成与 P1 部署续跑；维护围栏改为 `draining/cutover_locked` 两阶段及受控 abort；需求评估禁止任何账号临时覆盖策略；岗位经验反馈分离被归因对象与不可变生产者身份，复用增加冻结配置、保守合并、范围过滤和来源生产者隔离审核，并同步更新 PRD、技术规格、API、测试用例、实施计划、架构说明与术语。
- 定时作业模板“AI 执行器仓库任务”更名为“AI 执行器定时指令”，默认作业名同步改为“AI 执行器定时任务”；模板用于定时下达执行器指令并回写结果，不预设代码仓库，代码扫描使用代码仓库巡检模板。
- 插件管理收敛连接与动作的 HTTP 配置职责：HTTP 连接新增可视化“请求方法”并在连接测试时按 `request_config.method/query/headers/body` 真实调用；标准动作场景仅显示自动判定的“调用方式”，固定请求路径、工具名和参数继续由模板维护；“写入内部业务数据”场景不再选择外部连接或显示请求配置，只维护内部写入目标和结果映射；无场景的自定义动作仍可在高级配置中维护 HTTP/MCP 请求。
- 研发协同 2.0 方案按 Review 收口为实现级契约：统一策略增加 `policy_version` 和 active 唯一规则，禁止版本显式策略覆盖；不可变快照补齐非空、父链/CHECK、无差异复用 base，以及多需求确定性合并的 `version_resolved` 快照和延迟校验来源关系；产品版本增加可追溯 `scope_version`。命令幂等保存脱敏响应快照，claim token 使用到期擦除的独立 replay-secret；工作项取消补齐 Runner Outbox/迟到结果隔离及高风险继续时 ready/new-attempt 语义；决策补齐结构化 input、answers 独立权限和 selector。代码巡检现行目标统一为正式整改需求，历史任务字段只读；P0 基础反馈/协作前端与 P1 经验/部署前端分别验收，经验查询补齐全范围过滤。测试基线不再信任漂移中的旧哈希，须在代码稳定后双次采集且全部套件全绿。
- 帮助中心的页面截图支持点击预览原图，并提供键盘可聚焦的图片入口和关闭弹窗操作。
- 用户洞察列表新增“所属产品”列，详情中的“产品 ID”调整为“所属产品”；两处优先显示当前权限范围内的产品名称，无法解析时才回退显示产品 ID。
- 修复“用户洞察需求机会挖掘”编辑回填遗漏内部数据读取参数的问题；每类最大行数、源数据和时间范围会保留，历史空配置会恢复为用户洞察默认范围并在下次保存时持久化。
- 定时作业“运行结果详情”新增刷新按钮，执行中的作业可按当前运行编号重新获取状态、日志和结果动作写入记录，无需关闭弹窗或刷新整页。
- 用户反馈洞察作业补齐配置的通用结果动作执行：同步钉钉文档会在洞察写入后真实调用“钉钉文档 - 更新内容”插件并记录调用日志。运行详情新增“结果动作执行情况”，直接展示动作成功、失败或未执行状态，以及钉钉文档、写入方式、调用日志和错误原因；历史运行缺少调用证据时明确标记为未执行。
- 定时作业手动触发和复跑改为请求运行记录立即返回：前端传递 `return_immediately=true` 后，服务端在后台启动实际执行，运行记录及采集台账持久化完成即返回并自动切换到“运行记录”页签，避免耗时 AI 作业请求未结束时页面没有可见运行记录。
- 修复用户反馈洞察作业编辑时错误回填代码巡检结果动作的问题；“取数动作”更名为“数据读取动作”，结果区明确洞察自动写入，仅保留钉钉同步、通知等可选输出。
- 内部数据源连接的产品过滤改为按当前产品权限范围提供可搜索产品选择器，保存值继续写入 `request_config.query.product_id`；时间窗口的系统变量选择收纳到字段内变量图标菜单，修复三列动态表单中控件换行和大块留白的问题。
- 周度 AI 客服/用户反馈洞察作业支持将 AI 输出同步到钉钉文档：新增“新增钉钉文档更新”结果动作入口，配置文档链接、追加/覆盖方式和写入模板后，由 `dingtalk_markdown` 驱动写入；取数动作继续只允许读取数据，避免分析前执行外部更新。
...
### Fixed
- 修复钉钉 AI 表格 Base ID 配置仅接受手工截取 ID 的问题：连接和动作兜底字段均支持直接粘贴 `alidocs.dingtalk.com/i/nodes/...` 表格链接，页面立即提取 Base ID，保存、试运行和正式调用前也会统一规范化历史链接配置。
