...
### Added
- 定时作业增加内嵌调度 Worker：应用启动后按 Cron/固定间隔原子领取到期作业，以 `scheduler` 触发方式创建运行记录；作业配置列表新增“上次执行”列和“运行记录”行操作，可直接筛选当前作业的运行历史。
...
- 修复定时作业页面读取会推进 `next_run_at` 并跳过到期作业的问题：列表查询改为只读，由调度 Worker 在实际领取并创建运行记录时推进下次运行时间。
