...
### Added
- 发布候选部署门禁调整为复用既有的非生产 Jenkins 验收 Job：新增强制 CI 工作流和不可跳过的验收测试，只创建平台部署记录并触发、同步既有 Job；隔离 SSH、Docker、Jenkins Compose 环境改为可选协议回归。Runner 连通性探测响应新增任务排队/运行状态、剩余等待、失败分类、安全重试条件和部署范围日志链接，运维部署页同步展示进度、日志与可操作重试。
- 部署真实连通性证据补齐配置指纹和风险策略：Runner 本地目标的主机、目录、Compose 或命令配置变化会立即使旧探测失效；探测有效期按环境、风险等级和方案可选更短配置收紧。Jenkins 新增真实 API/Job 预检与短期证据，部署单支持“探测并启动”。新增隔离 SSH、Docker、Jenkins 外部 E2E 环境、可选自托管 CI 门禁和真实 Provider 回归脚本。
- 部署 Runner 新增平台受控真实连通性探测：SSH 仅执行固定参数连接和 `true`，Docker 仅校验 Engine 与 Compose 配置；心跳和目标上报不再能单独放行部署，成功探测证据 10 分钟后自动过期。Runner 测试入口、执行资源授权、部署方案候选和部署启动门禁同步展示并校验探测状态，SSH/Docker/Jenkins 的接单、日志、超时与回滚端到端回归同步补齐。
- Runner 安装包配置按目标系统推荐 CPU 架构，macOS 默认推荐 Apple 芯片 `arm64`；部署能力 Runner 自动固定到部署信任域，SSH/Docker 方案和候选资源服务端拒绝编码或验证信任域 Runner。部署方案、发起部署和执行资源授权页面新增就绪原因、配置入口与移动端弹窗适配。
- 新增 Worker 运维页面，展示后台事件积压、超时租约、死信、重试、心跳和外部操作对账状态；对账只执行 Provider 查询，不重放原动作。
- 知识中心新增图片检索页签，支持上传或粘贴图片，通过受控多模态 Profile 生成查询向量并显示文档、页码、区域和图片资产预览。
- 可信自治交付治理补齐真实执行闭环：高风险生产部署强制双人复核并受发布冻结控制；外部派发连接中断或超时时转待对账，避免盲目重试。
- 独立验证 Runner 缺失时任务直接转人工确认；Runner 信任域和签名公钥更新会重新校验并生成指纹。
- 验收计划激活后冻结用例范围，预算账本支持 PostgreSQL 读取与结算，视觉检索按文档、知识空间和产品范围过滤。
