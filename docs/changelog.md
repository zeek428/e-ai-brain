...
- 研发协同 2.0 方案按 Review 收口为实现级契约：统一策略增加 `policy_version` 和 active 唯一规则；不可变快照补齐非空、父链/CHECK 约束及“无收紧差异复用 base”；产品版本增加可追溯 `scope_version`；命令幂等保存脱敏响应快照并明确 claim 租约过期语义；新增工作项取消、Runner 取消 Outbox/迟到结果隔离和决策补充信息接口；决策选项冻结 outcome 与主体状态映射。代码巡检现行目标统一为正式整改需求，历史任务字段只读；P0 基础反馈归因/入口隔离与 P1 岗位经验治理分别验收，经验查询补齐业务大脑、产品、岗位、工作项、场景、风险、信任域、置信度和权限过滤。测试基线不再信任漂移中的旧哈希，须在代码稳定后双次采集且定时作业套件保持全绿。
