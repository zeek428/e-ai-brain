...
### Added
- 定时作业新增“用户洞察需求机会挖掘”可执行模板：平台自动提供零配置的“内部业务数据连接”和“读取内部业务数据”动作，可读取当前产品范围内的用户洞察、需求、产品和 Bug；模板默认以用户洞察/需求作为 AI 输入，并将可验证的高价值机会通过“创建需求”写入所选产品需求池。包含“创建需求”的新增、编辑和试运行请求未选择产品时，前后端会明确阻止保存。
