...
- OpenAI-compatible 模型网关，用于 AI 助手、任务执行和知识问答。
- DB-first 运行模式：生产和本地开发默认以 PostgreSQL 为事实源，进程内 MemoryStore 仅用于测试辅助。
...
管理定时作业、AI 能力配置和插件能力。定时作业可以编排采集、巡检、通知、写入报告等动作，并保留运行记录。

### 需求交付

覆盖需求管理、研发任务、研发执行器策略、迭代版本、Bug 管理和全链路追踪。高影响 AI 动作需要人工确认后继续推进。
...
包含用户、角色、菜单、系统设置、邮件发送配置、模型网关、AI 助手快捷任务和 `@` 能力管理。
...
- 知识检索必须在数据库查询层执行权限过滤。
- 模型调用必须经过 `model_gateway`，业务模块不直接调用模型供应商 SDK。
- AI 高影响动作必须保留人工确认点。
