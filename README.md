...
覆盖需求管理、研发任务、研发执行器策略、迭代版本、Bug 管理和全链路追踪。需求和 Bug 可推进为 AI Task，由 Agent 按策略形成“计划、编码、独立验证、失败分析、再次执行”的自治循环；任务详情保留执行上下文清单、每轮证据和停止原因。Runner 返回成功只代表编码阶段结束，自动合入还必须通过平台独立质量门禁，高风险变更仍转人工确认。

### 产品资产

维护产品、版本、模块、Git 仓库、相关系统和知识中心。知识中心支持文档上传、版本化解析、可插拔 OCR/版面/表格处理、多模态内容结构化，以及基于解析文本和向量的引用式检索、知识问答、过期治理和知识沉淀审核。

### 运营治理

包含日志监控、用户洞察、运维部署、审计与运行、执行诊断和代码巡检。运维部署支持人工、SSH、Docker、Jenkins 四种方式，以及部署窗口、预检、灰度/分批/蓝绿发布、健康检查和独立回滚运行；外部副作用通过事务 Outbox 和独立 Worker 派发。

### 系统管理

包含用户、角色、菜单、系统设置、邮件发送配置、模型网关、本地 AI 工具执行器、产品/环境执行资源授权、外部事件运行状态、AI 助手快捷任务和 `@` 能力管理。GitHub/GitLab、Jenkins 与可观测性 Webhook 先验签并幂等写入 Inbox，再由 Worker 投影为质量、发布或运行证据。
...
React + TypeScript + Ant Design Pro
  -> FastAPI modular monolith
     ...
     -> execution worker (Outbox / Inbox / deployment orchestration)
     -> PostgreSQL + pgvector
     -> Redis
...
- Agent 自治循环必须围绕可验证目标推进，每轮计划、修改、独立检查、失败分析和再规划都应可追踪；失败重试、高风险动作和权限边界需要被审计并可转入人工接管。
- 编码 Runner 自报成功不能作为自动合入的唯一证据；自动合入还要求来自独立验证信任域的签名证明、可执行验收计划和通过的质量门禁。
- 验收计划把需求标准、测试用例、提交/制品与验证结果绑定；同一输入和提交出现相反结果会被标记为 Flaky，并阻断自动推进。
- 生产发布支持 maker-checker、发布冻结和外部结果对账；不确定的 Git、Jenkins 或 Runner 回执进入对账或人工处理，绝不直接重复派发。
- Worker 心跳、队列状态和 Agent 预算账本可被集中观测；重复失败指纹会打开自治循环熔断，等待有理由的人工恢复。
- 部署业务状态、审计和外部派发意图在同一事务内写入，Runner、Jenkins 和 Git Provider 调用由带租约、幂等键和执行回执的 Outbox Worker 派发；外部结果不确定时进入对账或人工处理，不盲目重试。
- 知识检索必须在数据库查询层执行权限过滤；视觉检索返回资产、页码和区域引用，且同样受产品和知识权限范围约束。
