...
from app.services import assistant_knowledge_references as knowledge_reference_helpers
from app.services.assistant_action_reference_defaults import (
    ASSISTANT_ACTION_CANDIDATES,
    ASSISTANT_ACTION_QUERY_TRIGGERS,
    ASSISTANT_ACTION_REFERENCE_CONFIG_LIST_NAME,
    ASSISTANT_ACTION_REFERENCE_CONFIG_SORT_FIELDS,
    ASSISTANT_ACTION_REFERENCE_CONFIG_STATUSES,
    ASSISTANT_ACTION_STANDARD_SORT_STEP,
)
from app.services.assistant_reference_formatting import (
    ASSISTANT_ACTION_REFERENCE_TYPE,
...
from app.services.assistant_reference_formatting import (
    assistant_reference_for_entity as _assistant_reference_for_entity,
)
from app.services.assistant_reference_formatting import (
    assistant_reference_matches_query as _assistant_reference_matches_query,
)
from app.services.assistant_reference_formatting import (
    assistant_reference_sort_key as _assistant_reference_sort_key,
)
from app.services.assistant_reference_formatting import (
    assistant_reference_type_preferences as _assistant_reference_type_preferences,
)
from app.services.assistant_reference_formatting import (
    merge_reference_lists as _merge_reference_lists,
)
from app.services.assistant_reference_formatting import (
    merge_reference_lists_by_type as _merge_reference_lists_by_type,
)
from app.services.assistant_reference_formatting import (
    reference_permission_label as _reference_permission_label,
)
...
def assistant_reference_candidates(
    ...
    *,
    ...
    knowledge_spaces = knowledge_reference_helpers.readable_knowledge_spaces(
        current_store,
        query=None,
        user=current_user,
    )
    knowledge_folders = knowledge_reference_helpers.readable_knowledge_folders(
        current_store,
        query=None,
        user=current_user,
    )
...
def assistant_reference_candidates_response(
    ...
    *,
    ...
    if normalized_type == "knowledge_document":
        items = knowledge_reference_helpers.knowledge_document_reference_candidates(
            current_store,
            limit=normalized_limit,
            query=message,
            user=user,
    ...
    if normalized_type == "knowledge_chunk":
        items = knowledge_reference_helpers.knowledge_chunk_reference_candidates(
            current_store,
            limit=normalized_limit,
            query=message,
            user=user,
    ...
    if normalized_type:
        ...
        return {"items": enriched_items, "total": len(enriched_items)}
    knowledge_references = knowledge_reference_helpers.knowledge_document_reference_candidates(
        current_store,
        limit=normalized_limit,
        query=message,
        user=user,
    ...
    if message.strip():
        knowledge_references = _merge_reference_lists(
            knowledge_reference_helpers.knowledge_chunk_reference_candidates(
                current_store,
                limit=normalized_limit,
                query=message,
                user=user,
    ...
    *,
    ...
    for reference in normalized_requests:
        ...
        if reference_type == "knowledge_document":
            document = knowledge_reference_helpers.readable_knowledge_document(
                current_store,
            ...
            resolved.append(knowledge_reference_helpers.knowledge_document_reference(document))
            knowledge_context.extend(
                knowledge_reference_helpers.knowledge_context_for_document(
                    current_store,
                    document=document,
        ...
        if reference_type == "knowledge_chunk":
            chunk_record = knowledge_reference_helpers.readable_knowledge_chunk(
                current_store,
            ...
            resolved.append(knowledge_reference_helpers.knowledge_chunk_reference(document, chunk))
            if len(knowledge_context) < max_chunks:
                knowledge_context.append(
                    knowledge_reference_helpers.knowledge_context_for_chunk(document, chunk)
                )
            continue
        if reference_type == "knowledge_space":
            space = knowledge_reference_helpers.readable_knowledge_space(
                current_store,
            ...
            resolved.append(
                knowledge_reference_helpers.knowledge_space_reference_with_metadata(
                    current_store,
                    space,
                    user=user,
                )
            ...
            knowledge_context.extend(
                knowledge_reference_helpers.knowledge_context_for_space(
                    current_store,
                    max_chunks=max_chunks - len(knowledge_context),
                    space=space,
                    user=user,
        ...
        if reference_type == "knowledge_folder":
            folder = knowledge_reference_helpers.readable_knowledge_folder(
                current_store,
            ...
            resolved.append(
                knowledge_reference_helpers.knowledge_folder_reference_with_metadata(
                    current_store,
                    folder,
                    user=user,
                )
            ...
            knowledge_context.extend(
                knowledge_reference_helpers.knowledge_context_for_folder(
                    current_store,
                    folder=folder,
                    max_chunks=max_chunks - len(knowledge_context),
                    user=user,
    ...
    for reference in references:
        ...
        if reference_type == "knowledge_space" and item is not None:
            enriched_item.update(
                knowledge_reference_helpers.knowledge_scope_reference_metadata(
                    current_store,
                    folder=None,
                    space=item,
                    user=user,
