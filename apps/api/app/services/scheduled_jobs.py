...
from app.services.scheduled_job_async_worker import execute_queued_scheduled_job_run_response
...
def require_product_for_business_result_actions(
    product_id: Any,
    result_actions: list[dict[str, Any]],
) -> None:
    product_required_actions = {
        str(action.get("type"))
        for action in result_actions
        if action.get("type") in {"create_requirements", "write_internal_user_insights"}
    }
    if product_required_actions and not str(product_id or "").strip():
        raise api_error(
            400,
            "VALIDATION_ERROR",
            "product_id is required when result_actions includes "
            + ", ".join(sorted(product_required_actions)),
        )
...
def create_scheduled_job_response(
    *,
    ...
    require_product_for_business_result_actions(payload.product_id, result_actions)
...
def dry_run_scheduled_job_response(
    *,
    ...
    require_product_for_business_result_actions(payload.product_id, configured_result_actions)
    config_json = scheduled_job_config_with_code_inspection_defaults(
        current_store,
        config_json=payload.config_json,
        job_type=job_type,
...
def patch_scheduled_job_response(
    *,
    ...
    require_product_for_business_result_actions(payload_field(draft, "product_id"), result_actions)
    knowledge_document_ids = validate_knowledge_document_ids(
        current_store,
