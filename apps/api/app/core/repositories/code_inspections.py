...
is_full_scan, incremental_from_commit, incremental_file_count,
files_scanned, lines_scanned, rules_loaded,
...
class CodeInspectionReadRepository:
        ...
        for report in reports.values():
            cursor.execute(
                ...
                ON CONFLICT (id) DO UPDATE SET
                  ...
                  incremental_from_commit = EXCLUDED.incremental_from_commit,
                ...
                (
                    ...
                    report.get("incremental_from_commit"),
                    report.get("incremental_file_count"),
    ...
    def _report_from_row(self, row: Any) -> dict[str, Any]:
        return {
            ...
            "incremental_from_commit": row[18],
            "incremental_file_count": row[19],
            "files_scanned": row[20] or 0,
            "lines_scanned": row[21] or 0,
            "rules_loaded": row[22] or [],
            "coverage_warning": row[23],
            "artifact_ref": row[24],
            "checkout_path": row[25],
            "checkout_path_retained": bool(row[26]),
            "remote_url_hash": row[27],
            "remote_url_summary": row[28],
            "scan_started_at": row[29].isoformat() if row[29] else None,
            "scan_finished_at": row[30].isoformat() if row[30] else None,
            "scanner_version": row[31],
            "rules_version": row[32],
            "suppressed_finding_count": row[33] or 0,
            "suppression_summary": row[34] or {},
            "quality_gate": row[35] or {},
            "scan_profile": row[36] or {},
            "previous_report_id": row[37],
            "previous_comparison": row[38] or {},
            "finding_count": row[39],
            "severe_finding_count": row[40],
            "status": row[41],
            "result_actions": row[42] or [],
