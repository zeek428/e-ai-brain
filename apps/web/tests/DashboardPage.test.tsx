...
describe('Dashboard page', () => {
  ...
  it('renders dashboard and operation pages without placeholder data', async () => {
    ...
    expect(dashboardMetrics).toHaveClass('dashboard-focus-grid');
    expect(dashboardMetrics.querySelectorAll('.dashboard-focus-card')).toHaveLength(4);
    expect(screen.getByText('健康结论')).toBeInTheDocument();
    expect(screen.getByText('需重点治理')).toBeInTheDocument();
    expect(screen.getByText('治理优先队列')).toBeInTheDocument();
    expect(screen.getByText('交付负载')).toBeInTheDocument();
    expect(screen.getByText('风险压力')).toBeInTheDocument();
    expect(screen.getByText('工程活跃')).toBeInTheDocument();
    expect(screen.getByText('用户声音')).toBeInTheDocument();
