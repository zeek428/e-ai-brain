...
describe('IterationVersionsPage', () => {
  ...
  it('opens the product version dashboard with delivery blockers and linked records', async () => {
            ...
            release_readiness_checklist: {
              ...
              items: [
                ...
                },
                {
                  ...
                  detail: '后端清单：推进到 testing 仍有 1 条需求阻塞。',
                  key: 'status-impact',
                  level: 'error',
                  status: 'blocked',
                  title: '状态推进',
    ...
    expect(screen.getByText('后端清单：发布准备未通过。')).toBeInTheDocument();
    expect(screen.getByText('后端清单：需求推进阻塞 1 条。')).toBeInTheDocument();
    expect(screen.getByText('需求未满足准入')).toBeInTheDocument();
