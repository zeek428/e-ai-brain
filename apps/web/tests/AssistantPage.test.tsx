...
describe('AssistantPage', () => {
  ...
  it('rebuilds execution trace diagnostic prompt from trace id when route prompt is missing', async () => {
    window.history.pushState(
      {},
      '',
      '/assistant?reference_type=assistant_chat_run&reference_id=assistant_chat_run_trace&diagnostic_trace_id=assistant_chat_run_trace',
    );
    const fetchMock = vi.fn<typeof fetch>(async (input, init) => {
      expect(init?.headers).toMatchObject({ Authorization: 'Bearer token-admin' });
      if (input === '/api/assistant/conversations') {
      ...
      if (input === '/api/governance/execution-traces/assistant_chat_run_trace') {
        return new Response(
          JSON.stringify({
            data: {
              diagnostic_nodes: [
    ...
    await waitFor(() => {
      expect((screen.getByLabelText('发送给 AI 助手') as HTMLTextAreaElement).value).toContain(
        '请基于执行诊断链路「AI 助手运行 assistant_chat_run_trace」分析运行问题。',
      );
    });
    const inputValue = (screen.getByLabelText('发送给 AI 助手') as HTMLTextAreaElement).value;
    expect(inputValue).toContain('model_gateway_log_assistant_trace');
    expect(inputValue).toContain('最可能原因');
    expect(await screen.findByLabelText('本次上下文')).toHaveTextContent('AI 助手运行 assistant_chat_run_trace');
  });
  ...
  it('sends @ scheduled job run-once commands with the active candidate reference', async () => {
    ...
    expect(screen.queryByRole('button', { name: '生成运行修复草案' })).not.toBeInTheDocument();
  ...
  it('shows and sends @ scheduled job run-once candidates when the command has no whitespace', async () => {
    let chatRequestBody: Record<string, unknown> | undefined;
    const referenceRequestParams: URLSearchParams[] = [];
    const scheduledJobReference = {
      id: 'scheduled_job_feedback_weekly',
    ...
    const fetchMock = vi.fn<typeof fetch>(async (input, init) => {
      ...
      if (String(input).startsWith('/api/assistant/reference-candidates?')) {
        const params = new URLSearchParams(String(input).split('?')[1] ?? '');
        referenceRequestParams.push(params);
        const items = params.get('query') === '提取每周用户反馈有价值信息'
          ? [scheduledJobReference]
          : [];
        return new Response(
    ...
    const assistantInput = screen.getByLabelText('发送给 AI 助手');
    fireEvent.change(assistantInput, {
      target: { value: '@提取每周用户反馈有价值信息执行一次' },
    });
    ...
    expect(await screen.findByText(/已触发「提取每周用户反馈有价值信息」执行一次/)).toBeInTheDocument();
    expect(referenceRequestParams.some((params) => (
      params.get('query') === '提取每周用户反馈有价值信息'
      && params.get('type') === null
    ))).toBe(true);
    expect(chatRequestBody).toMatchObject({
      message: '@提取每周用户反馈有价值信息执行一次',
      references: [
        {
          id: 'scheduled_job_feedback_weekly',
          type: 'scheduled_job',
  ...
  it('supports full-width @ scheduled job run-once commands', async () => {
    let chatRequestBody: Record<string, unknown> | undefined;
    const scheduledJobReference = {
      id: 'scheduled_job_feedback_weekly',
      permission_label: '管理员可引用',
    ...
    const fetchMock = vi.fn<typeof fetch>(async (input, init) => {
      ...
      if (input === '/api/assistant/chat') {
        ...
        return new Response(
          JSON.stringify({
            data: {
              conversation_id: 'conversation_fullwidth_run_once',
              latency_ms: 12,
              message: {
                content: '已执行「提取每周用户反馈有价值信息」一次，运行记录 scheduled_job_run_001 已成功完成。',
                id: 'assistant_message_fullwidth_run_once',
                references: [
                  scheduledJobReference,
                  {
                    id: 'scheduled_job_run_001',
